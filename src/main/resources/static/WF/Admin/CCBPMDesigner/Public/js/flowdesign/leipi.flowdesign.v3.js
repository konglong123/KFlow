...
(function ($) {
    ...
    $.fn.Flowdesign = function (options) {
	    ...
	    // 1. 为鼠标左键绑定一个选择框
            // 2. 同时取得被框选的节点（nodes）信息
            Mouse1 : function (moveSelected) {
                ...
                var oldTop = 0;
                window.selectedList = [];//拖拽多选选中的块集合，全局
                ...
                $(document).mouseup(function (e) {
                        ...
                        if (e.which == 1) {
                            ...
                            selectedList2 = findSelected(nodes);
                            flag = false;
                            clearDragData();
                            oEvent.preventDefault();  // 阻止默认行为
                            oEvent.stopPropagation(); // 阻止事件冒泡
                        ...
                        if (selectedList2 != null) {
                            for (var i = 0; i < selectedList2.length; i++) {
                                window.selectedList.push(selectedList2[i]);// 将被选择节点信息push给全局变量
                                //获得节点的ID，改变节点的style，这里选择将节点的文本边红，区别未选中节点
                                var NodeIDselected = selectedList2[i].NodeID;
                                var obj = document.getElementById("window" + NodeIDselected);
                                obj.style.color = "red";
                ...
                function findSelected(nodes) {
                    ...
                    for (var i = 0; i < blockList.length; i++) {
                        //计算每个块的定位信息
                        var left = nodes[i].X;
                        var right = nodes[i].width + left;
                        var top = nodes[i].Y;
                        var bottom = nodes[i].height + top;


                        //判断每个块是否被遮罩盖住（即选中）
                        var leftFlag = moveSelected.style.left.split('px')[0] <= left && left <= moveSelected.style.right.split('px')[0];
                        var rightFlag = moveSelected.style.left.split('px')[0] <= right && right <= moveSelected.style.right.split('px')[0];
                        var topFlag = moveSelected.style.top.split('px')[0] <= top && top <= moveSelected.style.bottom.split('px')[0];
                        var bottomFlag = moveSelected.style.top.split('px')[0] <= bottom && bottom <= moveSelected.style.bottom.split('px')[0];
                        if ((leftFlag || rightFlag) && (topFlag || bottomFlag)) {
                            selectedList1.push(blockList[i]);
                            $(blockList[i]).addClass('selected');//这行没用到
                        }
                    }
	    ...
	    //左键选择右键对齐菜单功能，这里的对齐逻辑是按照ID顺序，取得节点的X,Y值进行对齐
            rightAlign1:function(list) {
                ...
                for (var idx = 1; idx < list.length; idx++) {
                    var node_new = list[idx];
                    node_new = new Entity("BP.WF.Template.NodeSimple", node_new);
                    node_new.X = node_1.X + 200;
                    node_new.Y = node_1.Y;
                    node_1 = node_new;
                    node_new.Update();
                }
            },
            downAlign1: function(list){
                var node_1=list[0];
                for (var idx = 1; idx <list.length; idx++) {
                    var node_new=list[idx];
                    var item=node_new.width;
                    node_new = new Entity("BP.WF.Template.NodeSimple", node_new);
                    node_new.X=node_1.X;
                    node_new.Y=node_1.Y+100;
                    node_1=node_new;
                    node_new.Update();
                }
            },
            leftAlign1: function(list){
                var node_1=list[0];
                for (var idx = 1; idx <list.length; idx++) {
                    var node_new=list[idx];
                    var item=node_new.width;
