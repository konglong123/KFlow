...
public class GenerWorkFlowView extends Entity {
	...
	public final String DoShift(String ToEmp, String Note) throws Exception {
		...
		try {
			BP.WF.Dev2Interface.Node_Shift(this.getFK_Flow(), this.getFK_Node(), this.getWorkID(), this.getFID(), ToEmp,
					Note);
			return "移交成功";
	...
	public final String DoDelete() throws Exception {
		try {
			if (BP.WF.Dev2Interface.Flow_IsCanViewTruck(this.getFK_Flow(), this.getWorkID(),
					WebUser.getNo()) == false) {
				return "您没有操作该流程数据的权限.";
			}
		} catch (Exception e) {
