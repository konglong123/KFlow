		...
		try {
			...
			for (Field temp : fields) {
				...
				if (null != objectValue && !"".equals(objectValue)) {
					...
					} else if(objectValue.toString().contains("com.kong.kflowweb.BP.") || objectValue instanceof Class){
	...
	*//**
	...
	public static <E> String ToJson(Iterable<E> array)
	{
		...
		for (Object item : array) {
			if(!StringUtils.isEmpty(item.getClass().getName()) && item.getClass().getName().contains("com.kong.kflowweb.BP.")){
				build.append(separate);
				Gson gson =  new GsonBuilder().serializeNulls().create();
