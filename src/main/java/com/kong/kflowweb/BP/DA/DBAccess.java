...
import com.kong.kflowweb.BP.En.SQLCash;
import com.kong.kflowweb.BP.Sys.SystemConfig;
import com.kong.kflowweb.BP.Tools.CRC32Helper;
...
public class DBAccess {
	...
	public static void SaveBytesToDB(byte[] bytes, String line, String tableName, String tablePK, String pkVal,
		...
		try {
			...
			if (com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBType() == DBType.MSSQL) {
				conn = com.kong.kflowweb.BP.DA.DBAccess.getGetAppCenterDBConn_MSSQL();
				pstmt = conn.prepareStatement(sql);
			} else if (com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBType() == DBType.Oracle) {
				conn = com.kong.kflowweb.BP.DA.DBAccess.getGetAppCenterDBConn_Oracle();
				pstmt = conn.prepareStatement(sql);
			} else if (com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBType() == DBType.MySQL) {
				conn = com.kong.kflowweb.BP.DA.DBAccess.getGetAppCenterDBConn_MySQL();
				pstmt = conn.prepareStatement(sql);
			}

			// 数据库字段类型不一致增加盘断
			if (com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBType() == DBType.MySQL
					|| com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBType() == DBType.MSSQL) {
	...
	public static int RunSP(String spName) {
		...
		case Informix:
			throw new RuntimeException("@没有实现...");
		default:
			throw new RuntimeException("Error: " + com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBType());
		}
	}
	...
	public static void RunSQLScriptGo(String sqlOfScriptFilePath) throws IOException, Exception {
		String str = com.kong.kflowweb.BP.DA.DataType.ReadTextFile(sqlOfScriptFilePath);
		/*
		...
		for (String s : strs) {
			...
			com.kong.kflowweb.BP.DA.DBAccess.RunSQL(mysql);
		}
	}
	...
	private static int RunSQLReturnResultSet_201809_SQL(String sql, Paras paras, Entities ens, Attrs attrs) {
		...
		try {
			...
			com.kong.kflowweb.BP.En.Map map = myen.getEnMap();
	...
	public static boolean IsExitsTabPK(String tab) {
		com.kong.kflowweb.BP.DA.Paras ps = new Paras();
		ps.Add("Tab", tab);
		String sql = "";
		switch (getAppCenterDBType()) {
		case MSSQL:
	...
	public static boolean IsExitsObject(String obj) throws Exception {
		...
		case Oracle:
			...
			String sql = "select object_name from all_objects WHERE  object_name = upper(:obj) and OWNER='"
					+ com.kong.kflowweb.BP.Sys.SystemConfig.getUser().toUpperCase() + "' ";
		...
		case MySQL:
			...
			return IsExits("SELECT table_name, table_type FROM information_schema.tables  WHERE table_name = '" + obj
					+ "' AND   TABLE_SCHEMA='" + com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBDatabase() + "' ");
		default:
	...
	public static boolean IsExitsTableCol(String table, String col) throws Exception {
		...
		case MySQL:
			String sql = "select count(*) FROM information_schema.columns WHERE TABLE_SCHEMA='"
					+ com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBDatabase() + "' AND table_name ='" + table
					+ "' and column_Name='" + col + "'";
	...
	public static String GetBigTextFromDB(String tableName, String tablePK, String pkVal, String fileSaveField)throws Exception {
		if (com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBType() == DBType.MSSQL
				|| com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBType() == DBType.MySQL){
			String strSQL = "SELECT " + fileSaveField + " FROM " + tableName + " WHERE " + tablePK + "='" + pkVal + "'";
	...
	public static byte[] GetByteFromDB(String tableName, String tablePK, String pkVal, String fileSaveField)  throws Exception{
		...
		{
			if (!com.kong.kflowweb.BP.DA.DBAccess.IsExitsTableCol(tableName, fileSaveField))
			{
				/*如果没有此列，就自动创建此列.*/
	...
	public static boolean TestIsConnection() {
		try {
			switch (com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBType()) {
			case MSSQL:
				com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnString("SELECT 1+2 ");
				break;
			case Oracle:
			case MySQL:
				com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnString("SELECT 1+2 FROM DUAL ");
				break;
			case Informix:
				com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnString("SELECT 1+2 FROM DUAL ");
				break;
