...
import com.kong.kflowweb.BP.En.SqlBuilder;
import com.kong.kflowweb.BP.Pub.RepBill;
...
import com.kong.kflowweb.BP.Tools.ConvertTools;
...
public class Cash {
	...
	public static String getCaseWhenSQL(String mTable, String key, String field, String enumKey, int def, String tag)
		...
		if (DataType.IsNullOrEmpty(tag) == true) {
			...
			if (ses.size() == 0) {
				com.kong.kflowweb.BP.Sys.XML.EnumInfoXml xml = new com.kong.kflowweb.BP.Sys.XML.EnumInfoXml(enumKey);
	...
	public static Object GetObj(String key, Depositary where) {
		...
		if (SystemConfig.getIsBSsystem()) {
			...
			// return com.kong.kflowweb.BP.Glo.HttpContextCurrent.Cache(key);
			{
				return getBS_Cash().get(key); // com.kong.kflowweb.BP.Glo.HttpContextCurrent.Cache(key);
			} else {
				/*
	...
	public static Object GetObjFormApplication(String key, Object isNullAsVal) {
		if (SystemConfig.getIsBSsystem()) {
			Object obj = getBS_Cash().get(key); // com.kong.kflowweb.BP.Glo.HttpContextCurrent.Cache(key);
			if (obj == null) {
				return isNullAsVal;
			} else {
				return obj;
	...
	public static void RemoveObj(String key, Depositary where) {
		...
		if (SystemConfig.getIsBSsystem()) {
			if (where == Depositary.Application) {
				/*
				 * warning com.kong.kflowweb.BP.Glo.getHttpContextCurrent().Cache.remove(key);
				 */
			} else {
				/*
				 * warning com.kong.kflowweb.BP.Glo.getHttpContextCurrent().Session.remove(key);
				 */
	...
	public static void AddObj(String key, Depositary where, Object obj) {
		...
		if (SystemConfig.getIsBSsystem()) {
			if (where == Depositary.Application) {
				...
				/*
				 * warning com.kong.kflowweb.BP.Glo.getHttpContextCurrent().Session(key) = obj;
				 */
	...
	public static boolean IsExits(String key, Depositary where) {
		if (SystemConfig.getIsBSsystem()) {
			if (where == Depositary.Application) {
				/*
				 * warning if (com.kong.kflowweb.BP.Glo.getHttpContextCurrent().Cache(key) ==
				 * null) { return false; } else { return true; }
				 */
				return true;
			} else {
				/*
				 * warning if (com.kong.kflowweb.BP.Glo.getHttpContextCurrent().Session(key) ==
				 * null) { return false; } else { return true; }
				 */
				return true;
			}
