package com.kong.kflowweb.BP.FlowEvent;

// 
import java.math.BigDecimal;

import com.kong.kflowweb.BP.WF.FlowEventBase;
...
//		//求出明细表的合计.
//		float hj = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValFloat("SELECT SUM(XiaoJi) as Num FROM ND101Dtl1 WHERE RefPK=" + this.getOID, 0);
//
//		//更新合计小写 , 把合计转化成大写.
//		String sql = "UPDATE ND101 SET DaXie='" + com.kong.kflowweb.BP.DA.DataType.ParseFloatToCash(hj) + "',HeJi="+hj+"  WHERE OID=" + this.getOID;
	...
	*/
	...
	public String SendSuccess() throws Exception
	{
		try
		{
			...
			// 分解执行的情况.
	        if(currNodeID==1808  ){
		    // com.kong.kflowweb.BP.WF.Entity.GenerWorkFlows ens =new  com.kong.kflowweb.BP.WF.Entity.GenerWorkFlows();
		    //ens.Retrieve(com.kong.kflowweb.BP.WF.Entity.GenerWorkFlowAttr.PWorkID, workid );
			    String sql="SELECT SUM(ShiJiHuaFeiChengBen) FROM ND19RPT WHERE PWorkID="+workid;
			    sjje=com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValDecimal(sql, sjje, 2);
