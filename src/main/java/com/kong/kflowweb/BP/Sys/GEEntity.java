...
public class GEEntity extends Entity
{
	...
	*/
	...
	public Map getEnMap()
	{
		// 注释掉，这里不需要再缓存了，因为有 com.kong.kflowweb.BP.Sys.MapData.GenerHisMap 里缓存，
...
//		}
		...
		try {
			this.set_enMap(com.kong.kflowweb.BP.Sys.MapData.GenerHisMap(this.FK_MapData));
		} catch (Exception e) {
	...
	public final void CopyToOID(long oid) throws Exception
	{
		...
		{
			//删除旧的数据.
			com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM " + dtl.getPTable() + " WHERE RefPK=" + this.getOID());
