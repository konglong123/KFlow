...
public class CCFormAPI
{
	...
	*/
	...
	{
		int num = com.kong.kflowweb.BP.DA.DBAccess.RunSQL("SELECT COUNT(MYPK) FROM Sys_FrmAttachmentDB WHERE FK_MapData='" + fk_mapdata + "' AND RefPKVal=" + pk);
		return "附件(" + num + ")";
	}
	...
	*/
			...
			throws Exception
	{
        ...
        if (attr.getLGType() == FieldTypeS.Normal)
        {
            ...
            attrH.setUIContralType( com.kong.kflowweb.BP.En.UIContralType.TB);
            attrH.setMinLen( 0);
            attrH.setMaxLen( 60);
            attrH.setMyDataType(com.kong.kflowweb.BP.DA.DataType.AppString);
	...
	*/
	...
	public static void NewEnumField(String fk_mapdata, String field, String fieldDesc, String enumKey, UIContralType ctrlType, float x, float y, int colSpan) throws Exception
	{
		...
		//删除可能存在的数据.
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM Sys_FrmRB WHERE KeyOfEn='" + ma.getKeyOfEn() + "' AND FK_MapData='" + ma.getFK_MapData() + "'");
	...
	*/
	private static void SaveFrm(String fk_mapdata, JSONObject formData) throws Exception
	{
		...
		com.kong.kflowweb.BP.Sys.CCFormParse.SaveLine(fk_mapdata, form_Lines);
		...
		if (form_Controls.isArray() == false || form_Controls.size() == 0)
		{
			...
			com.kong.kflowweb.BP.DA.DBAccess.RunSQLs(delSqls);
			return;
		}
	    ...
	    int nodeID = 0;
	    if (com.kong.kflowweb.BP.DA.DataType.IsNumStr(nodeIDStr) == true)
	    {
		...
		for (int idx = 0, jControl = form_Controls.size(); idx < jControl; idx++)
		{
			...
			if (shape.equals("Label")) //保存标签.
			{
					...
					{
						com.kong.kflowweb.BP.Sys.CCFormParse.SaveLabel(fk_mapdata, control, properties, labelPKs, ctrlID);
			...
			{
					com.kong.kflowweb.BP.Sys.CCFormParse.SaveButton(fk_mapdata, control, properties, btnsPKs, ctrlID);
					btnsPKs = btnsPKs.replace(ctrlID + "@", "@");
					continue;
			}
			else if (shape.equals("HyperLink")) //保存link.
			{
					com.kong.kflowweb.BP.Sys.CCFormParse.SaveHyperLink(fk_mapdata, control, properties, linkPKs, ctrlID);
					linkPKs = linkPKs.replace(ctrlID + "@", "@");
					continue;
			}
			else if (shape.equals("Image")) //保存Img.
			{
					com.kong.kflowweb.BP.Sys.CCFormParse.SaveImage(fk_mapdata, control, properties, imgPKs, ctrlID);
			...
			{
				com.kong.kflowweb.BP.Sys.CCFormParse.SaveMapAttr(fk_mapdata, ctrlID, shape, control, properties, attrPKs);
			...
			{
				//记录已经存在的ID， 需要当时保存.
				com.kong.kflowweb.BP.Sys.CCFormParse.SaveDtl(fk_mapdata, ctrlID, x, y, height, width);
			...
			{
				//记录已经存在的ID， 需要当时保存.
				com.kong.kflowweb.BP.Sys.CCFormParse.SaveAthMulti(fk_mapdata, ctrlID, x, y, height, width);
			...
			{
				//记录已经存在的ID， 需要当时保存.
				com.kong.kflowweb.BP.Sys.CCFormParse.SaveAthImg(fk_mapdata, ctrlID, x, y, height, width);
			...
			{
				//记录已经存在的ID， 需要当时保存.
				com.kong.kflowweb.BP.Sys.CCFormParse.SaveFrmEle(fk_mapdata, shape, ctrlID, x, y, height, width);
			...
			{
				//记录已经存在的ID， 需要当时保存.
				com.kong.kflowweb.BP.Sys.CCFormParse.SaveMapFrame(fk_mapdata, shape, ctrlID, x, y, height, width);
			...
			if (shape.equals("RadioButton"))
			{
				...
				String str = com.kong.kflowweb.BP.Sys.CCFormParse.SaveFrmRadioButton(fk_mapdata, ctrlID, x, y);
				if (str == null)
				{
					continue;
				}
...
//ORIGINAL LINE: public static string SaveEnum(string enumKey, string enumLab, string cfg, bool isNew,string lang = "CH")
	...
	*/
	...
	{
		 if (com.kong.kflowweb.BP.DA.DataType.IsNullOrEmpty(name) == true)
             return "";
		...
		try
		{
			if (isQuanPin == true)
			{
				s = com.kong.kflowweb.BP.DA.DataType.ParseStringToPinyin(name);
				if (s.length() > 15)
				{
					s = com.kong.kflowweb.BP.DA.DataType.ParseStringToPinyinWordFirst(name);
				}
			}
			else
			{
				s = com.kong.kflowweb.BP.DA.DataType.ParseStringToPinyinJianXie(name);
			}
	...
	*/
	public static String DB_SFTableList(int pageNumber, int pageSize) throws Exception
	{
		...
		return com.kong.kflowweb.BP.Tools.Entitis2Json.ConvertEntitis2GridJsonOnlyData(sftables, RowCount);
	}
	/** 
	 获取所有枚举
	...
	*/
	public static String DB_EnumerationList(int pageNumber, int pageSize) throws Exception
	{
		...
		return com.kong.kflowweb.BP.Tools.Entitis2Json.ConvertEntitis2GridJsonOnlyData(enumMains, RowCount);
	}
	/** 
	 获得隐藏字段集合.
	...
	*/
	public static String DB_Hiddenfielddata(String fk_mapdata) throws Exception
	{
		...
		return com.kong.kflowweb.BP.Tools.Entitis2Json.ConvertEntitis2GridJsonOnlyData(mapAttrs);
	}
	/**
	 * 清缓存
	 * @param frmID
	 ...
	 */
     ...
     {
         com.kong.kflowweb.BP.DA.CashFrmTemplate.Remove(frmID);
	...
	public static DataSet GenerHisDataSet(String fk_mapdata, String frmName) throws Exception {
		  ...
		  DataSet dsFrm = com.kong.kflowweb.BP.DA.CashFrmTemplate.GetFrmDataSetModel(fk_mapdata);
          if (dsFrm != null)
              return dsFrm;
		...
		//放入缓存.
		com.kong.kflowweb.BP.DA.CashFrmTemplate.Put(fk_mapdata, ds);
		return ds;
	}
