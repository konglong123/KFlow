...
public class MapAttr extends EntityMyPK {
	...
	public final EntitiesNoName getHisEntitiesNoName() throws Exception {
		if (this.getUIBindKey().contains(".")) {
			EntitiesNoName ens = (EntitiesNoName) com.kong.kflowweb.BP.En.ClassFactory.GetEns(this.getUIBindKey());
			if (ens == null) {
				return null;
			}
	...
	public final String getDefVal() throws Exception {
		...
		if (s.toLowerCase().equals("@webuser.no")) {
			...
			return com.kong.kflowweb.BP.Web.WebUser.getName();
		} else if (s.toLowerCase().equals("@webuser.fk_dept")) {
			return com.kong.kflowweb.BP.Web.WebUser.getFK_Dept();
		} else if (s.toLowerCase().equals("@webuser.fk_deptname")) {
			return com.kong.kflowweb.BP.Web.WebUser.getFK_DeptName();
		} else if (s.toLowerCase().equals("@webuser.FK_DeptNameOfFull")) {
			return com.kong.kflowweb.BP.Web.WebUser.getFK_DeptNameOfFull();
	...
	protected boolean beforeInsert() throws Exception {
		...
		if (this.getKeyOfEn() == null || this.getKeyOfEn().trim().equals("")) {
			try {
				this.setKeyOfEn(com.kong.kflowweb.BP.DA.DataType.ParseStringToPinyin(this.getName()));
				if (this.getKeyOfEn().length() > 20) {
					this.setKeyOfEn(com.kong.kflowweb.BP.DA.DataType.ParseStringToPinyinWordFirst(this.getName()));
				}
		...
		if (this.getIdx() == 0) {
			this.setIdx(999); // com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt("SELECT
    ...
    {
        SFTable sf = new SFTable(this.getUIBindKey());
        return com.kong.kflowweb.BP.Tools.Json.ToJson(sf.getGenerHisDataTable());
    }
}