...
public class SysEnum extends EntityMyPK
{
	...
	*/
	public SysEnum(String enumKey, int val) throws Exception
	{
		...
		this.setLang(com.kong.kflowweb.BP.Web.WebUser.getSysLang());
		this.setIntKey(val);
		this.setMyPK(this.getEnumKey() + "_" + this.getLang() + "_" + this.getIntKey());
		int i = this.RetrieveFromDBSources();
		if (i == 0)
		{
			i = this.Retrieve(SysEnumAttr.EnumKey, enumKey, SysEnumAttr.Lang, com.kong.kflowweb.BP.Web.WebUser.getSysLang(), SysEnumAttr.IntKey, this.getIntKey());
			SysEnums ses = new SysEnums();
			ses.Full(enumKey);
			if (i == 0)
			{
				//尝试注册系统的枚举的配置.
				com.kong.kflowweb.BP.Sys.SysEnums myee = new SysEnums(enumKey);

				throw new RuntimeException("@ EnumKey=" + getEnumKey() + " Val=" + val + " Lang=" + com.kong.kflowweb.BP.Web.WebUser.getSysLang() + " ...Error");
			}
		}
	}
