...
public class SysEnums extends Entities {
	...
	public final void RegIt(String EnumKey, String vals, Boolean isInsert) throws Exception {
		try {
			...
			for (String s : strs) {
				...
				se.setLang(com.kong.kflowweb.BP.Web.WebUser.getSysLang());
				if (isInsert == true)
					se.Insert();
				this.AddEntity(se);
			}
	...
	public final boolean Full(String enumKey) throws Exception {
		...
		Entities ens = (Entities) Cash.GetObjFormApplication("EnumOf" + enumKey + com.kong.kflowweb.BP.Web.WebUser.getSysLang(), null);
		if (ens != null) {
		...
		Cash.AddObj("EnumOf" + enumKey + com.kong.kflowweb.BP.Web.WebUser.getSysLang(), Depositary.Application, this);
		return true;
	}
