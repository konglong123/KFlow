	...
	*/
	private String _DoEventNode(String dotype, Entity en, String atPara) throws Exception
	{
		...
		if (nev.getHisDoType() == EventDoType.BuessUnit) {
			/* 获得业务单元，开始执行他 */
			try {
				BuessUnitBase enBuesss = com.kong.kflowweb.BP.Sys.Glo.GetBuessUnitEntityByEnName(nev.getDoDoc());
		...
		doc = doc.replaceAll("~", "'");
		doc = doc.replaceAll("@WebUser.No", com.kong.kflowweb.BP.Web.WebUser.getNo());
		doc = doc.replaceAll("@WebUser.Name", com.kong.kflowweb.BP.Web.WebUser.getName());
		doc = doc.replaceAll("@WebUser.FK_Dept", com.kong.kflowweb.BP.Web.WebUser.getFK_Dept());
		...
		switch (nev.getHisDoType())
		{
			...
			case URLOfSelf:
				...
				if (myURL.contains("http") == false)
				{
					...
					{
						String cfgBaseUrl = com.kong.kflowweb.BP.WF.Glo.getHostURL();
			...
			case EventBase: //执行事件类.
				...
				com.kong.kflowweb.BP.Sys.EventBase ev = null;
				try
				{
					ev = com.kong.kflowweb.BP.En.ClassFactory.GetEventBase(evName);
				}
				catch (RuntimeException ex)
				{
					throw new RuntimeException("@事件名称:" + evName + "拼写错误，或者系统不存在。说明：事件所在的类库必须是以com.kong.kflowweb.BP.开头，并且类库的com.kong.kflowweb.BP.xxx.dll。");
				}
				...
				try
				{
					...
					if (SystemConfig.getIsBSsystem()&&com.kong.kflowweb.BP.Sys.Glo.getRequest()!=null)
					{
						java.util.Enumeration params=com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameterNames();
						//如果是bs系统, 就加入外部url的变量.
						while (params.hasMoreElements())
						{
							String key = (String) params.nextElement();
							String val = com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter(
									key);
