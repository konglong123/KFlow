...
public abstract class FrmEventBase
{
	...
	*/
	public final String DoIt(String eventType, Entity en, Row row, String atPara)
	{
		...
		if (SystemConfig.getIsBSsystem() == true&&com.kong.kflowweb.BP.Sys.Glo.getRequest()!=null)
		{
			while (com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameterNames().hasMoreElements())
			{
			...
			//如果是bs系统, 就加入外部url的变量.
			while( com.kong.kflowweb.BP.Sys.Glo.getRequest().getAttributeNames().hasMoreElements())
			{
				String key = (String) com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameterNames().nextElement();
				String val = com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter(key);
				try
