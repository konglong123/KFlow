	...
	*/
	...
	protected void afterDelete() throws Exception
	{
		...
		//删除附件.
			...
			// @于庆海需要翻译.
			if (ath.getAthSaveWay()==com.kong.kflowweb.BP.Sys.AthSaveWay.WebServer)
			{
				new File(this.getFileFullName()).delete();
			}

			if (ath.getAthSaveWay() == com.kong.kflowweb.BP.Sys.AthSaveWay.FTPServer)
			{
				...
				{
				   SftpUtil en= com.kong.kflowweb.BP.WF.Glo.getSftpUtil();
				   en.deleteFile(this.getFileFullName());				   
				}
				
				if (SystemConfig.getFTPServerType().equals("FTP") )
				{
				   FtpUtil  en= com.kong.kflowweb.BP.WF.Glo.getFtpUtil();
				   en.deleteFile(this.getFileFullName());
    ...
    {
        if (saveWay == com.kong.kflowweb.BP.Sys.AthSaveWay.WebServer)
            return this.getFileFullName();

        if (saveWay == com.kong.kflowweb.BP.Sys.AthSaveWay.FTPServer)
            return this.MakeFullFileFromFtp();

        if (saveWay == com.kong.kflowweb.BP.Sys.AthSaveWay.DB)
			try {
