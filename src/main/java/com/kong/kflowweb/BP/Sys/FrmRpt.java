...
public class FrmRpt extends EntityNoName
{
	...
	*/
	...
	public final Map GenerMap() throws Exception
	{
		...
		if (isdebug == false)
		{
			Map m = com.kong.kflowweb.BP.DA.Cash.GetMap(this.getNo());
			if (m != null)
			{
				return m;
			}
		...
		com.kong.kflowweb.BP.DA.Cash.SetMap(this.getNo(), map);
		return map;
	}
	...
	*/
	...
	{
		return com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt("select COUNT(OID) from " + this.getPTable() + " WHERE WorkID=" + workID);
	}

	public final int GetCountByFK(String field, String val)
	{
		return com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt("select COUNT(OID) from " + this.getPTable() + " WHERE " + field + "='" + val + "'");
	}
	...
	{
		return com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt("SELECT COUNT(OID) from " + this.getPTable() + " WHERE " + f1 + "=" + val1 + " AND " + f2 + "='" + val2 + "'");
	}
	...
	protected boolean beforeDelete() throws Exception
	{
		...
		try
		{
			com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DROP TABLE " + this.getPTable());
		}
