...
public class MapAttrEnum extends EntityMyPK {
	...
	public Map getEnMap() {
		...
		// 解决:“SELECT IntKey as No, Lab as Name FROM Sys_Enum where
		// EnumKey='@UIBindKey'” 修改为 “SELECT IntKey \"No\", Lab \"Name\" FROM
		// Sys_Enum where EnumKey='@UIBindKey'”
		map.AddDDLSQL(MapAttrAttr.DefVal, "0", "默认值（选中）",
				"SELECT  IntKey \"No\", Lab \"Name\" FROM Sys_Enum where EnumKey='@UIBindKey'", true);
	...
	protected void afterDelete() throws Exception {
		// 删除可能存在的数据.
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM Sys_FrmRB WHERE KeyOfEn='" + this.getKeyOfEn() + "' AND FK_MapData='"
				+ this.getFK_MapData() + "'");
		super.afterDelete();
	}
	...
	public final String DoSysEnum() {
		return Glo.getCCFlowAppPath()
				+ "WF/Admin/CCFormDesigner/DialogCtr/EnumerationNew.htm?DoType=FrmEnumeration_SaveEnum&EnumKey="
				+ this.getUIBindKey();
	}
	...
	public final String DoDDLFullCtrl() {
		return Glo.getCCFlowAppPath() + "WF/Admin/FoolFormDesigner/MapExt/DDLFullCtrl.htm?FK_MapData="
	...
	public final String DoAutoFull() {
		return Glo.getCCFlowAppPath() + "WF/MapDef/MapExt/AutoFullDLL.jsp?FK_MapData=" + this.getFK_MapData()
				+ "&ExtType=AutoFull&KeyOfEn=" + this.getKeyOfEn() + "&RefNo=" + this.getMyPK();
	}
	...
	public final String DoRadioBtns() {
		return Glo.getCCFlowAppPath() + "WF/Admin/FoolFormDesigner/MapExt/RadioBtns.htm?FK_MapData="
				+ this.getFK_MapData() + "&ExtType=AutoFull&KeyOfEn=" + this.getKeyOfEn() + "&RefNo=" + this.getMyPK();
	}
