...
import com.kong.kflowweb.BP.Tools.Cryptos;
import com.kong.kflowweb.BP.Tools.En3Des;
import com.kong.kflowweb.BP.WF.StartWorkAttr;
...
public class Glo
{
	...
	public static BuessUnitBase GetBuessUnitEntityByEnName(String enName) {
		...
		if (myen == null) {
			...
			com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLineError("@根据类名称获取业务单元实例出现错误:" + enName + ",没有找到该类的实体.");
			return null;
		}
		return myen;
	}
...
*/
	public static FormEventBase GetFormEventBaseByEnName(String enName)
	{
		if (Htable_FormFEE == null)
		{
			...
			java.util.ArrayList al = com.kong.kflowweb.BP.En.ClassFactory.GetObjects("com.kong.kflowweb.BP.Sys.FormEventBase");
	...
	*/
	public static FormEventBaseDtl GetFormDtlEventBaseByEnName(String dtlEnName)
	{
		if (Htable_FormFEEDtl == null || Htable_FormFEEDtl.isEmpty())
		{
			...
			java.util.ArrayList al = com.kong.kflowweb.BP.En.ClassFactory.GetObjects("com.kong.kflowweb.BP.Sys.FormEventBaseDtl");
	...
	{
		com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLineWarning(msg);
	}
	...
	{
		com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLineError(msg);
	}
	...
	public static void WriteUserLog(String logType, String empNo, String msg) throws Exception
	{
		...
		try
		{
			...
			{
				ul.setIP(com.kong.kflowweb.BP.Sys.Glo.getRequest().getRemoteHost());
			}
...
*/
	public static void SendMessageToCCIM(String fromEmpNo, String sendToEmpNo, String msg, String now)
	{
		...
		//加密处理
		msg = com.kong.kflowweb.BP.Tools.SecurityDES.encrypt(msg);
		...
		long messgeID = com.kong.kflowweb.BP.DA.DBAccess.GenerOID("RecordMsgUser");
...
//}
	...
	public static String GenerMD5(Work wk)
	{
		...
		for (Attr attr : wk.getEnMap().getAttrs())
		{
			...
			} else if (attr.getKey() == com.kong.kflowweb.BP.WF.WorkAttr.RDT)
			{
