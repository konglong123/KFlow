...
import com.kong.kflowweb.BP.Tools.Json;
...
public class SFTable<CodeCompileUnit> extends EntityNoName {
	...
	public final DataTable getGenerHisDataTable() throws Exception {
		...
		if (this.getSrcType() == com.kong.kflowweb.BP.Sys.SrcType.BPClass) {
		...
		if (this.getSrcType() == com.kong.kflowweb.BP.Sys.SrcType.WebServices) {
			...
			for (String p : ps) {
				...
				try {
					if (pa[1].contains("WebUser.No")) {
						pa[1] = pa[1].replace("WebUser.No", com.kong.kflowweb.BP.Web.WebUser.getNo());
					}
					if (pa[1].contains("@WebUser.Name")) {
						pa[1] = pa[1].replace("@WebUser.Name", com.kong.kflowweb.BP.Web.WebUser.getName());
					}
					if (pa[1].contains("@WebUser.FK_DeptName")) {
						pa[1] = pa[1].replace("@WebUser.FK_DeptName", com.kong.kflowweb.BP.Web.WebUser.getFK_DeptName());
					}
					if (pa[1].contains("@WebUser.FK_Dept")) {
						pa[1] = pa[1].replace("@WebUser.FK_Dept", com.kong.kflowweb.BP.Web.WebUser.getFK_Dept());
					}
				...
				if (pa[1].contains("@WorkID")) {
					pa[1] = pa[1].replace("@WorkID", (com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter("WorkID") != null)
							? com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter("WorkID") : "");
				}
				if (pa[1].contains("@NodeID")) {
					pa[1] = pa[1].replace("@NodeID", (com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter("NodeID") != null)
							? com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter("NodeID") : "");
				}
				if (pa[1].contains("@FK_Node")) {
					pa[1] = pa[1].replace("@FK_Node", (com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter("FK_Node") != null)
							? com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter("FK_Node") : "");
				}
				if (pa[1].contains("@FK_Flow")) {
					pa[1] = pa[1].replace("@FK_Flow", (com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter("FK_Flow") != null)
							? com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter("FK_Flow") : "");
				}
				if (pa[1].contains("@FID")) {
					pa[1] = pa[1].replace("@FID", (com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter("FID") != null)
							? com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter("FID") : "");
				}
			...
			if ("DataSet".equals(td[1])) {
				return result == null ? new DataTable()
						: ((DataSet) ((result instanceof DataSet) ? result : null)).Tables.get(0);
				...
				/*
				 * if (!jdata.getIsArray()) { throw new
				 * RuntimeException("@返回的JSON格式字符串“" + ((result != null) ?
				 * result : "") + "”不正确"); }
				 */
				 ...
				 * dt.Rows.Add(jdata.getItem(i)["No"].toString(),
				 * jdata.getItem(i)["Name"].toString()); }
				 */
				return dt;
				 ...
				 * xml.LoadXml((String)((result instanceof String) ? result :
				 * null));
				 * 
				 * XmlNode root = null;
				 * 
				 * if (xml.ChildNodes.size() < 2) { root = xml.ChildNodes[0]; }
				 * else { root = xml.ChildNodes[1]; }
				 * 
				 * dt = new DataTable(); dt.Columns.Add("No", String.class);
				 * dt.Columns.Add("Name", String.class);
				 * 
		...
		if (this.getSrcType() == com.kong.kflowweb.BP.Sys.SrcType.SQL || this.getSrcType() == com.kong.kflowweb.BP.Sys.SrcType.TableOrView) {
			...
			if (runObj.contains("WebUser.No")) {
				runObj = runObj.replace("WebUser.No", com.kong.kflowweb.BP.Web.WebUser.getNo());
			}

			if (runObj.contains("@WebUser.Name")) {
				runObj = runObj.replace("@WebUser.Name", com.kong.kflowweb.BP.Web.WebUser.getName());
			}

			if (runObj.contains("@WebUser.FK_Dept")) {
				runObj = runObj.replace("@WebUser.FK_Dept", com.kong.kflowweb.BP.Web.WebUser.getFK_Dept());
			}

			/// #warning 这是写的什么？ 不是说了吗 不采用这样的表达式,不好转java. 写的这样长，谁能看懂了？
			/// 读完这行代码，还能传过来气吗？
			if (this.getSrcType() == com.kong.kflowweb.BP.Sys.SrcType.TableOrView) {
				runObj = "SELECT " + this.getColumnValue() + " No, " + this.getColumnText() + " Name"
						+ (this.getCodeStruct() == com.kong.kflowweb.BP.Sys.CodeStruct.Tree ? (", " + this.getParentValue() + " ParentNo")
								: "")
		...
		/// #region 自定义表.
		if (this.getSrcType() == com.kong.kflowweb.BP.Sys.SrcType.CreateTable) {
			String sql = "SELECT No, Name FROM " + this.getNo();
			return src.RunSQLReturnTable(sql);
		}

		// @浙商银行
		// throw new RuntimeException("@没有判断的数据类型." + this.getSrcType() + " - "
		// + this.getSrcTypeText());
		return null;
	}
	...
	public final Object InvokeWebService(String url, String methodname, Object[] args) {
		...
		/*
		 * String namespace = "com.kong.kflowweb.BP.RefServices"; try { if (url.endsWith(".asmx"))
		 * { url += "?wsdl"; } else if (url.endsWith(".svc")) { url +=
		 * "?singleWsdl"; }
		 * 
		 * //获取WSDL WebClient wc = new WebClient(); Stream stream =
		 * wc.OpenRead(url); ServiceDescription sd =
		 * ServiceDescription.Read(stream); String classname =
		 * sd.Services[0].getName(); ServiceDescriptionImporter sdi = new
		 * ServiceDescriptionImporter(); sdi.AddServiceDescription(sd, "", "");
		 * CodeNamespace cn = new CodeNamespace(namespace); //生成客户端代理类代码
		 * CodeCompileUnit ccu = new CodeCompileUnit(); ccu.Namespaces.Add(cn);
		 * sdi.Import(cn, ccu); CSharpCodeProvider csc = new
		 * CSharpCodeProvider(); ICodeCompiler icc = csc.CreateCompiler();
		 * //设定编译参数 CompilerParameters cplist = new CompilerParameters();
		 * cplist.GenerateExecutable = false; cplist.GenerateInMemory = true;
		 * cplist.ReferencedAssemblies.Add("System.dll");
		 * cplist.ReferencedAssemblies.Add("System.XML.dll");
		 * cplist.ReferencedAssemblies.Add("System.Web.Services.dll");
		 * cplist.ReferencedAssemblies.Add("System.Data.dll"); //编译代理类
		 * CompilerResults cr = icc.CompileAssemblyFromDom(cplist, ccu); if
		 * (true == cr.Errors.HasErrors) { StringBuilder sb = new
		 * StringBuilder(); for (System.CodeDom.Compiler.CompilerError ce :
		 * cr.Errors) { sb.append(ce.toString());
		 * sb.append(System.Environment.NewLine); } throw new
		 * RuntimeException(sb.toString()); }
		 * 
		 * //生成代理实例，并调用方法 System.Reflection.Assembly assembly =
		 * cr.CompiledAssembly; java.lang.Class t = assembly.GetType(namespace +
		 * "." + classname, true, true); Object obj =
		 * Activator.CreateInstance(t); java.lang.reflect.Method mi =
		 * t.getMethod(methodname);
		 * 
		 * return mi.invoke(obj, args); } catch (java.lang.Exception e) { return
		 * null; }
		 */
	}
	...
	public final boolean getIsTree() {
		if (this.getCodeStruct() == com.kong.kflowweb.BP.Sys.CodeStruct.NoName) {
			return false;
		}
		return true;
	}
	...
	public final SrcType getSrcType() {
		if (this.getNo().contains("com.kong.kflowweb.BP.") == true) {
			return SrcType.BPClass;
		} else {
			SrcType src = SrcType.forValue(this.GetValIntByKey(SFTableAttr.SrcType));
			if (src == com.kong.kflowweb.BP.Sys.SrcType.BPClass) {
				return com.kong.kflowweb.BP.Sys.SrcType.CreateTable;
			}
			return src;
		}
	}
	...
	public final String getSrcTypeText() {
		if (com.kong.kflowweb.BP.Sys.SrcType.TableOrView.equals(getSrcType())) {
			if (this.getIsClass()) {
				return "<img src='/WF/Img/Class.png' width='16px' broder='0' />实体类";
			} else {
				return "<img src='/WF/Img/Table.gif' width='16px' broder='0' />表/视图";
			}
		} else if (com.kong.kflowweb.BP.Sys.SrcType.TableOrView.equals(getSrcType())) {

		} else if (com.kong.kflowweb.BP.Sys.SrcType.SQL.equals(getSrcType())) {
			return "<img src='/WF/Img/SQL.png' width='16px' broder='0' />SQL表达式";
		} else if (com.kong.kflowweb.BP.Sys.SrcType.WebServices.equals(getSrcType())) {
	...
	public final EntitiesNoName getHisEns() throws Exception {
		if (this.getIsClass()) {
			EntitiesNoName ens = (EntitiesNoName) com.kong.kflowweb.BP.En.ClassFactory.GetEns(this.getNo());
			ens.RetrieveAll();
			return ens;
		}

		com.kong.kflowweb.BP.Sys.GENoNames ges = new GENoNames(this.getNo(), this.getName());
		ges.RetrieveAll();
		return ges;
	}
	...
	public SFTable(String mypk) throws Exception {
		...
		try {
			...
			if (this.getNo().equals("com.kong.kflowweb.BP.Pub.NYs")) {
				this.setName("年月");
				this.setFK_Val("FK_NY");
				this.Insert();
			} else if (this.getNo().equals("com.kong.kflowweb.BP.Pub.YFs")) {
				this.setName("月");
				this.setFK_Val("FK_YF");
				this.Insert();
			} else if (this.getNo().equals("com.kong.kflowweb.BP.Pub.Days")) {
				this.setName("天");
				this.setFK_Val("FK_Day");
				this.Insert();
			} else if (this.getNo().equals("com.kong.kflowweb.BP.Pub.NDs")) {
				this.setName("年");
	...
	public Map getEnMap() {
		...
		map.AddDDLSysEnum(SFTableAttr.SrcType, 0, "数据表类型", true, false, SFTableAttr.SrcType,
				"@0=本地的类@1=创建表@2=表或视图@3=SQL查询表@4=WebServices@5=微服务Handler外部数据源@6=JavaScript外部数据源");
		...
		// 数据源.
		map.AddDDLEntities(SFTableAttr.FK_SFDBSrc, "local", "数据源", new com.kong.kflowweb.BP.Sys.SFDBSrcs(), true);
	...
	public final String DoEdit() {
		if (this.getIsClass()) {
			...
			return SystemConfig.getCCFlowWebPath() + "WF/Admin/FoolFormDesigner/SFTableEditData.htm?FK_SFTable="
					+ this.getNo();
		}
	}
	...
	protected boolean beforeInsert() throws Exception {
		...
		if (this.getSrcType() == com.kong.kflowweb.BP.Sys.SrcType.CreateTable) {
			...
			String sql = "";
			if (this.getCodeStruct() == com.kong.kflowweb.BP.Sys.CodeStruct.NoName
					|| this.getCodeStruct() == com.kong.kflowweb.BP.Sys.CodeStruct.GradeNoName) {
			...
			if (this.getCodeStruct() == com.kong.kflowweb.BP.Sys.CodeStruct.Tree) {
				sql = "CREATE TABLE " + this.getNo() + " (";
				sql += "No  varchar(30) NOT NULL,";
				sql += "Name varchar(3900)  NULL,";
	...
	protected void afterInsert() throws Exception {
		if (this.getSrcType() == com.kong.kflowweb.BP.Sys.SrcType.TableOrView) {
			...
			if (SystemConfig.getAppCenterDBType() == com.kong.kflowweb.BP.DA.DBType.Oracle
					|| SystemConfig.getAppCenterDBType() == com.kong.kflowweb.BP.DA.DBType.MySQL) {
				sql += "No,";
				sql += "Name";
			} else {
				sql += "[No],";
			...
			sql += (this.getCodeStruct() == com.kong.kflowweb.BP.Sys.CodeStruct.Tree ? ",[ParentNo])" : ")");
			sql += " AS ";
		...
		if (dt.Rows.size() == 0) {
			...
			if (this.getCodeStruct() == com.kong.kflowweb.BP.Sys.CodeStruct.Tree) {
				...
				for (int i = 1; i < 4; i++) {
					...
					sql = "INSERT INTO " + this.getSrcTable() + " (No,Name,ParentNo) VALUES('" + no + "','Item" + no
							+ "','1') ";
					this.RunSQL(sql);
				}
			}
	...
	public String GenerDataOfJson() throws Exception {
		return com.kong.kflowweb.BP.Tools.Json.ToJson(this.getGenerHisDataTable());
	}
}