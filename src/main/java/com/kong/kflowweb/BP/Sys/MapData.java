...
public class MapData extends EntityNoName
{
   ...
   public void setFormEventEntity(String value){
    ...
    public FormEventBase getHisFEB(){
        ...
        if (_HisFEB == null)
            _HisFEB = com.kong.kflowweb.BP.Sys.Glo.GetFormEventBaseByEnName(this.getNo());
	...
	*/
	public final void Upgrade() throws Exception
	{
		String sql = "";
			...
			///#region 升级ccform控件.
		if (com.kong.kflowweb.BP.DA.DBAccess.IsExitsObject("Sys_FrmLine") == true)
		{
			//重命名.
			com.kong.kflowweb.BP.Sys.SFDBSrc dbsrc = new SFDBSrc("local");
			...
			{
				com.kong.kflowweb.BP.Sys.FrmEle ele = new FrmEle();
				ele.Copy(dr);
				ele.setEleType(com.kong.kflowweb.BP.Sys.FrmEle.Line);

				if (ele.getIsExits() == true)
				{
					ele.setMyPK(com.kong.kflowweb.BP.DA.DBAccess.GenerGUID());
				}
				ele.Insert();
			}
		}
		if (com.kong.kflowweb.BP.DA.DBAccess.IsExitsObject("Sys_FrmLab") == true)
		{
			//重命名.
			com.kong.kflowweb.BP.Sys.SFDBSrc dbsrc = new SFDBSrc("local");
			...
			for (DataRow dr : dt.Rows)
			{
				com.kong.kflowweb.BP.Sys.FrmEle ele = new FrmEle();
				ele.Copy(dr);
				ele.setEleType(com.kong.kflowweb.BP.Sys.FrmEle.Label);
		...
		if (com.kong.kflowweb.BP.DA.DBAccess.IsExitsObject("Sys_FrmBtn") == true)
		{
			//重命名.
			com.kong.kflowweb.BP.Sys.SFDBSrc dbsrc = new SFDBSrc("local");
			dbsrc.Rename("Table", "Sys_FrmLab", "Sys_FrmLabBak", sql);
	...
	*/
    ...
    {
        com.kong.kflowweb.BP.DA.CashFrmTemplate.Remove(this.getNo());
        CleanObject();
    }
	public static boolean getIsEditDtlModel()
	{
		String s = com.kong.kflowweb.BP.Web.WebUser.GetSessionByKey("IsEditDtlModel", "0");
		if (s.equals("0"))
		{
			return false;
		}
	...
	{
		com.kong.kflowweb.BP.Web.WebUser.SetSessionByKey("IsEditDtlModel", "1");
	}
	...
	*/
	...
	public final Entities getHisEns()
	{
		if (_HisEns == null)
		{
			_HisEns = com.kong.kflowweb.BP.En.ClassFactory.GetEns(this.getNo());
		}
		return _HisEns;
	}
...
*/
	...
	*/
	public static Map GenerHisMap(String no) throws Exception
	{
		...
		{
			Map map = com.kong.kflowweb.BP.DA.Cash.GetMap(no);
			if (map == null)
			{
				MapData md = new MapData();
				md.setNo(no);
				md.Retrieve();
				map = md.GenerHisMap();
				com.kong.kflowweb.BP.DA.Cash.SetMap(no, map);
			}
			return map;
		}
	}
...
//ORIGINAL LINE: case "WF_Node":
	...
	*/
	public final void RepairMap() throws Exception
	{
		...
		{
			attr = new com.kong.kflowweb.BP.Sys.MapAttr();
			attr.setFK_MapData(this.getNo());
...
/*		//检查特殊UIBindkey丢失的问题.
	...
	*/
	...
	protected boolean beforeUpdateInsertAction() throws Exception
	{
		...
		if (getIsHaveCA() == true)
		{
			...
			// attr.HisEditType = com.kong.kflowweb.BP.En.EditType.UnDel;
			//attr.KeyOfEn = "SealData";
			// attr.Name = "SealData";
			// attr.MyDataType = com.kong.kflowweb.BP.DA.DataType.AppString;
	...
	*/
	...
	{
		String sql = "UPDATE Sys_MapData SET VER='" + com.kong.kflowweb.BP.DA.DataType.getCurrentDataTimess() + "' WHERE No='" + this.getNo() + "'";
	...
	*/
	...
	{
		byte[] by = com.kong.kflowweb.BP.DA.DBAccess.GetByteFromDB(this.getPTable(), this.getEnPK(), (new Integer(oid)).toString(), "DBFile");
		if (by != null)
		{
			bytes.argvalue = by;
			return true;
		...
		{
			String tempExcel = com.kong.kflowweb.BP.Sys.SystemConfig.getPathOfDataUser() + "FrmOfficeTemplate/" + this.getNo() + ".xlsx";
			File file = new File(tempExcel);
			if (file.exists() == true)
			{
				bytes.argvalue = com.kong.kflowweb.BP.DA.DataType.ConvertFileToByte(tempExcel);
				return false;
			}
			else //模板文件也不存在时
			{
	...
	*/
	...
	{
		com.kong.kflowweb.BP.DA.DBAccess.SaveFileToDB(bty.toString(), this.getPTable(), this.getEnPK(), (new Integer(oid)).toString(), "DBFile");
	}
