...
import com.kong.kflowweb.BP.WF.Template.FrmWorkCheckAttr;
...
public class Node extends Entity {
	...
	public final Work getHisWork() throws Exception {
		...
		if (this.getIsStartNode()) {
			obj = new com.kong.kflowweb.BP.WF.GEStartWork(this.getNodeID(), this.getNodeFrmID());
			obj.setHisNode(this);
			obj.setNodeID(this.getNodeID());
			return obj;
		}
		...
		if (this.getFormType() != NodeFormType.FoolTruck || this.WorkID == 0 || this.getIsStartNode() == true) {
			obj = new com.kong.kflowweb.BP.WF.GEWork(this.getNodeID(), this.getNodeFrmID());
			obj.setHisNode(this);
		...
		obj.clearSQLCash();
		com.kong.kflowweb.BP.DA.Cash.getSQL_Cash().remove("ND" + this.getNodeID());
	...
	public static String CheckFlow(Flow fl) throws Exception {
		...
		// 更新是否是有完成条件的节点。
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("UPDATE WF_Node SET IsCCFlow=0  WHERE FK_Flow='" + fl.getNo() + "'");
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM WF_Direction WHERE Node=0 OR ToNode=0");
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM WF_Direction WHERE Node  NOT IN (SELECT NODEID FROM WF_Node )");
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM WF_Direction WHERE ToNode  NOT IN (SELECT NODEID FROM WF_Node) ");
	 ...
	 * 审核组件
	 */
	public final com.kong.kflowweb.BP.WF.Template.FrmWorkCheckSta getFrmWorkCheckSta() {
		return FrmWorkCheckSta.forValue(this.GetValIntByKey(NodeAttr.FWCSta));
	}

	public final void setFrmWorkCheckSta(com.kong.kflowweb.BP.WF.Template.FrmWorkCheckSta value) {
	...
	public final String getFormUrl() {
		...
		str = str.replace("@SDKFromServHost", com.kong.kflowweb.BP.Sys.SystemConfig.getAppSettings().get("SDKFromServHost").toString());
		return str;
	}
	...
	/// 单节点退回规则
	/// </summary>
	public int getReturnOneNodeRole() {

		return this.GetValIntByKey(NodeAttr.ReturnOneNodeRole);
	}

	public void setReturnOneNodeRole(int value) {
	...
	public final boolean getIsEnableSMSMessage() {
		int i = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt("SELECT SMSEnable FROM Sys_FrmEvent WHERE FK_MapData='ND"
	...
	protected boolean beforeDelete() throws Exception {
		...
		if (this.getNodePosType() == NodePosType.End) {
			...
			for (GenerWorkFlow gwf : gwfs.ToJavaList()) {
				com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoFlowOver(gwf.getFK_Flow(), gwf.getWorkID(), "流程成功结束",1);
			}
		...
		// 删除分组.
		com.kong.kflowweb.BP.Sys.GroupFields gfs = new com.kong.kflowweb.BP.Sys.GroupFields();
		gfs.Delete(com.kong.kflowweb.BP.Sys.GroupFieldAttr.EnName, md.getNo());

		try {
			// 删除它的明细。
			com.kong.kflowweb.BP.Sys.MapDtls dtls = new com.kong.kflowweb.BP.Sys.MapDtls(md.getNo());
			dtls.Delete();
		} catch (Exception e) {
		...
		// 删除框架
		try {
			com.kong.kflowweb.BP.Sys.MapFrames frams = new com.kong.kflowweb.BP.Sys.MapFrames(md.getNo());
			frams.Delete();
		} catch (Exception e) {
		...
		// 删除扩展
		try {
			com.kong.kflowweb.BP.Sys.MapExts exts = new com.kong.kflowweb.BP.Sys.MapExts(md.getNo());
			exts.Delete();
		} catch (Exception e) {
		...
		// 删除节点与岗位的对应.
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM WF_NodeStation WHERE FK_Node=" + this.getNodeID());
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM WF_NodeEmp  WHERE FK_Node=" + this.getNodeID());
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM WF_NodeDept WHERE FK_Node=" + this.getNodeID());
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM WF_FrmNode  WHERE FK_Node=" + this.getNodeID());
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM WF_CCEmp  WHERE FK_Node=" + this.getNodeID());
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM WF_CH  WHERE FK_Node=" + this.getNodeID());

		// 删除附件.
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM Sys_FrmAttachment  WHERE FK_MapData='" + this.getNodeID() + "'");
	 ...
	 * @throws Exception
	 */
	private void AddDocAttr(com.kong.kflowweb.BP.Sys.MapData md) throws Exception {
		// 如果是单据流程？
		com.kong.kflowweb.BP.Sys.MapAttr attr = new com.kong.kflowweb.BP.Sys.MapAttr();

		attr = new MapAttr();
		attr.setFK_MapData(md.getNo());
		attr.setKeyOfEn("KeyWord");
		attr.setName("主题词");
		attr.setMyDataType(com.kong.kflowweb.BP.DA.DataType.AppString);
		...
		attr.setMaxLen(300);
		attr.setIdx(-99);
		attr.setHisEditType(com.kong.kflowweb.BP.En.EditType.UnDel);
		attr.Insert();
	...
	public final String RepareMap() throws Exception {
		...
		if (attr.IsExit(MapAttrAttr.KeyOfEn, WorkAttr.RDT, MapAttrAttr.FK_MapData, md.getNo()) == false) {
			...
			attr.setMyDataType(com.kong.kflowweb.BP.DA.DataType.AppDateTime);
			attr.setUIContralType(UIContralType.TB);
			attr.setLGType(FieldTypeS.Normal);
			attr.setUIVisible(false);
			attr.setUIIsEnable(false);
	...
	public final void CreateMap() throws Exception {
		...
		if (this.getNodePosType() == NodePosType.Start) {
			...
			attr = new MapAttr();
			attr.setFK_MapData(md.getNo());
			attr.setHisEditType(com.kong.kflowweb.BP.En.EditType.Edit);
			// attr.setedit
