...
public class FullSA
{
	...
	*/
	public FullSA(WorkNode currWorkNode) throws Exception
	{
		...
		//求出已经路过的节点.
		DataTable dt = DBAccess.RunSQLReturnTable("SELECT FK_Node FROM WF_GenerWorkerList WHERE WorkID="+com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBVarStr()+"WorkID", "WorkID", workid);
		...
		for (Node item : nds.ToJavaList())
		{
			...
			{
				String sql = "SELECT No, Name FROM Port_Emp WHERE No IN (SELECT A.FK_Emp FROM " + com.kong.kflowweb.BP.WF.Glo.getEmpStation() + " A, WF_NodeStation B WHERE A.FK_Station=B.FK_Station AND B.FK_Node=" + item.getNodeID() + ")";
            ...
            if (item.getHisDeliveryWay() == DeliveryWay.ByDept)
            {
                ...
                if (dt.Rows.size() == 0)
                {
                    if (item.getHisFlow().getHisFlowAppType() == FlowAppType.Normal)
                    {
                        ...
                        else if (com.kong.kflowweb.BP.WF.Glo.getOSModel() == OSModel.OneMore)
                        {
			...
			if (item.getHisDeliveryWay() == DeliveryWay.ByStation)
			{
				...
				if (this.HisCurrWorkNode.getHisFlow().getFlowAppType() == FlowAppType.Normal)
				{
					switch (com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBType())
					{
						case MySQL:
						case MSSQL:
							sql = "select X.No from Port_Emp x inner join (select FK_Emp from " + com.kong.kflowweb.BP.WF.Glo.getEmpStation() + " a inner join WF_NodeStation b ";
							...
							break;
						default:
							sql = "SELECT No FROM Port_Emp WHERE NO IN " + "(SELECT  FK_Emp  FROM " + com.kong.kflowweb.BP.WF.Glo.getEmpStation() + " WHERE FK_Station IN (SELECT FK_Station FROM WF_NodeStation WHERE FK_Node=" + dbStr + "FK_Node) )" + " AND  NO IN " + "(SELECT No FROM Port_Emp WHERE FK_Dept =" + dbStr + "FK_Dept)";
							sql += " ORDER BY No ";
							break;
					}
