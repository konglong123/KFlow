...
import com.kong.kflowweb.BP.Sys.FrmWorkCheckSta;
import com.kong.kflowweb.BP.WF.Port.WFEmp;
import com.kong.kflowweb.BP.WF.Template.FWCOrderModel;
...
import com.kong.kflowweb.BP.WF.Template.NodeCancels;
...
public class WorkUnSend
{
	...
	*/
	private String DoThreadUnSend() throws Exception
	{
		...
		if (nd.getHisCancelRole() == CancelRole.OnlyNextStep)
		{
			...
			int num = wl.Retrieve(GenerWorkerListAttr.FK_Emp, com.kong.kflowweb.BP.Web.WebUser.getNo(), GenerWorkerListAttr.FK_Node, wnPri.getHisNode().getNodeID());
			if (num == 0)
			{
		...
		Node cancelToNode = new Node(cancelToNodeID);
		 switch(cancelToNode.getHisNodeWorkType())
         {
             ...
             case WorkFL:
                ...
                com.kong.kflowweb.BP.WF.Dev2Interface.Node_FHL_KillSubFlow(cancelToNode.getFK_Flow(), this.FID, this.WorkID); //杀掉子线程.
                // 调用撤消发送前事件。
		...
		if (this.getHisFlow().getHisDataStoreModel() == com.kong.kflowweb.BP.WF.Template.DataStoreModel.ByCCFlow)
		{
		...
		{
			//设置全部的人员不可用。
			com.kong.kflowweb.BP.DA.DBAccess.RunSQL("UPDATE WF_GenerWorkerlist SET IsPass=0,  IsEnable=-1 WHERE WorkID=" + this.WorkID + " AND FK_Node=" + gwf.getFK_Node());

			//设置当前人员可用。
			com.kong.kflowweb.BP.DA.DBAccess.RunSQL("UPDATE WF_GenerWorkerlist SET IsPass=0,  IsEnable=1  WHERE WorkID=" + this.WorkID + " AND FK_Node=" + gwf.getFK_Node() + " AND FK_Emp='" + WebUser.getNo() + "'");
		}
		else
		{
			com.kong.kflowweb.BP.DA.DBAccess.RunSQL("UPDATE WF_GenerWorkerlist SET IsPass=0  WHERE WorkID=" + this.WorkID + " AND FK_Node=" + gwf.getFK_Node());
		}

		//更新当前节点，到rpt里面。
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("UPDATE " + this.getHisFlow().getPTable() + " SET FlowEndNode=" + gwf.getFK_Node() + " WHERE OID=" + this.WorkID);
		...
		if (cancelToNode.getIsStartNode() == false && cancelToNode.getIsEnableTaskPool() == false)
		{
			...
			// com.kong.kflowweb.BP.DA.DBAccess.RunSQL("UPDATE WF_GenerWorkerList SET IsPass=0 WHERE FK_Node=" + backtoNodeID + " AND WorkID=" + this.WorkID);
	...
	*/
	public final String DoUnSend() throws Exception
	{
        ...
        if (this.UnSendToNode == nd.getNodeID()){
	            ...
	            || nd.getHisNodeWorkType() == NodeWorkType.WorkFHL)
	        {
	            ...
	            for(DataRow  dr : dt.Rows)
	            {
	                ...
	                com.kong.kflowweb.BP.WF.Dev2Interface.Node_FHL_KillSubFlow(threadnd.getFK_Flow(), this.WorkID, Long.parseLong(dr.getValue("WorkID").toString())); //杀掉子线程.
        ...
        if (com.kong.kflowweb.BP.WF.Dev2Interface.Flow_IsCanDoCurrentWork( this.WorkID, WebUser.getNo()) == true)
		...
		if (gwf.getHuiQianTaskSta() != HuiQianTaskSta.None)
		{
			...
			{
				gwf.setTodoEmps( WebUser.getNo() + "," + com.kong.kflowweb.BP.Web.WebUser.getName() + ";" + gwf.getTodoEmps());

			}
			else
			{
				gwf.setTodoEmps( gwf.getTodoEmps() + com.kong.kflowweb.BP.Web.WebUser.getName() + ";");
			}
	   ...
	   ///#region 求的撤销的节点.
		...
		if (nd.getHisCancelRole() == CancelRole.OnlyNextStep)
		{
			...
			GenerWorkerList wl = new GenerWorkerList();
			int num = wl.Retrieve(GenerWorkerListAttr.FK_Emp, com.kong.kflowweb.BP.Web.WebUser.getNo(), GenerWorkerListAttr.FK_Node, wnPri.getHisNode().getNodeID(),GenerWorkerListAttr.WorkID,this.WorkID);
			if (num == 0)
			{
				throw new RuntimeException("@您不能执行撤消发送，因为当前工作不是您发送的或下一步工作已处理。");
			}
             ...
             && nd.getHisRunModel() == RunModel.FL)
        {
            /* 检查一下是否还有没有完成的子线程，如果有就抛出不允许撤销的异常。 */
              ...
              if (com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt(sql) != 0)
        ...
        {
            com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM ND" + Integer.parseInt(nd.getFK_Flow()) + "Track WHERE WorkID = " + this.WorkID +
	...
	private String DoUnSendFeiLiu(GenerWorkFlow gwf) throws Exception
	{
		...
		for (Node ndNext : nd.getHisToNodes().ToJavaList())
		{
			...
			if (ndNext.getHisRunModel() == RunModel.SubThread)
			{
				...
				{
					com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DeleteSubThread(gwf.getFK_Flow(), en.getWorkID(), "合流节点撤销发送前，删除子线程.");
				}
				continue;
			}
		...
		//设置当前节点。
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("UPDATE WF_GenerWorkerlist SET IsPass=0 WHERE WorkID=" + this.WorkID + " AND FK_Node=" + gwf.getFK_Node() + " AND IsPass=1");
		...
		else
		{
			...
			if (com.kong.kflowweb.BP.Web.WebUser.getIsWap() == false)
			{
				if (!this.getHisFlow().getFK_FlowSort().equals("00"))
				{
					return "@撤消执行成功." + msg;
	...
	*/
	private String DoUnSendInFeiLiuHeiliu(GenerWorkFlow gwf) throws Exception
	{
		...
		gwl.setIsPassInt(0);
		gwl.setIsRead(false);
		gwl.setRDT(com.kong.kflowweb.BP.DA.DataType.getCurrentDataTime());
		gwl.setSDT(com.kong.kflowweb.BP.DA.DataType.getCurrentDataTime()); //这里计算时间有问题.
		gwl.Update();
		...
		gwf.setSender(com.kong.kflowweb.BP.Web.WebUser.getNo());
		gwf.setSendDT(com.kong.kflowweb.BP.DA.DataType.getCurrentDataTime());
		gwf.Update();
	...
	*/
	private String DoUnSendFeiLiu_bak(GenerWorkFlow gwf) throws Exception
	{
		...
		String sql = "SELECT FK_Node FROM WF_GenerWorkerList WHERE WorkID=" + this.WorkID + " AND FK_Emp='" + com.kong.kflowweb.BP.Web.WebUser.getNo() + "' AND FK_Node=" + gwf.getFK_Node();
		DataTable dt = DBAccess.RunSQLReturnTable(sql);
		if (dt.Rows.size() == 0)
		{
	...
	*/
	public final String DoUnSendHeiLiu_Main(GenerWorkFlow gwf) throws Exception
	{
		...
		GenerWorkerList wl = new GenerWorkerList();
		int i = wl.Retrieve(GenerWorkerListAttr.FK_Node, priFLNode.getNodeID(), GenerWorkerListAttr.FK_Emp, com.kong.kflowweb.BP.Web.WebUser.getNo());
		if (i == 0)
		{
...
///#warning
	...
	public final String DoUnSendSubFlow(GenerWorkFlow gwf) throws Exception
	{
		...
		{
			java.math.BigDecimal all = new java.math.BigDecimal(com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt("SELECT COUNT(*) NUM FROM dbo.WF_GenerWorkerList WHERE FID=" + this.FID + " AND FK_Node=" + wnPri.getHisNode().getNodeID()));
			java.math.BigDecimal ok = new java.math.BigDecimal(com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt("SELECT COUNT(*) NUM FROM dbo.WF_GenerWorkerList WHERE FID=" + this.FID + " AND IsPass=1 AND FK_Node=" + wnPri.getHisNode().getNodeID()));
