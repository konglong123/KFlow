...
public class PushMsg extends EntityMyPK
{
	...
	*/
	public final String DoSendMessage(Node currNode, Entity en, String atPara, SendReturnObjs objs, Node jumpToNode, String jumpToEmps) throws Exception
	{
		...
		if (en.getRow().containsKey("Title") == true)
		{
			...
			{
				title = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnStringIsNull("SELECT Title FROM WF_GenerWorkFlow WHERE WorkID=" + en.getPKVal(), "标题");
			}
		}
		else
		{
		...
		//生成URL.
		String hostUrl = com.kong.kflowweb.BP.WF.Glo.getHostURL();
							   ...
							   String openUrl, SendReturnObjs objs,Row r) throws Exception
	{
		...
		if (smsDoc.contains("@") == true)
			smsDoc = com.kong.kflowweb.BP.WF.Glo.DealExp(smsDoc, en, null);

		if (this.getFK_Event() == com.kong.kflowweb.BP.Sys.EventListOfNode.ReturnAfter)
		{
			//获取退回原因
			Paras ps = new Paras();
			ps.SQL = "SELECT BeiZhu,ReturnerName,IsBackTracking FROM WF_ReturnWork WHERE WorkID=" + com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBVarStr() + "WorkID  ORDER BY RDT DESC";
		...
		if (this.getSMSPushWay() == 2)
		{
			...
			//发送消息
			com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendMessage(toEmp, smsDoc, mailTitle, this.getFK_Event(), "WKAlt" + currNode.getNodeID() + "_" + workid, com.kong.kflowweb.BP.Web.WebUser.getNo(), openUrl, this.getSMSPushModel(),null,null);
		...
		if (this.getSMSPushWay() == 3)
		{
			...
			for(String nodeID : nodes)
			{
				...
				for(DataRow dr : dt.Rows)
				{
					...
					//发送消息
					com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendMessage(empNo, smsDoc, mailTitle, this.getFK_Event(), "WKAlt" + currNode.getNodeID() + "_" + workid, com.kong.kflowweb.BP.Web.WebUser.getNo(), openUrl, this.getSMSPushModel(),null,null);
					//处理短消息.
					toEmpIDs += empName + ",";
				}
			}
		...
		if(this.getSMSPushWay() == 4)
		{
			...
			if (bySQL.contains("@") == true)
				bySQL = com.kong.kflowweb.BP.WF.Glo.DealExp(bySQL, en, null);
		...
		{
			//工作到达、退回、移交、撤销
			if ((this.getFK_Event().equals(com.kong.kflowweb.BP.Sys.EventListOfNode.WorkArrive)
					|| this.getFK_Event().equals(com.kong.kflowweb.BP.Sys.EventListOfNode.ReturnAfter)
					|| this.getFK_Event().equals(com.kong.kflowweb.BP.Sys.EventListOfNode.ShitAfter)
					|| this.getFK_Event().equals(com.kong.kflowweb.BP.Sys.EventListOfNode.UndoneAfter))
			...
			//begin 节点发送成功后
			if (this.getFK_Event().equals(com.kong.kflowweb.BP.Sys.EventListOfNode.SendSuccess) && objs.getVarAcceptersID() != null)
			{
				/*如果向接受人发送消息.*/
				...
				for(String empNo : emps)
				{
					...
					com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendMessage(empNo, smsDoc, mailTitle, this.getFK_Event(), "WKAlt" + currNode.getNodeID() + "_" + workid, com.kong.kflowweb.BP.Web.WebUser.getNo(), openUrl, this.getSMSPushModel(), paras,null);
			...
			//begin 流程结束后、流程删除后
			if (this.getFK_Event().equals(com.kong.kflowweb.BP.Sys.EventListOfNode.FlowOverAfter)
					|| this.getFK_Event().equals(com.kong.kflowweb.BP.Sys.EventListOfNode.AfterFlowDel))
			{
				/*向所有参与人发送消息. */
			...
			//begin 节点预警、逾期
			if(this.getFK_Event().equals(com.kong.kflowweb.BP.Sys.EventListOfNode.NodeWarning)
					|| this.getFK_Event().equals(com.kong.kflowweb.BP.Sys.EventListOfNode.NodeOverDue))
			{
				//获取当前节点的接收人
				...
				for(String emp : emps)
				{
					...
					String paras = "@FK_Flow=" + currNode.getFK_Flow() + "&FK_Node=" + currNode.getNodeID() + "@WorkID=" + workid;
					com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendMessage(empA[0], smsDoc, mailTitle, this.getFK_Event(), "WKAlt" + currNode.getNodeID() + "_" + workid, com.kong.kflowweb.BP.Web.WebUser.getNo(), openUrl, this.getSMSPushModel(), paras,null);
				}
			}
			//end 节点预警、逾期
	...
	*/
	private String SendEmail(String title, String openWorkURl, Entity en, String jumpToEmps, Node currNode, long workid, SendReturnObjs objs, Row r) throws Exception
	{
		...
		{
			mailDocTmp = com.kong.kflowweb.BP.WF.Glo.DealExp(mailDocTmp, en, null);
		}
		...
		if (this.getMailPushWay() == 3)
		{
			...
			for (String nodeID : nodes)
			{
				...
				for (DataRow dr : dt.Rows)
				{
					...
					String paras = "@FK_Flow=" + currNode.getFK_Flow() + "&FK_Node=" + currNode.getNodeID() + "@WorkID=" + workid;
					//发送邮件
					com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendEmail(emailAddress, mailTitleTmp, mailDocTmp, this.getFK_Event(), this.getFK_Event() + workid, com.kong.kflowweb.BP.Web.WebUser.getNo(), null, empNo, paras);
		...
		if (this.getFK_Event().equals(com.kong.kflowweb.BP.Sys.EventListOfNode.WorkArrive) || this.getFK_Event().equals(com.kong.kflowweb.BP.Sys.EventListOfNode.ReturnAfter))
		{
			//工作到达.
			if (this.getMailPushWay() == 1)
			{
				...
				for (String emp : emps)
				{
					...
					//获得当前人的邮件.
					com.kong.kflowweb.BP.WF.Port.WFEmp empEn = new WFEmp(emp);

					//发送邮件.
					com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendEmail(empEn.getEmail(), mailTitleTmp, mailDocReal, this.getFK_Event(), "WKAlt" + currNode.getNodeID() + "_" + workid, com.kong.kflowweb.BP.Web.WebUser.getNo(), null, emp,null);
				}
			...
			if (this.getMailPushWay() == 2)
			{
				...
				//发送邮件
				com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendEmail(emailAddress, mailTitleTmp, mailDocTmp, this.getFK_Event(), "WKAlt" + currNode.getNodeID() + "_" + workid, com.kong.kflowweb.BP.Web.WebUser.getNo(), null, null,null);
				return "@已向:{" + emailAddress + "}发送提醒邮件.";
			}
		}
			///#region SendSuccess - 发送成功事件. - 邮件处理.
		if (this.getFK_Event().equals(com.kong.kflowweb.BP.Sys.EventListOfNode.SendSuccess))
		{
			//发送成功事件.
			if (this.getMailPushWay() == 1 && objs.getVarAcceptersID() != null)
			{
				...
				for (String emp : emps)
				{
					...
					//发送邮件.
					com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendEmail(empEn.getEmail(), mailTitleTmp, mailDocReal, this.getFK_Event(), "WKAlt" + objs.getVarToNodeID() + "_" + workid, com.kong.kflowweb.BP.Web.WebUser.getNo(), null, emp, paras);
				}
				return "@已向:{" + toEmpIDs + "}发送提醒邮件.";
			}
			...
			if (this.getMailPushWay() == 2)
			{
				...
				//发送邮件
				com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendEmail(emailAddress, mailTitleTmp, mailDocTmp, this.getFK_Event(), "WKAlt" + objs.getVarToNodeID() + "_" + workid, com.kong.kflowweb.BP.Web.WebUser.getNo(), null, null, paras);
		...
		if (this.getFK_Event().equals(com.kong.kflowweb.BP.Sys.EventListOfNode.FlowOverAfter))
		{
			...
			if (this.getMailPushWay() == 1)
			{
				...
				for (String emp : emps)
				{
					...
					//发送邮件.
					com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendEmail(empEn.getEmail(), mailTitleTmp, mailDocReal, this.getFK_Event(), "FlowOver" + workid, com.kong.kflowweb.BP.Web.WebUser.getNo(), null, emp, paras);
				}
				return "@已向:{" + toEmpIDs + "}发送提醒邮件.";
			}
			...
			if (this.getMailPushWay() == 2)
			{
				...
				//发送邮件
				com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendEmail(emailAddress, mailTitleTmp, mailDocTmp, this.getFK_Event(), "FlowOver" + workid, com.kong.kflowweb.BP.Web.WebUser.getNo(), null, null, paras);
				return "@已向:{" + emailAddress + "}发送提醒邮件.";
			}
		}
	...
	*/
	private String SendShortMessageToSpecNodes(String title, String openWorkURl, Entity en, String jumpToEmps, Node currNode, long workid, SendReturnObjs objs, Row r) throws Exception
	{
		...
		{
			smsDocTmp = com.kong.kflowweb.BP.WF.Glo.DealExp(smsDocTmp, en, null);
		}
		...
		if (this.getSMSPushWay() == 3)
		{
			...
			for (String nodeID : nodes)
			{
				...
				for (DataRow dr : dt.Rows)
				{
					...
					//发送邮件.
					com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendSMS(tel, mailDocReal, this.getFK_Event(), "WKAlt" + currNode.getNodeID() + "_" + workid, WebUser.getNo(), null, empNo, paras,null);
		...
		if (this.getFK_Event().equals(com.kong.kflowweb.BP.Sys.EventListOfNode.WorkArrive) || this.getFK_Event().equals(com.kong.kflowweb.BP.Sys.EventListOfNode.ReturnAfter))
		{
			...
			if (this.getSMSPushWay() == 1)
			{
				...
				for (String emp : emps)
				{
					...
					//发送短信.
					Dev2Interface.Port_SendSMS(empEn.getTel(), smsDocTmpReal, this.getFK_Event(), "WKAlt" + currNode.getNodeID() + "_" + workid, com.kong.kflowweb.BP.Web.WebUser.getNo(), null, emp, null,null);
				}
			...
			if (this.getSMSPushWay() == 2)
			{
				...
				//发送短信.
				String paras = "@FK_Flow=" + currNode.getFK_Flow() + "@WorkID=" + workid + "@FK_Node=" + currNode.getNodeID();
				com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendSMS(tel, smsDocTmp, this.getFK_Event(), "WKAlt" + currNode.getNodeID() + "_" + workid, com.kong.kflowweb.BP.Web.WebUser.getNo(), null, paras,null,null);
		...
		if (this.getFK_Event().equals(com.kong.kflowweb.BP.Sys.EventListOfNode.SendSuccess))
		{
			...
			if (this.getSMSPushWay() == 1 && objs.getVarAcceptersID() != null)
			{
				...
				for (String empID : emps)
				{
					...
					com.kong.kflowweb.BP.WF.Port.WFEmp empEn = new WFEmp(empID);

					String paras = "@FK_Flow=" + currNode.getFK_Flow() + "@WorkID=" + workid + "@FK_Node=" + currNode.getNodeID();

					//发送短信.
					Dev2Interface.Port_SendSMS(empEn.getTel(), smsDocTmpReal, this.getFK_Event(), "WKAlt" + objs.getVarToNodeID() + "_" + workid, com.kong.kflowweb.BP.Web.WebUser.getNo(), null, empID, paras,null);
				}
			...
			if (this.getSMSPushWay() == 2)
			{
				...
				if (tel != null || tel.length() > 6)
				{
					...
					//发送短信.
					com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendSMS(tel, smsDocTmp, this.getFK_Event(), "WKAlt" + objs.getVarToNodeID() + "_" + workid, com.kong.kflowweb.BP.Web.WebUser.getNo(), null, paras,null,null);
		...
		if (this.getFK_Event().equals(com.kong.kflowweb.BP.Sys.EventListOfNode.FlowOverAfter))
		{
			...
			if (this.getSMSPushWay() == 1)
			{
				...
				for (String empID : emps)
				{
					...
					com.kong.kflowweb.BP.WF.Port.WFEmp empEn = new WFEmp();
					empEn.setNo(empID);
					...
					//发送短信.
					Dev2Interface.Port_SendSMS(empEn.getTel(), smsDocTmpReal, this.getFK_Event(), "FlowOver" + workid, com.kong.kflowweb.BP.Web.WebUser.getNo(), null, empID, paras,null);
				}
				return "@已向:{" + toEmpIDs + "}发送提醒手机短信.";
			}
			...
			if (this.getSMSPushWay() == 2)
			{
				...
				if (tel != null || tel.length() > 6)
				{
					String paras = "@FK_Flow=" + currNode.getFK_Flow() + "@WorkID=" + workid + "@FK_Node=" + currNode.getNodeID();
					//发送短信.
					com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendSMS(tel, smsDocTmp, this.getFK_Event(), "FlowOver"+ workid, com.kong.kflowweb.BP.Web.WebUser.getNo(), null, paras,null,null);
					return "@已向:{" + tel + "}发送提醒手机短信.";
				}
			}
		}
