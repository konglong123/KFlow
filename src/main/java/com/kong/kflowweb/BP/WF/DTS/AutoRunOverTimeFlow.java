...
public class AutoRunOverTimeFlow extends Method{
    ...
    /// 不带有参数的方法
    /// </summary>
    public void AutoRunOverTimeFlow()
    {
        this.Title = "处理逾期的任务";
       ...
       // #endregion 找到要逾期的数据.
        ...
        for (DataRow row : generTab.Rows)
        {
            ...
            for (GenerWorkerList item : gwls.ToJavaList())
            {
                ...
                com.kong.kflowweb.BP.Port.Emp emp = new Emp(item.getFK_Emp());
            ...
            if (isLogin == false)
            {
                com.kong.kflowweb.BP.Port.Emp emp = new Emp("admin");
                com.kong.kflowweb.BP.Web.WebUser.SignInOfGener(emp);
            }
                ...
                switch (node.getHisOutTimeDeal())
                {
                    ...
                    case AutoJumpToSpecNode: //跳转到指定的节点.
                        try
                        {
                            ...
                            //设置默认同意.
                            com.kong.kflowweb.BP.WF.Dev2Interface.WriteTrackWorkCheck(jumpToNode.getFK_Flow(), node.getNodeID(), workid, 0,
                                "同意（预期自动审批）", null);

                            //执行发送.
                            info = com.kong.kflowweb.BP.WF.Dev2Interface.Node_SendWork(fk_flow, workid, null, null, jumpToNode.getNodeID(), null).ToMsgOfText();

                            // info = com.kong.kflowweb.BP.WF.Dev2Interface.Flow_Schedule(workid, jumpToNode.NodeID, emp.No);
                            msg = "流程 '" + node.getFlowName() + "',标题: '" + title + "'的应该完成时间为'" + compleateTime + "',当前节点'" + node.getEnName() +
                                  "'超时处理规则为'自动跳转'," + info;


                            SetText(msg);
                            com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLine(LogType.Info, msg);

                        }
                        catch (Exception ex)
                        {
                            msg = "流程 '" + node.getFlowName() + "',WorkID=" + workid + ",标题: '" + title + "'的应该完成时间为'" + compleateTime + "',当前节点'" + node.getEnName() +
                                  "'超时处理规则为'自动跳转',跳转异常:" + ex.getMessage();
                            SetText(msg);
                            com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLine(LogType.Error, msg);
                        }
                        break;
                    case AutoShiftToSpecUser: //走动移交给.
                        ...
                        try
                        {
                            com.kong.kflowweb.BP.WF.Dev2Interface.Node_Shift(fk_flow, fk_node, workid, 0, empShift.getNo(),
                                "流程节点已经逾期,系统自动移交");
                    ...
                    case AutoTurntoNextStep:
                        try
                        {
                            ...
                            com.kong.kflowweb.BP.Web.WebUser.SignInOfGener(workerList.getHisEmp());
                        ...
                        break;
                    case DeleteFlow:
                        info = com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoDeleteFlowByReal(fk_flow, workid, true);
                        msg = "流程  '" + node.getFlowName() + "',标题: '" + title + "'的应该完成时间为'" + compleateTime + "',当前节点'" + node.getName() +
                              "'超时处理规则为'删除流程'," + info;
                        SetText(msg);
                        com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLine(LogType.Info, msg);
                    ...
                    case RunSQL:
                        try
                        {
                            com.kong.kflowweb.BP.WF.Work wk = node.getHisWork();
                            wk.setOID(workid);
                            wk.Retrieve();

                            doOutTime = com.kong.kflowweb.BP.WF.Glo.DealExp(doOutTime, wk, null);
                    ...
                    case SendMsgToSpecUser:
                        try
                        {
                            ...
                            boolean boo = com.kong.kflowweb.BP.WF.Dev2Interface.WriteToSMS(myemp.getNo(), DateUtils.getCurrentDate("yyyy-MM-dd HH:mm:ss"), "系统发送逾期消息",
            ...
            catch (Exception ex)
            {
                ...
                com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLine(LogType.Error, ex.toString());
