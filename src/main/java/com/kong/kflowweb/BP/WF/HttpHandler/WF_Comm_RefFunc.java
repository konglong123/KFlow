...
import com.kong.kflowweb.BP.GPM.StationType;
import com.kong.kflowweb.BP.Sys.OSModel;
import com.kong.kflowweb.BP.Tools.StringHelper;
import com.kong.kflowweb.BP.WF.Glo;
import com.kong.kflowweb.BP.WF.Comm.JsonResultEmp;
import com.kong.kflowweb.BP.WF.Comm.JsonResultInnerData;
import com.kong.kflowweb.BP.WF.Comm.JsonResultStation;
import com.kong.kflowweb.BP.WF.Template.NodeDept;
import com.kong.kflowweb.BP.WF.Template.NodeDeptAttr;
...
public class WF_Comm_RefFunc extends WebContralBase {
    ...
    public String Dot2DotTreeDeptEmpModel_SaveNodeEmps() throws Exception
    {
        ...
        if (StringHelper.isNullOrEmpty(data))
            data = "";
        com.kong.kflowweb.BP.WF.Template.NodeEmps nemps = new com.kong.kflowweb.BP.WF.Template.NodeEmps();
        String[] empNos = data.split(",");

        //提交内容过长时，采用分段式提交
        if (StringHelper.isNullOrEmpty(partno))
        {
            nemps.Delete(com.kong.kflowweb.BP.WF.Template.NodeEmpAttr.FK_Node, nid);
        }
        else
        {
        ...
        com.kong.kflowweb.BP.WF.Template.NodeEmp nemp = null;
        ...
        for(String empNo : empNos)
        {
            ...
            nemp = new com.kong.kflowweb.BP.WF.Template.NodeEmp();
            nemp.setFK_Node(nid);
        ...
        return transction(com.kong.kflowweb.BP.Tools.Json.ToJson(jre),msg);
    }
    ...
    public String Dot2DotTreeDeptEmpModel_GetNodeEmps()
    {
        ...
        {
        	if (Glo.Plant == com.kong.kflowweb.BP.WF.Plant.JFlow&&(DBAccess.getAppCenterDBType() == DBType.Oracle))
        		row.setValue("Code", com.kong.kflowweb.BP.Tools.chs2py.ConvertStr2Code(row.getValue("NAME").toString()));
    		else
        		row.setValue("Code", com.kong.kflowweb.BP.Tools.chs2py.ConvertStr2Code(row.getValue("Name").toString()));
        ...
        //jr.setInnerData(dt);
        return transction(com.kong.kflowweb.BP.Tools.Json.ToJson(dt),"");
    }
    ...
    public String Dot2DotTreeDeptModel_SaveNodeDepts() throws Exception
    {
        ...
        com.kong.kflowweb.BP.WF.Template.NodeDepts ndepts = new com.kong.kflowweb.BP.WF.Template.NodeDepts();
        String[] deptNos = data.split("\\|");

        //提交内容过长时，采用分段式提交
        if (StringHelper.isNullOrEmpty(partno))
        {
            ndepts.Delete(com.kong.kflowweb.BP.WF.Template.NodeDeptAttr.FK_Node, nid);
        }
        else
        {
        ...
        com.kong.kflowweb.BP.WF.Template.NodeDept nemp = null;

        for(String deptNo : deptNos)
        {
            if (dtDepts.selectx("No="+deptNo).size() + dtDepts.selectx("NO="+deptNo).size() == 0)
                continue;
            nemp = new com.kong.kflowweb.BP.WF.Template.NodeDept();
            nemp.setFK_Node(nid);
       ...
       // return Newtonsoft.Json.JsonConvert.SerializeObject(jr);
        return com.kong.kflowweb.BP.Tools.Json.ToJson(jr);
    }
    ...
    public String Dot2DotTreeDeptModel_GetNodeDepts()
    {
        ...
        String re = com.kong.kflowweb.BP.Tools.Json.ToJson(jr);
        if(Glo.Plant == com.kong.kflowweb.BP.WF.Plant.JFlow){
...
//			int.TryParse(nodeid, out nid)
        ...
        com.kong.kflowweb.BP.WF.Template.NodeStations nsts = new com.kong.kflowweb.BP.WF.Template.NodeStations();
        String[] stNos = data.split("\\|");

        //提交内容过长时，采用分段式提交
        if (StringHelper.isNullOrEmpty(partno))
        {
            nsts.Delete(com.kong.kflowweb.BP.WF.Template.NodeStationAttr.FK_Node, nid);
        }
        else
        {
            String[] parts = partno.split("/",0);
        ...
        com.kong.kflowweb.BP.WF.Template.NodeStation nst = null;

        for(String stNo : stNos)
        {
            if (dtSts.selectx("No="+stNo).size() + dtSts.selectx("NO="+stNo).size() == 0)
                continue;
            nst = new com.kong.kflowweb.BP.WF.Template.NodeStation();
            nst.setFK_Node(nid);
