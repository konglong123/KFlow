...
import com.kong.kflowweb.BP.En.EntityTree;
...
public class WF_MyFlow extends WebContralBase {
	...
	public String SaveFlow_ToDraftRole() throws Exception
	{
		...
		// 获取表单树的数据
		com.kong.kflowweb.BP.WF.WorkNode workNode = new WorkNode(this.getWorkID(), this.getFK_Node());
		...
		if (nd.getIsStartNode() == true )
		{
			...
			//修改RPT表的标题
			wk.SetValByKey(com.kong.kflowweb.BP.WF.Data.GERptAttr.Title, title);
			wk.Update();
	 ...
	 * @return
	 */
	private com.kong.kflowweb.BP.WF.Template.FlowFormTrees appFlowFormTree = new FlowFormTrees();

	public final String FlowFormTree_Init() throws Exception {
		// add root
		com.kong.kflowweb.BP.WF.Template.FlowFormTree root = new com.kong.kflowweb.BP.WF.Template.FlowFormTree();
		root.setNo("00");
		root.setParentNo("0");
		root.setName("目录");
			...
			for (MapData md : mds.ToJavaList()) {
				...
				for (SysFormTree formTree : formTrees.ToJavaList()) {
					...
					if (appFlowFormTree.Contains("No", formTree.getNo()) == false) {
						com.kong.kflowweb.BP.WF.Template.FlowFormTree nodeFolder = new com.kong.kflowweb.BP.WF.Template.FlowFormTree();
				...
				com.kong.kflowweb.BP.WF.Template.FlowFormTree nodeForm = new com.kong.kflowweb.BP.WF.Template.FlowFormTree();
		...
		for (NodeToolbar item : extToolBars.ToJavaList()) {
			...
			com.kong.kflowweb.BP.WF.Template.FlowFormTree formTree = new com.kong.kflowweb.BP.WF.Template.FlowFormTree();
	...
	private void AppendFolder(SysFormTrees formTrees) {
		com.kong.kflowweb.BP.WF.Template.FlowFormTrees parentFolders = new com.kong.kflowweb.BP.WF.Template.FlowFormTrees();
		// 二级目录
		for (com.kong.kflowweb.BP.WF.Template.FlowFormTree folder : appFlowFormTree.ToJavaList()) {
		...
		// 找到父级目录添加到集合
		for (com.kong.kflowweb.BP.WF.Template.FlowFormTree folderapp : parentFolders.ToJavaList()) {
		...
		for (com.kong.kflowweb.BP.WF.Template.FlowFormTree folder : appFlowFormTree.ToJavaList()) {
			...
			boolean bHave = false;
			for (com.kong.kflowweb.BP.WF.Template.FlowFormTree child : appFlowFormTree.ToJavaList()) {
		...
		// 修改根节点编号
		for (com.kong.kflowweb.BP.WF.Template.FlowFormTree folder : parentFolders.ToJavaList()) {
			for (com.kong.kflowweb.BP.WF.Template.FlowFormTree folderApp : appFlowFormTree.ToJavaList()) {
	...
	public final void TansEntitiesToGenerTree(Entities ens, String rootNo, String checkIds) {
		...
		// attributes
		com.kong.kflowweb.BP.WF.Template.FlowFormTree formTree = (com.kong.kflowweb.BP.WF.Template.FlowFormTree) ((root instanceof com.kong.kflowweb.BP.WF.Template.FlowFormTree)
				? root : null);
		if (formTree != null) {
	...
	public final void AddChildren(EntityTree parentEn, Entities ens, String checkIds) {
		...
		for (EntityTree item : convertEntityMultiTree(ens)) {
			...
			// attributes
			com.kong.kflowweb.BP.WF.Template.FlowFormTree formTree = (com.kong.kflowweb.BP.WF.Template.FlowFormTree) ((item instanceof com.kong.kflowweb.BP.WF.Template.FlowFormTree)
					? item : null);
			if (formTree != null) {
				String url = formTree.getUrl() == null ? "" : formTree.getUrl();
	...
	public String MyFlow_Init() throws Exception {
		...
		String userNo = com.kong.kflowweb.BP.Web.WebUser.getNo();
		...
		if (this.getcurrND().getIsStartNode() && this.getIsCC() == false && this.getWorkID() == 0) {
			if (com.kong.kflowweb.BP.WF.Dev2Interface.Flow_IsCanStartThisFlow(this.getFK_Flow(), WebUser.getNo(), this.getPFlowNo(),
					this.getPNodeID(), this.getPWorkID()) == false) {
				// 是否可以发起流程？
				return "err@您(" + com.kong.kflowweb.BP.Web.WebUser.getNo() + ")没有发起或者处理该流程的权限.";
			}
		}

		if (this.getWorkID() == 0 && this.getcurrND().getIsStartNode() && this.GetRequestVal("IsCheckGuide") == null) {
			long workid = com.kong.kflowweb.BP.WF.Dev2Interface.Node_CreateBlankWork(this.getFK_Flow());
		...
		if (this.getWorkID() != 0 && !StringUtils.isEmpty(this.GetRequestVal("IsCheckGuide"))) {
			...
			DataTable dt = com.kong.kflowweb.BP.WF.Glo.StartGuidEnties(this.getWorkID(), this.getFK_Flow(), this.getFK_Node(), key);
			...
			if (dt != null && dt.Rows.size() > 0 && dt.Columns.contains("PFlowNo") == true) {
				...
				// 设置父子关系.
				com.kong.kflowweb.BP.WF.Dev2Interface.SetParentInfo(this.getFK_Flow(), this.getWorkID(), pFlowNo, pWorkID, pNodeID, pEmp);
			}
		}
		...
		if (this.getcurrND().getHisFormType() == NodeFormType.SheetTree
			...
			if (this.getWorkID() == 0) {
				...
				this.setWorkID(
						com.kong.kflowweb.BP.WF.Dev2Interface.Node_CreateBlankWork(this.getFK_Flow(), null, null, WebUser.getNo(), null));
			...
			if (this.getcurrND().getIsStartNode()) {
				...
				if (com.kong.kflowweb.BP.WF.Glo.CheckIsCanStartFlow_InitStartFlow(this.getcurrFlow()) == false) {
					// 如果启用了限制就把信息提示出来.
					String msg1 = com.kong.kflowweb.BP.WF.Glo.DealExp(this.getcurrFlow().getStartLimitAlert(), currWK, null);
					return "err@" + msg1;
				}
			}
		...
		if (frmtype != NodeFormType.RefOneFrmTree) {
			...
			if (this.getcurrND().getNodeFrmID().contains(nodeID) == false) {
				...
				com.kong.kflowweb.BP.WF.Node nd = new Node(Integer.parseInt(refNodeID));
	...
	public String SaveParas() throws Exception {
		com.kong.kflowweb.BP.WF.Dev2Interface.Flow_SaveParas(this.getWorkID(), this.GetRequestVal("Paras"));
		return "保存成功";
	}
	...
	public String InitToolBar() throws Exception {
		...
		try {
			...
			// #region //加载自定义的button.
			com.kong.kflowweb.BP.WF.Template.NodeToolbars bars = new NodeToolbars();
			...
			// com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLineError(ex);
	...
	public final String Send() throws Exception {
		try {
			...
			objs = com.kong.kflowweb.BP.WF.Dev2Interface.Node_SendWork(this.getFK_Flow(), this.getWorkID(), ht, null, this.getToNode(),
					null);
			...
			case SpecUrl:
				...
				myurl = myurl.replace("@WebUser.No", com.kong.kflowweb.BP.Web.WebUser.getNo());
				myurl = myurl.replace("@WebUser.Name", com.kong.kflowweb.BP.Web.WebUser.getName());
				myurl = myurl.replace("@WebUser.FK_Dept", com.kong.kflowweb.BP.Web.WebUser.getFK_Dept());

				if (myurl.contains("@")) {
					com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendMsg("admin",
			...
			case TurnToByCond:
				...
				return msg;
			default:
				msg = msg.replace("@WebUser.No", com.kong.kflowweb.BP.Web.WebUser.getNo());
				msg = msg.replace("@WebUser.Name", com.kong.kflowweb.BP.Web.WebUser.getName());
				msg = msg.replace("@WebUser.FK_Dept", com.kong.kflowweb.BP.Web.WebUser.getFK_Dept());
				return msg;
			}
			/// #endregion
	...
	public String Save() throws Exception {
		try {
			...
			return com.kong.kflowweb.BP.WF.Dev2Interface.Node_SaveWork(this.getFK_Flow(), this.getFK_Node(), this.getWorkID(),
	...
	public String DeleteFlow() {
		try {
			return com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoDeleteFlowByReal(this.getFK_Flow(), this.getWorkID(), true);
		} catch (Exception ex) {
	...
	public String GenerWorkNode() throws Exception {
		try {
			...
			if (this.getDoType1() != null && this.getDoType1().toUpperCase().equals("VIEW")) {
				DataTable trackDt = com.kong.kflowweb.BP.WF.Dev2Interface.DB_GenerTrack(this.getFK_Flow(), this.getWorkID(),
						this.getFID()).Tables.get(0);
				ds.Tables.add(trackDt);
				return com.kong.kflowweb.BP.Tools.Json.ToJson(ds);
			}

			ds = com.kong.kflowweb.BP.WF.CCFlowAPI.GenerWorkNode(this.getFK_Flow(), this.getFK_Node(), this.getWorkID(), this.getFID(),
					com.kong.kflowweb.BP.Web.WebUser.getNo(), "0");
	...
	public String Confirm() throws Exception {
		// 确认
		com.kong.kflowweb.BP.WF.Dev2Interface.Flow_Confirm(this.getWorkID());
		return "设置成功.";
	}
	...
	public String Dtl_Init() throws Exception {
		...
		// 获得他的描述,与数据.
		DataSet ds = com.kong.kflowweb.BP.WF.CCFormAPI.GenerDBForCCFormDtl(mdtl.getFK_MapData(), mdtl,
	 ...
	 * @throws Exception
	 */
	public final String MyFlow_Init_DealUrl(com.kong.kflowweb.BP.WF.Node currND, Work currWK, String url) throws Exception {
		if (url == null) {
	...
	public final String MyFlow_StopFlow() throws Exception {
		try {
			String str = com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoFlowOver(this.getFK_Flow(), this.getWorkID(), "流程成功结束",1);
	...
	public final String StartGuide_Init() throws Exception {
		...
		try {
			...
			case BySQLOne:
			case BySystemUrlOneEntity:
				josnData = com.kong.kflowweb.BP.Tools.Json.ToJson(dt);
				break;
			case BySQLMulti:
				josnData = com.kong.kflowweb.BP.Tools.Json.ToJson(dt);
				break;
	...
	public String BathFlow() throws Exception {
		...
		for (String workId : workIdArray) {
			...
			for (DataRow dr : dt.Rows) {
				...
				SendReturnObjs returnObj = com.kong.kflowweb.BP.WF.Dev2Interface.Node_SendWork(dr.getValue("FK_Flow").toString(),
	...
	public String DeleteWorkFlow() throws Exception {
		boolean isCan = com.kong.kflowweb.BP.WF.Dev2Interface.Flow_IsCanDeleteFlowInstance(this.getFK_Flow(), this.getWorkID(),
				WebUser.getNo());
		if (isCan == false)
			return "你没有删除流程的权限";
		return com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoDeleteFlowByReal(this.getFK_Flow(), this.getWorkID(), true);
