...
public abstract class Work extends Entity
{
	...
	*/
	public final String getRec() throws Exception
	{
		...
		if (str.equals(""))
		{
			this.SetValByKey(WorkAttr.Rec, com.kong.kflowweb.BP.Web.WebUser.getNo());
		}
	...
	*/
	...
	{
		String appName = com.kong.kflowweb.BP.Sys.Glo.getRequest().getRemoteAddr();
		String ip = (String) SystemConfig.getAppSettings().get("CIP");
		if (ip == null || ip.equals(""))
		{
			throw new RuntimeException("@您没有设置CIP");
		}
		return "http://" + ip + "/" + appName + "/WF/Port.jsp?UserNo=" + com.kong.kflowweb.BP.Web.WebUser.getNo() + "&DoWhat=DoNode&WorkID=" + this.getOID() + "&FK_Node=" + this.getHisNode().getNodeID() + "&Key=MyKey";
	}
	...
	*/
	...
	public Map getEnMap()
	{
		...
		try {
			map = com.kong.kflowweb.BP.Sys.MapData.GenerHisMap(this.NodeFrmID);
		} catch (Exception ex) {
			return null;
		}
