...
import com.kong.kflowweb.BP.WF.CCStaWay;
...
public class CC extends Entity
{
	...
	*/
	 public DataTable GenerCCers(Entity rpt, long workid) throws Exception
     {
         ...
         {
             /*如果抄送到部门. */
             if (Glo.getOSModel() == com.kong.kflowweb.BP.Sys.OSModel.OneOne)
         ...
         if (this.getCCIsStations() == true)
         {
             ...
             if (this.getCCStaWay() == CCStaWay.StationSmartCurrNodeWorker || this.getCCStaWay() == CCStaWay.StationSmartNextNodeWorker)
             {
                 ...
                 if (this.getCCStaWay() == CCStaWay.StationSmartCurrNodeWorker)
                     deptNo = com.kong.kflowweb.BP.Web.WebUser.getFK_Dept();
                 else
                     deptNo = DBAccess.RunSQLReturnStringIsNull("SELECT FK_Dept FROM WF_GenerWorkerlist WHERE WorkID=" + workid + " AND IsEnable=1 AND IsPass=0", com.kong.kflowweb.BP.Web.WebUser.getFK_Dept());
                 ...
                 this.getCCStaWay() == CCStaWay.StationDeptUpLevelNextNodeWorker )
             {
                 ...
                 while (true)
                 {
                     com.kong.kflowweb.BP.Port.Dept dept = new Dept(deptNo);
         ...
         if (this.getCCIsSQLs() == true)
         {
             ...
             if (sql.contains("@") == true)
                 sql = com.kong.kflowweb.BP.WF.Glo.DealExp(sql, rpt, null);
	...
	*/
	...
	public Map getEnMap()
	{
		...
		//map.AddSearchAttr(CCAttr.CCCtrlWay);
			...
			// 相关功能。
		map.getAttrsOfOneVSM().Add(new com.kong.kflowweb.BP.WF.Template.CCStations(), new com.kong.kflowweb.BP.WF.Port.Stations(), NodeStationAttr.FK_Node, NodeStationAttr.FK_Station, DeptAttr.Name, DeptAttr.No, "抄送岗位");

		map.getAttrsOfOneVSM().Add(new com.kong.kflowweb.BP.WF.Template.CCDepts(), new com.kong.kflowweb.BP.WF.Port.Depts(), NodeDeptAttr.FK_Node, NodeDeptAttr.FK_Dept, DeptAttr.Name, DeptAttr.No, "抄送部门");

		map.getAttrsOfOneVSM().Add(new com.kong.kflowweb.BP.WF.Template.CCEmps(), new com.kong.kflowweb.BP.WF.Port.Emps(), NodeEmpAttr.FK_Node, NodeEmpAttr.FK_Emp, DeptAttr.Name, DeptAttr.No, "抄送人员");
