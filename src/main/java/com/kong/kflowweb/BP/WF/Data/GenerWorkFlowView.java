...
public class GenerWorkFlowView extends Entity {
	...
	public Map getEnMap() {
		...
		map.AddDDLEntities(GenerWorkFlowViewAttr.FK_Dept, null, "部门", new com.kong.kflowweb.BP.Port.Depts(), false);
	...
	public final String DoShift(String ToEmp, String Note) throws Exception {
		...
		try {
			com.kong.kflowweb.BP.WF.Dev2Interface.Node_Shift(this.getFK_Flow(), this.getFK_Node(), this.getWorkID(), this.getFID(), ToEmp,
					Note);
			return "移交成功";
	...
	public final String DoDelete() throws Exception {
		try {
			if (com.kong.kflowweb.BP.WF.Dev2Interface.Flow_IsCanViewTruck(this.getFK_Flow(), this.getWorkID(),
					WebUser.getNo()) == false) {
				return "您没有操作该流程数据的权限.";
			}
		} catch (Exception e) {
	...
	public final String DoSkip() {
		try {
			PubClass.WinOpen(ContextHolderUtils.getResponse(),
					SystemConfig.getCCFlowWebPath() + "WF/Admin/FlowDB/FlowSkip.jsp?WorkID=" + this.getWorkID()
							+ "&FID=" + this.getFID() + "&FK_Flow=" + this.getFK_Flow() + "&FK_Node="
							+ this.getFK_Node(),
					900, 800);
		} catch (IOException e) {
	...
	public final String Rollback() {
		return Glo.getCCFlowAppPath() + "WF/Admin/Rollback.htm?WorkID=" + this.getWorkID() + "&FID=" + this.getFID()
				+ "&FK_Flow=" + this.getFK_Flow() + "&FK_Node=" + this.getFK_Node();
	}
