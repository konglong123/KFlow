...
import com.kong.kflowweb.BP.En.AttrOfSearch;
import com.kong.kflowweb.BP.En.AttrSearch;
import com.kong.kflowweb.BP.En.AttrSearchs;
import com.kong.kflowweb.BP.En.Attrs;
import com.kong.kflowweb.BP.En.ClassFactory;
import com.kong.kflowweb.BP.En.DDLShowType;
import com.kong.kflowweb.BP.En.Entities;
import com.kong.kflowweb.BP.En.EntitiesTree;
import com.kong.kflowweb.BP.En.Entity;
import com.kong.kflowweb.BP.En.EntityTree;
import com.kong.kflowweb.BP.En.EntityTreeAttr;
...
import com.kong.kflowweb.BP.En.RefMethods;
...
import com.kong.kflowweb.BP.Sys.DefVals;
import com.kong.kflowweb.BP.Sys.EnCfg;
import com.kong.kflowweb.BP.Sys.EventListOfNode;
import com.kong.kflowweb.BP.Sys.GEEntitys;
...
import com.kong.kflowweb.BP.Sys.UserRegedit;
import com.kong.kflowweb.BP.Sys.XML.ActiveAttr;
import com.kong.kflowweb.BP.Sys.XML.ActiveAttrAttr;
import com.kong.kflowweb.BP.Sys.XML.ActiveAttrs;
import com.kong.kflowweb.BP.Tools.DataTableConvertJson;
...
public class WF_Comm extends WebContralBase {
	...
	public String Tree_Init() throws Exception {
		...
		return com.kong.kflowweb.BP.Tools.Json.ToJson(ens.ToDataTableField("TreeTable"));
	}
	// #endregion 树的实体
	...
	public String Method_Done() {
		...
		Method rm = com.kong.kflowweb.BP.En.ClassFactory.GetMethod(ensName);
		int mynum = 0;
		...
		for (Attr attr : rm.getHisAttrs()) {
			...
			try {
				...
				case TB:
					...
					case com.kong.kflowweb.BP.DA.DataType.AppString:
					case com.kong.kflowweb.BP.DA.DataType.AppDate:
					case com.kong.kflowweb.BP.DA.DataType.AppDateTime:
						String str1 = this.GetValFromFrmByKey(attr.getKey());
						rm.SetValByKey(attr.getKey(), str1);
						break;
					case com.kong.kflowweb.BP.DA.DataType.AppInt:
						...
						break;
					case com.kong.kflowweb.BP.DA.DataType.AppFloat:
						...
						break;
					case com.kong.kflowweb.BP.DA.DataType.AppDouble:
					case com.kong.kflowweb.BP.DA.DataType.AppMoney:
						...
						break;
					case com.kong.kflowweb.BP.DA.DataType.AppBoolean:
	...
	public String Entity_Init() throws Exception {
		try {
			...
			if (pkval == null || pkval.equals("0") || pkval.equals("") || pkval.equals("undefined")) {
				...
				for (com.kong.kflowweb.BP.En.Attr attr : en.getEnMap().getAttrs())
	...
	public final String Entity_DoMethodReturnString() throws Exception {
		// 创建类实体.
		com.kong.kflowweb.BP.En.Entity en = ClassFactory.GetEn(this.getEnName()); // Activator.CreateInstance(System.Type.GetType("com.kong.kflowweb.BP.En.Entity"))
																// as
																// com.kong.kflowweb.BP.En.Entity;
		en.setPKVal(this.getPKVal());
		en.RetrieveFromDBSources();
	...
	public final String MethodLink_Init() throws Exception {
		ArrayList al = com.kong.kflowweb.BP.En.ClassFactory.GetObjects("com.kong.kflowweb.BP.En.Method");
		String html = "";
		Iterator it1 = al.iterator();
		while (it1.hasNext()) {

			com.kong.kflowweb.BP.En.Method en = (Method) it1.next();
	...
	public final String Search_SearchAttrs() throws Exception {
		...
		for (AttrSearch item : attrs) {
			...
			if (attr.getUIDDLShowType() == DDLShowType.BindSQL) {
				...
				sql = com.kong.kflowweb.BP.WF.Glo.DealExp(sql, null, null);
	...
	public String Search_Exp() throws Exception {
		...
		String name = "数据导出";
		String filename = name + "_" + com.kong.kflowweb.BP.DA.DataType.getCurrentDataCNOfLong() + "_" + WebUser.getName() + ".xls";
	...
	public final String Refmethod_Init() throws Exception {
		...
		Entities ens = com.kong.kflowweb.BP.En.ClassFactory.GetEns(ensName);
		Entity en = ens.getGetNewEntity();
		com.kong.kflowweb.BP.En.RefMethod rm = en.getEnMap().getHisRefMethods().get(index);
		...
		for (MapAttr item : attrs.ToJavaList()) {
			...
			if (v.indexOf('@') == -1)
				...
				if (v.equals("@WebUser.No"))
					mydrMain.setValue(item.getKeyOfEn(), com.kong.kflowweb.BP.Web.WebUser.getNo());
				else if (v.equals("@WebUser.Name"))
					mydrMain.setValue(item.getKeyOfEn(), com.kong.kflowweb.BP.Web.WebUser.getName());
				else if (v.equals("@WebUser.FK_Dept"))
					mydrMain.setValue(item.getKeyOfEn(), com.kong.kflowweb.BP.Web.WebUser.getFK_Dept());
				else if (v.equals("@WebUser.FK_DeptName"))
					mydrMain.setValue(item.getKeyOfEn(), com.kong.kflowweb.BP.Web.WebUser.getFK_DeptName());
				else if (v.equals("@WebUser.FK_DeptNameOfFull") || v.equals("@WebUser.FK_DeptFullName"))
					mydrMain.setValue(item.getKeyOfEn(), com.kong.kflowweb.BP.Web.WebUser.getFK_DeptNameOfFull());
		...
		for (DataRow dr : mapAttrs.Rows) {
			...
			if (ds.Tables.contains(uiBindKey) == false)
				ds.Tables.add(com.kong.kflowweb.BP.Sys.PubClass.GetDataTableByUIBineKey(uiBindKey));
		}
		...
		for (Attr attr : rm.getHisAttrs()) {
			...
			if (attr.getUIBindKey().contains("SELECT") == true || attr.getUIBindKey().contains("select") == true) {
				...
				sqlBindKey = com.kong.kflowweb.BP.WF.Glo.DealExp(sqlBindKey, en, null);
	...
	public final String Refmethod_Done() throws Exception {
		Entities ens = com.kong.kflowweb.BP.En.ClassFactory.GetEns(this.getEnsName());
		Entity en = ens.getGetNewEntity();
		String msg = "";
	...
	public final String DoOneEntity(Entity en, int rmIdx) {
		com.kong.kflowweb.BP.En.RefMethod rm = en.getEnMap().getHisRefMethods().get(rmIdx);
		rm.HisEn = en;
		int mynum = 0;
		for (Attr attr : rm.getHisAttrs()) {
			if (attr.getMyFieldType() == FieldType.RefText) {
		...
		for (Attr attr : rm.getHisAttrs()) {
			...
			case DDL:
				try {
					if (attr.getMyDataType() == com.kong.kflowweb.BP.DA.DataType.AppString) {
						String str = this.GetValFromFrmByKey(attr.getKey());
						objs[idx] = str;
	...
	public final String SFTable() throws Exception {
		com.kong.kflowweb.BP.Sys.SFTable sftable = new com.kong.kflowweb.BP.Sys.SFTable(this.GetRequestVal("SFTable"));
	...
	public final String Exec(String clsName, String methodName, String paras) throws Exception {
		...
		if (clsName.contains(".HttpHandler.") == true) {
			...
			Object tempVar = null;
			try {
				tempVar = java.lang.Class.forName("com.kong.kflowweb.BP.WF.HttpHandler.DirectoryPageBase").newInstance();
		...
		try {
			// 创建类实体.
			Object tempVar2 = java.lang.Class.forName("com.kong.kflowweb.BP.En.Entity").newInstance();
			com.kong.kflowweb.BP.En.Entity en = (com.kong.kflowweb.BP.En.Entity) ((tempVar2 instanceof com.kong.kflowweb.BP.En.Entity) ? tempVar2 : null);
			en.setPKVal(this.getPKVal());
			en.RetrieveFromDBSources();
	...
	public String SQLList() {
		...
		com.kong.kflowweb.BP.Sys.XML.SQLList sqlXml = new com.kong.kflowweb.BP.Sys.XML.SQLList(sqlKey);
		// 获得SQL
	...
	public String WebUser_Init() throws Exception {
		...
		if (DataType.IsNullOrEmpty(userNo) == true) {
			...
			ht.put("CustomerNo", com.kong.kflowweb.BP.Sys.SystemConfig.getCustomerNo());
			ht.put("CustomerName", com.kong.kflowweb.BP.Sys.SystemConfig.getCustomerName());
		...
		// ht.put("orgNo", WebUser.getFK_Dept() == null ? "" :
		// WebUser.getFK_Dept());
		return com.kong.kflowweb.BP.Tools.Json.ToJson(ht);
	}
    ...
    {
        com.kong.kflowweb.BP.WF.Dev2Interface.Port_Login(WebUser.getAuth());
        return "登录成功";
    }
	...
	public String EntityAth_Upload() throws Exception {
		...
		if (SystemConfig.getCustomerNo().equals("TianYe")) {
			...
			String savePath = com.kong.kflowweb.BP.Sys.SystemConfig.getPathOfDataUser() + className;
	...
	private DataSet GroupSearchSet(Entities ens, Entity en, Map map, UserRegedit ur, DataSet ds, ActiveAttrs aas)
		...
		// 获取查询的注册表
		com.kong.kflowweb.BP.Sys.UserRegedit searchUr = new UserRegedit();
		...
		if (en.getEnMap().IsShowSearchKey && DataType.IsNullOrEmpty(keyWord) == false && keyWord.length() >= 1) {
			...
			for (Attr attr : map.getAttrs()) {
				...
				if (attr.getField().equals("MyFileExt") || attr.getField().equals("MyFilePath")
			...
			for (DataRow dr : dt.Rows) {
				...
				try {
					...
					if (val == null || val.length() <= 1) {
						...
						try {
							com.kong.kflowweb.BP.Port.Dept Dept = new com.kong.kflowweb.BP.Port.Dept(val);
	...
	public final String Entities_Save() throws Exception {
		try {
			/// #region 查询出来s实体数据.
			Entities dtls = com.kong.kflowweb.BP.En.ClassFactory.GetEns(this.getEnsName());
			Entity en = dtls.getGetNewEntity();
			QueryObject qo = new QueryObject(dtls);
			qo.DoQuery();
			Map map = en.getEnMap();
	...
	private String readTxt() {
		try {
			String path = com.kong.kflowweb.BP.Sys.SystemConfig.getPathOfDataUser() + "Fastenter\\" + getFK_MapData() + "\\"
			...
			for (File file : folderArray) {
				if (count >= index && count < iPageSize * iPageNumber) {
					dt.Rows.get(count).setValue("MyPk", com.kong.kflowweb.BP.DA.DBAccess.GenerGUID());
