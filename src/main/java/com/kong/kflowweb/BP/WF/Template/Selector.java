...
import com.kong.kflowweb.BP.GPM.EmpAttr;
import com.kong.kflowweb.BP.GPM.StationAttr;
...
//map.AddTBStringDoc(SelectorAttr.SelectorP1, null, "分组参数,可以为空", true, false, true);
       ...
       // #region 对应关系
        //平铺模式.
        map.getAttrsOfOneVSM().AddGroupPanelModel(new com.kong.kflowweb.BP.WF.Template.NodeStations(), new com.kong.kflowweb.BP.WF.Port.Stations(),
            com.kong.kflowweb.BP.WF.Template.NodeStationAttr.FK_Node,
            com.kong.kflowweb.BP.WF.Template.NodeStationAttr.FK_Station, "绑定岗位(平铺)", StationAttr.FK_StationType,"Name","No");

        map.getAttrsOfOneVSM().AddGroupListModel(new com.kong.kflowweb.BP.WF.Template.NodeStations(), new com.kong.kflowweb.BP.WF.Port.Stations(),
          com.kong.kflowweb.BP.WF.Template.NodeStationAttr.FK_Node,
          com.kong.kflowweb.BP.WF.Template.NodeStationAttr.FK_Station, "绑定岗位(树)", StationAttr.FK_StationType,"Name","No");


        //节点绑定部门. 节点绑定部门.
        map.getAttrsOfOneVSM().AddBranches(new com.kong.kflowweb.BP.WF.Template.NodeDepts(), new com.kong.kflowweb.BP.Port.Depts(),
           com.kong.kflowweb.BP.WF.Template.NodeDeptAttr.FK_Node,
           com.kong.kflowweb.BP.WF.Template.NodeDeptAttr.FK_Dept, "绑定部门", EmpAttr.Name, EmpAttr.No, "@WebUser.FK_Dept");

        //节点绑定人员. 使用树杆与叶子的模式绑定.
        map.getAttrsOfOneVSM().AddBranchesAndLeaf(new com.kong.kflowweb.BP.WF.Template.NodeEmps(), new com.kong.kflowweb.BP.Port.Emps(),
           com.kong.kflowweb.BP.WF.Template.NodeEmpAttr.FK_Node,
           com.kong.kflowweb.BP.WF.Template.NodeEmpAttr.FK_Emp, "绑定接受人", EmpAttr.FK_Dept, EmpAttr.Name, EmpAttr.No, "@WebUser.FK_Dept");
        //#endregion
	...
	*/
	private DataSet BySQL(int nodeID, Entity en) throws Exception
	{
		...
		{
			 sqlGroup = com.kong.kflowweb.BP.WF.Glo.DealExp(sqlGroup, en, null);  //@祝梦娟
			DataTable dt = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(sqlGroup);
			dt.TableName = "Depts";
			ds.Tables.add(dt);
		}
		...
		String sqlDB = this.getSelectorP2();
		 sqlDB = com.kong.kflowweb.BP.WF.Glo.DealExp(sqlDB, en, null);
		DataTable dtEmp = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(sqlDB);
		...
		if (!this.getSelectorP3().equals(""))
		{
			...
			sqlDB = com.kong.kflowweb.BP.WF.Glo.DealExp(sqlDB, en, null);
			DataTable dtDef = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(sqlDB);
		...
		if (!this.getSelectorP4().equals(""))
		{
			...
			DataTable dtForce = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(sqlDB);
	...
	*/
	private DataSet ByDept(int nodeID)
	{
		...
		DataTable dtEmp = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(sql);
		ds.Tables.add(dtEmp);
		dtEmp.TableName = "Emps";
		return ds;
	}
	...
	*/
	...
	private DataSet AccepterOfDeptStationOfCurrentOper(int nodeID, Entity en) throws Exception
	{
        ...
        else //如果没人，就查询父级
        {
            ...
            com.kong.kflowweb.BP.WF.WorkNode node = new com.kong.kflowweb.BP.WF.WorkNode(workID, nodeID);
            ...
            sql = "SELECT DISTINCT b.No,b.Name,b.FK_Dept   FROM " + com.kong.kflowweb.BP.WF.Glo.getEmpStation() + " a,Port_Emp b  WHERE FK_Station IN "
