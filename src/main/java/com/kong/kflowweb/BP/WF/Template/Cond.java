...
public class Cond extends EntityMyPK {
	...
	public final String getSpecOper() throws Exception {
		...
		if (way == SpecOperWay.CurrOper) {
			return com.kong.kflowweb.BP.Web.WebUser.getNo();
		}

		if (way == SpecOperWay.SpecNodeOper) {
			String sql = "SELECT FK_Emp FROM WF_GenerWorkerlist WHERE FK_Node=" + this.getSpecOperPara()
					+ " AND WorkID=" + this.getWorkID();
			String fk_emp = DBAccess.RunSQLReturnStringIsNull(sql, null);
			if (fk_emp == null) {
				throw new RuntimeException(
						"@您在配置方向条件时错误，求指定的人员的时候，按照指定的节点[" + this.getSpecOperPara() + "]，作为处理人，但是该节点上没有人员。查询SQL:" + sql);
			}
			return fk_emp;
		}
		...
		if (way == SpecOperWay.SpecSheetField) {
			if (this.en.getRow().containsKey(this.getSpecOperPara().replace("@", "")) == false) {
				throw new RuntimeException(
						"@您在配置方向条件时错误，求指定的人员的时候，按照指定的字段[" + this.getSpecOperPara() + "]作为处理人，但是该字段不存在。");
			}
		...
		if (way == SpecOperWay.SpenEmpNo) {
			if (StringHelper.isNullOrEmpty(this.getSpecOperPara()) == false) {
				throw new RuntimeException(
						"@您在配置方向条件时错误，求指定的人员的时候，按照指定的人员[" + this.getSpecOperPara() + "]作为处理人，但是人员参数没有设置。");
			}
	...
	protected boolean beforeUpdateInsertAction() throws Exception {
		...
		this.setMyPOID(com.kong.kflowweb.BP.DA.DBAccess.GenerOID());
	...
	public final void setFK_Attr(String value) throws Exception {
		...
		com.kong.kflowweb.BP.Sys.MapAttr attr = new com.kong.kflowweb.BP.Sys.MapAttr(value);
	...
	public final void DoUp(int fk_node) {
		...
		this.DoOrderUp(CondAttr.FK_Node, (new Integer(fk_node)).toString(), CondAttr.CondType,
				(new Integer(condtypeInt)).toString(), CondAttr.PRI);
	}
	...
	public final void DoDown(int fk_node) {
		int condtypeInt = this.getHisCondType().getValue();
		this.DoOrderDown(CondAttr.FK_Node, (new Integer(fk_node)).toString(), CondAttr.CondType,
				(new Integer(condtypeInt)).toString(), CondAttr.PRI);
	}
	...
	public boolean getIsPassed() throws Exception {
		...
		if (this.getHisDataFrom() == ConnDataFrom.Stas) {
			...
			if (com.kong.kflowweb.BP.WF.Glo.getOSModel() == com.kong.kflowweb.BP.Sys.OSModel.OneMore) {
				com.kong.kflowweb.BP.GPM.DeptEmpStations sts = new com.kong.kflowweb.BP.GPM.DeptEmpStations();
				sts.Retrieve("FK_Emp", this.getSpecOper());
				for (com.kong.kflowweb.BP.GPM.DeptEmpStation st : sts.ToJavaList()) {
					if (strs.contains("@" + st.getFK_Station() + "@")) {
						this.MsgOfCond = "@以岗位判断方向，条件为true：岗位集合" + strs + "，操作员(" + com.kong.kflowweb.BP.Web.WebUser.getNo() + ")岗位:"
								+ st.getFK_Station() + st.getFK_StationT();
						return true;
					}
			...
			this.MsgOfCond = "@以岗位判断方向，条件为false：岗位集合" + strs + "，操作员(" + com.kong.kflowweb.BP.Web.WebUser.getNo() + ")岗位:" + strs1;
			return false;
		}
		...
		if (this.getHisDataFrom() == ConnDataFrom.Depts) {
			...
			com.kong.kflowweb.BP.GPM.DeptEmps sts = new com.kong.kflowweb.BP.GPM.DeptEmps();
			if (SystemConfig.getOSModel() == OSModel.OneMore) {
				sts.Retrieve(com.kong.kflowweb.BP.GPM.DeptEmpAttr.FK_Emp, this.getSpecOper());

				// @于庆海.
				com.kong.kflowweb.BP.Port.Emp emp = new com.kong.kflowweb.BP.Port.Emp(this.getSpecOper());
				emp.setNo(this.getSpecOper());
				if (emp.RetrieveFromDBSources() == 1) {
					com.kong.kflowweb.BP.GPM.DeptEmp de = new com.kong.kflowweb.BP.GPM.DeptEmp();
				...
				com.kong.kflowweb.BP.GPM.DeptEmp myen = new com.kong.kflowweb.BP.GPM.DeptEmp();
			...
			String strs1 = "";
			for (com.kong.kflowweb.BP.GPM.DeptEmp st : sts.ToJavaList()) {
				if (strs.contains("@" + st.getFK_Dept() + "@")) {
					this.MsgOfCond = "@以岗位判断方向，条件为true：部门集合" + strs + "，操作员(" + com.kong.kflowweb.BP.Web.WebUser.getNo() + ")部门:"
							+ st.getFK_Dept();
					return true;
				}
				strs1 += st.getFK_Dept();
			...
			this.MsgOfCond = "@以部门判断方向，条件为false：部门集合" + strs + "，操作员(" + com.kong.kflowweb.BP.Web.WebUser.getNo() + ")部门:" + strs1;
			return false;
		}
		...
		if (this.getHisDataFrom() == ConnDataFrom.Url) {
			...
			url = url.replace("@SDKFromServHost", (String) com.kong.kflowweb.BP.Sys.SystemConfig.getAppSettings().get("SDKFromServHost"));
			url = com.kong.kflowweb.BP.WF.Glo.DealExp(url, this.en, "");
			...
			if (url.contains("&SID") == false) {
				url += "&SID=" + com.kong.kflowweb.BP.Web.WebUser.getSID();
			}

			if (url.contains("&UserNo") == false) {
				url += "&UserNo=" + com.kong.kflowweb.BP.Web.WebUser.getNo();
			}

			if (SystemConfig.getIsBSsystem() && com.kong.kflowweb.BP.Sys.Glo.getRequest() != null) {
				// 是bs系统，并且是url参数执行类型.
				String myurl = com.kong.kflowweb.BP.Sys.Glo.getRequest().getRequestURI();
				if (myurl.indexOf('?') != -1) {
			...
			if (url.contains("http") == false) {
				...
				if (SystemConfig.getIsBSsystem() && com.kong.kflowweb.BP.Sys.Glo.getRequest() != null) {
					// 在cs模式下自动获取
					String host = com.kong.kflowweb.BP.Sys.Glo.getRequest().getRemoteHost();
					if (url.contains("@AppPath")) {
						url = url.replace("@AppPath", "http://" + host + com.kong.kflowweb.BP.Sys.Glo.getRequest().getRemoteAddr());
					} else {
						url = "http://" + com.kong.kflowweb.BP.Sys.Glo.getRequest().getRemoteHost() + url;
					}
				}
		...
		if (this.getHisDataFrom() == ConnDataFrom.Paras) {
			...
			return com.kong.kflowweb.BP.WF.Glo.CondExpPara(this.getOperatorValueStr(), ht, en.getOID());
		...
		if (this.getHisDataFrom() == ConnDataFrom.NodeForm) {
			if (en.getEnMap().getAttrs().Contains(this.getAttrKey()) == false)
				throw new RuntimeException(
						"err@判断条件方向出现错误：实体：" + en.getEnDesc() + " 属性" + this.getAttrKey() + "已经被删除方向条件判断失败.");

			this.MsgOfCond = "@以表单值判断方向，值 " + en.getEnDesc() + "." + this.getAttrKey() + " ("
					+ en.GetValStringByKey(this.getAttrKey()) + ") 操作符:(" + this.getFK_Operator() + ") 判断值:("
	...
	public boolean CheckIsPass(Entity en) throws Exception {
		...
		try {
			...
			throw new RuntimeException("@判断条件:Node=[" + this.getNodeID() + "," + nd23.getEnDesc() + "], 出现错误。@"
					+ ex.getMessage() + "。有可能您设置了非法的条件判断方式。");
		}
	}
	...
	public Map getEnMap() {
		...
		// map.AddDDLSysEnum(CondAttr.CondType, 0, "条件类型", true, false,
		// CondAttr.CondType,"@0=节点完成条件@1=流程完成条件@2=方向条件");
		...
		// true, true, NodeAttr.CondOrAnd,
		// "@0=And(条件集合中所有的都成立)@1=Or(条件集合中只有一个成立)");
