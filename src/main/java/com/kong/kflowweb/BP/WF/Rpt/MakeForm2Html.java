...
import com.kong.kflowweb.BP.Sys.FrmUI.ExtImg;
import com.kong.kflowweb.BP.Tools.AesEncodeUtil;
import com.kong.kflowweb.BP.Tools.BaseFileUtils;
import com.kong.kflowweb.BP.Tools.QrCodeUtil;
...
public class MakeForm2Html 
{
    ...
    public static StringBuilder GenerHtmlOfFree(MapData mapData, String frmID, long workid, Entity en, String path
        ...
        if (eles.size()>= 1)
        {
            for( com.kong.kflowweb.BP.Sys.FrmEle ele :eles.ToJavaList())
            {
            	  float y = ele.getY();
                  x = ele.getX() + wtX;
        ...
        for(com.kong.kflowweb.BP.Sys.FrmImg img :imgs.ToJavaList())
        {
            float y = img.getY();
            String imgSrc = "";
           ...
           //  ////#region 图片类型
            if (img.getHisImgAppType() == ImgAppType.Img)
            {
                ...
                if (img.getImgSrcType() == 1)
                {
                    ...
                    {
                        /*如果有变量*/
                        imgSrc = com.kong.kflowweb.BP.WF.Glo.DealExp(imgSrc, en, "");
                    }
                }
            ...
            if (img.getHisImgAppType() == ImgAppType.Seal)
            {
                ...
                if ((img.getIsEdit() == 1))
                {
                   //  ////#region 加载签章
                    ...
                    sql = String.format(" select FK_Station from Port_DeptStation where FK_Dept ='{0}' and FK_Station in (select FK_Station from " + com.kong.kflowweb.BP.WF.Glo.getEmpStation()+ " where FK_Emp='{1}')", fk_dept, WebUser.getNo());
                ...
                else
                {
                    ...
                    if (realDB != null)
                    {
                        ...
                        x =  img.getX() + wtX;
                        sb.append("\t\n<DIV id=" + img.getMyPK() + " style='position:absolute;left:" + x + "px;top:" + y + "px;text-align:left;vertical-align:top' >");
                        sb.append("\t\n<img src='" + imgSrc + "' onerror='javascript:this.src='" + appPath + "DataUser/ICON/" + com.kong.kflowweb.BP.Sys.SystemConfig.getCustomerNo() + "/LogBiger.png';' style='padding: 0px;margin: 0px;border-width: 0px;width:" + img.getW() + "px;height:" + img.getH() + "px;' />");
                        sb.append("\t\n</DIV>");
                    }
                }
            }
        ...
        for (FrmBtn btn : btns.ToJavaList())
        {
            ...
            String doDoc = com.kong.kflowweb.BP.WF.Glo.DealExp(btn.getEventContext(), en, null);
       ...
       //  ////#region  输出 rb.
        com.kong.kflowweb.BP.Sys.FrmRBs myrbs = mapData.getFrmRBs();
        for (com.kong.kflowweb.BP.Sys.FrmRB rb : myrbs.ToJavaList())
        {
            x = rb.getX() + wtX;
        ...
        for(FrmAttachment ath : aths.ToJavaList())
        {
            ...
            {
                /* 单个文件 */
            	FrmAttachmentDBs athDBs = com.kong.kflowweb.BP.WF.Glo.GenerFrmAttachmentDBs(ath,String.valueOf(workid), ath.getMyPK());
        ...
        , String path, String flowNo,String FK_Node,String basePath) throws Exception
{
    ...
    public static String MakeCCFormToPDF(Node node, long workid,String flowNo,String fileNameFormat,boolean urlIsHostUrl,String basePath) throws Exception{
		...
		String frmID = "ND"+node.getNodeID();
         ...
         fileNameFormat = com.kong.kflowweb.BP.DA.DataType.PraseStringToFileName(fileNameFormat);
    ...
    public static String MakeHtmlDocument(String frmID, long workid, String flowNo , String fileNameFormat , 
        ...
        try
        {
           ...
           if(mapData.getHisFrmType() == FrmType.Url){
            	...
            	String  docs = com.kong.kflowweb.BP.DA.DataType.ReadTextFile(SystemConfig.getPathOfDataUser() + "InstancePacketOfData/Template/indexUrl.htm");
            	...
            	if(gwf!=null)
            	 ...
            	 com.kong.kflowweb.BP.DA.DataType.WriteFile(indexFile, docs);
            	return indexFile;
            }
            ...
            if(SystemConfig.GetValByKeyBoolen("IsShowShuiYin",false) == true){
	            ...
	            if (words.contains("@") == true)
	                words = com.kong.kflowweb.BP.WF.Glo.DealExp(words, en);
            ...
            if (new File(tempFile).exists() == false){
                ...
                {
                    docs = com.kong.kflowweb.BP.DA.DataType.ReadTextFile(SystemConfig.getPathOfDataUser() + "InstancePacketOfData/Template/indexFool.htm");
                    sb =GenerHtmlOfFool(mapData, frmID, workid, en, path, flowNo,nodeID,basePath);
                    docs = docs.replace("@Width", String.valueOf(mapData.getFrmW())+"px");
                }
                else if(mapData.getHisFrmType() == FrmType.FreeFrm)
                {
                    docs = com.kong.kflowweb.BP.DA.DataType.ReadTextFile(SystemConfig.getPathOfDataUser() + "InstancePacketOfData/Template/indexFree.htm");
            ...
            if (flowNo != null )
            {
                ...
                if (gwf.getWFState() == WFState.Runing)
                {
                    if ( SystemConfig.getCustomerNo()=="TianYe" && gwf.getNodeName().contains("反馈") == true)
                    {
                        ...
                        {
                            //让流程自动结束.
                            com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoFlowOver(gwf.getFK_Flow(), workid, "打印并自动结束",0);
                        }
                    }
                }
    ...
    {
        com.kong.kflowweb.BP.DA.Log.DebugWriteInfo("@开始生成PDF:" + pdfFileExe + "@pdf=" + pdf + "@htmFile=" + htmFile);
