...
public class Emp extends EntityNoName
{
	...
	*/
	...
	public Map getEnMap()
	{
		...
		map.AddDDLEntities(EmpAttr.FK_Dept, null, "部门", new com.kong.kflowweb.BP.Port.Depts(), true);
	...
	*/
	public final String DoChangeUserNo(String userNo) throws Exception
	{
		...
		i= com.kong.kflowweb.BP.DA.DBAccess.RunSQL(sql);
		if (i != 0)
		{
			msg += "@待办更新[" + i + "]个。";
		}
		...
		//更换流程信息的数据表
		com.kong.kflowweb.BP.WF.Flows fls = new Flows();
		fls.RetrieveAll();
		...
		//更新其他字段.
		com.kong.kflowweb.BP.Sys.MapAttrs attrs = new MapAttrs();
		attrs.RetrieveAll();
		for (com.kong.kflowweb.BP.Sys.MapAttr attr : attrs.ToJavaList())
		{
			if (attr.getDefValReal().contains("WebUser.No") == true)
			{
				try
				{
					com.kong.kflowweb.BP.Sys.MapData md = new MapData(attr.getFK_MapData());
