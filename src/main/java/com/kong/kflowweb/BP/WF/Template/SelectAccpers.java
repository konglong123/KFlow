...
public class SelectAccpers extends EntitiesMyPK
{
	...
	*/
	public final int QueryAccepter(int fk_node, String rec, long workid) throws Exception
	{
		...
		//找出最近的工作ID
		int maxWorkID = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt("SELECT Max(WorkID) FROM WF_SelectAccper WHERE Rec='" + rec + "' AND FK_Node=" + fk_node, 0);
		if (maxWorkID == 0)
		{
			return 0;
		}
	...
	*/
	...
	{
		//找出最近的工作ID.
		int maxWorkID = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt("SELECT Max(WorkID) FROM WF_SelectAccper WHERE " + SelectAccperAttr.IsRemember + "=1 AND Rec='" + com.kong.kflowweb.BP.Web.WebUser.getNo() + "' AND FK_Node=" + fk_node, 0);
		if (maxWorkID == 0)
		{
			return 0;
		}
	...
	*/
	...
	{
		com.kong.kflowweb.BP.En.QueryObject qo = new QueryObject(this);
