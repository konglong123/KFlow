...
import com.kong.kflowweb.BP.WF.FlowRunWay;
import com.kong.kflowweb.BP.WF.ImpFlowTempleteModel;
import com.kong.kflowweb.BP.WF.Node;
import com.kong.kflowweb.BP.WF.Nodes;
import com.kong.kflowweb.BP.WF.Platform;
import com.kong.kflowweb.BP.WF.Template.DTSField;
import com.kong.kflowweb.BP.WF.Template.FlowDTSWay;
import com.kong.kflowweb.BP.WF.Template.StartGuideWay;
import com.kong.kflowweb.BP.WF.Template.TruckViewPower;
...
public class WF_Admin_AttrFlow extends WebContralBase {
	...
	public String APICodeFEE_Init() throws Exception {
		...
		if (com.kong.kflowweb.BP.WF.Glo.getPlatform() == Platform.CCFlow)
		...
		String code = com.kong.kflowweb.BP.DA.DataType.ReadTextFile(tmpPath);
		...
		String[] lineStrings = { "namespace com.kong.kflowweb.BP.FlowEvent", ": com.kong.kflowweb.BP.WF.FlowEventBase", "public override string FlowMark",
	...
	public String FlowFields_Init() throws NumberFormatException, Exception {
		com.kong.kflowweb.BP.Sys.MapAttrs attrs = new com.kong.kflowweb.BP.Sys.MapAttrs("ND" + Integer.parseInt(this.getFK_Flow()) + "Rpt");
		return attrs.ToJson();
	}
	...
	public String AutoStart_Init() throws Exception {
		com.kong.kflowweb.BP.WF.Flow en = new com.kong.kflowweb.BP.WF.Flow();
	...
	public String AutoStart_Save() throws Exception {
		// 执行保存.
		com.kong.kflowweb.BP.WF.Flow en = new com.kong.kflowweb.BP.WF.Flow(this.getFK_Flow());

		en.SetValByKey(com.kong.kflowweb.BP.WF.Template.FlowAttr.FlowRunWay, this.GetRequestValInt("RB_FlowRunWay"));
	...
	public String DTSBTable_Init() throws Exception {
		...
		// 获得数据源的表.
		com.kong.kflowweb.BP.Sys.SFDBSrc src = new SFDBSrc("local");
		DataTable dt = src.GetTables();
		dt.TableName = "Tables";
		ds.Tables.add(dt);

		// 把节点信息放入.
		com.kong.kflowweb.BP.WF.Nodes nds = new Nodes(this.getFK_Flow());
	...
	public String DTSBTable_Save() throws Exception {
		...
		com.kong.kflowweb.BP.WF.Template.FlowDTSWay dtsWay = (FlowDTSWay.forValue(this.GetRequestValInt("RB_DTSWay")));
	...
	public String DTSBTableExt_Save() throws Exception {
		...
		// com.kong.kflowweb.BP.Web.Controls.RadioBtn rb = this.Pub1.GetRadioBtnByID("rb_workId");
	...
	public String StartGuide_Init() throws Exception {
		...
		com.kong.kflowweb.BP.WF.Template.FrmNodes fns = new com.kong.kflowweb.BP.WF.Template.FrmNodes(Integer.parseInt(this.getFK_Flow() + "01"));
		...
		en.getRow().put("userId", com.kong.kflowweb.BP.Web.WebUser.getSID());
		// en.Row = ht;
		return en.ToJson();
	...
	public String StartGuide_Save() {
		try {
			...
			en.SetValByKey(com.kong.kflowweb.BP.WF.Template.FlowAttr.StartGuideWay, val);

			if (en.getStartGuideWay() == StartGuideWay.None) {
				en.setStartGuideWay(com.kong.kflowweb.BP.WF.Template.StartGuideWay.None);
			}
			...
			if (en.getStartGuideWay() == StartGuideWay.ByHistoryUrl) {
				...
				en.setStartGuideWay(com.kong.kflowweb.BP.WF.Template.StartGuideWay.ByHistoryUrl);
			}

			if (en.getStartGuideWay() == StartGuideWay.BySelfUrl) {
				en.setStartGuidePara1(this.GetRequestVal("TB_SelfURL"));
				en.setStartGuidePara2("");
				en.setStartGuideWay(com.kong.kflowweb.BP.WF.Template.StartGuideWay.BySelfUrl);
			}
			...
			if (en.getStartGuideWay() == StartGuideWay.BySQLOne) {
				...
				en.setStartGuideWay(com.kong.kflowweb.BP.WF.Template.StartGuideWay.BySQLOne);
			}
			...
			if (en.getStartGuideWay() == StartGuideWay.SubFlowGuide) {
				...
				en.setStartGuideWay(com.kong.kflowweb.BP.WF.Template.StartGuideWay.SubFlowGuide);
			}

			com.kong.kflowweb.BP.WF.Template.FrmNodes fns = new com.kong.kflowweb.BP.WF.Template.FrmNodes(Integer.parseInt(this.getFK_Flow() + "01"));
			if (fns.size() >= 2) {
				if (en.getStartGuideWay() == (StartGuideWay.ByFrms))
					en.setStartGuideWay(com.kong.kflowweb.BP.WF.Template.StartGuideWay.ByFrms);
			}
	...
	public String TruckViewPower_Init() throws Exception {
		if (StringHelper.isNullOrEmpty(getFK_Flow())) {
			...
			com.kong.kflowweb.BP.WF.Template.TruckViewPower en = new com.kong.kflowweb.BP.WF.Template.TruckViewPower(getFK_Flow());
			en.Retrieve();
			return en.ToJson();
		}
	}
	...
	public String TruckViewPower_Save() {
		try {
			...
			en = (TruckViewPower) com.kong.kflowweb.BP.Sys.PubClass.CopyFromRequestByPost(en, getRequest());
			en.Save(); // 执行保存.
			return "保存成功";
		} catch (Exception e) {
			return "err@保存失败";
	...
	//	string path= com.kong.kflowweb.BP.sys..SystemConfig
...
//		}
		...
		// 执行导入
		com.kong.kflowweb.BP.WF.Flow flow;
		try {
			flow = com.kong.kflowweb.BP.WF.Flow.DoLoadFlowTemplate(FK_FlowSort, xmlFile.getAbsolutePath(), model, flowNo);
