...
public class WorkFlowBuessRole
{
	...
	*/
	public static String GenerTitle(Flow fl, Work wk, Emp emp, String rdt) throws Exception
	{
		...
		if (DotNetToJavaStringHelper.isNullOrEmpty(titleRole))
		{
			...
			if (DotNetToJavaStringHelper.isNullOrEmpty(titleRole) || titleRole.contains("@") == false)
			{
	...
	*/
	public static String GenerTitle(Flow fl, Work wk) throws Exception
	{
		...
		if (titleRole.equals("@OutPara") || DotNetToJavaStringHelper.isNullOrEmpty(titleRole) == true)
		{
			titleRole = "@WebUser.FK_DeptName-WebUser.No,@WebUser.Name在@RDT发起.";
		}
...
///#warning 为什么，加这个代码？牺牲了很多效率，我注销了. by zhoupeng 2016.8.15
	...
	*/
	public static String GenerBillNo(String billNo, long workid, Entity en, String flowPTable) throws Exception
	{
		...
		if (billNo.contains("@"))
		{
			billNo = com.kong.kflowweb.BP.WF.Glo.DealExp(billNo, en, null);
		}
		...
		if (billNo.contains("{ParentBillNo}"))
		{
			...
			for (int i = 2; i < 9; i++)
			{
				...
				num = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt(sql, 0);
		...
		else
		{
			...
			java.util.ArrayList<java.util.Map.Entry<Integer, Integer>> loc = new java.util.ArrayList<java.util.Map.Entry<Integer, Integer>>(); //流水号位置，流水号位数
			String lsh; //流水号设置码
			...
			for (int i = 2; i < 9; i++)
			{
				...
				while (supposeBillNo.contains(lsh))
				{
					...
					//将找到的流水号码替换成假设的流水号
					supposeBillNo = (lshIdx == 0 ? "" : supposeBillNo.substring(0, lshIdx)) + StringHelper.padLeft("", i, '_') + (lshIdx + 6 + 1 < supposeBillNo.length() ? supposeBillNo.substring(lshIdx + 6) : "");
			...
			//sql = "SELECT BillNo FROM " + flowPTable + " WHERE BillNo LIKE '" + supposeBillNo.replace("[", "[[]") + "'" + (flowPTable.toLowerCase().equals("wf_generworkflow") || System.Text.RegularExpressions.Regex.IsMatch(flowPTable.toLowerCase(),"^nd+[\\d]+rpt$") ? " AND WFState >1" : "") + (flowPTable.toLowerCase().equals("wf_generworkflow") ? (" AND WorkID <> " + workid) : (" AND OID <> " + workid)) + " ORDER BY BillNo DESC";
			sql = "SELECT BillNo FROM " + flowPTable + " WHERE BillNo LIKE '" + supposeBillNo.replace("[", "[[]") + "'" + (flowPTable.toLowerCase().equals("wf_generworkflow") || flowPTable.toLowerCase().matches("^nd+[\\d]+rpt$") ? " AND WFState >1" : "") + (flowPTable.toLowerCase().equals("wf_generworkflow") ? (" AND WorkID <> " + workid) : (" AND OID <> " + workid)) + " ORDER BY BillNo DESC";
			String maxBillNo = DBAccess.RunSQLReturnString(sql);
			int ilsh = 0;

			if (StringHelper.isNullOrWhiteSpace(maxBillNo))
			{
				//没有数据，则所有流水号都从1开始
			...
			else
			{
				...
				java.util.HashMap<Integer, Integer> mlsh = new java.util.HashMap<Integer, Integer>();
				int plus1idx = -1;

				for (int i = loc.size() - 1; i >= 0; i--)
				{
					//获取单据号中当前位的流水码数
					ilsh = Integer.parseInt(maxBillNo.substring(loc.get(i).getKey(), loc.get(i).getKey() + loc.get(i).getValue()));
	...
	public static Node RequestNextNode(Node currNode, long workid, GenerWorkFlow currWorkFlow, GERpt enPara) throws NumberFormatException, Exception
	{
		...
		if (currNode.getCondModel() == CondModel.ByUserSelected)
		{
			...
			for (String item : mynodes)
			{
				...
				//排除到达自身节点
				if ((new Integer(currNode.getNodeID())).toString().equals(item))
				{
					continue;
				}
		...
		if (myNodes.size() == 0)
		{
			throw new RuntimeException("@当前用户(" + com.kong.kflowweb.BP.Web.WebUser.getName() + "),定义节点的方向条件错误:从{" + currNode.getNodeID() + currNode.getName() + "}节点到其它节点,定义的所有转向条件都不成立.");
		}
	...
	*/
	public static DataTable RequetNextNodeWorkers(Flow fl, Node currNode, Node toNode, Entity enParas, long workid) throws Exception
	{
		...
		if (toNode.getHisDeliveryWay() == DeliveryWay.ByPreviousNodeEmp)
		{
			...
			dr.setValue(0,com.kong.kflowweb.BP.Web.WebUser.getNo());
			dt.Rows.add(dr);
			return dt;
		}
				...
				|| toNode.getHisDeliveryWay() == DeliveryWay.BySQLAsSubThreadEmpsAndData)
		{
			...
			sql = sql.replace("@NodeID", (new Integer(toNode.getNodeID())).toString());
		...
		if (toNode.getHisDeliveryWay() == DeliveryWay.ByDtlAsSubThreadEmps)
		{
			...
			com.kong.kflowweb.BP.Sys.MapDtls dtls = new com.kong.kflowweb.BP.Sys.MapDtls(currNode.getNodeFrmID());
			String msg = null;
			for (com.kong.kflowweb.BP.Sys.MapDtl dtl : dtls.ToJavaList())
			{
				try
				{
		...
		if (toNode.getHisDeliveryWay() == DeliveryWay.ByPreviousNodeFormEmpsField)
		{
			...
			for (String s : myemps)
			{
				...
				//if (com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt("SELECT COUNT(NO) AS NUM FROM Port_Emp WHERE NO='" + s + "' or name='"+s+"'", 0) == 0)
				//    continue;
		...
		if (toNode.getHisDeliveryWay() == DeliveryWay.ByDept)
		{
			...
			if (flowAppType == FlowAppType.PRJ)
			{
				...
				if (dt.Rows.size() == 0)
				{
					...
					sql += "AND NO IN ";
					sql += "(";
					sql += "SELECT FK_Emp FROM " + com.kong.kflowweb.BP.WF.Glo.getEmpStation() + " WHERE FK_Station IN ";
		...
		{
			sql = "SELECT A.FK_Emp FROM " + com.kong.kflowweb.BP.WF.Glo.getEmpStation() + " A, WF_NodeStation B WHERE A.FK_Station=B.FK_Station AND B.FK_Node=" + dbStr + "FK_Node ORDER BY A.FK_Emp";
			ps = new Paras();
			ps.Add("FK_Node", toNode.getNodeID());
			ps.SQL = sql;
			dt = DBAccess.RunSQLReturnTable(ps);
		...
		if (toNode.getHisDeliveryWay() == DeliveryWay.BySpecNodeEmpStation)
		{
			...
			else
			{
				...
				com.kong.kflowweb.BP.Port.Emp em = new com.kong.kflowweb.BP.Port.Emp(empNo);
		...
		if (currNode.getIsStartNode() == false)
		{
			...
			if (flowAppType == FlowAppType.Normal || flowAppType == FlowAppType.DocFlow)
			{
				...
				sql = "SELECT DISTINCT FK_Emp  FROM " + com.kong.kflowweb.BP.WF.Glo.getEmpStation() + " WHERE FK_Station IN " + "(SELECT FK_Station FROM WF_NodeStation WHERE FK_Node=" + toNode.getNodeID() + ") " + "AND FK_Emp IN (SELECT FK_Emp FROM WF_GenerWorkerlist WHERE WorkID=" + dbStr + "WorkID AND FK_Node IN (" + DataType.PraseAtToInSql(toNode.getGroupStaNDs(), true) + ") )";
			...
			{
				// 如果当前的节点不是开始节点， 从轨迹里面查询。
				sql = "SELECT DISTINCT FK_Emp  FROM Prj_EmpPrjStation WHERE FK_Station IN " + "(SELECT FK_Station FROM WF_NodeStation WHERE FK_Node=" + dbStr + "FK_Node ) AND FK_Prj=" + dbStr + "FK_Prj " + "AND FK_Emp IN (SELECT FK_Emp FROM WF_GenerWorkerlist WHERE WorkID=" + dbStr + "WorkID AND FK_Node IN (" + DataType.PraseAtToInSql(toNode.getGroupStaNDs(), true) + ") )";
				sql += " ORDER BY FK_Emp ";
				...
				{
					// 如果项目组里没有工作人员就提交到公共部门里去找。
					sql = "SELECT DISTINCT FK_Emp  FROM " + com.kong.kflowweb.BP.WF.Glo.getEmpStation() + " WHERE FK_Station IN " + "(SELECT FK_Station FROM WF_NodeStation WHERE FK_Node=" + dbStr + "FK_Node ) " + "AND FK_Emp IN (SELECT FK_Emp FROM WF_GenerWorkerlist WHERE WorkID=" + dbStr + "WorkID AND FK_Node IN (" + DataType.PraseAtToInSql(toNode.getGroupStaNDs(), true) + ") )";
					sql += " ORDER BY FK_Emp ";
		...
		if (!currNode.getGroupStaNDs().equals(toNode.getGroupStaNDs()))
		{
			...
			if (flowAppType == FlowAppType.Normal)
			{
				...
				{
					DataTable dtStas = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable("SELECT FK_Station FROM WF_NodeStation WHERE FK_Node=" + toNode.getNodeID());
			...
			if (flowAppType == FlowAppType.PRJ)
			{
				...
				if (dt.Rows.size() == 0)
				{
					// 如果项目组里没有工作人员就提交到公共部门里去找。  
						...
						sql = "SELECT No FROM Port_Emp WHERE NO IN " + "(SELECT  FK_Emp  FROM " + com.kong.kflowweb.BP.WF.Glo.getEmpStation() + " WHERE FK_Station IN (SELECT FK_Station FROM WF_NodeStation WHERE FK_Node=" + dbStr + "FK_Node))" + " AND  NO IN " + "(SELECT FK_Emp FROM Port_DeptEmp WHERE FK_Dept =" + dbStr + "FK_Dept)";
						sql += " ORDER BY No ";
			...
			else
			{
				...
				{
					if (dr.getValue(0).toString().equals(com.kong.kflowweb.BP.Web.WebUser.getNo()))
					{
...
//             * 
		...
		while (true)
		{
			com.kong.kflowweb.BP.Port.Dept myDept = new com.kong.kflowweb.BP.Port.Dept(nowDeptID);
			...
			{
				//如果父亲级没有，就找父级的平级. 
				return FindWorker.Func_GenerWorkerList_SpecDept_SameLevel(myDept.getParentNo(), empNo,toNode);
	 ...
	 * @throws Exception 
	*/
	private static DataTable RequetNextNodeWorkers_DiGui_ByDepts(com.kong.kflowweb.BP.Port.Depts subDepts, String empNo, Node toNode) throws Exception
	{
		for (com.kong.kflowweb.BP.Port.Dept item : subDepts.ToJavaList())
		{
	...
	*/
	private static DataTable RequetNextNodeWorkers_DiGui(String deptNo, String empNo, Node toNode) throws Exception
	{
		...
		if (dt.Rows.size() == 0)
		{
			...
			sql = "SELECT No FROM Port_Emp WHERE No IN ";
			sql += "(SELECT  FK_Emp  FROM " + com.kong.kflowweb.BP.WF.Glo.getEmpStation() + " WHERE FK_Station IN (SELECT FK_Station FROM WF_NodeStation WHERE FK_Node=" + dbStr + "FK_Node ) )";
			sql += " AND No IN ";
			...
			{
				com.kong.kflowweb.BP.Port.Dept deptP = new com.kong.kflowweb.BP.Port.Dept(deptNo);
    ...
    public static String DoCCAuto(Node node, GERpt rpt, long workid, long fid) throws Exception
    {
        ...
        String basePath = com.kong.kflowweb.BP.WF.Glo.getHostURL();
        ...
        for(DataRow dr : dt.Rows)
        {
            ...
            //生成标题与内容.
        	String ccTitle = ccEn.getCCTitle();
            ccTitle = com.kong.kflowweb.BP.WF.Glo.DealExp(ccTitle, rpt, null);

            String ccDoc = ccEn.getCCDoc();
            ccDoc = com.kong.kflowweb.BP.WF.Glo.DealExp(ccDoc, rpt, null);
            ...
            if (com.kong.kflowweb.BP.WF.Glo.getIsEnableSysMessage() == true)
            {
                //     //写入消息提示.
                //     ccMsg += list.CCTo + "(" + dr[1].ToString() + ");";
                //     com.kong.kflowweb.BP.WF.Port.WFEmp wfemp = new Port.WFEmp(list.CCTo);
                ...
                //     com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendMsg(wfemp.No, title, mytemp, null, com.kong.kflowweb.BP.Sys.SMSMsgType.CC, list.FK_Flow, list.FK_Node, list.WorkID, list.FID);
            }
        }
        //写入日志.
        return ccMsg;
    ...
    public static String DoCCByEmps(Node nd, GERpt rptGE, long workid, long fid) throws Exception
    {
        ...
        //生成标题与内容.
        String ccTitle = cc.getCCTitle();
        ccTitle = com.kong.kflowweb.BP.WF.Glo.DealExp(ccTitle, rptGE, null);

        String ccDoc = cc.getCCDoc();
        ccDoc = com.kong.kflowweb.BP.WF.Glo.DealExp(ccDoc, rptGE, null);

        //取出抄送人列表
        String ccers = rptGE.GetValStrByKey("SysCCEmps");
        if (StringHelper.isNullOrEmpty(ccers) == false)
            return "";
        ...
        String mailTemp = com.kong.kflowweb.BP.DA.DataType.ReadTextFile2Html(com.kong.kflowweb.BP.Sys.SystemConfig.getPathOfDataUser() + "/EmailTemplete/CC_" + WebUser.getSysLang() + ".txt");
        for (Object item : ht.keySet())
		{
            ...
            if (com.kong.kflowweb.BP.WF.Glo.getIsEnableSysMessage() == true)
            {
                ...
                com.kong.kflowweb.BP.WF.Port.WFEmp wfemp = new WFEmp(list.getCCTo());
				...
				com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendMsg(wfemp.getNo(), title, mytemp, null, com.kong.kflowweb.BP.WF.SMSMsgType.CC, list.getFK_Flow(), list.getFK_Node(), list.getWorkID(), list.getFID());
            }
            //写入消息机制.
        }
