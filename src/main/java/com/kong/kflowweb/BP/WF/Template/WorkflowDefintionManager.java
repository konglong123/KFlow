...
public class WorkflowDefintionManager
{
	...
	public static String SaveFlow(String fk_flow, String nodes, String dirs, String labes) throws Exception
	{
		try
		{
			...
			for (String lab : mylabs)
			{
				...
				ln.setMyPK(com.kong.kflowweb.BP.DA.DBAccess.GenerGUID()); // ap.GetValStrByKey("MyPK");
	...
	*/
	...
	{
		com.kong.kflowweb.BP.WF.Flow fl1 = new com.kong.kflowweb.BP.WF.Flow(flowNo);
		try
		{
			fl1.DoDelete();
			return "删除成功.";
		...
		{
			com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLineError("Do Method DelFlow Branch has a error , para:\t" + flowNo + ex.getMessage());
			return ex.getMessage();
		}
	}
	public static String DeleteFrmTemplate(String v1) throws Exception{
		...
		return null;
			...
			com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLineError("Do Method DelFrm Branch has a error , para:\t" + v1 + e.getMessage());
			return e.getMessage();
		}
