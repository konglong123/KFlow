...
import com.kong.kflowweb.BP.WF.PortalInterface;
...
public class FindWorker {
	...
	public final DataTable FindByWorkFlowModel() throws Exception {
		...
		if (town.getHisNode().getHisDeliveryWay() == DeliveryWay.BySQL
			...
			sql = com.kong.kflowweb.BP.WF.Glo.DealExp(sql, this.currWn.rptGe, null);
			if (sql.contains("@")) {
				if (com.kong.kflowweb.BP.WF.Glo.getSendHTOfTemp() != null) {
					for (Object key : com.kong.kflowweb.BP.WF.Glo.getSendHTOfTemp().keySet()) {
						sql = sql.replace("@" + key, com.kong.kflowweb.BP.WF.Glo.getSendHTOfTemp().get(key).toString());
					}
				}
			}
		...
		if (town.getHisNode().getHisDeliveryWay() == DeliveryWay.ByDtlAsSubThreadEmps) {
			...
			com.kong.kflowweb.BP.Sys.MapDtls dtls = new com.kong.kflowweb.BP.Sys.MapDtls(this.currWn.getHisNode().getNodeFrmID());
			String msg = null;
		...
		if (town.getHisNode().getHisDeliveryWay() == DeliveryWay.BySpecNodeEmp
			...
			for (String nd : nds) {
				...
				ps = new Paras();
				ps.SQL = "SELECT DISTINCT(FK_Emp) FROM WF_GenerWorkerList WHERE "+workSQL+"  AND FK_Node=" + dbStr
		...
		if (town.getHisNode().getHisDeliveryWay() == DeliveryWay.ByPreviousNodeFormEmpsField) {
			...
			for (String s : myemps) {
				...
				// if (com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt("SELECT COUNT(NO) AS
		...
		if (town.getHisNode().getHisDeliveryWay() == DeliveryWay.ByStationOnly) {
			sql = "SELECT A.FK_Emp FROM " + com.kong.kflowweb.BP.WF.Glo.getEmpStation()
		...
		if (this.currWn.getHisNode().getIsStartNode() == false) {
			...
			if (flowAppType == FlowAppType.Normal || flowAppType == FlowAppType.DocFlow) {
				...
				sql = "SELECT DISTINCT FK_Emp  FROM " + com.kong.kflowweb.BP.WF.Glo.getEmpStation() + " WHERE FK_Station IN "
		...
		if ((DataType.IsNullOrEmpty(toNodeGroupStaNDs) == true && DataType.IsNullOrEmpty(currGroupStaNDs) == true)
			...
			if (flowAppType == FlowAppType.Normal) {
				...
				if (com.kong.kflowweb.BP.Sys.SystemConfig.getOSDBSrc() == OSDBSrc.WebServices) {
					DataTable dtStas = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(
							"SELECT FK_Station FROM WF_NodeStation WHERE FK_Node=" + town.getHisNode().getNodeID());
				...
				if (dt.Rows.size() == 0) {
					...
					if (com.kong.kflowweb.BP.WF.Glo.getOSModel() == OSModel.OneOne) {
						sql = "SELECT No FROM Port_Emp WHERE NO IN " + "(SELECT  FK_Emp  FROM "
								+ com.kong.kflowweb.BP.WF.Glo.getEmpStation()
					...
					if (specName.equals("SpecStations")) // 按岗位
					{
						...
						for (String station : stations) {
							...
							DataTable dt_Emps = DBAccess.RunSQLReturnTable("SELECT FK_Emp FROM "
									+ com.kong.kflowweb.BP.WF.Glo.getEmpStation() + " WHERE FK_Station='" + station + "'");
