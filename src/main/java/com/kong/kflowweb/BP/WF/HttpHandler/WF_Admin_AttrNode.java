...
import com.kong.kflowweb.BP.DA.DataRowCollection;
...
import com.kong.kflowweb.BP.Sys.FrmBtn;
import com.kong.kflowweb.BP.Sys.FrmBtns;
import com.kong.kflowweb.BP.Sys.FrmEventAttr;
import com.kong.kflowweb.BP.Sys.FrmEvents;
import com.kong.kflowweb.BP.Sys.GroupCtrlType;
...
import com.kong.kflowweb.BP.Tools.DealString;
import com.kong.kflowweb.BP.Tools.StringHelper;
import com.kong.kflowweb.BP.WF.BatchRole;
...
import com.kong.kflowweb.BP.WF.TeamLeaderConfirmRole;
import com.kong.kflowweb.BP.WF.TodolistModel;
...
import com.kong.kflowweb.BP.WF.Template.NodeCancel;
import com.kong.kflowweb.BP.WF.Template.NodeCancelAttr;
import com.kong.kflowweb.BP.WF.Template.NodeReturn;
import com.kong.kflowweb.BP.WF.Template.NodeReturnAttr;
import com.kong.kflowweb.BP.WF.Template.TemplateFileModel;
import com.kong.kflowweb.BP.WF.Template.WebOfficeWorkModel;
import com.kong.kflowweb.BP.WF.XML.EventLists;
...
public class WF_Admin_AttrNode extends WebContralBase {
	...
	public String getShowType() {
		...
		if (this.getFK_Node() == 0 && com.kong.kflowweb.BP.DA.DataType.IsNullOrEmpty(this.getFK_Flow()) == false
				&& this.getFK_Flow().length() >= 3)
			return "Flow";

		if (this.getFK_Node() == 0 && com.kong.kflowweb.BP.DA.DataType.IsNullOrEmpty(this.getFK_MapData()) == false)
			return "Frm";
	...
	public String Action_Init() throws Exception {
		...
		if (com.kong.kflowweb.BP.DA.DataType.IsNullOrEmpty(this.getFK_MapData()) == false)
	...
	public String ActionDtl_Init() throws Exception {
		...
		ArrayList<BuessUnitBase> al = com.kong.kflowweb.BP.En.ClassFactory.GetObjects("com.kong.kflowweb.BP.Sys.BuessUnitBase");
		...
		return com.kong.kflowweb.BP.Tools.Json.ToJson(dtBuess);
	}
	...
	public final String NodeFromWorkModel_Save() throws Exception {
		...
		com.kong.kflowweb.BP.Sys.MapData md = new com.kong.kflowweb.BP.Sys.MapData("ND" + this.getFK_Node());
		...
		if (selectFModel.equals("DefFrm")) {
			...
			if (frmModel.equals("0")) {
				...
				md.setHisFrmType(com.kong.kflowweb.BP.Sys.FrmType.FreeFrm);
				md.Update();
			} else {
				nd.setFormType(NodeFormType.FoolForm);
				nd.DirectUpdate();

				md.setHisFrmType(com.kong.kflowweb.BP.Sys.FrmType.FoolForm);
				md.Update();
			}
		...
		if (selectFModel.equals("FoolTruck")) {
			...
			md.setHisFrmType(com.kong.kflowweb.BP.Sys.FrmType.FoolForm); // 同时更新表单表住表.
			md.Update();
		}
		...
		if (selectFModel.equals("SelfForm")) {
			...
			md.setHisFrmType(com.kong.kflowweb.BP.Sys.FrmType.Url); // 同时更新表单表住表.
		...
		if (selectFModel.equals("SDKForm")) {
			...
			md.setHisFrmType(com.kong.kflowweb.BP.Sys.FrmType.Url);
		...
		if (selectFModel.equals("SheetTree")) {
			...
			if (sheetTreeModel.equals("0")) {
				...
				md.setHisFrmType(com.kong.kflowweb.BP.Sys.FrmType.FreeFrm); // 同时更新表单表住表.
				md.Update();
			} else {
		...
		if (selectFModel.equals("WebOffice")) {
			...
			if (WebOfficeFrmType.equals("0")) {
				// btn.setWebOfficeFrmModel( com.kong.kflowweb.BP.Sys.FrmType.FreeFrm);

				md.setHisFrmType(com.kong.kflowweb.BP.Sys.FrmType.FreeFrm); // 同时更新表单表住表.
				md.Update();
			} else {
				/// btn.setWebOfficeFrmModel(com.kong.kflowweb.BP.Sys.FrmType.FoolForm);
	...
	private void _BindData4SortingMapAttrs_Init(MapDatas mapdatas, MapAttrs attrs, GroupFields groups, MapDtls dtls,
		...
		if (mapdata != null) {
            ...
            if(DataType.IsNullOrEmpty(this.getFK_Flow())==false){
	            ...
	            nodes = new Nodes();
	            nodes.Retrieve(com.kong.kflowweb.BP.WF.Template.NodeAttr.FK_Flow, getFK_Flow(), com.kong.kflowweb.BP.WF.Template.NodeAttr.Step);
	            ...
	            if (nodes.size() == 0)
	            {
	                ...
	                if (nodeid.length() > 2)
	                {
	                    ...
	                    nodes.Retrieve(com.kong.kflowweb.BP.WF.Template.NodeAttr.FK_Flow, flowno, com.kong.kflowweb.BP.WF.Template.NodeAttr.Step);
	                }
	            }
	...
	public String NodeAttrs_Init() throws Exception {
		...
		for (Node node : nodes.ToJavaList()) {
			...
			if (node.getHisDeliveryWay() == com.kong.kflowweb.BP.WF.DeliveryWay.ByStation) {
				dr.setValue("HisDeliveryWayJsFnPara", "ByStation");
				dr.setValue("HisDeliveryWayCountLabel", "岗位");
				com.kong.kflowweb.BP.WF.Template.NodeStations nss = new com.kong.kflowweb.BP.WF.Template.NodeStations();
				intHisDeliveryWayCount = nss.Retrieve(com.kong.kflowweb.BP.WF.Template.NodeStationAttr.FK_Node, node.getNodeID());
			} else if (node.getHisDeliveryWay() == com.kong.kflowweb.BP.WF.DeliveryWay.ByDept) {
				dr.setValue("HisDeliveryWayJsFnPara", "ByDept");
				dr.setValue("HisDeliveryWayCountLabel", "部门");
				com.kong.kflowweb.BP.WF.Template.NodeDepts nss = new com.kong.kflowweb.BP.WF.Template.NodeDepts();
				intHisDeliveryWayCount = nss.Retrieve(com.kong.kflowweb.BP.WF.Template.NodeDeptAttr.FK_Node, node.getNodeID());
			} else if (node.getHisDeliveryWay() == com.kong.kflowweb.BP.WF.DeliveryWay.ByBindEmp) {
				dr.setValue("HisDeliveryWayJsFnPara", "ByDept");
				dr.setValue("HisDeliveryWayCountLabel", "人员");
				com.kong.kflowweb.BP.WF.Template.NodeEmps nes = new com.kong.kflowweb.BP.WF.Template.NodeEmps();
				intHisDeliveryWayCount = nes.Retrieve(com.kong.kflowweb.BP.WF.Template.NodeStationAttr.FK_Node, node.getNodeID());
			}
			...
			// 消息&事件Count
			com.kong.kflowweb.BP.Sys.FrmEvents fes = new com.kong.kflowweb.BP.Sys.FrmEvents();
			dr.setValue("HisFrmEventsCount", fes.Retrieve(com.kong.kflowweb.BP.Sys.FrmEventAttr.FK_MapData, "ND" + node.getNodeID()));

			// 流程完成条件Count
			com.kong.kflowweb.BP.WF.Template.Conds conds = new com.kong.kflowweb.BP.WF.Template.Conds(com.kong.kflowweb.BP.WF.Template.CondType.Flow, node.getNodeID());
	...
	public String TurnToDeal_Save() {
		try {
			...
			com.kong.kflowweb.BP.Sys.MapAttrs attrs = new com.kong.kflowweb.BP.Sys.MapAttrs("ND" + nodeID);
			com.kong.kflowweb.BP.WF.Node nd = new com.kong.kflowweb.BP.WF.Node(nodeID);

			int val = this.GetRequestValInt("TurnToDeal");

			// 遍历页面radiobutton
			if (0 == val) {
				nd.setHisTurnToDeal(com.kong.kflowweb.BP.WF.TurnToDeal.CCFlowMsg);
			} else if (1 == val) {
				nd.setHisTurnToDeal(com.kong.kflowweb.BP.WF.TurnToDeal.SpecMsg);
				nd.setTurnToDealDoc(this.GetRequestVal("TB_SpecMsg"));
			} else {
				nd.setHisTurnToDeal(com.kong.kflowweb.BP.WF.TurnToDeal.SpecUrl);
	...
	public String SepcFiledsSepcUsers_Init() {
		 ...
		 * 
		 * 
		 * com.kong.kflowweb.BP.Sys.MapAttrs attrs = new com.kong.kflowweb.BP.Sys.MapAttrs(fk_mapdata);
		 * 
		 * com.kong.kflowweb.BP.Sys.FrmImgs imgs = new com.kong.kflowweb.BP.Sys.FrmImgs(fk_mapdata);
		 * 
		 * com.kong.kflowweb.BP.Sys.MapExts exts = new com.kong.kflowweb.BP.Sys.MapExts(); int mecount =
		 * exts.Retrieve(com.kong.kflowweb.BP.Sys.MapExtAttr.FK_MapData, fk_mapdata,
		 * com.kong.kflowweb.BP.Sys.MapExtAttr.Tag, this.GetRequestVal("FK_Node"),
		 * com.kong.kflowweb.BP.Sys.MapExtAttr.ExtType, "SepcFiledsSepcUsers");
		 * 
		 * com.kong.kflowweb.BP.Sys.FrmAttachments aths = new com.kong.kflowweb.BP.Sys.FrmAttachments(fk_mapdata);
		 * 
		 * exts = new com.kong.kflowweb.BP.Sys.MapExts();
		 * exts.Retrieve(com.kong.kflowweb.BP.Sys.MapExtAttr.FK_MapData, fk_mapdata,
		 * com.kong.kflowweb.BP.Sys.MapExtAttr.Tag, this.GetRequestVal("FK_Node"),
		 * com.kong.kflowweb.BP.Sys.MapExtAttr.ExtType, "SepcAthSepcUsers");
		 */
		return "";// toJson
	}
	...
	public String BatchStartFields_Init() throws Exception {
		...
		com.kong.kflowweb.BP.Sys.SysEnums ses = new com.kong.kflowweb.BP.Sys.SysEnums(com.kong.kflowweb.BP.WF.Template.NodeAttr.BatchRole);
		//获取当前节点设置的批处理规则
        String srole = "";
	...
	public String BatchStartFields_Save() throws Exception {
		...
		switch (selectval) {
		case 0:
			nd.setHisBatchRole(com.kong.kflowweb.BP.WF.BatchRole.None);
			break;
		case 1:
			nd.setHisBatchRole(com.kong.kflowweb.BP.WF.BatchRole.Ordinary);
			break;
		default:
			nd.setHisBatchRole(com.kong.kflowweb.BP.WF.BatchRole.Group);
			break;
		}
		// 批处理的数量
	...
	public String BlockModel_Save() throws Exception {
		...
		nd.SetValByKey(com.kong.kflowweb.BP.WF.Template.NodeAttr.BlockModel, val);
		if (nd.getBlockModel() == com.kong.kflowweb.BP.WF.BlockModel.None)
			nd.setBlockModel(com.kong.kflowweb.BP.WF.BlockModel.None);
		if (nd.getBlockModel() == com.kong.kflowweb.BP.WF.BlockModel.CurrNodeAll) {
			nd.setBlockModel(com.kong.kflowweb.BP.WF.BlockModel.CurrNodeAll);
		}
		if (nd.getBlockModel() == com.kong.kflowweb.BP.WF.BlockModel.SpecSubFlow) {
			nd.setBlockModel(com.kong.kflowweb.BP.WF.BlockModel.SpecSubFlow);
			nd.setBlockExp(this.GetRequestVal("TB_SpecSubFlow"));
		}
		if (nd.getBlockModel() == com.kong.kflowweb.BP.WF.BlockModel.BySQL) {
			nd.setBlockModel(com.kong.kflowweb.BP.WF.BlockModel.BySQL);
			nd.setBlockExp(this.GetRequestVal("TB_SQL"));
		}
		if (nd.getBlockModel() == com.kong.kflowweb.BP.WF.BlockModel.ByExp) {
			nd.setBlockModel(com.kong.kflowweb.BP.WF.BlockModel.ByExp);
	...
	public String CanCancelNodes_Init() throws Exception {
		...
		com.kong.kflowweb.BP.WF.Node mynd = new com.kong.kflowweb.BP.WF.Node();
		mynd.setNodeID(this.getFK_Node());
		mynd.RetrieveFromDBSources();

		com.kong.kflowweb.BP.WF.Template.NodeCancels rnds = new com.kong.kflowweb.BP.WF.Template.NodeCancels();
		rnds.Retrieve(NodeCancelAttr.FK_Node, this.getFK_Node());

		com.kong.kflowweb.BP.WF.Nodes nds = new Nodes();
		nds.Retrieve(com.kong.kflowweb.BP.WF.Template.NodeAttr.FK_Flow, this.getFK_Flow());
	...
	public String CanCancelNodes_Save() throws Exception {
		com.kong.kflowweb.BP.WF.Template.NodeCancels rnds = new com.kong.kflowweb.BP.WF.Template.NodeCancels();
		rnds.Delete(com.kong.kflowweb.BP.WF.Template.NodeCancelAttr.FK_Node, this.getFK_Node());
	...
	public String CanReturnNodes_Init() throws Exception {
		...
		com.kong.kflowweb.BP.WF.Template.NodeReturns rnds = new com.kong.kflowweb.BP.WF.Template.NodeReturns();
	...
	public String CanReturnNodes_Save() throws Exception {
		com.kong.kflowweb.BP.WF.Template.NodeReturns rnds = new com.kong.kflowweb.BP.WF.Template.NodeReturns();
		rnds.Delete(com.kong.kflowweb.BP.WF.Template.NodeReturnAttr.FK_Node, this.getFK_Node());
	...
	public String PushMessage_Init() throws Exception {
		com.kong.kflowweb.BP.WF.Template.PushMsg enDel = new com.kong.kflowweb.BP.WF.Template.PushMsg();
	...
	public String PushMessage_ShowHidden() {
		com.kong.kflowweb.BP.WF.XML.EventLists xmls = new com.kong.kflowweb.BP.WF.XML.EventLists();
		xmls.RetrieveAll();
		for (com.kong.kflowweb.BP.WF.XML.EventList item : xmls.ToJavaList()) {
			if (item.getIsHaveMsg() == false)
				continue;

		}
		return com.kong.kflowweb.BP.Tools.Json.ToJson(xmls);

	}

	public String PushMessageEntity_Init() throws Exception {
		String fk_node = GetRequestVal("FK_Node");
		com.kong.kflowweb.BP.WF.Template.PushMsg en = new com.kong.kflowweb.BP.WF.Template.PushMsg();
	...
	public String PushMessageEntity_Save() throws Exception {
		com.kong.kflowweb.BP.WF.Template.PushMsg msg = new com.kong.kflowweb.BP.WF.Template.PushMsg();
		msg.setMyPK(this.getMyPK());
		msg.RetrieveFromDBSources();
		msg.setFK_Event(this.getFK_Event());
		msg.setFK_Node(this.getFK_Node());
		// msg = com.kong.kflowweb.BP.Sys.PubClass.CopyFromRequestByPost(msg, context.Request) as
		// com.kong.kflowweb.BP.WF.Template.PushMsg;
		msg.Save(); // 执行保存.
	...
	public String PushMsg_Init() throws Exception {
		...
		com.kong.kflowweb.BP.WF.Template.PushMsgs ens = new com.kong.kflowweb.BP.WF.Template.PushMsgs(nodeID);
		return ens.ToJson();
	}
	...
	public final String PushMsgEntity_Init() throws Exception {
		...
		com.kong.kflowweb.BP.Sys.MapAttrs attrs = new com.kong.kflowweb.BP.Sys.MapAttrs();
		attrs.Retrieve(com.kong.kflowweb.BP.Sys.MapAttrAttr.FK_MapData, "ND" + this.getFK_Node(), "LGType", 0, "MyDataType", 1);
		ds.Tables.add(attrs.ToDataTableField("FrmFields"));

		// 节点
		// TODO 数据太多优化一下
		com.kong.kflowweb.BP.WF.Node nd = new com.kong.kflowweb.BP.WF.Node(this.getFK_Node());
		com.kong.kflowweb.BP.WF.Nodes nds = new com.kong.kflowweb.BP.WF.Nodes(nd.getFK_Flow());
		ds.Tables.add(nds.ToDataTableField("Nodes"));
	...
	public final String PushMsg_Save() throws Exception {
		...
		String nodesOfSMS = "";
		for (com.kong.kflowweb.BP.WF.Node mynd : nds.ToJavaList()) {
		...
		//节点预警
		if (this.getFK_Event() == com.kong.kflowweb.BP.Sys.EventListOfNode.NodeWarning) {
		...
		//节点逾期
		if (this.getFK_Event() == com.kong.kflowweb.BP.Sys.EventListOfNode.NodeOverDue) {
			int noticeType = Integer.parseInt(this.GetRequestVal("RB_NoticeType").replace("RB_NoticeType", ""));
			msg.SetPara("NoticeType", noticeType);
		...
		if (DataType.IsNullOrEmpty(msg.getMyPK()) == true) {
			msg.setMyPK(com.kong.kflowweb.BP.DA.DBAccess.GenerGUID());
			msg.Insert();
		} else {
			msg.Update();
		}
