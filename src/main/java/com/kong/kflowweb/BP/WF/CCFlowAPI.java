...
import com.kong.kflowweb.BP.WF.Template.FrmEleType;
...
import com.kong.kflowweb.BP.WF.Template.SubFlowYanXus;
...
public class CCFlowAPI {
	...
	public static DataSet GenerWorkNode(String fk_flow, int fk_node, long workID, 
		...
		if (workID == 0)
			workID = com.kong.kflowweb.BP.WF.Dev2Interface.Node_CreateBlankWork(fk_flow, null, null, userNo, null, 0, 0, null, 0, null, 0,
					null);
		...
		try {
			...
			DataSet myds = com.kong.kflowweb.BP.Sys.CCFormAPI.GenerHisDataSet(md.getNo(), nd.getName());
			...
			if (nd.getFormType() == NodeFormType.FoolTruck && nd.getIsStartNode() == false
				...
				if (com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBType() == DBType.MSSQL) {
					myFrmIDs = myFrmIDs.replace("'", "");
					sqlOrder += " ORDER BY CHARINDEX(FrmID, '" + myFrmIDs + "'), Idx";
				}

				if (com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBType() == DBType.MySQL) {
					myFrmIDs = myFrmIDs.replace("'", "");
					sqlOrder += " ORDER BY INSTR('" + myFrmIDs + "',FrmID), Idx";
				}
				...
				// 把扩展放入里面去.
				myFrmIDs = wk.HisPassedFrmIDs + ",'ND" + fk_node + "'";
				com.kong.kflowweb.BP.Sys.MapExts exts = new MapExts();
                ...
                //把从表放里面
                myFrmIDs = wk.HisPassedFrmIDs + ",'ND" + fk_node + "'";
                com.kong.kflowweb.BP.Sys.MapDtls dtls = new MapDtls();
                ...
                //把附件放里面
                myFrmIDs = wk.HisPassedFrmIDs + ",'ND" + fk_node + "'";
                com.kong.kflowweb.BP.Sys.FrmAttachment frmAtchs = new FrmAttachment();
			...
			// #region 流程设置信息.
			com.kong.kflowweb.BP.WF.Dev2Interface.Node_SetWorkRead(fk_node, workID);
			if(nd.getIsStartNode() == false){
				if (gwf.getTodoEmps().contains(com.kong.kflowweb.BP.Web.WebUser.getName() + ";") == false)
                {
                    gwf.setTodoEmps(gwf.getTodoEmps() +com.kong.kflowweb.BP.Web.WebUser.getNo() + "," + com.kong.kflowweb.BP.Web.WebUser.getName());
                    gwf.Update();
                }
			}
			...
			if (me.RetrieveFromDBSources() == 1) {
				...
				Entity tempVar = com.kong.kflowweb.BP.WF.Glo.DealPageLoadFull(wk, me, attrs, dtls);
...
*/
			...
			case AskForReplay: // 返回加签的信息.
				...
				DataTable mydt = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(mysql);
			...
			case Shift:
				...
				com.kong.kflowweb.BP.En.QueryObject qo = new QueryObject(fws);
