...
public class CCMobile extends WebContralBase
{
	...
	*/
	...
	public final String Home_Init() throws Exception
	{
		...
		ht.put("Todolist_Complete", com.kong.kflowweb.BP.WF.Dev2Interface.getTodolist_Complete());
//		ht.put("Todolist_Sharing", com.kong.kflowweb.BP.WF.Dev2Interface.getTodolist_Sharing());
		ht.put("Todolist_CCWorks", com.kong.kflowweb.BP.WF.Dev2Interface.getTodolist_CCWorks());
//		ht.put("Todolist_Apply", com.kong.kflowweb.BP.WF.Dev2Interface.getTodolist_Apply()); //申请下来的任务个数.
//		ht.put("Todolist_Draft", com.kong.kflowweb.BP.WF.Dev2Interface.getTodolist_Draft()); //草稿数量.

		ht.put("Todolist_HuiQian", com.kong.kflowweb.BP.WF.Dev2Interface.getTodolist_HuiQian()); //会签数量.
	...
	*/
	public final String Home_Init_WorkCount() throws Exception
	{
		...
		DataTable dt1 = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(sql);
     ...
     {
         com.kong.kflowweb.BP.WF.HttpHandler.WF_MyFlow wfPage = new WF_MyFlow(this.context);
         return wfPage.MyFlow_Init();
     }
	public final String Runing_Init() throws Exception
	{
		com.kong.kflowweb.BP.WF.HttpHandler.WF wfPage = new WF();
	...
	*/
	public final String Todolist_Init() throws Exception
	{
		...
		DataTable dt = com.kong.kflowweb.BP.WF.Dev2Interface.DB_Todolist(WebUser.getNo(), this.getFK_Node());
		return com.kong.kflowweb.BP.Tools.Json.ToJson(dt);
	}
	...
	{
		DataTable dt=null;
		dt=com.kong.kflowweb.BP.WF.Dev2Interface.DB_FlowComplete();
		return  com.kong.kflowweb.BP.Tools.Json.ToJson(dt);
	}
	...
	*/
	...
	///#region 关键字查询.
			...
			*/
			...
			{
				com.kong.kflowweb.BP.WF.HttpHandler.WF_RptSearch search = new WF_RptSearch();
			...
			*/
			public final String Search_Init() throws Exception
			{
				...
				{
					sql = "SELECT  TSpan as No, COUNT(WorkID) as Num FROM WF_GenerWorkFlow WHERE (Emps LIKE '%" + WebUser.getNo() + "%' OR TodoEmps LIKE '%"+com.kong.kflowweb.BP.Web.WebUser.getNo()+",%' OR Starter='" + WebUser.getNo() + "') AND WFState > 1 AND FID = 0 GROUP BY TSpan";
				}
				else
				{
					sql = "SELECT  TSpan as No, COUNT(WorkID) as Num FROM WF_GenerWorkFlow WHERE FK_Flow='" + this.getFK_Flow() + "' AND WFState > 1 AND FID = 0 AND (Emps LIKE '%" + WebUser.getNo() + "%' OR TodoEmps LIKE '%"+com.kong.kflowweb.BP.Web.WebUser.getNo()+",%' OR Starter='" + WebUser.getNo() + "')  GROUP BY TSpan";
				}
				...
				if (tSpan == null || tSpan.equals("-1"))
	                sql = "SELECT  FK_Flow as No, FlowName as Name, COUNT(WorkID) as Num FROM WF_GenerWorkFlow WHERE (Emps LIKE '%" + WebUser.getNo() + "%' OR TodoEmps LIKE '%" + com.kong.kflowweb.BP.Web.WebUser.getNo() + ",%' OR Starter='" + WebUser.getNo() + "')  AND WFState > 1 AND FID = 0 GROUP BY FK_Flow, FlowName";
				else 
	                sql = "SELECT  FK_Flow as No, FlowName as Name, COUNT(WorkID) as Num FROM WF_GenerWorkFlow WHERE TSpan=" + tSpan + " AND (Emps LIKE '%" + WebUser.getNo() + "%' OR TodoEmps LIKE '%"+com.kong.kflowweb.BP.Web.WebUser.getNo()+",%' OR Starter='" + WebUser.getNo() + "')  AND WFState > 1 AND FID = 0 GROUP BY FK_Flow, FlowName";
				...
				qo.addOr();
		        qo.AddWhere(GenerWorkFlowAttr.Starter, com.kong.kflowweb.BP.Web.WebUser.getNo());
		        qo.addOr();
		        qo.AddWhere(GenerWorkFlowAttr.TodoEmps," LIKE ","%" + com.kong.kflowweb.BP.Web.WebUser.getNo()+ ",%");
		        qo.addRightBracket();
			...
			*/
	        ...
	        {
	            com.kong.kflowweb.BP.WF.HttpHandler.WF_WorkOpt_OneWork en = new WF_WorkOpt_OneWork(this.context);
	            return en.OP_UnSend();
	        }
}