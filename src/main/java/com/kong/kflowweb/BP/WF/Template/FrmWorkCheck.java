...
public class FrmWorkCheck extends Entity {
	...
	public Map getEnMap() {
		...
		map.AddDDLSysEnum(FrmWorkCheckAttr.FWCSta, FrmWorkCheckSta.Disable.getValue(), "审核组件状态", true, true,
				FrmWorkCheckAttr.FWCSta, "@0=禁用@1=启用@2=只读");
		map.AddDDLSysEnum(FrmWorkCheckAttr.FWCShowModel, FrmWorkShowModel.Free.getValue(), "显示方式", true, true,
				FrmWorkCheckAttr.FWCShowModel, "@0=表格方式@1=自由模式"); // 此属性暂时没有用.

		map.AddDDLSysEnum(FrmWorkCheckAttr.FWCType, FWCType.Check.getValue(), "审核组件", true, true,
		...
		// map.AddTBInt(FrmWorkCheckAttr.FWCAth, 0, "轨迹图是否显示？", true, true);

		map.AddDDLSysEnum(FrmWorkCheckAttr.FWCAth, 0, "附件上传", true, true, FrmWorkCheckAttr.FWCAth,
		...
		map.AddDDLSysEnum(FrmWorkCheckAttr.FWCOrderModel, 0, "协作模式下操作员显示顺序", true, true, FrmWorkCheckAttr.FWCOrderModel,
				"@0=按审批时间先后排序@1=按照接受人员列表先后顺序(官职大小)");
