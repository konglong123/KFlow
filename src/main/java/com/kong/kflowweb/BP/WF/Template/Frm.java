...
import com.kong.kflowweb.BP.WF.FormRunType;
...
public class Frm extends EntityNoName
{
	...
	*/
	...
	public Map getEnMap()
	{
		...
		map.AddTBString(FrmAttr.FK_Flow, null, "独立表单属性:FK_Flow", true, false, 0, 50, 10);
		 //   map.AddDDLSysEnum(FrmAttr.FrmType, 0, "独立表单属性:运行类型", true, false, FrmAttr.FrmType);
			...
			//表单的运行类型.
		map.AddDDLSysEnum(FrmAttr.FrmType, com.kong.kflowweb.BP.Sys.FrmType.FreeFrm.getValue(), "表单类型",true, false, FrmAttr.FrmType);
		...
		map.AddTBInt(com.kong.kflowweb.BP.Sys.MapDataAttr.FrmW, 900, "表单宽度", true, false);
		map.AddTBInt(com.kong.kflowweb.BP.Sys.MapDataAttr.FrmH, 1200, "表单高度", true, false);
	...
	{
		return this.GetValIntByKey(com.kong.kflowweb.BP.Sys.MapDataAttr.FrmH);
	}
