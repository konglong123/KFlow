...
public class CCFormAPI extends Dev2Interface {
	...
	public static void getCreateOrSaveAthMulti(String fk_mapdata, String no, String name, float x, float y)
		...
		if (i == 0) {
			// ath.setSaveTo(SystemConfig.getPathOfDataUser() + "/UploadFile/" +
			// fk_mapdata + "/");
		}
	...
	public static void NewEnumField(String fk_mapdata, String field, String fieldDesc, String enumKey,
		...
		// 删除可能存在的数据.
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM Sys_FrmRB WHERE KeyOfEn='" + ma.getKeyOfEn() + "' AND FK_MapData='"
				+ ma.getFK_MapData() + "'");
	...
	public static DataSet GenerDBForVSTOExcelFrmModel(String frmID, int pkval) throws Exception {
		...
		for (MapDtl item : md.getMapDtls().ToJavaList()) {
			...
			dtMapAttr = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(sql);
			...
			for (DataRow dr : dtMapAttr.Rows) {
				...
				} else // 无范围限制时
				{
					...
					myds.Tables.add(com.kong.kflowweb.BP.Sys.PubClass.GetDataTableByUIBineKey(uiBindKey));
				}
				...
				if (me.RetrieveFromDBSources() == 1) {
					...
					Entity tempVar3 = com.kong.kflowweb.BP.WF.Glo.DealPageLoadFull(wk, me, attrs, dtls);
				...
				for (DataRow dr1 : dtMapAttr.Rows) {
					...
					if (lgType1.equals("1")) {
						...
						String mysql = "SELECT IntKey AS No, Lab as Name FROM Sys_Enum WHERE EnumKey='" + uiBindKey1
								+ "' ORDER BY IntKey ";
						DataTable dtEnum = DBAccess.RunSQLReturnTable(mysql);
					...
					Object tempVar4 = mes.GetEntityByKey(MapExtAttr.ExtType, MapExtXmlList.AutoFullDLL,
							MapExtAttr.AttrOfOper, keyOfEn1);
					...
					dt = com.kong.kflowweb.BP.Sys.PubClass.GetDataTableByUIBineKey(uiBindKey1);
	...
	public static DataSet GenerDBForCCFormDtl(String frmID, MapDtl dtl, int pkval, String atParas, long fid)
		...
		for (MapAttr attr : dtlAttrs.ToJavaList()) {
			...
			// #region 修改区分大小写.
			if (com.kong.kflowweb.BP.DA.DBType.Oracle.equals(SystemConfig.getAppCenterDBType())) {
