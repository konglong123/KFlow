...
import com.kong.kflowweb.BP.DA.AtPara;
...
import com.kong.kflowweb.BP.DA.TWay;
import com.kong.kflowweb.BP.Difference.ContextHolderUtils;
import com.kong.kflowweb.BP.En.Attr;
import com.kong.kflowweb.BP.En.Attrs;
import com.kong.kflowweb.BP.En.Entity;
import com.kong.kflowweb.BP.En.FieldType;
import com.kong.kflowweb.BP.En.QueryObject;
import com.kong.kflowweb.BP.En.Row;
import com.kong.kflowweb.BP.En.UIContralType;
import com.kong.kflowweb.BP.Port.Emp;
import com.kong.kflowweb.BP.Sys.AthCtrlWay;
import com.kong.kflowweb.BP.Sys.AthUploadWay;
import com.kong.kflowweb.BP.Sys.DefVal;
import com.kong.kflowweb.BP.Sys.DtlOpenType;
import com.kong.kflowweb.BP.Sys.FrmAttachment;
import com.kong.kflowweb.BP.Sys.FrmAttachmentAttr;
import com.kong.kflowweb.BP.Sys.FrmAttachmentDBAttr;
import com.kong.kflowweb.BP.Sys.FrmAttachments;
import com.kong.kflowweb.BP.Sys.FrmEvent;
import com.kong.kflowweb.BP.Sys.FrmImg;
import com.kong.kflowweb.BP.Sys.FrmRB;
import com.kong.kflowweb.BP.Sys.FrmTree;
import com.kong.kflowweb.BP.Sys.FrmTrees;
import com.kong.kflowweb.BP.Sys.GEDtl;
...
import com.kong.kflowweb.BP.Sys.MapExtAttr;
import com.kong.kflowweb.BP.Sys.MapExtXmlList;
import com.kong.kflowweb.BP.Sys.MapExts;
import com.kong.kflowweb.BP.Sys.OSModel;
import com.kong.kflowweb.BP.Sys.SFDBSrc;
import com.kong.kflowweb.BP.Sys.SFTable;
import com.kong.kflowweb.BP.Sys.SystemConfig;
import com.kong.kflowweb.BP.Sys.ToolbarExcel;
import com.kong.kflowweb.BP.Tools.DateUtils;
import com.kong.kflowweb.BP.Tools.FtpUtil;
import com.kong.kflowweb.BP.Tools.SftpUtil;
import com.kong.kflowweb.BP.Tools.StringHelper;
import com.kong.kflowweb.BP.WF.Data.CH;
import com.kong.kflowweb.BP.WF.Data.CHSta;
import com.kong.kflowweb.BP.WF.Data.GERpt;
import com.kong.kflowweb.BP.WF.Data.GERptAttr;
import com.kong.kflowweb.BP.WF.Port.WFEmp;
import com.kong.kflowweb.BP.WF.Rpt.MapRpts;
import com.kong.kflowweb.BP.WF.Template.CCList;
import com.kong.kflowweb.BP.WF.Template.Cond;
import com.kong.kflowweb.BP.WF.Template.Direction;
import com.kong.kflowweb.BP.WF.Template.FlowAttr;
import com.kong.kflowweb.BP.WF.Template.FlowExt;
import com.kong.kflowweb.BP.WF.Template.FlowSort;
import com.kong.kflowweb.BP.WF.Template.FlowSorts;
import com.kong.kflowweb.BP.WF.Template.FrmField;
import com.kong.kflowweb.BP.WF.Template.FrmFieldAttr;
import com.kong.kflowweb.BP.WF.Template.FrmFields;
import com.kong.kflowweb.BP.WF.Template.FrmNode;
import com.kong.kflowweb.BP.WF.Template.FrmTransferCustom;
import com.kong.kflowweb.BP.WF.Template.FrmWorkCheck;
import com.kong.kflowweb.BP.WF.Template.MapDataExt;
import com.kong.kflowweb.BP.WF.Template.NodeExt;
import com.kong.kflowweb.BP.WF.Template.NodeToolbar;
import com.kong.kflowweb.BP.WF.Template.SelectAccper;
import com.kong.kflowweb.BP.WF.Template.SubFlowYanXu;
import com.kong.kflowweb.BP.WF.Template.SysForm;
...
public class Glo {
	...
	public static SftpUtil getSftpUtil() throws Exception {
		...
		String pass = com.kong.kflowweb.BP.Sys.Glo.String_JieMi_FTP(SystemConfig.getFTPUserPassword());
	...
	public static FtpUtil getFtpUtil() throws Exception {
		// 获取
		String ip = com.kong.kflowweb.BP.Sys.Glo.String_JieMi_FTP(SystemConfig.getFTPServerIP());

		String userNo = com.kong.kflowweb.BP.Sys.Glo.String_JieMi_FTP(SystemConfig.getFTPUserNo());
		String pass = com.kong.kflowweb.BP.Sys.Glo.String_JieMi_FTP(SystemConfig.getFTPUserPassword());
	...
	public static ShortMessageWriteTo getShortMessageWriteTo() {
		return ShortMessageWriteTo.forValue(com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKeyInt("ShortMessageWriteTo", 0));
	}
	 ...
	 */
	public static Plant Plant = com.kong.kflowweb.BP.WF.Plant.JFlow;
	/**
	 * 当前版本号-为了升级使用.
	 */
	...
	public static String UpdataCCFlowVer() throws Exception {
		...
		int currVer = DBAccess.RunSQLReturnValInt(sql, 0);
		 ...
		 //检查子流程表.
        if (com.kong.kflowweb.BP.DA.DBAccess.IsExitsObject("WF_NodeSubFlow") == true)
        {
            if (com.kong.kflowweb.BP.DA.DBAccess.IsExitsTableCol("WF_NodeSubFlow", "OID") == true)
            {
		...
		try {
			...
			if (DBAccess.IsExitsTableCol("Sys_FrmEvent", "DoType") == true) {
				com.kong.kflowweb.BP.Sys.FrmEvent fe = new FrmEvent();
			...
			// 序列号.
			com.kong.kflowweb.BP.Sys.Serial se = new com.kong.kflowweb.BP.Sys.Serial();
			se.CheckPhysicsTable();

			com.kong.kflowweb.BP.WF.Template.NodeExt ext = new com.kong.kflowweb.BP.WF.Template.NodeExt();
			ext.CheckPhysicsTable();

			com.kong.kflowweb.BP.Sys.EnCfg cfg = new com.kong.kflowweb.BP.Sys.EnCfg();
			cfg.CheckPhysicsTable();

			// 执行升级 2016.04.08
			com.kong.kflowweb.BP.WF.Template.Cond cnd = new Cond();
			cnd.CheckPhysicsTable();
			
			 //增加列FlowStars
            com.kong.kflowweb.BP.WF.Port.WFEmp wfemp = new com.kong.kflowweb.BP.WF.Port.WFEmp();
            wfemp.CheckPhysicsTable();
            //#region 更新wf_emp. 的字段类型. 2019.06.19
            DBType dbtype = com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBType();
			...
			// /#region 标签Ext
			sql = "DELETE FROM Sys_EnCfg WHERE No='com.kong.kflowweb.BP.WF.Template.NodeExt'";
			com.kong.kflowweb.BP.DA.DBAccess.RunSQL(sql);
			sql = "INSERT INTO Sys_EnCfg(No,GroupTitle) VALUES ('com.kong.kflowweb.BP.WF.Template.NodeExt','";
			sql += "@NodeID=基本配置";
			...
			sql = "DELETE FROM Sys_EnCfg WHERE No='com.kong.kflowweb.BP.WF.Template.FlowExt'";
			com.kong.kflowweb.BP.DA.DBAccess.RunSQL(sql);
			sql = "INSERT INTO Sys_EnCfg(No,GroupTitle) VALUES ('com.kong.kflowweb.BP.WF.Template.FlowExt','";
			sql += "@No=基本配置";
			sql += "')";
			com.kong.kflowweb.BP.DA.DBAccess.RunSQL(sql);

			sql = "DELETE FROM Sys_EnCfg WHERE No='com.kong.kflowweb.BP.WF.Template.MapDataExt'";
			com.kong.kflowweb.BP.DA.DBAccess.RunSQL(sql);
			sql = "INSERT INTO Sys_EnCfg(No,GroupTitle) VALUES ('com.kong.kflowweb.BP.WF.Template.MapDataExt','";
			sql += "@No=基本属性";
			sql += "@Designer=设计者信息";
			sql += "')";
			com.kong.kflowweb.BP.DA.DBAccess.RunSQL(sql);
			...
			// /#region 标签
			sql = "DELETE FROM Sys_EnCfg WHERE No='com.kong.kflowweb.BP.WF.Template.NodeSheet'";
			com.kong.kflowweb.BP.DA.DBAccess.RunSQL(sql);
			sql = "INSERT INTO Sys_EnCfg(No,GroupTitle) VALUES ('com.kong.kflowweb.BP.WF.Template.NodeSheet','";
			sql += "@NodeID=基本配置";
			sql += "@FormType=表单";
			sql += "@FWCSta=审核组件,适用于sdk表单审核组件与ccform上的审核组件属性设置.";
			...
			sql = "DELETE FROM Sys_EnCfg WHERE No='com.kong.kflowweb.BP.WF.Template.FlowSheet'";
			com.kong.kflowweb.BP.DA.DBAccess.RunSQL(sql);
			sql = "INSERT INTO Sys_EnCfg(No,GroupTitle) VALUES ('com.kong.kflowweb.BP.WF.Template.FlowSheet','";
			sql += "@No=基本配置";
			...
			com.kong.kflowweb.BP.WF.GenerWorkFlow gwf = new GenerWorkFlow();
			...
			for (DataRow dr : dt.Rows) {
				sql = "UPDATE WF_GenerWorkFlow SET WeekNum="
						+ com.kong.kflowweb.BP.DA.DataType.WeekOfYear(DataType.ParseSysDateTime2DateTime(dr.getValue(1).toString()))
						+ " WHERE WorkID=" + dr.getValue(0).toString();
				com.kong.kflowweb.BP.DA.DBAccess.RunSQL(sql);
			}
			// 查询.
			com.kong.kflowweb.BP.WF.Data.CH ch = new CH();
			...
			try {
				...
				// oracle数据库无法识别“+”
				// 增加oracle数据库判断
				if (SystemConfig.getAppCenterDBType() == com.kong.kflowweb.BP.DA.DBType.Oracle) {
				...
				} else if (SystemConfig.getAppCenterDBType() == com.kong.kflowweb.BP.DA.DBType.MySQL) {
				...
				com.kong.kflowweb.BP.DA.DBAccess.RunSQLs(sqls);
			...
			// 升级
			com.kong.kflowweb.BP.Sys.SFTable tab = new SFTable();
			...
			com.kong.kflowweb.BP.WF.Template.NodeSheet nodeSheet = new com.kong.kflowweb.BP.WF.Template.NodeSheet();
			nodeSheet.CheckPhysicsTable();

			// /#region 把节点的toolbarExcel, word 信息放入mapdata
			com.kong.kflowweb.BP.WF.Template.NodeSheets nss = new com.kong.kflowweb.BP.WF.Template.NodeSheets();
			nss.RetrieveAll();
			for (com.kong.kflowweb.BP.WF.Template.NodeSheet ns : nss.ToJavaList()) {
			...
			com.kong.kflowweb.BP.WF.Template.FlowFormTree tree = new com.kong.kflowweb.BP.WF.Template.FlowFormTree();
			...
			// /#region 执行sql．
			com.kong.kflowweb.BP.DA.DBAccess.RunSQL(
			...
			com.kong.kflowweb.BP.DA.DBAccess.RunSQLs(sql);
			...
			com.kong.kflowweb.BP.WF.SMS sms = new SMS();
			sms.CheckPhysicsTable();
			// /#region 重新生成view WF_EmpWorks, 2013-08-06.
			try {

				if (DBAccess.IsExitsObject("WF_EmpWorks") == true)
					com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DROP VIEW WF_EmpWorks");

				if (DBAccess.IsExitsObject("V_FlowStarter") == true)
					com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DROP VIEW V_FlowStarter");

				if (DBAccess.IsExitsObject("V_FlowStarterBPM") == true)
					com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DROP VIEW V_FlowStarterBPM");

				if (DBAccess.IsExitsObject("V_TOTALCH") == true)
					com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DROP VIEW V_TOTALCH");

				if (DBAccess.IsExitsObject("V_TOTALCHYF") == true)
					com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DROP VIEW V_TOTALCHYF");

				if (DBAccess.IsExitsObject("V_TOTALCHWeek") == true)
					com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DROP VIEW V_TOTALCHWeek");
				
				if (DBAccess.IsExitsObject("V_WF_Delay") == true)
					com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DROP VIEW V_WF_Delay");
			} catch (java.lang.Exception e4) {
			}

			String sqlscript = "";
			// 执行必须的sql.
			switch (com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBType()) {
			case Oracle:
				sqlscript = com.kong.kflowweb.BP.Sys.SystemConfig.getCCFlowAppPath() + "/WF/Data/Install/SQLScript/InitView_Ora.sql";
				break;
			case MSSQL:
			case Informix:
				sqlscript = com.kong.kflowweb.BP.Sys.SystemConfig.getCCFlowAppPath() + "/WF/Data/Install/SQLScript/InitView_SQL.sql";
				break;
			case MySQL:
				sqlscript = com.kong.kflowweb.BP.Sys.SystemConfig.getCCFlowAppPath() + "/WF/Data/Install/SQLScript/InitView_MySQL.sql";
				break;
			default:
				break;
			}
			...
			com.kong.kflowweb.BP.DA.DBAccess.RunSQLScript(sqlscript);
			...
			if (emp.RetrieveFromDBSources() == 1) {
				com.kong.kflowweb.BP.Web.WebUser.SignInOfGener(emp, true);
			} else {
				emp.setNo("admin");
				emp.setName("admin");
				emp.setFK_Dept("01");
			...
			com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLineError("系统升级错误:" + err, ex);
			return "0";
	...
	public static void UpdataCCFlowVerSQLScript() {
		...
		if ("".equals(currDBVer) || Integer.parseInt(currDBVer) != Integer.parseInt(myVer)) {
			com.kong.kflowweb.BP.DA.DBAccess.RunSQLScript(SystemConfig.getPathOfData() + "/UpdataCCFlowVer.sql");
	...
	public static String getCCFlowAppPath() {
		...
		if (request == null || request.getServerName() == null) {
			basePath = com.kong.kflowweb.BP.WF.Glo.getHostURL();
	...
	public static boolean IsCanInstall() throws Exception {
		 ...
		 try
         {
             ...
             com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(sql);
	...
	public static void DoInstallDataBase(String lang, int demoType) throws Exception {
		...
		ArrayList al = null;
		String info = "com.kong.kflowweb.BP.En.Entity";
		al = com.kong.kflowweb.BP.En.ClassFactory.GetObjects(info);

		FrmRB frmRb = new FrmRB();
		frmRb.CheckPhysicsTable();

		com.kong.kflowweb.BP.Sys.SysEnum se = new com.kong.kflowweb.BP.Sys.SysEnum();
		se.CheckPhysicsTable();

		com.kong.kflowweb.BP.Sys.SysEnumMain sem = new com.kong.kflowweb.BP.Sys.SysEnumMain();
		...
		if (com.kong.kflowweb.BP.DA.DBAccess.IsExitsTableCol("WF_Emp", "StartFlows") == false)
		{
			String sql = "";
			//增加StartFlows这个字段
		...
		for (Object obj : al) {
			...
			// 不安装CCIM的表.
			if (clsName != null && clsName.contains("com.kong.kflowweb.BP.CCIM")) {
				continue;
			}
			...
			// 不安装Work的表.
			if (clsName != null && clsName.equals("com.kong.kflowweb.BP.WF.Work")) {
				continue;
			}

			if (clsName != null && clsName.equals("com.kong.kflowweb.BP.WF.GEWork")) {
				continue;
			}

			// 抽象的类不允许创建表.
			switch (clsName) {
			case "com.kong.kflowweb.BP.WF.StartWork":
			case "com.kong.kflowweb.BP.WF.Work":
			case "com.kong.kflowweb.BP.WF.GEStartWork":
			case "com.kong.kflowweb.BP.En.GENoName":
			case "com.kong.kflowweb.BP.En.GETree":
			case "com.kong.kflowweb.BP.WF.Data.GERpt":
			case "com.kong.kflowweb.BP.WF.GEWork":
				continue;
			default:
				break; 
			}
			...
			if (isInstallFlowDemo == false) {
				// 如果不安装demo.
				if (clsName != null) {
					if (clsName.contains("com.kong.kflowweb.BP.CN") || clsName.contains("com.kong.kflowweb.BP.Demo")) {
						continue;
					}
				}
			}
		...
		// 2, 注册枚举类型 SQL
		// 2, 注册枚举类型。
		com.kong.kflowweb.BP.Sys.XML.EnumInfoXmls xmls = new com.kong.kflowweb.BP.Sys.XML.EnumInfoXmls();
		xmls.RetrieveAll();
		for (com.kong.kflowweb.BP.Sys.XML.EnumInfoXml xml : xmls.ToJavaList()) {
			com.kong.kflowweb.BP.Sys.SysEnums ses = new com.kong.kflowweb.BP.Sys.SysEnums();
		...
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM Port_DeptStation");

		String sqlscript = "";

		sqlscript = com.kong.kflowweb.BP.Sys.SystemConfig.getCCFlowAppPath() + "\\WF\\Data\\Install\\SQLScript\\Port_Inc_CH_BPM.sql";
		com.kong.kflowweb.BP.DA.DBAccess.RunSQLScript(sqlscript);
		...
		int i = 0;
		for (com.kong.kflowweb.BP.Port.Emp emp : emps.ToJavaList()) {
			i++;
			com.kong.kflowweb.BP.WF.Port.WFEmp wfEmp = new com.kong.kflowweb.BP.WF.Port.WFEmp();
			wfEmp.Copy(emp);
		...
		if (isInstallFlowDemo == true) {
			...
			com.kong.kflowweb.BP.Port.Emp emp = new com.kong.kflowweb.BP.Port.Emp("admin");
			com.kong.kflowweb.BP.Web.WebUser.SignInOfGener(emp);
			com.kong.kflowweb.BP.Sys.Glo.WriteLineInfo("开始装载模板...");

			// 装载数据模版.
			com.kong.kflowweb.BP.WF.DTS.LoadTemplete l = new com.kong.kflowweb.BP.WF.DTS.LoadTemplete();
			Object tempVar = l.Do();
			String msg = (String) ((tempVar instanceof String) ? tempVar : null);

			com.kong.kflowweb.BP.Sys.Glo.WriteLineInfo("装载模板完成......");
		...
		if (isInstallFlowDemo == true) {
			// 增加图片签名
			com.kong.kflowweb.BP.WF.DTS.GenerSiganture gs = new com.kong.kflowweb.BP.WF.DTS.GenerSiganture();
			gs.Do();
		...
		if (isInstallFlowDemo == true) {
			...
			for (Flow fl : fls.ToJavaList()) {
				try {
					...
					com.kong.kflowweb.BP.DA.Log.DebugWriteError(ex.getMessage());
				}
			}
		}

		com.kong.kflowweb.BP.Sys.Glo.WriteLineInfo("*********************************************");

		com.kong.kflowweb.BP.Sys.Glo.WriteLineInfo("1. jflow 已经成功安装如果有demo流程在运行之前执行流程检查,就会自动创建数据表。");

		com.kong.kflowweb.BP.Sys.Glo.WriteLineInfo("2. 再运行的时候有时候会出现数据库字段的异常信息，请刷新一次，系统就可以自动修复字段。");

		com.kong.kflowweb.BP.Sys.Glo.WriteLineInfo("3. 登录界面 http://loaclhost:8080/  默认密码为pub 或者123 。");
	...
	public static void DoInstallDataBase(String lang, boolean isInstallFlowDemo, boolean isInstallCCIM)
		...
		com.kong.kflowweb.BP.GPM.Emp empP = new com.kong.kflowweb.BP.GPM.Emp();
		...
		if (isInstallFlowDemo) {
			...
			DataTable dtStudent = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable("SELECT No FROM Demo_Student");
		...
		if (isInstallFlowDemo == true) {
            ...
            //创建一个空白的流程，不然，整个结构就出问题。
            com.kong.kflowweb.BP.Sys.FrmTrees frmTrees = new FrmTrees();
	...
	public static boolean CheckTreeRoot() throws Exception {
		...
		if (dt.Rows.size() == 0) {
			com.kong.kflowweb.BP.Port.Dept rootDept = new com.kong.kflowweb.BP.Port.Dept();
			rootDept.setName("总部");
			rootDept.setParentNo("0");
			try {
				rootDept.Insert();
			} catch (RuntimeException ex) {
				com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLineWarning("@尝试向port_dept插入数据失败，应该是视图问题. 技术信息:" + ex.getMessage());
			}
			throw new RuntimeException("@没有找到部门树为0个根节点, 有可能是因为您在集成cc的时候，没有遵守cc的规则，部门树的根节点必须是ParentNo=0。");
		}

		if (com.kong.kflowweb.BP.WF.Glo.getOSModel() == getOSModel().OneOne) {
			try {
				com.kong.kflowweb.BP.Port.Dept dept = new com.kong.kflowweb.BP.Port.Dept();
				dept.Retrieve(com.kong.kflowweb.BP.Port.DeptAttr.ParentNo, "0");
			} catch (RuntimeException ex) {
		...
		if (com.kong.kflowweb.BP.WF.Glo.getOSModel() == getOSModel().OneMore) {
			try {
				// com.kong.kflowweb.BP.GPM.Depts rootDepts = new com.kong.kflowweb.BP.GPM.Depts("0");
			} catch (RuntimeException ex) {
				throw new RuntimeException(
	...
	*/
	 ...
	 * @param emps
	 */
	public static void DealNote(String note, com.kong.kflowweb.BP.Port.Emps emps) {
	...
	public static FlowEventBase GetFlowEventEntityByEnName(String enName) {
		if (Htable_FlowFEE == null || Htable_FlowFEE.isEmpty()) {
			...
			ArrayList<FlowEventBase> al = com.kong.kflowweb.BP.En.ClassFactory.GetObjects("com.kong.kflowweb.BP.WF.FlowEventBase");
		...
		if (myen == null) {
			com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLineError("@根据类名称获取流程事件实体实例出现错误:" + enName + ",没有找到该类的实体.");
			return null;
		}
		return myen;
	}
	...
	public static void DealBuinessAfterSendWork(String fk_flow, long workid, String doFunc, String WorkIDs,
		...
		if (doFunc.equals("SetParentFlow")) {
			...
			for (String id : workids) {
				...
				// 设置当前流程的ID
				com.kong.kflowweb.BP.WF.Dev2Interface.SetParentInfo(gwfSubFlow.getFK_Flow(), workidC, gwfParent.getFK_Flow(),
						gwfParent.getWorkID(), gwfParent.getFK_Node(), WebUser.getNo());

				// 是否可以执行？
				if (com.kong.kflowweb.BP.WF.Dev2Interface.Flow_IsCanDoCurrentWork(workidC, WebUser.getNo()) == true) {
					// 执行向下发送.
					try {
						com.kong.kflowweb.BP.WF.Dev2Interface.Node_SendWork(gwfSubFlow.getFK_Flow(), workidC);
						okworkids += workidC;
					} catch (RuntimeException ex) {

						// 如果有一个发送失败，就撤销子流程与父流程.
						// 首先把主流程撤销发送.
						com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoUnSend(fk_flow, workid, 0);
						...
						for (String okwokid : myokwokid) {
							...
							com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoUnSend(cFlowNo, workidC, 0);
						}
	...
	public static String getEmpDept() {
		if (com.kong.kflowweb.BP.WF.Glo.getOSModel() == com.kong.kflowweb.BP.Sys.OSModel.OneMore) {
			return "Port_DeptEmp";
		} else {
			return "Port_EmpDept";
		}
	...
	public static boolean getIsAdmin() throws Exception {
		...
		return s.contains(com.kong.kflowweb.BP.Web.WebUser.getNo());
	}
	...
	public static String MapDataLikeKeyV1(String flowNo, String colName) {
		...
		String len = com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBLengthStr();
		if (flowNo.length() == 1) {
	...
	public static int getSMSSendTimeFromHour() {
		try {
			return Integer.parseInt(com.kong.kflowweb.BP.Sys.SystemConfig.getAppSettings().get("SMSSendTimeFromHour").toString());
		} catch (java.lang.Exception e) {
			return 8;
		}
	}
	...
	public static int getSMSSendTimeToHour() {
		try {
			return Integer.parseInt(com.kong.kflowweb.BP.Sys.SystemConfig.getAppSettings().get("SMSSendTimeToHour").toString());
		} catch (java.lang.Exception e) {
			return 8;
		}
	}
	...
	public static String Simulation_RunOne(String flowNo, String empNo, String paras) throws Exception {
		...
		com.kong.kflowweb.BP.WF.Dev2Interface.Port_Login(empNo);

		workid = com.kong.kflowweb.BP.WF.Dev2Interface.Node_CreateBlankWork(flowNo, ht, null, emp.getNo(), null, 0, 0, null, 0, null, 0,
				null);
		SendReturnObjs objs = com.kong.kflowweb.BP.WF.Dev2Interface.Node_SendWork(flowNo, workid, ht, null);
	...
	public static String GenerBillNo(String billNo, long workid, Entity en, String flowPTable) throws Exception {
		...
		return com.kong.kflowweb.BP.WF.WorkFlowBuessRole.GenerBillNo(billNo, workid, en, flowPTable);
	}
	...
	public static boolean IsMobile() {
		...
		String agent = com.kong.kflowweb.BP.Sys.Glo.getRequest().getHeader("UserAgent").toLowerCase().trim();
	...
	public static boolean getIsEnableDraft() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKeyBoolen("IsEnableDraft", false);
	}
	...
	public static boolean ExeExp(String exp, Entity en) throws Exception {
		...
		val = val.replace("%", "");
		val = val.replace("~", "");
		com.kong.kflowweb.BP.En.Row row = en.getRow();
	...
	public static boolean CondExpSQL(String sqlExp, Hashtable ht,long myWorkID) throws Exception {
		String sql = sqlExp;
		sql = sql.replace("~", "'");
		sql = sql.replace("@WebUser.No", com.kong.kflowweb.BP.Web.WebUser.getNo());
		sql = sql.replace("@WebUser.Name", com.kong.kflowweb.BP.Web.WebUser.getName());
		sql = sql.replace("@WebUser.FK_Dept", com.kong.kflowweb.BP.Web.WebUser.getFK_Dept());
	...
	public static boolean CondExpPara(String exp, Hashtable ht, long myWorkID) throws Exception {
		...
		if (ht.containsKey(key) == false) {
			...
			if (isHave == false) {
				if (com.kong.kflowweb.BP.WF.Glo.getSendHTOfTemp().containsKey(key) == false) {
					try {
				...
				valPara = com.kong.kflowweb.BP.WF.Glo.getSendHTOfTemp().get(key).toString().trim();
			}
...
;
	 ...
	 * @return
	 */
	public static String GenerMD5(com.kong.kflowweb.BP.WF.Work wk) {
		String s = null;
	...
	public static String LoadFlowDataWithToSpecNode(String xlsFile) throws Exception {
		DataTable dt = com.kong.kflowweb.BP.DA.DBLoad.GetTableByExt(xlsFile);
		String err = "";
		String info = "";
		...
		for (DataRow dr : dt.Rows) {
			...
			// /#region 检查数据是否完整。
			com.kong.kflowweb.BP.Port.Emp emp = new com.kong.kflowweb.BP.Port.Emp();
			emp.setNo(executer);
			...
			wk.SetValByKey(WorkAttr.Rec, com.kong.kflowweb.BP.Web.WebUser.getNo());
			wk.SetValByKey(StartWorkAttr.FK_Dept, com.kong.kflowweb.BP.Web.WebUser.getFK_Dept());
	...
	public static String LoadFlowDataWithToSpecEndNode(String xlsFile) throws Exception {
		...
		for (DataRow dr : dt.Rows) {
			...
			emp = new com.kong.kflowweb.BP.Port.Emp();
			emp.setNo(starter);
			if (emp.RetrieveFromDBSources() == 0) {
				err += "@账号:" + starter + ",不存在。";
				continue;
			...
			// 结束点结束。
			emp = new com.kong.kflowweb.BP.Port.Emp(executer);
			com.kong.kflowweb.BP.Web.WebUser.SignInOfGener(emp);
			...
			wkEnd.SetValByKey(WorkAttr.Rec, com.kong.kflowweb.BP.Web.WebUser.getNo());
			wkEnd.SetValByKey(StartWorkAttr.FK_Dept, com.kong.kflowweb.BP.Web.WebUser.getFK_Dept());
	...
	public static void setSessionMsg(String value) throws Exception {
		...
		if (i == 0) {
			// 如果没有更新到.
			com.kong.kflowweb.BP.WF.Port.WFEmp emp = new WFEmp();
			emp.setNo(com.kong.kflowweb.BP.Web.WebUser.getNo());
			emp.setName(com.kong.kflowweb.BP.Web.WebUser.getName());
			emp.setFK_Dept(com.kong.kflowweb.BP.Web.WebUser.getFK_Dept());
			emp.Insert();
			DBAccess.RunSQL(p);
		}
	}
	...
	public static String getFromPageType() {
		...
		if (_FromPageType == null) {
			try {
				String url = com.kong.kflowweb.BP.Sys.Glo.getRequest().getRemoteAddr();
	...
	public static java.util.Hashtable getSendHTOfTemp() throws Exception {
		...
		return (java.util.Hashtable) ((_SendHTOfTemp.get(com.kong.kflowweb.BP.Web.WebUser.getNo()) instanceof java.util.Hashtable)
				? _SendHTOfTemp.get(com.kong.kflowweb.BP.Web.WebUser.getNo()) : null);
	}
	...
	public static void setSendHTOfTemp(java.util.Hashtable value) throws Exception {
		...
		if (null != value)
			_SendHTOfTemp.put(com.kong.kflowweb.BP.Web.WebUser.getNo(), value);
		else
			_SendHTOfTemp.put(com.kong.kflowweb.BP.Web.WebUser.getNo(), new java.util.Hashtable());
	}
	...
	public static String getFlowFileBill() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.getPathOfDataUser()  + "/Bill/";
	}
	...
	public static String getIntallPath() {
		if (_IntallPath == null) {
			if (SystemConfig.getIsBSsystem() == true) {
				_IntallPath = com.kong.kflowweb.BP.WF.Glo.getCCFlowAppPath();
			}
		}
	...
	public static String getFlowCtrlBtnPos() {
		String s = (String) ((com.kong.kflowweb.BP.Sys.SystemConfig.getAppSettings().get("FlowCtrlBtnPos") instanceof String)
				? com.kong.kflowweb.BP.Sys.SystemConfig.getAppSettings().get("FlowCtrlBtnPos") : null);
	...
	public static String getGloSID() {
		String s = (String) ((com.kong.kflowweb.BP.Sys.SystemConfig.getAppSettings().get("GloSID") instanceof String)
				? com.kong.kflowweb.BP.Sys.SystemConfig.getAppSettings().get("GloSID") : null);
		if (s == null || s.equals("")) {
	...
	public static boolean getIsEnableCheckUseSta() {
		String s = (String) ((com.kong.kflowweb.BP.Sys.SystemConfig.getAppSettings().get("IsEnableCheckUseSta") instanceof String)
				? com.kong.kflowweb.BP.Sys.SystemConfig.getAppSettings().get("IsEnableCheckUseSta") : null);
	...
	public static boolean getIsEnableMyNodeName() {
		String s = (String) ((com.kong.kflowweb.BP.Sys.SystemConfig.getAppSettings().get("IsEnableMyNodeName") instanceof String)
				? com.kong.kflowweb.BP.Sys.SystemConfig.getAppSettings().get("IsEnableMyNodeName") : null);
		if (s == null || s.equals("0")) {
			return false;
		}
		return true;
	...
	public static boolean getIsQL() {
		String s = com.kong.kflowweb.BP.Sys.SystemConfig.getAppSettings().get("IsQL").toString();
		if (s == null || s.equals("0")) {
			return false;
		}
		return true;
	...
	public static boolean getIsEnableTaskPool() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKeyBoolen("IsEnableTaskPool", false);
	}
	...
	public static boolean getIsShowTitle() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKeyBoolen("IsShowTitle", false);
	}
	...
	public static boolean getIsEnablePRI() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKeyBoolen("IsEnablePRI", false);
	}
	...
	public static UserInfoShowModel getUserInfoShowModel() {
		return UserInfoShowModel.forValue(com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKeyInt("UserInfoShowModel", 0));
	}
	...
	public static String getUpdataMainDeptSQL() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKey("UpdataMainDeptSQL",
				"UPDATE Port_Emp SET FK_Dept=" + com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBVarStr() + "FK_Dept WHERE No="
						+ com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBVarStr() + "No");
	}
	...
	public static String getUpdataSID() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKey("UpdataSID",
				"UPDATE Port_Emp SET SID=" + com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBVarStr() + "SID WHERE No="
						+ com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBVarStr() + "No");
	}
	...
	public static String getSilverlightDownloadUrl() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKey("SilverlightDownloadUrl",
	...
	public static String DealUserInfoShowModel(String no, String name) {
		switch (com.kong.kflowweb.BP.WF.Glo.getUserInfoShowModel()) {
		case UserIDOnly:
			return "(" + no + ")";
		case UserIDUserName:
			return "(" + no + "," + name + ")";
	...
	public static boolean getIsEnable_DingDing() {
		// 如果两个参数都不为空说明启用
		String corpid = com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKey("Ding_CorpID", "");
		String corpsecret = com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKey("Ding_CorpSecret", "");
	...
	public static boolean getIsEnable_WeiXin() {
		// 如果两个参数都不为空说明启用
		String corpid = com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKey("WX_CorpID", "");
		String corpsecret = com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKey("WX_AppSecret", "");
		if (StringHelper.isNullOrEmpty(corpid) || StringHelper.isNullOrEmpty(corpsecret)) {
			return false;
		}
		return true;
	...
	public static OSModel getOSModel() {
		 ...
		 * OSModel.forValue(com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKeyInt("OSModel", 0));
		 * return os;
		 */
	...
	public static boolean getIsUnit() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKeyBoolen("IsUnit", false);
	}
	...
	public static boolean getIsEnableZhiDu() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKeyBoolen("IsEnableZhiDu", false);
	}
	...
	public static boolean getIsDeleteGenerWorkFlow() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKeyBoolen("IsDeleteGenerWorkFlow", false);
	}
	...
	public static boolean getIsEnableCheckFrmTreeIsNull() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKeyBoolen("IsEnableCheckFrmTreeIsNull", true);
	}
	...
	public static int getIsWinOpenStartWork() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKeyInt("IsWinOpenStartWork", 1);
	}
	...
	public static boolean getIsWinOpenEmpWorks() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKeyBoolen("IsWinOpenEmpWorks", true);
	}
	...
	public static boolean getIsEnableSysMessage() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKeyBoolen("IsEnableSysMessage", true);
	}
	...
	public static int getAutoNodeDTSTimeSpanMinutes() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKeyInt("AutoNodeDTSTimeSpanMinutes", 2);
	}
	...
	public static String getCCIMDBName() {
		String baseUrl = com.kong.kflowweb.BP.Sys.SystemConfig.getAppSettings().get("CCIMDBName").toString();
	...
	public static String getHostURL() {
		if (com.kong.kflowweb.BP.Sys.SystemConfig.getIsBSsystem()) {
			// 如果是BS 就要求 路径.
		}

		String baseUrl = com.kong.kflowweb.BP.Sys.SystemConfig.getAppSettings().get("HostURL").toString();
		if (StringHelper.isNullOrEmpty(baseUrl) == true) {
	...
	public static String getCurrPageID() {
		try {
			String url = com.kong.kflowweb.BP.Sys.Glo.getRequest().getRequestURI();
	...
	public static String getGetUserStyle() {
		// com.kong.kflowweb.BP.WF.Port.WFEmp emp = new WFEmp(WebUser.getNo());
		// if(string.IsNullOrEmpty(emp.Style) || emp.Style=="0")
		// {
		String userStyle = com.kong.kflowweb.BP.Sys.SystemConfig.getAppSettings().get("UserStyle").toString();
	...
	public static java.util.Date SetToWorkTime(java.util.Date dt) throws Exception {
		if (com.kong.kflowweb.BP.Sys.GloVar.getHolidays().contains(DateUtils.format(dt, "MM-dd"))) {
			dt = DateUtils.addDay(dt, 1);
			// 如果当前是节假日，就要从下一个有效期计算。
			while (true) {
				if (com.kong.kflowweb.BP.Sys.GloVar.getHolidays().contains(DateUtils.format(dt, "MM-dd")) == false) {
					{
						break;
					}
				}
	...
	private static void InitCH2017(Flow fl, Node nd, long workid, long fid, String title, String prvRDT, String sdt,
		...
		if (nd.getIsEndNode() == true) {
			...
			DataTable dt = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(ps);
			if (dt.Rows.size() == 0) {
				return;
			}
		...
		try {
			...
			if (ch.getIsExits()) {
				...
				ch.setMyPK(com.kong.kflowweb.BP.DA.DBAccess.GenerGUID());
				ch.Insert();
			}
		}
	...
	public static String getAMFrom() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKey("AMFrom", "08:30");
	}
	...
	public static float getAMPMHours() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKeyFloat("AMPMHours", 8);
	}
	...
	public static float getAMPMTimeSpan() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKeyFloat("AMPMTimeSpan", 1);
	}
	...
	public static String getAMTo() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKey("AMTo", "11:30");
	}
	...
	public static String getPMFrom() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKey("PMFrom", "13:30");
	}
	...
	public static String getPMTo() {
		return com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKey("PMTo", "17:30");
	}
	...
	public static com.kong.kflowweb.BP.Sys.FrmAttachmentDBs GenerFrmAttachmentDBs(FrmAttachment athDesc, String pkval,
			String FK_FrmAttachment) throws Exception {

		com.kong.kflowweb.BP.Sys.FrmAttachmentDBs dbs = new com.kong.kflowweb.BP.Sys.FrmAttachmentDBs();
		if (athDesc.getHisCtrlWay() == AthCtrlWay.PWorkID) {
			String pWorkID = Integer.toString(
					com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt("SELECT PWorkID FROM WF_GenerWorkFlow WHERE WorkID=" + pkval, 0));
			if (pWorkID == null || pWorkID == "0")
				pWorkID = pkval;

			if (athDesc.getAthUploadWay() == AthUploadWay.Inherit) {
				/* 继承模式 */
				com.kong.kflowweb.BP.En.QueryObject qo = new com.kong.kflowweb.BP.En.QueryObject(dbs);
	...
	public static String GenerActiveAths(MapExts exts, Node nd, Entity en, MapData md, MapAttrs attrs)
		...
		for (MapExt me : exts.ToJavaList()) {
			...
			if (!me.getTag1().equals("")) {
				...
				if (tag1.contains(com.kong.kflowweb.BP.Web.WebUser.getNo() + ",")) {
					// 根据设置的人员计算.
					isCando = true;
				}
			}
			...
			if (!me.getTag2().equals("")) {
				...
				sql = com.kong.kflowweb.BP.WF.Glo.DealExp(sql, en, null);
				if (com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValFloat(sql) > 0) {
					isCando = true;
				}
			}

			if (!me.getTag3().equals("") && com.kong.kflowweb.BP.Web.WebUser.getFK_Dept() == me.getTag3()) {
				// 根据部门编号判断.
				isCando = true;
			}
	...
	public static void ToMsg(String info, HttpServletResponse response) {
		// string rowUrl = com.kong.kflowweb.BP.Sys.Glo.getRequest().getRequestURI();
	...
	public static boolean CheckIsCanStartFlow_SendStartFlow(Flow flow, Work wk) throws Exception {
		...
		if (role == StartLimitRole.ResultIsZero) {
			sql = com.kong.kflowweb.BP.WF.Glo.DealExp(flow.getStartLimitPara(), wk, null);
			if (DBAccess.RunSQLReturnValInt(sql, 0) == 0) {
				return true;
			} else {
				return false;
	...
	public static boolean getIsEnableTrackRec() {
		String s = (String) com.kong.kflowweb.BP.Sys.SystemConfig.getAppSettings().get("IsEnableTrackRec");// get("IsEnableTrackRec").toString();
	...
	public static boolean getIsBeta() {
		String s = com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKey("IsBeta", "");
		if (DotNetToJavaStringHelper.isNullOrEmpty(s)) {
			return false;
		}
	...
	public static DataTable StartGuidEnties(long WorkID, String FK_Flow, int FK_Node, String sKey) throws Exception {
		...
		case BySQLOne:
			...
			// 保存
			com.kong.kflowweb.BP.WF.Dev2Interface.Node_SaveWork(FK_Flow, FK_Node, WorkID, ht);
			return dt;
