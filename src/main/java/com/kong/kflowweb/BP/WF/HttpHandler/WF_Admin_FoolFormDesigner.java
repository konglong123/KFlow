...
import com.kong.kflowweb.BP.En.EntityNoNameAttr;
import com.kong.kflowweb.BP.En.FieldTypeS;
import com.kong.kflowweb.BP.En.UIContralType;
import com.kong.kflowweb.BP.Tools.chs2py;
import com.kong.kflowweb.BP.WF.DotNetToJavaStringHelper;
import com.kong.kflowweb.BP.WF.ExtContral;
import com.kong.kflowweb.BP.WF.RefObject;
import com.kong.kflowweb.BP.WF.WSMethod;
import com.kong.kflowweb.BP.WF.Template.FrmNodeComponent;
import com.kong.kflowweb.BP.WF.Template.MapFrmFool;
...
public class WF_Admin_FoolFormDesigner extends WebContralBase
{
	...
	*/
	...
	public String SysEnum_Save() throws Exception {
		...
		com.kong.kflowweb.BP.DA.Cash.DelObjFormApplication(keyApp);
		return "保存成功";
	}
	...
	public String getSTable() throws Exception {
		if (_STable == null) {
			...
			if (_STable == null || "".equals(_STable)) {
				com.kong.kflowweb.BP.En.Entity en = com.kong.kflowweb.BP.En.ClassFactory.GetEn(this.getFK_MapData());
				if (en != null) {
...
//		return com.kong.kflowweb.BP.Tools.Json.ToJson(ds);
		...
		return com.kong.kflowweb.BP.Tools.Json.ToJson(map);
	}
	...
	public String ImpTableField_Save() throws Exception {
		...
		while (e.hasMoreElements()) {
			...
			if (name.startsWith("HID_Idx_")) {
				...
				ma.setLGType(com.kong.kflowweb.BP.En.FieldTypeS.Normal);
				//
				if (!"".equals(ma.getUIBindKey())) {
					...
					if (se.GetCountByKey(SysEnumAttr.EnumKey, ma.getUIBindKey()) > 0) {	// ?
						ma.setMyDataType(com.kong.kflowweb.BP.DA.DataType.AppInt);
						ma.setLGType(com.kong.kflowweb.BP.En.FieldTypeS.Enum);
						ma.setUIContralType(com.kong.kflowweb.BP.En.UIContralType.DDL);
					}
					SFTable tb = new SFTable();
					tb.setNo(ma.getUIBindKey());
					if (tb.IsExit(EntityNoNameAttr.No, ma.getUIBindKey())) {	// ?
						ma.setMyDataType(com.kong.kflowweb.BP.DA.DataType.AppString);
						ma.setLGType(com.kong.kflowweb.BP.En.FieldTypeS.FK);
						ma.setUIContralType(com.kong.kflowweb.BP.En.UIContralType.DDL);
					}
				}
				if (ma.getMyDataType() == com.kong.kflowweb.BP.DA.DataType.AppBoolean) {
					ma.setUIContralType(com.kong.kflowweb.BP.En.UIContralType.CheckBok);
				}
				...
				if (isLeft) {
					maxEnd = maxEnd + 40;
					lab = new FrmLab();
					lab.setMyPK(com.kong.kflowweb.BP.DA.DBAccess.GenerGUID());
	...
	*/
	public final String Designer_Init() throws Exception
	{
		...
		{
            if (this.getFK_MapData().contains("com.kong.kflowweb.BP.") == true)
            {
                /*如果是类的实体.*/
	...
	*/
	public final String MapDefDtlFreeFrm_Init() throws Exception
	{
		...
		if (dtl.RetrieveFromDBSources() == 0)
		{
			com.kong.kflowweb.BP.Sys.CCFormAPI.CreateOrSaveDtl(this.getFK_MapData(), this.getFK_MapDtl(), dtl.getName(), 100, 200);
		}
		else
		{
			com.kong.kflowweb.BP.Sys.CCFormAPI.CreateOrSaveDtl(this.getFK_MapData(), this.getFK_MapDtl(), dtl.getName(), dtl.getX(), dtl.getY());
		}
	...
	*/
	...
	public final String ParseStringToPinyin() //转拼音方法.
	{
		...
		String flag = GetRequestVal("flag");
	     ...
	     return com.kong.kflowweb.BP.Sys.CCFormAPI.ParseStringToPinyinField(name, flag.equals("true"), true, 20);
...
*/
	...
	public final String SFList_SaveSFField() throws Exception
	{
          ...
          com.kong.kflowweb.BP.Sys.CCFormAPI.SaveFieldSFTable(this.getFK_MapData(), this.getKeyOfEn(),
          ...
          if (sf.getSrcType() == SrcType.TableOrView || sf.getSrcType() == SrcType.BPClass
              ...
              return "../../Comm/En.htm?EnName=com.kong.kflowweb.BP.Sys.FrmUI.MapAttrSFTable&PKVal=" + attr.getMyPK();
          else
              return "../../Comm/En.htm?EnName=com.kong.kflowweb.BP.Sys.FrmUI.MapAttrSFSQL&PKVal=" + attr.getMyPK();
	}
	...
	*/
	public final String Designer_NewFrame() throws Exception
	{
		...
		//com.kong.kflowweb.BP.Sys.CCFormAPI.CreateOrSaveAthMulti(this.getFK_MapData(), this.GetRequestVal("FrameNo"), "我的附件", 100, 200);
		return frm.getMyPK();
	}
	/**
	 创建一个多附件
	...
	*/
	public final String Designer_AthNew() throws Exception
	{
		...
		com.kong.kflowweb.BP.Sys.CCFormAPI.CreateOrSaveAthMulti(this.getFK_MapData(), this.GetRequestVal("AthNo"), "我的附件", 100, 200);
		return ath.getMyPK();
	}
	/**
	 返回信息.
	...
	*/
    ...
    {
        com.kong.kflowweb.BP.Sys.GroupField gf = new GroupField();
    ...
    public final String GroupField_SaveCheck() throws Exception
    {
        ...
        com.kong.kflowweb.BP.Sys.CCFormAPI.CreateCheckGroup(this.getFK_MapData(), lab, prx);
        return "创建成功...";
    }
	...
	*/
	public final String GroupField_DeleteCheck() throws Exception
	{
		...
		com.kong.kflowweb.BP.WF.Template.MapFrmFool md = new com.kong.kflowweb.BP.WF.Template.MapFrmFool(this.getFK_MapData());
	...
	public final String EditFExtContral_Save() throws Exception
	{
		...
		switch (en.getUIContralType())
		{
			case AthShow:
				...
				ath.Update();
				com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM Sys_GroupField WHERE FrmID='" + this.getFK_MapData() + "' AND CtrlID='" + en.getAthRefObj() + "'");
	...
	*/
	public final String MapFrame_Init() throws Exception
	{
		...
		if (this.getMyPK() == null)
		{
			...
			mf.setName("我的框架.");
			mf.setFK_MapData(this.getFK_MapData());
			mf.setMyPK(com.kong.kflowweb.BP.DA.DBAccess.GenerGUID());
		}
		else
		{
	...
	*/
	...
	{
		MapFrame mf = new MapFrame();
		Object tempVar = com.kong.kflowweb.BP.Sys.PubClass.CopyFromRequestByPost(mf, this.getRequest());
	...
	public String FieldTypeListChoseOneField_Save() throws Exception
      {
          ...
          if (com.kong.kflowweb.BP.DA.DataType.AppBoolean == dataType)
	...
	*/
	public final String FieldTypeSelect_Create() throws Exception
	{
		...
		if (attr.getMyDataType() == DataType.AppString)
		{
			...
			attr.setUIContralType(UIContralType.TB);
			attr.Insert();
			return "url@../../Comm/En.htm?EnName=com.kong.kflowweb.BP.Sys.FrmUI.MapAttrString&MyPK=" + attr.getMyPK() + "&FK_MapData=" + this.getFK_MapData() + "&KeyOfEn=" + newNo + "&FType=" + attr.getMyDataType() + "&DoType=Edit&GroupField=" + this.getGroupField();
		}
		...
		if (attr.getMyDataType() == DataType.AppInt)
		{
			...
			return "url@../../Comm/En.htm?EnName=com.kong.kflowweb.BP.Sys.FrmUI.MapAttrNum&MyPK=" + attr.getMyPK() + "&FK_MapData=" + this.getFK_MapData() + "&KeyOfEn=" + newNo + "&FType=" + attr.getMyDataType() + "&DoType=Edit&GroupField=" + this.getGroupField();
		...
		if (attr.getMyDataType() == DataType.AppDate)
		{
			...
			return "url@../../Comm/En.htm?EnName=com.kong.kflowweb.BP.Sys.FrmUI.MapAttrDT&MyPK=" + attr.getMyPK() + "&FK_MapData=" + this.getFK_MapData() + "&KeyOfEn=" + newNo + "&FType=" + attr.getMyDataType() + "&DoType=Edit&GroupField=" + this.getGroupField();
		...
		if (attr.getMyDataType() == DataType.AppBoolean)
		{
			...
			return "url@../../Comm/En.htm?EnName=com.kong.kflowweb.BP.Sys.FrmUI.MapAttrBoolen&MyPK=" + attr.getMyPK() + "&FK_MapData=" + this.getFK_MapData() + "&KeyOfEn=" + newNo + "&FType=" + attr.getMyDataType() + "&DoType=Edit&GroupField=" + this.getGroupField();
	...
	*/
	public final String FieldInitGroupAndSysEnum() throws Exception
	{
		...
		// com.kong.kflowweb.BP.DA.DataType.WriteFile("c:\\FieldInitGroupAndSysEnum.json", json);
		return json;
	}
	...
	*/
	public final String EditF_Save() throws Exception
	{
		try
		{
			...
			if (attr.getMyDataType() == com.kong.kflowweb.BP.DA.DataType.AppString && lgType == FieldTypeS.Normal)
			{
	...
	*/
	public final String DtlSave() throws Exception
	{
		try
		{
			...
			com.kong.kflowweb.BP.Sys.PubClass.CopyFromRequest(dtl, this.getRequest());
...
//			using (FileStream iStream = File.OpenRead(fileFullName))
	...
	*/
	public final String Attachment_Save() throws Exception
	{
		...
		int i = ath.RetrieveFromDBSources();
		Object tempVar = com.kong.kflowweb.BP.Sys.PubClass.CopyFromRequestByPost(ath, this.getRequest());
	...
	*/
	...
	public final String SFGuide_Getmtds() throws Exception
	{
		...
		return com.kong.kflowweb.BP.Tools.Json.ToJson(mtds);
	}
	public final String SFGuide_GetCols() throws Exception
	{
		...
		return com.kong.kflowweb.BP.Tools.Json.ToJsonUpper(dt);
	}
	/** 
	 //获取表/视图列表
	...
	*/
	public final String SFGuide_GetClass() throws Exception
	{
		...
		{
			case 0:
				arr = ClassFactory.GetObjects("com.kong.kflowweb.BP.En.EntityNoName");
				break;
			case 1:
				arr = ClassFactory.GetObjects("com.kong.kflowweb.BP.En.EntitySimpleTree");
				break;
			default:
		...
		///#endregion
			...
			*/
			public final String ImpTableFieldSelectBindKey_Init() throws Exception
			{
				...
				com.kong.kflowweb.BP.Sys.SysEnumMains ens = new com.kong.kflowweb.BP.Sys.SysEnumMains();
				ens.RetrieveAll();
				ds.Tables.add(ens.ToDataTableField("EnumMain"));

				com.kong.kflowweb.BP.Sys.SFTables tabs = new com.kong.kflowweb.BP.Sys.SFTables();
				tabs.RetrieveAll();
...
public String FieldTypeListChoseOneField_Init() throws Exception
{
    ...
    //获得原始数据.
    DataTable dt = com.kong.kflowweb.BP.DA.DBAccess.GetTableSchema(ptable);

    //创建样本.
    DataTable mydt = com.kong.kflowweb.BP.DA.DBAccess.GetTableSchema(ptable);
    mydt.Rows.clear();
