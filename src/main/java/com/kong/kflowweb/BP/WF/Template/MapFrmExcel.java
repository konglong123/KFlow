...
public class MapFrmExcel extends EntityNoName
{
	...
	*/
	...
	public Map getEnMap()
	{
		...
		map.AddTBString(MapFrmExcelAttr.Name, null, "表单名称", true, false, 0, 500, 20,true);
			//数据源.
		map.AddDDLEntities(MapFrmExcelAttr.DBSrc, "local", "数据源", new com.kong.kflowweb.BP.Sys.SFDBSrcs(), true);
		map.AddDDLEntities(MapFrmExcelAttr.FK_FormTree, "01", "表单类别", new SysFormTrees(), true);
			//表单的运行类型.
		map.AddDDLSysEnum(MapFrmExcelAttr.FrmType, com.kong.kflowweb.BP.Sys.FrmType.FreeFrm.getValue(), "表单类型", true, false, MapFrmExcelAttr.FrmType);
			///#region 模版属性。
		...
		rm.Title = "手机端表单";
		rm.GroupName = "高级设置";
		rm.Icon = com.kong.kflowweb.BP.WF.Glo.getCCFlowAppPath() + "WF/Admin/CCFormDesigner/Img/telephone.png";
	...
	*/
	public final String DoNodeFrmCompent()
	{
		...
		{
			return Glo.getCCFlowAppPath() + "WF/Comm/EnOnly.htm?EnName=com.kong.kflowweb.BP.WF.Template.FrmNodeComponent&PK=" + this.getNo().replace("ND", "") + "&t=" + DataType.getCurrentDataTime();
		}
		else
		{
