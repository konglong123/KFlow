...
public class WF_Admin extends WebContralBase {
    ...
    {
        return com.kong.kflowweb.BP.Sys.SystemConfig.getRunOnPlant();
    }
	...
	public final String TestFlow_Init() throws Exception {
		...
		// 让admin 登录.
		com.kong.kflowweb.BP.WF.Dev2Interface.Port_Login("admin");

		if (this.getRefNo() != null) {
			HttpSession session = getRequest().getSession();
			Emp emp = new Emp(this.getRefNo());
			com.kong.kflowweb.BP.Web.WebUser.SignInOfGener(emp);
			com.kong.kflowweb.BP.Web.WebUser.SetSessionByKey("FK_Flow", this.getFK_Flow());
		...
		try {
			...
			if (nd.getHisDeliveryWay().equals(DeliveryWay.ByDeptAndStation)) {
				...
				if (com.kong.kflowweb.BP.WF.Glo.getOSModel() == com.kong.kflowweb.BP.Sys.OSModel.OneOne) {
					...
					sql += "AND No IN ";
					sql += "(";
					sql += "SELECT FK_Emp FROM " + com.kong.kflowweb.BP.WF.Glo.getEmpStation()
							+ " WHERE FK_Station IN ";
																		...
																		// 2016年11月开始约定此规则.
			{
				...
				if (dt.Rows.size() > 300) {
					if (SystemConfig.getAppCenterDBType() == com.kong.kflowweb.BP.DA.DBType.MSSQL) {
					...
					if (SystemConfig.getAppCenterDBType() == com.kong.kflowweb.BP.DA.DBType.MySQL) {
			...
			for (DataRow dr : dt.Rows) {
				...
				com.kong.kflowweb.BP.Port.Emp emp = new Emp(myemp);
			...
			// 返回数据源.
			return com.kong.kflowweb.BP.Tools.Json.DataTableToJson(dtEmps, false);
	...
	public final String TestFlow_ReturnToUser() throws Exception {
		...
		String sid = com.kong.kflowweb.BP.WF.Dev2Interface.Port_Login(userNo);
	...
	public final String DBInstall_Init() throws Exception {
		 ...
		 try
         {
             ...
             //判断是否可以安装,不能安装就抛出异常.
             com.kong.kflowweb.BP.WF.Glo.IsCanInstall();

             //判断是不是有.
             if (com.kong.kflowweb.BP.DA.DBAccess.IsExitsObject("WF_Flow") == true)
                 return "err@info数据库已经安装上了，您不必在执行安装. 点击:<a href='./CCBPMDesigner/Login.htm' >这里直接登录流程设计器</a>";


             Hashtable ht = new Hashtable();
             ht.put("OSModel", (int)com.kong.kflowweb.BP.WF.Glo.getOSModel().getValue()); //组织结构类型.
             ht.put("DBType", SystemConfig.getAppCenterDBType().toString()); //数据库类型.
             ht.put("Ver", com.kong.kflowweb.BP.WF.Glo.Ver); //版本号.
	...
	public final String DBInstall_Submit() {
		...
		try {
			...
			com.kong.kflowweb.BP.WF.Glo.DoInstallDataBase(lang, demoTye);

			// 执行jflow的升级。
			com.kong.kflowweb.BP.WF.Glo.UpdataCCFlowVer();

			// 加注释.
			com.kong.kflowweb.BP.Sys.PubClass.AddComment();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public final String BindFrms_Init() throws Exception {
		// 注册这个枚举，防止第一次运行出错.
		com.kong.kflowweb.BP.Sys.SysEnums ses = new SysEnums("FrmEnableRole");
	...
	public final String ReLoginSubmit() throws Exception {
	   ...
	   */
			public final String SettingTemplate_Init()
			{
				...
				com.kong.kflowweb.BP.WF.Template.SQLTemplates sqls = new SQLTemplates();
				//sqls.Retrieve(com.kong.kflowweb.BP.WF.Template.SQLTemplateAttr.SQLType, sqlType);
				...
				for (DataRow dr : dt.Rows)
				{
					com.kong.kflowweb.BP.WF.Template.SQLTemplate en = new SQLTemplate();
	...
	public final void TansEntitiesToGenerTree(Entities ens, String rootNo,
		...
		// attributes
		com.kong.kflowweb.BP.WF.Template.FlowFormTree formTree = (com.kong.kflowweb.BP.WF.Template.FlowFormTree) ((root instanceof com.kong.kflowweb.BP.WF.Template.FlowFormTree) ? root
				: null);
		if (formTree != null) {
	...
	public final void AddChildren(EntityTree parentEn, Entities ens,
		...
		for (EntityTree item : convertEntityMultiTree(ens)) {
			...
			// attributes
			com.kong.kflowweb.BP.WF.Template.FlowFormTree formTree = (com.kong.kflowweb.BP.WF.Template.FlowFormTree) ((item instanceof com.kong.kflowweb.BP.WF.Template.FlowFormTree) ? item
					: null);
			if (formTree != null) {
				String url = formTree.getUrl() == null ? "" : formTree.getUrl();
