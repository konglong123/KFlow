...
*/
	...
	*/
	...
	public final String FrmTable_GetSFTableList() throws Exception
	{
		...
		return com.kong.kflowweb.BP.Sys.CCFormAPI.DB_SFTableList(pageNumber, pageSize);
	}
	/** 
	 默认执行的方法
	 @return 
	...
	*/
	...
	public String DoDefaultMethod() throws Exception
	{
		...
		if("NewSFTableField".equals(getDoType()))//创建一个SFTable字段.
		{
			//调用接口,执行保存.
			try {
				com.kong.kflowweb.BP.Sys.CCFormAPI.SaveFieldSFTable(this.getFK_MapData(), this.getKeyOfEn(), this.GetRequestVal("Name"), this.GetRequestVal("UIBindKey"), this.GetRequestValFloat("x"), this.GetRequestValFloat("y"),0);
			} catch (Exception e) {
				return "保存外部字段失败";
			}
			return "执行成功.";
