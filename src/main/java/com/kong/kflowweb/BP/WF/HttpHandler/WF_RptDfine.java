...
import com.kong.kflowweb.BP.En.EntitiesSimpleTree;
...
import com.kong.kflowweb.BP.Sys.UserRegeditAttr;
...
*/
	public final String Default_Init() throws Exception
	{
		...
		if (rd.getMyDeptRole() == 0)
		{
			...
			{
				String sql = "SELECT Leader FROM Port_Dept WHERE No='" + com.kong.kflowweb.BP.Web.WebUser.getFK_Dept() + "'";
				String strs = DBAccess.RunSQLReturnStringIsNull(sql, null);
				if (strs != null && strs.contains(com.kong.kflowweb.BP.Web.WebUser.getNo()) == true)
				{
		...
		if (com.kong.kflowweb.BP.Web.WebUser.getIsAdmin())
		{
			ht.put("Adminer", "高级查询");
		}
		...
		return com.kong.kflowweb.BP.Tools.Json.ToJsonEntitiesNoNameModel(ht);
	}
	...
	public final String FlowSearch_Init() throws Exception
	{
		...
		{
			//如果没有找到，就让其重置一下.
			com.kong.kflowweb.BP.WF.Rpt.RptDfine rd = new RptDfine(this.getFK_Flow());
		...
		{
				qo.AddWhere(com.kong.kflowweb.BP.WF.Data.GERptAttr.FlowStarter, WebUser.getNo());
		}

		else if (this.getSearchType().equals("MyDept")) //我部门发起的.
		{
				qo.AddWhere(com.kong.kflowweb.BP.WF.Data.GERptAttr.FK_Dept, WebUser.getFK_Dept());
		}

		else if (this.getSearchType().equals("MyJoin")) //我参与的.
		{
				qo.AddWhere(com.kong.kflowweb.BP.WF.Data.GERptAttr.FlowEmps, " LIKE ", "%" + WebUser.getNo() + "%");
		}
		...
		for (MapAttr attr1 : rptAttrs.ToJavaList())
		{
			...
			switch (attr.getUIContralType())
			{
				...
				case DDL:
					...
					if (attr.getUIBindKey().equals("com.kong.kflowweb.BP.Port.Depts") || attr.getUIBindKey().equals("com.kong.kflowweb.BP.Port.Units")) //判断特殊情况。
					{
