...
import com.kong.kflowweb.BP.WF.TaskSta;
...
import com.kong.kflowweb.BP.WF.Template.FlowSheet;
...
public class MyDeptTodolist extends Entity
{
	...
	*/
	...
	{
		com.kong.kflowweb.BP.WF.WFState ws = (WFState)this.getWFState();
		switch(ws)
		{
			case Complete:
				return "已完成";
	...
	*/
	...
	public Map getEnMap()
	{
		...
		map.AddTBMyNum();
		map.AddDDLEntities(MyDeptTodolistAttr.FK_Emp, null, "当前处理人", new com.kong.kflowweb.BP.WF.Data.MyDeptEmps(), false);
		...
		map.AddSearchAttr(MyDeptTodolistAttr.FK_Emp);
			...
			//增加隐藏的查询条件.
		AttrOfSearch search = new AttrOfSearch(MyDeptTodolistAttr.WorkerDept, "部门", MyDeptTodolistAttr.WorkerDept, "=", com.kong.kflowweb.BP.Web.WebUser.getFK_Dept(), 0, true);
		...
		rm.getHisAttrs().AddDDLEntities("ToEmp", null, "移交给:", new com.kong.kflowweb.BP.WF.Flows(), true);
	...
	*/
	...
	{
		try {
			if (com.kong.kflowweb.BP.WF.Dev2Interface.Flow_IsCanViewTruck(this.getFK_Flow(), this.getWorkID(), null) == false)
			{
		...
		try
		{
			com.kong.kflowweb.BP.WF.Dev2Interface.Node_Shift(this.getFK_Flow(), this.getFK_Node(), this.getWorkID(), this.getFID(), ToEmp, Note);
			return "移交成功";
		}
	...
	*/
	public final String DoDelete() throws Exception
	{
		...
		try
		{
			com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoDeleteFlowByReal(this.getFK_Flow(), this.getWorkID(), true);
			return "删除成功";
		}
		catch (RuntimeException ex)
		{
	...
	*/
	...
	{
		com.kong.kflowweb.BP.WF.Template.FlowExt fl = new FlowExt(this.getFK_Flow());
