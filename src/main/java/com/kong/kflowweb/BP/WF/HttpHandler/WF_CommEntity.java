...
import com.kong.kflowweb.BP.En.AttrOfOneVSM;
import com.kong.kflowweb.BP.En.Attrs;
import com.kong.kflowweb.BP.En.AttrsOfOneVSM;
import com.kong.kflowweb.BP.En.ClassFactory;
import com.kong.kflowweb.BP.En.Dot2DotModel;
import com.kong.kflowweb.BP.En.EnDtl;
import com.kong.kflowweb.BP.En.EnDtls;
...
public class WF_CommEntity extends WebContralBase {
    ...
    public String BranchesAndLeaf_Init() throws Exception
    {
        ...
        String json=com.kong.kflowweb.BP.Tools.Json.ToJson(dtSelected);
  ...
  //  #region 实体的操作.
    ...
    public final String EntityOnly_Init()
    {
        try
        {
           ...
           // com.kong.kflowweb.BP.DA.DataType.WriteFile("C:\\EntityOnly_Init.TXT", str);
            return str;
            ...
            if (oneVsM.size() > 0)
            {
                for (AttrOfOneVSM vsM : oneVsM)
                {
                    ...
                    if (en.getPKVal() != null)
                    {
                        ...
                        {
                            // url = "Dot2Dot.aspx?EnsName=" + en.GetNewEntities.ToString() + "&EnName=" + this.EnName + "&AttrKey=" + vsM.EnsOfMM.ToString();
                            url = "Dot2Dot.htm?EnName=" + this.getEnName() + "&Dot2DotEnsName=" + vsM.getEnsOfMM().toString(); //比如:com.kong.kflowweb.BP.WF.Template.NodeStations
                            url += "&AttrOfOneInMM=" + vsM.getAttrOfOneInMM(); //存储表那个与主表关联. 比如: FK_Node
                            url += "&AttrOfMInMM=" + vsM.getAttrOfMInMM();  //dot2dot存储表那个与实体表.  比如:FK_Station.
                            url += "&EnsOfM=" + vsM.getEnsOfM().toString(); //默认的B实体.   //比如:com.kong.kflowweb.BP.Port.Stations
                            url += "&DefaultGroupAttrKey=" + vsM.DefaultGroupAttrKey; //默认的B实体分组依据.  比如:FK_Station.
   ...
   // #endregion 实体的操作.
    ...
    public String Dot2Dot_GenerGroupEntitis() throws Exception
    {
        ...
        if (attr.getMyFieldType() == FieldType.Enum)
        {
            ...
            com.kong.kflowweb.BP.Pub.NYs nys = new com.kong.kflowweb.BP.Pub.NYs();
            for (SysEnum item : ses.ToJavaList())
            {
                com.kong.kflowweb.BP.Pub.NY ny =new com.kong.kflowweb.BP.Pub.NY();
       ...
       // #region 把外键与枚举放入里面去.
        ...
        for (Attr attr : dtl.getEnMap().getAttrs())
        {
            ...
            if (attr.getUIBindKey().contains("SELECT") == true || attr.getUIBindKey().contains("select") == true)
            {
                ...
                sqlBindKey = com.kong.kflowweb.BP.WF.Glo.DealExp(sqlBindKey, null, null);
    ...
    public String Dtl_Exp() throws Exception
    {
        ...
        String filename = refPKVal + "_" + en.toString() +"_"+com.kong.kflowweb.BP.DA.DataType.getCurrentDataCNOfLong()+ "_" + name  +".xls";
