...
import com.kong.kflowweb.BP.WF.Template.Bill;
import com.kong.kflowweb.BP.WF.Template.BillFileType;
...
public class WorkNode {
	...
	public final String getVirPath() {
		if (_VirPath == null && com.kong.kflowweb.BP.Sys.SystemConfig.getIsBSsystem()) {
			_VirPath = Glo.getCCFlowAppPath(); // com.kong.kflowweb.BP.Sys.Glo.Request.ApplicationPath;
		}
		return _VirPath;
	}
	...
	public final String getAppType() {
		if (com.kong.kflowweb.BP.Sys.SystemConfig.getIsBSsystem() == false) {
			return "CCFlow";
		}

		if (_AppType == null && com.kong.kflowweb.BP.Sys.SystemConfig.getIsBSsystem()) {
			if (com.kong.kflowweb.BP.Web.WebUser.getIsWap()) {
				_AppType = "WF/WAP";
			} else {
				if (com.kong.kflowweb.BP.Sys.Glo.getRequest() == null)
					_AppType = "WF";
				else {
					boolean b = com.kong.kflowweb.BP.Sys.Glo.getRequest().getRequestURI().toLowerCase().contains("oneflow");
					if (b) {
						_AppType = "WF/OneFlow";
					} else {
						_AppType = "WF";
	...
	private GenerWorkerLists InitWorkerLists(WorkNode town, DataTable dt, long fid) throws Exception {
		...
		if (dt.Rows.size() == 1) {
			...
			if (town.getHisNode().getIsGuestNode()) {
				if (!this.getHisGenerWorkFlow().getGuestNo().equals("")) {
					...
					if (town.getHisNode().getHisDeliveryWay() == DeliveryWay.BySQL) {
						...
						DataTable mydt = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(Glo.DealExp(mysql, this.rptGe, null));
			...
			rm.setObjsExt(rm.getObjsExt() + com.kong.kflowweb.BP.WF.Glo.DealUserInfoShowModel(wl.getFK_Emp(), wl.getFK_EmpText()));
			rm.setEmps("@" + wl.getFK_Emp() + "@");
			rm.setEmpsExt(com.kong.kflowweb.BP.WF.Glo.DealUserInfoShowModel(wl.getFK_Emp(), wl.getFK_EmpText()));
				...
				for (GenerWorkerList wl : this.HisWorkerLists.ToJavaList()) {
					if (wl.getIsEnable() == false) {
						empExts += "<strike><font color=red>"
								+ com.kong.kflowweb.BP.WF.Glo.DealUserInfoShowModel(wl.getFK_Emp(), wl.getFK_EmpText())
								+ "</font></strike>、";
					} else {
						empExts += com.kong.kflowweb.BP.WF.Glo.DealUserInfoShowModel(wl.getFK_Emp(), wl.getFK_EmpText());
					}

					if (wl.getIsEnable() == true) {
						objs += wl.getFK_Emp() + "@";
						objsExt += com.kong.kflowweb.BP.WF.Glo.DealUserInfoShowModel(wl.getFK_Emp(), wl.getFK_EmpText());
					}
				}
		...
		if (at != ActionType.SubThreadForward) {
			if (this.HisWorkerLists.size() == 1) {
				...
				String emps = "";
				for (GenerWorkerList wl : this.HisWorkerLists.ToJavaList()) {
					info += com.kong.kflowweb.BP.WF.Glo.DealUserInfoShowModel(wl.getFK_DeptT(), wl.getFK_EmpText()) + ",";
		...
		if (ygflows.size() > 0 && 1 == 2) {
			for (SubFlowYanXu item : ygflows.ToJavaList()) {
				...
				if (item.getExpType() == ConnDataFrom.Paras)
					isPass = com.kong.kflowweb.BP.WF.Glo.CondExpPara(item.getCondExp(), this.rptGe.getRow(), this.getWorkID());

				if (item.getExpType() == ConnDataFrom.SQL)
					isPass = com.kong.kflowweb.BP.WF.Glo.CondExpSQL(item.getCondExp(), this.rptGe.getRow(), this.getWorkID());
	...
	public final Node NodeSend_GenerNextStepNode_Ext1() throws Exception {
		...
		while (true) {
			...
			if (!nd.getNodeFrmID().equals(mywork.NodeFrmID)) {
				...
				if (this.rptGe==null||this.rptGe.equals(""))
			...
			if (nd.getAutoJumpRole2()) {
				...
				for (DataRow dr : dt.Rows) {
					String sql = "SELECT COUNT(*) FROM WF_GenerWorkerList WHERE FK_Emp='" + dr.getValue(0).toString()
							+ "' AND WorkID=" + this.getWorkID() + " AND FK_Node="
				...
				if (isHave == true) {
					...
					com.kong.kflowweb.BP.WF.Glo.DealPageLoadFull(wk, item, nd.getMapData().getMapAttrs(), nd.getMapData().getMapDtls());
					...
					for (FrmImg img : imgs.ToJavaList()) {
						...
						if (!StringHelper.isNullOrEmpty(img.getTag0()) && img.getTag0().contains("^")
							...
							if (fk_dept.equals("all")) {
								...
								if (sealType.equals("1")) {
									...
									fk_dept = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnString(sql);
								}
								// 表单字段
						...
						sql = String.format(
								" select FK_Station from Port_DeptStation where FK_Dept ='%1$s' and FK_Station in (select FK_Station from "
										+ com.kong.kflowweb.BP.WF.Glo.getEmpStation() + " where FK_Emp='%2$s')",
								fk_dept, WebUser.getNo());
						dt = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(sql);
						for (DataRow dr : dt.Rows) {
						...
						try {
							imgSrc = com.kong.kflowweb.BP.WF.Glo.getCCFlowAppPath() + "DataUser/Seal/" + fk_dept + "_" + stationNo
									+ ".jpg";
							// 设置主键
	...
	private void Func_CheckCompleteCondition() throws Exception {
		...
		try {
			...
			if (this.getHisNode().getIsCCFlow() && this.getHisFlowCompleteConditions().getIsPass()) {
				...
				// string path = com.kong.kflowweb.BP.Sys.Glo.Request.ApplicationPath;
	...
	private void NodeSend_11(Node toND) throws Exception {
		...
		try {
			...
			com.kong.kflowweb.BP.DA.DBAccess.RunSQL(updateLengthSql);
	...
	private void NodeSend_24_UnSameSheet(Nodes toNDs) throws Exception {
		...
		for (Node nd : toNDs.ToJavaList()) {
			...
			wk.setOID(com.kong.kflowweb.BP.DA.DBAccess.GenerOID("WorkID"));
			wk.BeforeSave();
			wk.DirectInsert();
	...
	private void NodeSend_24_SameSheet(Node toNode) throws Exception {
		if (this.getHisGenerWorkFlow().getTitle().equals("未生成")) {
			this.getHisGenerWorkFlow()
					.setTitle(com.kong.kflowweb.BP.WF.WorkFlowBuessRole.GenerTitle(this.getHisFlow(), this.getHisWork()));
		}
		...
		if (toNode.getHisDeliveryWay() == DeliveryWay.BySQLAsSubThreadEmpsAndData) {
			...
			dtWork = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(sql);
		}
		...
		for (GenerWorkerList wl : current_gwls.ToJavaList()) {
			...
			if (IsHaveFH) {
				...
				if (this.IsHaveSubThreadGroupMark == true) {
					...
					if (dt.Rows.size() == 0) {
						mywk.setOID(DBAccess.GenerOID("WorkID")); // com.kong.kflowweb.BP.DA.DBAccess.GenerOID();
					} else {
					...
					for (FrmAttachmentDB athDB : athDBs.ToJavaList()) {
						...
						if (athDB_N.getHisAttachmentUploadType().equals(AttachmentUploadType.Single)) {
							...
							try {
								...
								athDB_N.setMyPK(com.kong.kflowweb.BP.DA.DBAccess.GenerGUID());
								athDB_N.Insert();
							}
						} else {
							try {
			...
			if (gwf.RetrieveFromDBSources() == 0) {
				...
				if (this.getHisNode().getIsStartNode()) {
					gwf.setTitle(com.kong.kflowweb.BP.WF.WorkFlowBuessRole.GenerTitle(this.getHisFlow(), this.getHisWork()) + "("
							+ wl.getFK_EmpText() + ")");
				} else {
				...
				gwf.setSender(com.kong.kflowweb.BP.WF.Glo.DealUserInfoShowModel(WebUser.getNo(), WebUser.getName()));
			...
			if (WebUser.getIsWap()) {
				this.addMsg("UnDoNew",
						"@<a href='" + getVirPath() + "WF/Wap/MyFlowInfo.jsp?DoType=UnSend&WorkID=" + this.getWorkID()
								+ "&FK_Flow=" + toNode.getFK_Flow()
								+ "'><img src='./Img/Action/UnSend.png' border=0/>撤销本次发送</a>， <a href='" + getVirPath()
	...
	private void NodeSend_53_SameSheet_To_HeLiu(Node toNode) throws Exception {
		...
		if (toNodeWK.RetrieveFromDBSources() == 0) // 查询出来数据.
			toNodeWK.DirectInsert();

		toNodeWK.Copy(this.getHisWork()); // 执行copy.
		...
		if (fid == 0) {
			...
			String strs = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnStringIsNull(
		...
		String emps = "";
		for (GenerWorkerList wl : current_gwls.ToJavaList()) {
			toEmpsStr += com.kong.kflowweb.BP.WF.Glo.DealUserInfoShowModel(wl.getFK_Emp(), wl.getFK_EmpText());
			if (current_gwls.size() == 1)
				emps = toEmpsStr;
			else
				emps += "@" + toEmpsStr;
		...
		// 增加变量.
		this.addMsg(SendReturnMsgFlag.VarAcceptersID, emps.replaceAll("@", ","), SendReturnMsgType.SystemMsg);
		...
		for (DataColumn dc : dt.Columns)
			mainWK.SetValByKey(dc.ColumnName, dt.Rows.get(0).get(dc.ColumnName));
		...
		// 处理国机的需求, 把最后一个子线程的主表数据同步到合流节点的Rpt里面去.
	...
	public final void CopyData(Work toWK, Node toND, boolean isSamePTable) throws Exception {
		...
		if (dtls.size() > 0) {
			...
			{
				startDtls = new com.kong.kflowweb.BP.Sys.MapDtls("ND" + Integer.parseInt(toND.getFK_Flow()) + "01");
			}
			...
			for (MapDtl dtl : dtls.ToJavaList()) {
				...
				if (com.kong.kflowweb.BP.DA.DBAccess.IsExitsObject(toDtl.getPTable())) {
					DBAccess.RunSQL("DELETE FROM " + toDtl.getPTable() + " WHERE RefPK=" + dbStr + "RefPK", "RefPK",
							(new Long(this.getWorkID())).toString());
				}
				...
				for (GEDtl gedtl : gedtls.ToJavaList()) {
					...
					com.kong.kflowweb.BP.Sys.GEDtl dtCopy = new GEDtl(toDtl.getNo());
					dtCopy.Copy(gedtl);
					dtCopy.FK_MapDtl = toDtl.getNo();
				...
				if (isEnablePass) {
					...
					for (GEDtl gedtl : gedtls.ToJavaList()) {
						...
						com.kong.kflowweb.BP.Sys.GEDtl dtCopy = new GEDtl(startDtl.getNo());
						dtCopy.Copy(gedtl);
						dtCopy.setOID(0);
	...
	public final void DealEvalUn() throws Exception {
		// 数据发送。
		com.kong.kflowweb.BP.WF.Data.Eval eval = new Eval();
	...
	private void CallSubFlow() throws Exception {
			...
			for (String keyval : keyvals) {
				...
				if (this.getHisNode().getSubFlowStartWay() == SubFlowStartWay.BySheetField) {
					...
					for (String emp : empStrs) {
						...
						com.kong.kflowweb.BP.Web.WebUser.SignInOfGener(empEn);
						...
						msgInfo += com.kong.kflowweb.BP.WF.Dev2Interface.Node_StartWork(FlowNo, null, null, 0, emp, this.getWorkID(),
								FlowNo);
					}
				}
		...
		// com.kong.kflowweb.BP.WF.Dev2Interface.Flow_NewStartWork(
		DataTable dt;
	 ...
	 * 
	 */
	private com.kong.kflowweb.BP.WF.Template.FrmNodes frmNDs = null;
	...
	public final void CheckFrmHuiZongToDtl() throws Exception {
		...
		for (FrmNode fn : frmNDs.ToJavaList()) {
			...
			dtlEn.setOID((int) this.getWorkID());
			dtlEn.setRDT(com.kong.kflowweb.BP.DA.DataType.getCurrentDataTime());
			dtlEn.setRec(com.kong.kflowweb.BP.Web.WebUser.getNo());
	...
	public final boolean CheckFrmIsNotNull() throws Exception {
		...
		if (this.getHisNode().getHisFormType() == NodeFormType.SheetTree) {
			...
			for (FrmNode item : nds.ToJavaList()) {
				...
				if (item.getFrmSln() == FrmSln.Self) {
					...
					ffs.Retrieve(FrmFieldAttr.FK_Node, this.getHisNode().getNodeID(), FrmFieldAttr.FK_MapData,
							md.getNo());
					// 获取整合后的mapAttrs
				...
				for (MapAttr mapAttr : mapAttrs.ToJavaList()) {
					...
					if (row.get(mapAttr.getKeyOfEn()) == null)
						err += "@表单{" + md.getName() + "; 字段" + mapAttr.getKeyOfEn() + " ; " + mapAttr.getName()
								+ "}，不能为空。";

					if ((row.get(mapAttr.getKeyOfEn()) instanceof String) == true) {
						String str = (String) (DataType.IsNullOrEmpty(row.get(mapAttr.getKeyOfEn()).toString()) == true
								? null : row.get(mapAttr.getKeyOfEn()));
						// 如果是检查不能为空
						if (str == null || DotNetToJavaStringHelper.isNullOrEmpty(str) == true
								|| str.trim().equals("")) {
							err += "@表单{" + md.getName() + "; 字段" + mapAttr.getKeyOfEn() + " ; " + mapAttr.getName()
									+ "}，不能为空。";
						}
		...
		if (this.getHisNode().getHisFormType() == NodeFormType.FreeForm
			...
			for (MapAttr attr : attrs.ToJavaList()) {
				...
				if ((row.get(attr.getKeyOfEn()) instanceof String) == true) {
					String str = (String) (DataType.IsNullOrEmpty(row.get(attr.getKeyOfEn()).toString()) == true ? null
							: row.get(attr.getKeyOfEn()));
					// 如果是检查不能为空
			...
			for (FrmAttachment ath : this.getHisWork().getHisFrmAttachments().ToJavaList()) {
				...
				if (ath.getReadRole() != 0) {
					...
					if (ids.equals("ALL") == false) {
						// 获得当前节点的上传附件.
						String sql = "SELECT MyPK,FileName FROM Sys_FrmAttachmentDB WHERE RefPKVal=" + this.getWorkID()
								+ " AND FK_FrmAttachment='" + ath.getMyPK() + "' AND Rec!='" + com.kong.kflowweb.BP.Web.WebUser.getNo()
								+ "'";
						DataTable dt = DBAccess.RunSQLReturnTable(sql);
		...
		com.kong.kflowweb.BP.WF.Template.FrmNodes frmNDs = new FrmNodes(this.getHisNode().getFK_Flow(), this.getHisNode().getNodeID());
		err = "";
	...
	public final boolean DealTeamUpNode() throws Exception {
		...
		for (GenerWorkerList item : gwls.ToJavaList()) {
			if (item.getIsPassInt() == 0 || item.getIsPassInt() == 90) {
				if (item.getFK_Emp().equals(WebUser.getNo()) == false)
					todoEmps += com.kong.kflowweb.BP.WF.Glo.DealUserInfoShowModel(item.getFK_Emp(), item.getFK_EmpText()) + " ";
				num++;
			}
		}
		...
		if (num == 1) {
			if (this.getHisGenerWorkFlow().getHuiQianTaskSta() == HuiQianTaskSta.None) {
				this.getHisGenerWorkFlow()
						.setSender(com.kong.kflowweb.BP.WF.Glo.DealUserInfoShowModel(com.kong.kflowweb.BP.Web.WebUser.getNo(), com.kong.kflowweb.BP.Web.WebUser.getName()));
				this.getHisGenerWorkFlow().setTodoEmpsNum(1);
				...
				this.getHisGenerWorkFlow().setSender(com.kong.kflowweb.BP.WF.Glo.DealUserInfoShowModel(huiqianNo, huiqianName));
				this.getHisGenerWorkFlow().setTodoEmpsNum(1);
				this.getHisGenerWorkFlow().setTodoEmps(WebUser.getName() + ";");
		...
		for (GenerWorkerList gwl : gwls.ToJavaList()) {
			...
			// 处理会签问题
			this.addMsg(SendReturnMsgFlag.OverCurr, "@会签工作已完成@当前工作未处理的会签人有: " + todoEmps + " .", null,
					SendReturnMsgType.Info);
			return true;
		}
	...
	public final String DealAlertZhuChiRen() throws Exception {
		...
		com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendMsg(gwl.getFK_Emp(), "工作会签完毕",
				this.getHisGenerWorkFlow().getTitle() + " 工作会签完毕,请到待办查看.",
				"HuiQian" + this.getWorkID() + "_" + WebUser.getNo(), "HuiQian",
				this.getHisGenerWorkFlow().getFK_Flow(), this.getHisGenerWorkFlow().getFK_Node(), this.getWorkID(), 0);

		// 设置为未读.
		com.kong.kflowweb.BP.WF.Dev2Interface.Node_SetWorkUnRead(this.getHisGenerWorkFlow().getWorkID());
		...
		return "您是最后一个会签该工作的处理人，已经提醒主持人(" + gwl.getFK_Emp() + "," + gwl.getFK_EmpText() + ")处理当前工作。";
	...
	public final boolean TeamupGroupLeader() throws Exception {
		...
		String sql = "SELECT COUNT(No) AS num FROM Port_Dept WHERE No='" + WebUser.getFK_Dept() + "' AND Leader='"
				+ WebUser.getNo() + "'";
		if (com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt(sql, 0) == 1)
			return false;
	...
	public boolean DealTeamupGroupLeader() throws Exception {
		...
		if (this.getHisNode().getTeamLeaderConfirmRole() == TeamLeaderConfirmRole.HuiQianLeader) {
			if (this.getHisGenerWorkFlow().getTodoEmps().contains(com.kong.kflowweb.BP.Web.WebUser.getNo() + ",") == true) {
				...
				if (num == 1) {
					this.getHisGenerWorkFlow().setSender(
							com.kong.kflowweb.BP.WF.Glo.DealUserInfoShowModel(com.kong.kflowweb.BP.Web.WebUser.getNo(), com.kong.kflowweb.BP.Web.WebUser.getName()));
		...
		for (GenerWorkerList item : gwls.ToJavaList()) {
			if (item.getIsPassInt() == 0 || item.getIsPassInt() == 90) {
				if (item.getFK_Emp().equals(WebUser.getNo()) == false)
					todoEmps1 += com.kong.kflowweb.BP.WF.Glo.DealUserInfoShowModel(item.getFK_Emp(), item.getFK_EmpText()) + " ";
				mynum++;
			}
		}
	...
	private boolean CheckBlockModel() throws Exception {
		...
		if (this.getHisNode().getBlockModel() == BlockModel.ByExp) {
			...
			com.kong.kflowweb.BP.En.Row row = this.rptGe.getRow();
	...
	private SendReturnObjs NodeSendToYGFlow(Node node, String toEmpIDs) throws Exception {
		...
		if (StringUtils.isBlank(toEmpIDs)) {
			...
			if (node.getHisDeliveryWay() == DeliveryWay.ByDeptAndStation) {
				...
				if (dt.Rows.size() == 0)
					try {
						throw new Exception(
								"@节点访问规则(" + node.getHisDeliveryWay().toString() + ")错误:节点(" + node.getNodeID() + ","
										+ node.getNodeID() + "), 按照岗位与部门的交集确定接受人的范围错误，没有找到人员:SQL=" + sql);
					} catch (Exception e) {
			...
			if (node.getHisDeliveryWay() == DeliveryWay.ByStationOnly) {
				sql = "SELECT A.FK_Emp No FROM " + com.kong.kflowweb.BP.WF.Glo.getEmpStation()
				...
				if (dt.Rows.size() == 0)
					try {
						throw new Exception("@节点访问规则错误:节点(" + node.getNodeID() + "," + node.getName()
								+ "), 仅按岗位计算，没有找到人员:SQL=" + ps.getSQLNoPara());
					} catch (Exception e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
			...
			if (node.getHisDeliveryWay() == DeliveryWay.ByDept) {
				...
				ps.SQL = "SELECT FK_Emp as No FROM WF_SelectAccper WHERE FK_Node=" + dbStr + "FK_Node AND WorkID="
						+ dbStr + "WorkID AND AccType=0 ORDER BY IDX";
				dt = DBAccess.RunSQLReturnTable(ps);
				if (dt.Rows.size() == 0) {
			...
			if (node.getHisDeliveryWay() == DeliveryWay.BySelected || dt == null) {
				sql = "SELECT FK_Emp No, EmpName Name FROM WF_SelectAccper WHERE FK_Node=" + node.getNodeID()
		...
		long workid = com.kong.kflowweb.BP.WF.Dev2Interface.Node_CreateBlankWork(node.getFK_Flow(), null, null, toEmpIDs, null,
				this.getWorkID(), 0, this.getHisNode().getFK_Flow(), this.getHisNode().getNodeID(),
				com.kong.kflowweb.BP.Web.WebUser.getNo(), 0, null);
		...
		// 为接受人显示待办.
		com.kong.kflowweb.BP.WF.Dev2Interface.Node_SetDraft2Todolist(node.getFK_Flow(), workid);

		// 设置变量.
		// this.addMsg(SendReturnMsgFlag.VarToNodeID,
		// node.getNodeID().toString(), workid.toString(),
		// SendReturnMsgType.SystemMsg);
	...
	public final SendReturnObjs NodeSend(Node jumpToNode, String jumpToEmp) throws Exception {
		...
		if (com.kong.kflowweb.BP.WF.Dev2Interface.Flow_IsCanDoCurrentWork(
		...
		if (sendWhen != null && sendWhen.indexOf("@") >= 0) {
			...
			if (StringHelper.isNullOrEmpty(toEmps) == false)
				jumpToEmp = toEmps;
		...
		if (this.getHisNode().getIsStartNode() == false && this.getHisGenerWorkFlow().getWFState() == WFState.Askfor) {
			...
			for (GenerWorkerList item : gwls.ToJavaList()) {
				...
				if (item.getIsPassInt() == AskforHelpSta.AfterDealSendByWorker.getValue()) {
					...
					// 让加签人，设置成工作未读。
					com.kong.kflowweb.BP.WF.Dev2Interface.Node_SetWorkUnRead(this.getWorkID(), item.getFK_Emp());
		...
		if (this.getHisNode().getIsHL() || this.getHisNode().getHisRunModel() == RunModel.FHL) {
			...
			if (dtWL.Rows.size() != 0) {
				...
				if (this.getHisNode().getThreadKillRole() == ThreadKillRole.ByAuto) {
					// 删除每个子线程，然后向下运动。
					for (DataRow dr : dtWL.Rows) {
						com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DeleteSubThread(this.getHisFlow().getNo(),
		...
		try {
			...
			this.getHisGenerWorkFlow().setSender(com.kong.kflowweb.BP.WF.Glo.DealUserInfoShowModel(WebUser.getNo(), WebUser.getName()));
			...
			if (this.getIsStopFlow()) {
				...
				if (this.getHisGenerWorkFlow().getWFState() == WFState.ReturnSta) {
					com.kong.kflowweb.BP.DA.DBAccess.RunSQL("UPDATE WF_ReturnWork SET IsBackTracking=0 WHERE WorkID=" + this.getWorkID());
				}
			...
			if (this.getHisNode().getHisPrintDocEnable() == true
				...
				for (BillTemplate func : reffunc.ToJavaList()) {
					...
					if (func.getHisBillFileType() != BillFileType.RuiLang) {
						...
						com.kong.kflowweb.BP.Pub.RTFEngine rtf = new com.kong.kflowweb.BP.Pub.RTFEngine();
						...
						try {
							...
							path = com.kong.kflowweb.BP.WF.Glo.getFlowFileBill() + year + "\\" + this.getExecerDeptNo() + "\\"
									+ func.getNo() + "\\";
							...
							com.kong.kflowweb.BP.WF.DTS.InitBillDir dir = new com.kong.kflowweb.BP.WF.DTS.InitBillDir();
							dir.Do();
							path = com.kong.kflowweb.BP.WF.Glo.getFlowFileBill() + year + "\\" + this.getExecerDeptNo() + "\\"
									+ func.getNo() + "\\";
							String msgErr = "@" + String.format("生成单据失败，请让管理员检查目录设置") + "["
									+ com.kong.kflowweb.BP.WF.Glo.getFlowFileBill() + "]。@Err：" + ex.getMessage() + " @File=" + file
									+ " @Path:" + path;
			...
			if (this.getHisNode().getSFActiveFlows().length() > 2) {
				...
				for (String flowNo : fls) {
					...
					// 启动子流程.
					SendReturnObjs sendObjs = com.kong.kflowweb.BP.WF.Dev2Interface.Node_StartWork(fl.getNo(), this.getHisWork().getRow(),
			...
			if (this.getHisNode().getIsStartNode()) {
				if (this.rptGe.getPWorkID() != 0 && this.getHisGenerWorkFlow().getPWorkID() == 0) {
					com.kong.kflowweb.BP.WF.Dev2Interface.SetParentInfo(this.getHisFlow().getNo(), this.getWorkID(),
					...
					Glo.AddToTrack(ActionType.StartChildenFlow, rptGe.getPFlowNo(), rptGe.getPWorkID(), fid,
							...
							paras);
				} else if (SystemConfig.getIsBSsystem() == true && com.kong.kflowweb.BP.Sys.Glo.getRequest() != null) {
					// 如果是BS系统
					String pflowNo = com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter("PFlowNo");
					if (StringHelper.isNullOrEmpty(pflowNo) == false) {
						String pWorkID = com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter("PWorkID");
						String pNodeID = com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter("PNodeID");
						String pEmp = com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter("PEmp");

						// 设置成父流程关系.
						com.kong.kflowweb.BP.WF.Dev2Interface.SetParentInfo(this.getHisFlow().getNo(), this.getWorkID(), pflowNo,
	...
	private void Deal_Event() throws Exception {
		...
		if (this.town != null) {
			// this.town
			String toMsg = this.getHisFlow().DoFlowEventEntity(EventListOfNode.WorkArrive, this.town.getHisNode(),
					this.rptGe, null, this.HisMsgObjs);
	...
	private void WhenTranscactionRollbackError(Exception ex) throws Exception {
		...
		if (this.town != null && this.town.getHisNode().getHisNodeWorkType() == NodeWorkType.SubThreadWork
			...
			// 删除子线程数据.
			if (com.kong.kflowweb.BP.DA.DBAccess.IsExitsObject(this.town.getHisWork().getEnMap().getPhysicsTable()) == true)
		...
		try {
			...
			if (gwf.getWFState().ordinal() != 0 || gwf.getFK_Node() != this.getHisNode().getNodeID()) {
				...
				gwf.setWFState(WFState.Runing);
				this.getHisGenerWorkFlow().setSender(com.kong.kflowweb.BP.WF.Glo.DealUserInfoShowModel(oldSender, oldSender));
				gwf.Update();
			}
	...
	private void InitStartWorkDataV2() throws Exception {
		...
		if (SystemConfig.getIsBSsystem() && com.kong.kflowweb.BP.Sys.Glo.getRequest() != null) {
			String fk_nodeFrom = com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter("FromNode");
			if (DotNetToJavaStringHelper.isNullOrEmpty(fk_nodeFrom) == false) {
				...
				if (DotNetToJavaStringHelper.isNullOrEmpty(PWorkID)) {
					PWorkID = com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter("PWorkID");
				}
				...
				// 记录父流程被调起。
				com.kong.kflowweb.BP.WF.Dev2Interface.WriteTrack(this.getHisFlow().getNo(), this.getHisNode().getNodeID(),
		...
		if (DotNetToJavaStringHelper.isNullOrEmpty(rptGe.getBillNo())) {
			...
			if (DotNetToJavaStringHelper.isNullOrEmpty(billNo) == false) {
				billNo = com.kong.kflowweb.BP.WF.WorkFlowBuessRole.GenerBillNo(billNo, this.getWorkID(), this.rptGe,
		...
		if (DotNetToJavaStringHelper.isNullOrEmpty(note) == false) {
			note = com.kong.kflowweb.BP.WF.Glo.DealExp(note, this.rptGe, null);
		}
	...
	public final void SendMsgToThem(GenerWorkerLists gwls) throws Exception {
		if (com.kong.kflowweb.BP.WF.Glo.getIsEnableSysMessage() == false) {
			return;
		}
	...
	public final void AddToTrack(ActionType at, String toEmp, String toEmpName, int toNDid, String toNDName, String msg,
		...
		t.setEmpFromT(this.getExecerName());
		t.FK_Flow = this.getHisNode().getFK_Flow();
		t.setTag("@SendNode=" + this.getHisNode().getNodeID());
		if (toNDid == 0) {
			toNDid = this.getHisNode().getNodeID();
			toNDName = this.getHisNode().getName();
		}
	...
	private void DealAutoRunEnable() throws Exception {
		...
		exp = exp.replace("WebUser.No", com.kong.kflowweb.BP.Web.WebUser.getNo());
		exp = exp.replace("@WebUser.Name", com.kong.kflowweb.BP.Web.WebUser.getName());
		exp = exp.replace("@WebUser.FK_DeptName", com.kong.kflowweb.BP.Web.WebUser.getFK_DeptName());

		exp = exp.replace("@WebUser.FK_Dept", com.kong.kflowweb.BP.Web.WebUser.getFK_Dept());
		...
		Dev2Interface.Flow_SetFlowTransferCustom(this.getHisFlow().getNo(), this.getWorkID(),
				com.kong.kflowweb.BP.WF.TransferCustomType.ByWorkerSet, strs);
		// 重新执行查询.
	...
	private void CheckCompleteCondition() throws Exception {
		...
		this.getHisGenerWorkFlow().setSender(com.kong.kflowweb.BP.Web.WebUser.getNo());
		this.getHisGenerWorkFlow().setSendDT(DataType.getCurrentDataTimess());

		this.rptGe.setFlowEnder(com.kong.kflowweb.BP.Web.WebUser.getNo());
		...
		try {
			...
			if (this.getHisNode().getIsCCFlow() && this.getHisFlowCompleteConditions().getIsPass()) {
				...
				// string path = com.kong.kflowweb.BP.Sys.Glo.Request.ApplicationPath;
				this.addMsg(SendReturnMsgFlag.MacthFlowOver, "@符合工作流程完成条件" + stopMsg + "" + overMsg, "@符合工作流程完成条件"
						+ stopMsg + ""
						+ overMsg /*
									 * + " @查看<img src='" + getVirPath() +
									 * "WF/Img/Btn/PrintWorkRpt.gif' ><a href='WFRpt.jsp?WorkID="
