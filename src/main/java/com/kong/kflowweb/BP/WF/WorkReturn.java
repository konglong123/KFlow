...
import com.kong.kflowweb.BP.WF.Template.TimelineRole;
...
public class WorkReturn
{
	...
	*/
	...
	private String dbStr = com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBVarStr();
	private Paras ps;
	...
	*/
	public WorkReturn(String fk_flow, long workID, long fid, int currNodeID, int ReturnToNodeID, String reutrnToEmp, boolean isBackTrack, String returnInfo) throws Exception
	{
        ...
        if (ReturnToNodeID == 0)
        {
            DataTable dt = com.kong.kflowweb.BP.WF.Dev2Interface.DB_GenerWillReturnNodes(currNodeID, workID, fid);
            if (dt.Rows.size()==0)
	...
	*/
	private void DeleteSpanNodesGenerWorkerListData()
	{
		...
		if (dt.Rows.size() >= 1)
		{
			...
			ps.Add("RDT", rdt);
			ps.Add("WorkID", this.WorkID);
			dt = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(ps);
	...
	*/
	public final String DoOrderReturn() throws Exception
	{
		...
		if (Glo.getIsEnableSysMessage() == true)
		{
			...
			com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendMsg(returnToEmp.getNo(), title, Msg, "RE" + this.HisNode.getNodeID() + this.WorkID, com.kong.kflowweb.BP.WF.SMSMsgType.ReturnAfter, ReturnToNode.getFK_Flow(), ReturnToNode.getNodeID(), this.WorkID, this.FID);
		}
...
//        gwfP.setTodoEmps(toEmps);
       ...
       // #region 写入退回提示.
        ...
        //删除当前的流程.
        com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoDeleteFlowByReal(gwf.getFK_Flow(), this.WorkID, true);

        //设置当前为未读的状态.
        com.kong.kflowweb.BP.WF.Dev2Interface.Node_SetWorkUnRead(gwfP.getWorkID());
...
//             * 
		...
		{
			//获得可以退回的节点，这个节点是有顺序的.
			DataTable dt= com.kong.kflowweb.BP.WF.Dev2Interface.DB_GenerWillReturnNodes(this.HisNode.getNodeID(), this.WorkID, this.FID);
		...
		//删除.
		com.kong.kflowweb.BP.WF.Dev2Interface.DeleteCheckInfo(this.HisNode.getFK_Flow(), this.WorkID, this.HisNode.getNodeID());
	...
	*/
	private String ExeReturn2_4() throws Exception
	{
		...
		//把分流节点的待办去掉. 
		gwls = new GenerWorkerLists();
		gwls.Retrieve(GenerWorkerListAttr.WorkID, this.WorkID, GenerWorkerListAttr.FID, this.FID, GenerWorkerListAttr.FK_Emp, com.kong.kflowweb.BP.Web.WebUser.getNo());
		for (GenerWorkerList item : gwls.ToJavaList())
		{
			item.setIsPassInt(-2);
			item.Update();
	...
	*/
	private String ExeReturn5_2() throws Exception
	{
		...
		com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendMsg(toEmp, gwf.getTitle(), Msg, "RE" + this.HisNode.getNodeID() + this.WorkID, com.kong.kflowweb.BP.WF.SMSMsgType.ReturnAfter, ReturnToNode.getFK_Flow(), ReturnToNode.getNodeID(), this.WorkID, this.FID);
	...
	*/
	private String ExeReturn3_4() throws Exception
	{
		...
		for (MapDtl dtl : dtls.ToJavaList())
		{
			...
			if (dtl.getIsHLDtl())
			{
				com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM " + dtl.getPTable() + " WHERE OID=" + this.WorkID);
			}
		}
	...
	*/
	private String ExeReturn3_2() throws Exception
	{
        ...
        toWL.Retrieve(GenerWorkerListAttr.WorkID, this.WorkID,
            GenerWorkerListAttr.FK_Node, this.ReturnToNode.getNodeID());
	...
	*/
	private String ExeReturn1_1() throws Exception
	{
	   ...
	   //如果事件返回的信息不是null，就终止执行。
        ...
        GenerWorkFlow gwf = new GenerWorkFlow(this.WorkID);
        Date dtOfShould;
        if (fl.getHisTimelineRole() == TimelineRole.ByFlow)
        {
            /*如果整体流程是按流程设置计算 */
            dtOfShould = DataType.ParseSysDateTime2DateTime(gwf.getSDTOfFlow());
        }
        else
        {
            //增加天数. 考虑到了节假日.             
        ...
        // 应完成日期.
        String sdt = DataType.getDateByFormart(dtOfShould, DataType.getSysDataTimeFormat());
...
//        gwf.setTodoEmps(toDoEmps);
      ...
      //更新待办状态.
				...
				|| this.HisNode.getTodolistModel() == TodolistModel.Teamup)
		{
			...
			if (com.kong.kflowweb.BP.DA.DBAccess.RunSQL(sql) == 0 && 1==2)
	...
	*/
	...
	{
//		String filePath = com.kong.kflowweb.BP.Sys.SystemConfig.getPathOfDataUser() + "ReturnLog/" + this.HisNode.getFK_Flow() + "/";
...
//		
		...
		try
		{		
		filePath = com.kong.kflowweb.BP.Sys.SystemConfig.getPathOfDataUser() + "ReturnLog/" + this.HisNode.getFK_Flow() + "/";
		if ((new java.io.File(filePath)).isDirectory() == false)
		{
			(new java.io.File(filePath)).mkdirs();
		}

		  file = filePath + File.separator + rw.getMyPK();
		...
		{
			Log.DebugWriteInfo("filePath:"+filePath+"file:"+file +"err@"+ex.getMessage() );
			//Log.DebugWriteInfo("退回信息文件路径1:"+filePath);
		}
		// this.HisWork.Delete();
	}
	...
	*/
	public final void DeleteToNodesData(Nodes nds) throws Exception
	{
		...
		for (Node nd : nds.ToJavaList())
		{
			...
			// 删除表单附件信息。
			com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM Sys_FrmAttachmentDB WHERE RefPKVal=" + dbStr + "WorkID AND FK_MapData=" + dbStr + "FK_MapData ", "WorkID", (new Long(this.WorkID)).toString(), "FK_MapData", "ND" + nd.getNodeID());
			// 删除签名信息。
			com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM Sys_FrmEleDB WHERE RefPKVal=" + dbStr + "WorkID AND FK_MapData=" + dbStr + "FK_MapData ", "WorkID", (new Long(this.WorkID)).toString(), "FK_MapData", "ND" + nd.getNodeID());
	...
	private WorkNode DoReturnSubFlow(int backtoNodeID, String msg, boolean isHiden) throws Exception
	{
		...
		// 如果是隐性退回。
		com.kong.kflowweb.BP.WF.ReturnWork rw = new ReturnWork();
		...
		{
			rw.setMyPK(rw.getReturnToNode() + "_" + rw.getWorkID() + "_" + com.kong.kflowweb.BP.DA.DBAccess.GenerOID());
			rw.Insert();
		}
		...
		if (Glo.getIsEnableSysMessage())
		{
			...
			com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendMsg(wn.getHisWork().getRec(), title, msg, "RESub" + backtoNodeID + "_" + this.WorkID, com.kong.kflowweb.BP.WF.SMSMsgType.SendSuccess, nd.getFK_Flow(), nd.getNodeID(), this.WorkID, this.FID);
		}
		return wn;
	}
}