...
import com.kong.kflowweb.BP.WF.Template.SelectorModel;
import com.kong.kflowweb.BP.WF.TransferCustom;
import com.kong.kflowweb.BP.WF.TransferCustoms;
...
import com.kong.kflowweb.BP.Sys.FrmRBAttr;
...
import com.kong.kflowweb.BP.WF.Rpt.MakeForm2Html;
...
public class WF_WorkOpt extends WebContralBase {
	...
	public final String SelectEmps_Init() throws Exception {
		...
		DataTable dtEmps = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(sql);
	...
	public final String WorkCheck_Init() throws Exception {
		...
		if (isReadonly == true)
			...
			isCanDo = com.kong.kflowweb.BP.WF.Dev2Interface.Flow_IsCanDoCurrentWork(this.getWorkID(), WebUser.getNo());
		...
		if (wcDesc.getFWCListEnable() == true) {
			...
			for (Track tk : tks.ToJavaList()) {
				...
				if (tk.getNDFrom() == this.getFK_Node() && tk.getEmpFrom() == com.kong.kflowweb.BP.Web.WebUser.getNo() && isCanDo
						&& isDoc == false)
					isDoc = true;
		...
		String str = com.kong.kflowweb.BP.Tools.Json.ToJson(ds);
		return str;
	}
	...
	public final String WorkCheck_GetNewUploadedAths() throws Exception {
		...
		return com.kong.kflowweb.BP.Tools.Json.ToJson(athDt);
	}
	...
	public String CC_Init() throws Exception {
		...
		// 根据他判断是否显示权限组。
		if (com.kong.kflowweb.BP.DA.DBAccess.IsExitsObject("GPM_Group") == true) {
			ht.put("IsGroup", "1");
		} else {
			ht.put("IsGroup", "0");
		}
	...
	public String CC_SelectDepts() throws Exception {
		// 选择部门呈现信息.
		com.kong.kflowweb.BP.Port.Depts depts = new com.kong.kflowweb.BP.Port.Depts();
		depts.RetrieveAll();
		return depts.ToJson();
	}
	...
	public String CC_SelectStations() {
		...
		DataTable dtSta = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(sqlStas);
	...
	public String CC_Send() throws Exception {
		...
		// 调用抄送接口执行抄送.
		String ccRec = com.kong.kflowweb.BP.WF.Dev2Interface.Node_CC_WriteTo_CClist(this.getFK_Node(), this.getWorkID(), title, doc, emps,
	...
	public String DealSubThreadReturnToHL_Init() throws Exception {
		...
		// 如果工作节点退回了
		com.kong.kflowweb.BP.WF.ReturnWorks rws = new com.kong.kflowweb.BP.WF.ReturnWorks();
		rws.Retrieve(com.kong.kflowweb.BP.WF.ReturnWorkAttr.ReturnToNode, this.getFK_Node(), com.kong.kflowweb.BP.WF.ReturnWorkAttr.WorkID,
				this.getWorkID(), com.kong.kflowweb.BP.WF.ReturnWorkAttr.RDT);

		String msgInfo = "";
		if (rws.size() != 0) {
			for (com.kong.kflowweb.BP.WF.ReturnWork rw : rws.ToJavaList()) {
		...
		com.kong.kflowweb.BP.WF.Template.BtnLab btn = new BtnLab(this.getFK_Node());
		com.kong.kflowweb.BP.WF.Node nd = new Node(this.getFK_Node());
	...
	public String DealSubThreadReturnToHL_Done() throws Exception {
		...
		if (actionType.equals("Return")) {
			// 如果是退回.
			com.kong.kflowweb.BP.WF.ReturnWork rw = new com.kong.kflowweb.BP.WF.ReturnWork();
			rw.Retrieve(com.kong.kflowweb.BP.WF.ReturnWorkAttr.ReturnToNode, this.getFK_Node(), com.kong.kflowweb.BP.WF.ReturnWorkAttr.WorkID,
					this.getWorkID());
			String info = com.kong.kflowweb.BP.WF.Dev2Interface.Node_ReturnWork(this.getFK_Flow(), this.getWorkID(), this.getFID(),
		...
		if (actionType.equals("Shift")) {
			...
			return com.kong.kflowweb.BP.WF.Dev2Interface.Node_Shift(this.getFK_Flow(), this.getFK_Node(), this.getWorkID(), this.getFID(),
					toEmps, note);
		}

		if (actionType.equals("Kill")) {
			msg = com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DeleteSubThread(this.getFK_Flow(), this.getWorkID(), "手工删除");
			// 提示信息.
		...
		if (actionType.equals("UnSend")) {
			return com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoUnSend(this.getFK_Flow(), this.getFID(), this.getFK_Node());
		}
	...
	public final String DeleteFlowInstance_Init() {
		try {
			if (com.kong.kflowweb.BP.WF.Dev2Interface.Flow_IsCanDeleteFlowInstance(this.getFK_Flow(), this.getWorkID(),
					com.kong.kflowweb.BP.Web.WebUser.getNo()) == false) {
	...
	public String DeleteFlowInstance_DoDelete() throws Exception {
		...
		if (deleteWay.equals("0")) {
			com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoDeleteFlowByFlag(this.getFK_Flow(), this.getWorkID(), doc, isDelSubFlow);
		}

		// 彻底删除.
		if (deleteWay.equals("1")) {
			com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoDeleteFlowByReal(this.getFK_Flow(), this.getWorkID(), isDelSubFlow);
		}

		// 彻底并放入到删除轨迹里.
		if (deleteWay.equals("2")) {
			com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoDeleteFlowByWriteLog(this.getFK_Flow(), this.getWorkID(), doc, isDelSubFlow);
		}
	...
	public String ViewWorkNodeFrm() throws Exception {
		...
		// 表单模版.
		DataSet myds = com.kong.kflowweb.BP.Sys.CCFormAPI.GenerHisDataSet(nd.getNodeFrmID(), null);
		String json = com.kong.kflowweb.BP.WF.Dev2Interface.CCFrom_GetFrmDBJson(this.getFK_Flow(), this.getMyPK());
		DataTable mainTable = com.kong.kflowweb.BP.Tools.Json.ToDataTableOneRow(json);
	...
	public String AskForRe() throws Exception {
		...
		return com.kong.kflowweb.BP.WF.Dev2Interface.Node_AskforReply(this.getWorkID(), note);
	}
	...
	public String Askfor() throws Exception {
		...
		com.kong.kflowweb.BP.WF.AskforHelpSta sta = com.kong.kflowweb.BP.WF.AskforHelpSta.AfterDealSend;
		if (model.equals("1")) {
			sta = com.kong.kflowweb.BP.WF.AskforHelpSta.AfterDealSendByWorker;
		}
		return com.kong.kflowweb.BP.WF.Dev2Interface.Node_Askfor(workID, sta, toEmp, note);
	}
	...
	public DataTable GetNextWorks(long WorkID,String FK_Node,String FK_Flow,int toNodeID) throws Exception {
		...
		if (nd.getTodolistModel() == TodolistModel.Teamup && nd.getIsStartNode() == false) {
			if (gwf.getTodoEmps().contains(WebUser.getNo() + ",")) {
				...
				/* 不是主持人就执行发送，返回发送结果. */
				SendReturnObjs objs = com.kong.kflowweb.BP.WF.Dev2Interface.Node_SendWork(FK_Flow, WorkID);
	...
	public String Accepter_Init() throws Exception {
		...
		if (nd.getTodolistModel() == TodolistModel.Teamup && nd.getIsStartNode() == false) {
             ...
             {
                 /* 如果不是最后一位，返回发送结果. */
                 SendReturnObjs objs = com.kong.kflowweb.BP.WF.Dev2Interface.Node_SendWork(this.getFK_Flow(), this.getWorkID());
	...
	public String AccepterSave12() throws Exception {
		try {
			...
			// 保存接受人.
			com.kong.kflowweb.BP.WF.Dev2Interface.Node_AddNextStepAccepters(this.getWorkID(), toNodeID, selectEmps, true);
	...
	public String Accepter_Send() throws Exception {
		try {
			...
			// 执行发送.
			SendReturnObjs objs = com.kong.kflowweb.BP.WF.Dev2Interface.Node_SendWork(this.getFK_Flow(), this.getWorkID(), toNodeID,
					selectEmps);
	...
	public String Return_Init() {
		...
		try {
			...
			DataTable dt = com.kong.kflowweb.BP.WF.Dev2Interface.DB_GenerWillReturnNodes(this.getFK_Node(), this.getWorkID(),
					this.getFID());
            ...
            if (dt.Rows.size() == 1)
            {
               ...
               if (nd.getReturnOneNodeRole() != 0)
               {
                   ...
                   String info = com.kong.kflowweb.BP.WF.Dev2Interface.Node_ReturnWork(this.getFK_Flow(), this.getWorkID(), 0, this.getFK_Node(), toNodeID, returnMsg, false);
                   return "info@" + info;
               }
            }
			...
			String str = com.kong.kflowweb.BP.Tools.Json.ToJson(dt);
	...
	public String DoReturnWork() {
		...
		try {
			...
			return com.kong.kflowweb.BP.WF.Dev2Interface.Node_ReturnWork(this.getFK_Flow(), this.getWorkID(), this.getFID(),
	...
	public String Allot() throws Exception {
		String msg = this.GetRequestVal("Message");
		String toEmp = this.GetRequestVal("ToEmp");
		return com.kong.kflowweb.BP.WF.Dev2Interface.Node_Allot(this.getFK_Flow(), this.getFK_Node(), this.getWorkID(), this.getFID(),
				toEmp, msg);
	}

	public String UnShift() throws Exception {

		return com.kong.kflowweb.BP.WF.Dev2Interface.Node_ShiftUn(this.getFK_Flow(), this.getWorkID());
	}
	...
	public String Press() throws Exception {
		...
		// 调用API.
		return com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoPress(this.getWorkID(), msg, true);
	}

	public String FlowBBSList() {
		Paras ps = new Paras();
		ps.SQL = "SELECT * FROM ND" + Integer.parseInt(this.getFK_Flow()) + "Track WHERE ActionType="
				+ com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBVarStr() + "ActionType";
		ps.Add("ActionType", com.kong.kflowweb.BP.WF.ActionType.FlowBBS.getValue());

		// 转化成json
		return com.kong.kflowweb.BP.Tools.Json.ToJson(com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(ps));
	}
	...
	public final String HuiQian_Init() throws Exception {
		...
		for (GenerWorkerList item : ens.ToJavaList()) {
			...
			// 标记为自己.
			if (item.getFK_Emp().equals(com.kong.kflowweb.BP.Web.WebUser.getNo())) {
				item.setFK_EmpText("" + item.getFK_EmpText());
		...
		return com.kong.kflowweb.BP.Tools.Json.ToJson(mydt);
	}
	...
	public final String HuiQian_Delete() throws Exception {
		...
		// 要找到主持人.
		GenerWorkFlow gwf = new GenerWorkFlow(this.getWorkID());
		if (gwf.getTodoEmps().contains(com.kong.kflowweb.BP.Web.WebUser.getNo() + ",") == false) {
		...
		// 从待办里移除.
		com.kong.kflowweb.BP.Port.Emp myemp = new com.kong.kflowweb.BP.Port.Emp(this.getFK_Emp());
	...
	public final String HuiQian_AddEmps() throws Exception {
		...
		// 发送消息.
		com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendMsg(emp.getNo(), "bpm会签邀请",
	...
	public final String HuiQian_SaveAndClose() throws Exception {
		...
		// 执行会签,写入日志.
		// com.kong.kflowweb.BP.WF.Dev2Interface.WriteTrack(gwf.getFK_Flow(), gwf.getFK_Node(),
		// workid, fid, msg, at, tag, cFlowInfo, optionMsg, empNoTo, empNameTo);
		// com.kong.kflowweb.BP.WF.Dev2Interface.WriteTrack(gwf.getFK_Flow(),
		// gwf.getFK_Node(), gwf.getNodeName(), gwf.getWorkID(),
		// gwf.getFID(), empsOfHuiQian, ActionType.HuiQian, "执行会签", null);

		com.kong.kflowweb.BP.WF.Dev2Interface.WriteTrack(gwf.getFK_Flow(), gwf.getFK_Node(), gwf.getWorkID(), gwf.getFID(), "执行会签",
		...
		if (nd.getTodolistModel() == TodolistModel.TeamupGroupLeader) {
			...
			// 删除自己的意见，以防止其他人员看到.
			com.kong.kflowweb.BP.WF.Dev2Interface.DeleteCheckInfo(gwf.getFK_Flow(), this.getWorkID(), gwf.getFK_Node());
			return str;
		}
		...
		if (i == 0) {
			...
			if (dt.Rows.size() != 0) {
				...
				if (emps.contains(",") == false)
					com.kong.kflowweb.BP.WF.Dev2Interface.Node_AddNextStepAccepters(this.getWorkID(), toNodeID, emps, false);
	...
	public String AccepterOfGener_Delete() throws Exception {
		// 删除指定的人员.
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM WF_SelectAccper WHERE WorkID=" + this.getWorkID() + " AND FK_Emp='"
				+ this.getFK_Emp() + "'");
	...
	public final String DBTemplate_Init() throws Exception {
		...
		for (DataRow dr : dtTemplate.Rows) {
			...
			com.kong.kflowweb.BP.DA.AtPara ap = new AtPara(str);
	...
	public final String Packup_Init() throws Exception {
		try {
			...
			if((nd.getHisFormType() == NodeFormType.SheetTree && nd.getHisPrintPDFModle() == 1) == false)
				...
				return "info@"+com.kong.kflowweb.BP.Tools.Json.ToJson(mds.ToDataTableField());
			}
		} catch (RuntimeException ex) {
			return "err@" + ex.getMessage();
		}
    ...
    public String PrintDoc_DoneIt(String billTemplateNo) throws Exception
    {
        ...
        try
        {
            ...
            if (func.getNodeID() != 0)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    else
                    {
                        ...
                        dir.Do();
                        path = com.kong.kflowweb.BP.WF.Glo.getFlowFileBill() + DataType.getCurrentYear() + "/" + WebUser.getFK_Dept() + "/" + func.getNo() + "/";
                        String msgErr = "@" + String.format("生成单据失败，请让管理员检查目录设置") + "[" + com.kong.kflowweb.BP.WF.Glo.getFlowFileBill() + "]。@Err：" + ex.getMessage() + " @File=" + file + " @Path:" + path;
                ...
                //把审核日志表加入里面去.
                Paras ps = new com.kong.kflowweb.BP.DA.Paras();
                ...
                rtf.dtTrack=com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(ps);
            }

            paths = file.split("_");
            path = paths[0] + "/" + paths[1] + "/" + paths[2] + "/";

            String billUrl = "url@" + com.kong.kflowweb.BP.WF.Glo.getCCFlowAppPath() + "DataUser/Bill/" + path + file;
            ...
            if (func.getHisBillFileType() == BillFileType.PDF)
            {
                ...
                try
                {
                    com.kong.kflowweb.BP.WF.Glo.Rtf2PDF(rtfPath, pdfPath);
                }
                catch (Exception ex)
                {
                    return "err@" + ex.getMessage();
    ...
    public String PrintDoc_FormDoneIt(Node nd, long workID, long fid, String formID, BillTemplate func) throws Exception
    {
        ...
        try
        {
            ...
            String billUrl = "url@" + fileModelT + "@" + com.kong.kflowweb.BP.WF.Glo.getCCFlowAppPath() + "DataUser/Bill/" + path + file;
	...
	public String RadioBtns_Save() throws Exception {
		...
		DataTable dt = com.kong.kflowweb.BP.Tools.Json.ToDataTable(json);
   ...
   {
       Paras ps = new Paras();
       ps.SQL = "SELECT * FROM ND" + Integer.parseInt(this.getFK_Flow()) + "Track WHERE ActionType=" + com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBVarStr() + "ActionType AND WorkID=" + com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBVarStr() + "WorkID";
       ps.Add("ActionType", com.kong.kflowweb.BP.WF.ActionType.FlowBBS.getValue());
   ...
   public String Note_Save() throws Exception
   {
       ...
       //增加track
       Node nd = new Node(this.getFK_Node());
       com.kong.kflowweb.BP.WF.Glo.AddToTrack(ActionType.FlowBBS, this.getFK_Flow(), this.getWorkID(), this.getFID(), nd.getNodeID(), nd.getName(),  WebUser.getNo(), WebUser.getName(),nd.getNodeID(), nd.getName(), WebUser.getNo(), WebUser.getName(), msg, null);
       ...
       for(String emp : emps)
       {
           if (DataType.IsNullOrEmpty(emp))
               continue;
           //获得当前人的邮件.
           com.kong.kflowweb.BP.WF.Port.WFEmp empEn = new com.kong.kflowweb.BP.WF.Port.WFEmp(emp);

           com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendMsg(empEn.getNo(), title,msg, null,"NoteMessage",this.getFK_Flow(),this.getFK_Node(),this.getWorkID(),this.getFID());
   ...
   //节点备注的设置
	...
	{
		com.kong.kflowweb.BP.WF.Template.FrmNodes fns = new com.kong.kflowweb.BP.WF.Template.FrmNodes();

		QueryObject qo = new QueryObject(fns);
		qo.AddWhere(FrmNodeAttr.FK_Node, Integer.parseInt(this.getFK_Flow() + "01"));
		qo.addAnd();
		qo.AddWhere(FrmNodeAttr.FrmEnableRole, com.kong.kflowweb.BP.WF.Template.FrmEnableRole.WhenHaveFrmPara.getValue());
		qo.addOrderBy(FrmNodeAttr.Idx);
		qo.DoQuery();

		for(com.kong.kflowweb.BP.WF.Template.FrmNode item : fns.ToJavaList())
		{
