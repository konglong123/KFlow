...
import com.kong.kflowweb.BP.Sys.AppType;
...
import com.kong.kflowweb.BP.Sys.FrmRpts;
...
public class MapDataExt extends EntityNoName
{
	...
	*/
	...
	public Map getEnMap()
	{
		...
		map.AddTBString(MapDataAttr.PTable, null, "存储表", true, false, 0, 500, 20);
			...
			//表单的运行类型.
		map.AddDDLSysEnum(MapDataAttr.FrmType, com.kong.kflowweb.BP.Sys.FrmType.FreeFrm.getValue(), "表单类型",true, false, MapDataAttr.FrmType);

		map.AddTBString(MapDataAttr.Url, null, "URL连接(对嵌入式表单有效)", true, false, 0, 500, 20, true);
			//数据源.
		map.AddDDLEntities(MapDataAttr.DBSrc, "local", "数据源", new com.kong.kflowweb.BP.Sys.SFDBSrcs(), true);
	...
	*/
	...
	{
		String url = Glo.getCCFlowAppPath() + "WF/Admin/FoolFormDesigner/CCForm/Frm.jsp?FK_MapData=" + this.getNo() + "&UserNo=" + com.kong.kflowweb.BP.Web.WebUser.getNo() + "&SID=" + com.kong.kflowweb.BP.Web.WebUser.getSID() + "&AppCenterDBType=" + com.kong.kflowweb.BP.DA.DBAccess.getAppCenterDBType() + "&CustomerNo=" + com.kong.kflowweb.BP.Sys.SystemConfig.getCustomerNo();
		try {
	...
	*/
	...
	{
		String url = Glo.getCCFlowAppPath() + "WF/Admin/FoolFormDesigner/Designer.htm?FK_MapData=" + this.getNo() + "&UserNo=" + com.kong.kflowweb.BP.Web.WebUser.getNo() + "&SID=" + com.kong.kflowweb.BP.Web.WebUser.getSID() + "&AppCenterDBType=" + com.kong.kflowweb.BP.DA.DBAccess.getAppCenterDBType() + "&IsFirst=1&CustomerNo=" + com.kong.kflowweb.BP.Sys.SystemConfig.getCustomerNo();
		try {
			PubClass.WinOpen(ContextHolderUtils.getResponse(), url, 800, 650);
		} catch (IOException e) {
	...
	*/
	...
	{
		return Glo.getCCFlowAppPath() + "WF/Comm/Search.htm?s=34&FK_MapData=" + this.getNo() + "&EnsName=com.kong.kflowweb.BP.Sys.SFDBSrcs";
	}
