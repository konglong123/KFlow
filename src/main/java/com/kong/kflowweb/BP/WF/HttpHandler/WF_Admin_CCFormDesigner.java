...
public class WF_Admin_CCFormDesigner extends WebContralBase {
	...
	public final String FrmEnumeration_NewEnumField() throws Exception {
		...
		com.kong.kflowweb.BP.Sys.CCFormAPI.NewEnumField(fk_mapdata, keyOfEn, fieldDesc, enumKeyOfBind, ctrl, x, y, 1);
		return "绑定成功.";
	}
	...
	public final String NewSFTableField() {
		...
		try {
			...
			// 调用接口,执行保存.
			try {
				com.kong.kflowweb.BP.Sys.CCFormAPI.SaveFieldSFTable(fk_mapdata, keyOfEn, fieldDesc, sftable, x, y, 0);
			} catch (Exception e) {
				return "设置失败";
			}
			return "设置成功";
	...
	public final String ParseStringToPinyin() {
		...
		if (flag.equals("true")) {
			return com.kong.kflowweb.BP.Sys.CCFormAPI.ParseStringToPinyinField(name, true);
		} else {
			return com.kong.kflowweb.BP.Sys.CCFormAPI.ParseStringToPinyinField(name, false);
		}
	}
	...
	public final String Hiddenfielddata() throws Exception {
		return com.kong.kflowweb.BP.Sys.CCFormAPI.DB_Hiddenfielddata(this.getFK_MapData());
	}
...
//		mdHid.setMyDataType(dataType);
	...
	public final String NewFrmGuide_GenerPinYin() throws Exception {
		...
		if (isQuanPin.equals("1")) {
			str = com.kong.kflowweb.BP.Sys.CCFormAPI.ParseStringToPinyinField(name, true);
		} else {
			str = com.kong.kflowweb.BP.Sys.CCFormAPI.ParseStringToPinyinField(name, false);
		}
...
*/
	...
	public final String NewFrmGuide_Create() throws Exception {
		...
		//表单的物理表.
        if(md.getHisFrmType() == com.kong.kflowweb.BP.Sys.FrmType.Url ||  md.getHisFrmType() == com.kong.kflowweb.BP.Sys.FrmType.Entity)
            md.setPTable(this.GetRequestVal("TB_PTable"));
        else
		...
		//增加上OID字段.
        com.kong.kflowweb.BP.Sys.CCFormAPI.RepareCCForm(md.getNo());
		
		if (md.getHisFrmType() == com.kong.kflowweb.BP.Sys.FrmType.WordFrm || md.getHisFrmType() == com.kong.kflowweb.BP.Sys.FrmType.ExcelFrm) {
			/* 把表单模版存储到数据库里 */
			return "url@../../Comm/En.htm?EnName=com.kong.kflowweb.BP.WF.Template.MapFrmExcel&PK=" + md.getNo();
		}

		if (md.getHisFrmType() == com.kong.kflowweb.BP.Sys.FrmType.FreeFrm) {
			return "url@FormDesigner.htm?FK_MapData=" + md.getNo();
		}
		
		if (md.getHisFrmType() == com.kong.kflowweb.BP.Sys.FrmType.Entity)
	...
	public final String CCFormDesignerSL_Init() {
		return com.kong.kflowweb.BP.WF.Glo.getSilverlightDownloadUrl();
	}

	public final String GoToFrmDesigner_Init() throws Exception {
		// 根据不同的表单类型转入不同的表单设计器上去.
		com.kong.kflowweb.BP.Sys.MapData md = new com.kong.kflowweb.BP.Sys.MapData(this.getFK_MapData());
		if (md.getHisFrmType() == com.kong.kflowweb.BP.Sys.FrmType.FoolForm) {
			/* 傻瓜表单 */
		...
		if (md.getHisFrmType() == com.kong.kflowweb.BP.Sys.FrmType.VSTOForExcel) {
			/* 自由表单 */
			return "url@FormDesigner.htm?FK_MapData=" + this.getFK_MapData();

			// return
			// "url@../../Comm/En.htm?EnsName=com.kong.kflowweb.BP.WF.Template.MapFrmExcels&PK=" +
			// this.FK_MapData;
			// return
		   ...
		   if (md.getHisFrmType() == com.kong.kflowweb.BP.Sys.FrmType.Url)
           {
               /* 自由表单 */
               return "url@../../Comm/EnOnly.htm?EnName=com.kong.kflowweb.BP.WF.Template.MapDataURL&No=" + this.getFK_MapData();
           }
		   if (md.getHisFrmType() == com.kong.kflowweb.BP.Sys.FrmType.Entity)
			   return "url@../../Comm/Ens.htm?EnsName=" + md.getPTable();
	...
	public final String PublicNoNameCtrlCreate() throws Exception {
		try {
			float x = Float.parseFloat(this.GetRequestVal("x"));
			float y = Float.parseFloat(this.GetRequestVal("y"));
			com.kong.kflowweb.BP.Sys.CCFormAPI.CreatePublicNoNameCtrl(this.getFrmID(), this.GetRequestVal("CtrlType"),
	 ...
	 */
	 public String NewImage()
     {
         ...
         try
         {
             com.kong.kflowweb.BP.Sys.CCFormAPI.NewImage(this.GetRequestVal("FrmID"),
	...
	public String NewField() throws Exception {
		try {
			com.kong.kflowweb.BP.Sys.CCFormAPI.NewField(this.GetRequestVal("FrmID"), this.GetRequestVal("KeyOfEn"),
	...
	public final String CCForm_AllElements_ResponseJson() throws Exception {
		...
		try {
              ...
              com.kong.kflowweb.BP.Sys.FrmUI.MapFrameExts mapFrameExts = new com.kong.kflowweb.BP.Sys.FrmUI.MapFrameExts(this.getFK_MapData());
	...
	public final String SaveForm() throws Exception {
		  ...
		  //清缓存
        com.kong.kflowweb.BP.Sys.CCFormAPI.AfterFrmEditAction(this.getFK_MapData());
        
		com.kong.kflowweb.BP.Sys.CCFormAPI.SaveFrm(this.getFK_MapData(), this.GetRequestVal("diagram"));
	...
	*/
	...
	public final String Tables_Init() throws Exception {
		...
		for (DataRow dr : dt.Rows) {
			// 求引用数量.
			int refNum = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt(
	...
	public final String Tables_Delete() throws Exception {
		try {
			com.kong.kflowweb.BP.Sys.SFTable tab = new com.kong.kflowweb.BP.Sys.SFTable();
	...
	public final String TableRef_Init() throws Exception {
		com.kong.kflowweb.BP.Sys.MapAttrs mapAttrs = new com.kong.kflowweb.BP.Sys.MapAttrs();
		mapAttrs.RetrieveByAttr(com.kong.kflowweb.BP.Sys.MapAttrAttr.UIBindKey, this.getFK_SFTable());
	...
	public final String Home_Init() throws Exception {
		...
		ht.put("MapFrmFrees", "../../Comm/En.htm?EnName=com.kong.kflowweb.BP.WF.Template.MapFrmFree&PKVal=" + no); // 自由表单属性.
		ht.put("MapFrmFools", "../../Comm/En.htm?EnName=com.kong.kflowweb.BP.WF.Template.MapFrmFool&PKVal=" + no); // 傻瓜表单属性.
		ht.put("MapFrmExcels", "../../Comm/En.htm?EnName=com.kong.kflowweb.BP.WF.Template.MapFrmExcel&PKVal=" + no); // Excel表单属性.
		ht.put("MapDataURLs", "../../Comm/En.htm?EnName=com.kong.kflowweb.BP.WF.Template.MapDataURL&PKVal=" + no); // 嵌入式表单属性.

		return com.kong.kflowweb.BP.DA.DataType.ToJsonEntityModel(ht);
	}
