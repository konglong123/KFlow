...
public class BillTemplate extends EntityNoName
{
	...
	*/
    ...
    {
    	 byte[] bytes = com.kong.kflowweb.BP.DA.DBAccess.GetByteFromDB(this.getEnMap().getPhysicsTable(), "No", this.getNo(), "DBFile");
         if (bytes != null)
             return bytes;

         //如果没有找到，就看看默认的文件是否有.
         String tempExcel = com.kong.kflowweb.BP.Sys.SystemConfig.getPathOfDataUser() + "CyclostyleFile\\" + this.getNo() + ".rtf";
         if (new File(tempExcel).exists() == false)
             tempExcel = com.kong.kflowweb.BP.Sys.SystemConfig.getPathOfDataUser() + "CyclostyleFile\\Word单据模版定义演示.docx";

         bytes = com.kong.kflowweb.BP.DA.DataType.ConvertFileToByte(tempExcel);
         return bytes;
    }
