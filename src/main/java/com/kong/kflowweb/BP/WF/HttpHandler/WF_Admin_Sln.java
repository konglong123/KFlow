...
public class WF_Admin_Sln extends WebContralBase {
	...
	public final String BindForm_GetFlowNodeDropList() throws Exception {
		...
		nodes.Retrieve(com.kong.kflowweb.BP.WF.Template.NodeAttr.FK_Flow, this.getFK_Flow(),
				com.kong.kflowweb.BP.WF.Template.NodeAttr.Step);
		...
		for (SysFormTree formTree : formTrees.Tolist()) {
			...
			if (mapS != null && mapS.size() > 0) {
				for (MapData map : mapS.Tolist()) {
					com.kong.kflowweb.BP.WF.Template.FlowFormTree formFolder = new com.kong.kflowweb.BP.WF.Template.FlowFormTree();
	...
	public final String Fields_Init() throws Exception
	{
		...
		{
			if(attr.getKeyOfEn() == com.kong.kflowweb.BP.WF.WorkAttr.RDT)
				continue;
			else if(attr.getKeyOfEn() == com.kong.kflowweb.BP.WF.WorkAttr.FID)
				continue;
			else if(attr.getKeyOfEn() == com.kong.kflowweb.BP.WF.WorkAttr.OID)
				continue;
			else if(attr.getKeyOfEn() == com.kong.kflowweb.BP.WF.WorkAttr.Rec)
				continue;
			else if(attr.getKeyOfEn() == com.kong.kflowweb.BP.WF.WorkAttr.MyNum)
				continue;
			else if(attr.getKeyOfEn() == com.kong.kflowweb.BP.WF.WorkAttr.MD5)
				continue;
			else if(attr.getKeyOfEn() == com.kong.kflowweb.BP.WF.WorkAttr.Emps)
				continue;
			else if(attr.getKeyOfEn() == com.kong.kflowweb.BP.WF.WorkAttr.CDT)
				continue;
	...
	// /#endregion 字段权限.
	  ...
	  ///#region 附件权限.
			...
			{
				com.kong.kflowweb.BP.Sys.FrmAttachments fas = new com.kong.kflowweb.BP.Sys.FrmAttachments();
				...
				int idx = 0;
				for (com.kong.kflowweb.BP.Sys.FrmAttachment item : fas.ToJavaList())
				{
	...
	public final String Dtls_Init() throws Exception {
		 com.kong.kflowweb.BP.Sys.MapDtls dtls = new com.kong.kflowweb.BP.Sys.MapDtls();
			...
			for (com.kong.kflowweb.BP.Sys.MapDtl item : dtls.ToJavaList())
			{
				if (item.getFK_Node() != 0)
				{
					continue;
