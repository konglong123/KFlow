...
import com.kong.kflowweb.BP.WF.SendReturnObjs;
...
public class AutoRunStratFlows extends Method {
	...
	public Object Do() throws Exception {
		...
		for (Flow fl : fls.ToJavaList()) {
			if (fl.getHisFlowRunWay() == com.kong.kflowweb.BP.WF.FlowRunWay.HandWork)
				continue;
			...
			if (fl.getRunObj() == null || fl.getRunObj() == "") {
				...
				com.kong.kflowweb.BP.DA.Log.DebugWriteError(msg);
				continue;
			}
			...
			case SpecEmp: // 指定人员按时运行。
				...
				if (emp.RetrieveFromDBSources() == 0) {
					com.kong.kflowweb.BP.DA.Log.DebugWriteError("启动自动启动流程错误：发起人(" + fk_emp + ")不存在。");
					continue;
				}
				...
				try {
					...
					// 创建空白工作, 发起开始节点.
					long workID = com.kong.kflowweb.BP.WF.Dev2Interface.Node_CreateBlankWork(fl.getNo());

					// 执行发送.
					SendReturnObjs objs = com.kong.kflowweb.BP.WF.Dev2Interface.Node_SendWork(fl.getNo(), workID);

					// string info_send=
					// com.kong.kflowweb.BP.WF.Dev2Interface.Node_StartWork(fl.No,);
					com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLineInfo(
							"流程:" + fl.getNo() + fl.getName() + "的定时任务\t\n -------------- \t\n" + objs.ToMsgOfText());

				} catch (Exception ex) {
					com.kong.kflowweb.BP.DA.Log.DebugWriteError(
		...
		if (com.kong.kflowweb.BP.Web.WebUser.getNo() != "admin") {
			com.kong.kflowweb.BP.Port.Emp empadmin = new com.kong.kflowweb.BP.Port.Emp("admin");
			com.kong.kflowweb.BP.Web.WebUser.SignInOfGener(empadmin);
		}
		// #endregion 发送消息
	...
	public void DTS_Flow(com.kong.kflowweb.BP.WF.Flow fl) throws Exception {
		// #region 读取数据.
		...
		if (dtMain.Rows.size() == 0) {
			com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLineError("流程(" + fl.getName() + ")此时无任务.");
			return;
		}
		...
		for (DataRow dr : dtMain.Rows) {
			...
			try {
				...
				if (toNodeID == fl.getStartNodeID()) {
					...
					long workID = com.kong.kflowweb.BP.WF.Dev2Interface.Node_CreateBlankWork(fl.getNo(), null, null, WebUser.getNo(),
							null);
					if (workID != wk.getOID())
				...
				com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLineWarning("@" + fl.getName() + ",第" + idx + "条,发起人员:" + WebUser.getNo() + "-"
