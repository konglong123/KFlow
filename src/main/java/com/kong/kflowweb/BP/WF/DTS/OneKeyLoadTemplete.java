	...
	*/
	...
	public Object Do() throws Exception
	{
		...
		try {
			...
			if (com.kong.kflowweb.BP.Sys.SystemConfig.getOSModel() == OSModel.OneMore)
			{
				//EmpDepts.
				com.kong.kflowweb.BP.GPM.DeptEmps eds = new com.kong.kflowweb.BP.GPM.DeptEmps();
				eds.ClearTable();
				dt = ds.hashTables.get("Port_DeptEmp");
				Object tempVar4 = QueryObject.InitEntitiesByDataTable(eds, dt, null);
				eds = (com.kong.kflowweb.BP.GPM.DeptEmps)((tempVar4 instanceof com.kong.kflowweb.BP.GPM.DeptEmps) ? tempVar4 : null);
				for (com.kong.kflowweb.BP.GPM.DeptEmp item : eds.ToJavaList())
				{
					item.DirectInsert(); //插入数据.
				}
			}
			...
			for (File dir : dirs)
			{
				...
				{
					msg += "\t\n@开始调度流程模板文件:" + filePath;
					Flow myflow = com.kong.kflowweb.BP.WF.Flow.DoLoadFlowTemplate(fs.getNo(), filePath.getPath(), ImpFlowTempleteModel.AsTempleteFlowNo);
		...
		//删除多余的空格.
		com.kong.kflowweb.BP.WF.DTS.DeleteBlankGroupField dts = new DeleteBlankGroupField();
		dts.Do();
