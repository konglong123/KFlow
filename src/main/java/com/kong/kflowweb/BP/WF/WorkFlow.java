...
public class WorkFlow
{
	...
	*/
	public final boolean IsCanDoCurrentWork(String empId) throws Exception
	{
		...
		return com.kong.kflowweb.BP.WF.Dev2Interface.Flow_IsCanDoCurrentWork( wn.getWorkID(), empId);
	}
	/** 
	 执行驳回
	 应用场景:子流程向分合点驳回时
	...
	*/
	public final void DoDeleteWorkFlowByFlag(String msg) throws Exception
	{
		try
		{
			...
			com.kong.kflowweb.BP.WF.Node nd = new Node(gwf.getFK_Node());
			...
			//设置产生的工作流程为.
			gwf.setWFState(com.kong.kflowweb.BP.WF.WFState.Delete);
			gwf.Update();
	...
	*/
	public final void DoUnDeleteWorkFlowByFlag(String msg) throws Exception
	{
		try
		{
			...
			//设置产生的工作流程为.
			GenerWorkFlow gwf = new GenerWorkFlow(this.getWorkID());
			gwf.setWFState(com.kong.kflowweb.BP.WF.WFState.Runing);
			gwf.Update();
	...
	*/
	public static String DoDeleteWorkFlowAlreadyComplete(String flowNo, long workID, boolean isDelSubFlow, String note) throws Exception
	{
		...
		if (isDelSubFlow)
		{
			...
			{
				com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoDeleteFlowByReal(item.getFK_Flow(), item.getWorkID(), true);
			}
		}
		com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLineInfo("@[" + fl.getName() + "]流程被[" + com.kong.kflowweb.BP.Web.WebUser.getNo() + com.kong.kflowweb.BP.Web.WebUser.getName() + "]删除，WorkID[" + workID + "]。");
	...
	*/
	private String DoDeleteSubThread() throws Exception
	{
		...
		if (1 == 2)
		{
			...
			///#region 写入删除日志.
		wn.AddToTrack(ActionType.DeleteSubThread, empOfWorker.getNo(), empOfWorker.getName(), wn.getHisNode().getNodeID(), wn.getHisNode().getName(), "子线程被:" + com.kong.kflowweb.BP.Web.WebUser.getName() + "删除.");
...
//             
		...
		int num = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt(mysql);
		if (num == 0)
		{
...
//                 * 
			...
			int i = gwl.Retrieve(GenerWorkerListAttr.FK_Node, gwfMain.getFK_Node(), GenerWorkerListAttr.WorkID, gwfMain.getWorkID(), GenerWorkerListAttr.FK_Emp, com.kong.kflowweb.BP.Web.WebUser.getNo());
			if (i == 0)
			{
...
//                         
					...
					num = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt(mysql);
					if (num == 0)
					{
...
//                             
						...
						int fenLiuNodeID = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt(mysql);
						...
						for (GenerWorkerList item : gwls.ToJavaList())
						{
							...
							item.setIsPassInt(0);
							item.setRDT(com.kong.kflowweb.BP.DA.DataType.getCurrentDataTimess());
							item.setSDT(com.kong.kflowweb.BP.DA.DataType.getCurrentDataTimess());
							item.Update();
						}
					}
				}
			...
			else
			{
				gwl.setIsRead(false);
				gwl.setIsPassInt(0);
				gwl.setRDT(com.kong.kflowweb.BP.DA.DataType.getCurrentDataTimess());
				gwl.setSDT(com.kong.kflowweb.BP.DA.DataType.getCurrentDataTimess());
				gwl.Update();
	...
	*/
	public final String DoDeleteWorkFlowByReal(boolean isDelSubFlow) throws Exception
	{
		...
		DBAccess.RunSQL("DELETE FROM WF_CHEval WHERE  WorkID=" + this.getWorkID()); // 删除质量考核数据。
			///#region 正常的删除信息.
		com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLineInfo("@[" + this.getHisFlow().getName() + "]流程被[" + com.kong.kflowweb.BP.Web.WebUser.getNo() + com.kong.kflowweb.BP.Web.WebUser.getName() + "]删除，WorkID[" + this.getWorkID() + "]。");
		String msg = "";
		try
		{
			long workId = this.getWorkID();
	...
	*/
	public final void DoComeBackWorkFlow(String msg) throws Exception
	{
		try
		{
			...
			com.kong.kflowweb.BP.WF.MsgsManager.AddMsgs(wls, "恢复的流程", wn.getHisNode().getName(), "回复的流程");
		}
		catch (RuntimeException ex)
		{
	...
	*/
	public final WorkNode GetCurrentWorkNode() throws Exception
	{
		...
		work.SetValByKey("FK_Dept", com.kong.kflowweb.BP.Web.WebUser.getFK_Dept());
		if (work.RetrieveFromDBSources() == 0)
		{
			...
			work.setRec(com.kong.kflowweb.BP.Web.WebUser.getNo());
			try
			{
				work.Insert();
			}
	...
	*/
	...
	{
		// 查询出来有少没有完成的流程。
		int i = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt("SELECT COUNT(*) FROM WF_GenerWorkFlow WHERE FID=" + gwf.getFID() + " AND WFState!=1");
		switch (i)
		{
			case 0:
				throw new RuntimeException("@不应该的错误。");
			case 1:
				com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM WF_GenerWorkFlow  WHERE FID=" + gwf.getFID() + " OR WorkID=" + gwf.getFID());
				com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM WF_GenerWorkerlist WHERE FID=" + gwf.getFID() + " OR WorkID=" + gwf.getFID());
			...
			default:
				com.kong.kflowweb.BP.DA.DBAccess.RunSQL("UPDATE WF_GenerWorkFlow SET WFState=1 WHERE WorkID=" + this.getWorkID());
				com.kong.kflowweb.BP.DA.DBAccess.RunSQL("UPDATE WF_GenerWorkerlist SET IsPass=1 WHERE WorkID=" + this.getWorkID());
				return "@当前的工作已经完成。";
		}
	}
	/** 
	...
	*/
	public final String LetParentFlowAutoSendNextSetp() throws Exception
	{
		...
		{
			  /*让父流程显示待办.*/
            com.kong.kflowweb.BP.DA.DBAccess.RunSQL("UPDATE WF_GenerWorkerlist SET IsPass=0 WHERE IsPass=80 AND WorkID="+this.getHisGenerWorkFlow().getPWorkID());
            return "";
		}
        ...
        {
            com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoFlowOver(this.getHisGenerWorkFlow().getPFlowNo(),
		...
		// 检查是否是最后的一个.
		int num = com.kong.kflowweb.BP.WF.Dev2Interface.Flow_NumOfSubFlowRuning(this.getHisGenerWorkFlow().getPWorkID(), this.getHisGenerWorkFlow().getWorkID());
		if (num != 0)
		{
			return "";
		}
        ...
        //当前登录用户.
        String currUserNo = com.kong.kflowweb.BP.Web.WebUser.getNo();
		...
		try
		{
			...
			//GERpt rpt = new GERpt("ND" + Integer.parseInt(this.getHisFlow().getNo()) + "Rpt", this.getWorkID());
			  ...
			  //@袁丽娜.
            if (com.kong.kflowweb.BP.WF.Dev2Interface.Flow_IsCanDoCurrentWork( pGWF.getWorkID(), WebUser.getNo()) == false)
            {
                /*没有权限的情况下，就移交给当前人员，让其在发送. */
                com.kong.kflowweb.BP.WF.Dev2Interface.Node_Shift(this.getHisGenerWorkFlow().getPFlowNo(), pGWF.getFK_Node(), pGWF.getWorkID(), 0, WebUser.getNo(), "工作自动移交，让其运行到下一步。");
            }

			// 让当前人员向下发送，但是这种发送一定不要检查发送权限，否则的话就出错误，不能发送下去.
			SendReturnObjs objs = com.kong.kflowweb.BP.WF.Dev2Interface.Node_SendWork(this.getHisGenerWorkFlow().getPFlowNo(), pGWF.getWorkID(),null, null, 0, null, emp.getNo(), emp.getName(), emp.getFK_Dept(), emp.getFK_DeptText(), null);
		...
		catch (RuntimeException ex)
		{
			...
			this.getHisGenerWorkFlow().DirectUpdate();
			   ...
			   //切换到当前流程节点.
            com.kong.kflowweb.BP.WF.Dev2Interface.Port_Login(currUserNo);
	...
	*/
	public final String CCTo(DataTable dt) throws Exception
	{
		...
		for (DataRow dr : dt.Rows)
		{
			...
			emps += com.kong.kflowweb.BP.WF.Glo.DealUserInfoShowModel(no, name);
		}

		Paras pss = new Paras();
		pss.Add("Sender", com.kong.kflowweb.BP.Web.WebUser.getNo());
		pss.Add("Receivers", emps);
		pss.Add("Title", "工作流抄送：工作名称:" + this.getHisFlow().getName() + "，最后处理人：" + com.kong.kflowweb.BP.Web.WebUser.getName());
	...
	*/
	public final String DoFix(String fixMsg) throws Exception
	{
		...
		//    //写入消息。
		//    com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendMsg(wl.FK_Emp, title, mailDoc, "Fix" + wl.WorkID, com.kong.kflowweb.BP.Sys.SMSMsgType.Etc, wl.FK_Flow, wl.FK_Node, wl.WorkID, wl.FID);
		//}
	...
	*/
	public final String DoUnFix(String unFixMsg) throws Exception
	{
		...
		//    //写入消息。
		//    com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendMsg(wl.FK_Emp, title, mailDoc, "Fix" + wl.WorkID, com.kong.kflowweb.BP.Sys.SMSMsgType.Self, wl.FK_Flow, wl.FK_Node, wl.WorkID, wl.FID);
		//}
	...
	*/
	public static boolean IsCanDoWorkCheckByEmpStation(int nodeId, String empId)
	{
		boolean isCan = false;
		// 判断岗位对应关系是不是能够执行.
		String sql = "SELECT a.FK_Node FROM WF_NodeStation a,  " + com.kong.kflowweb.BP.WF.Glo.getEmpStation() + " b WHERE (a.FK_Station=b.FK_Station) AND (a.FK_Node=" + nodeId + " AND b.FK_Emp='" + empId + "' )";
		...
		// 判断他的主要工作岗位能不能执行它.
		sql = "select FK_Node from WF_NodeStation WHERE FK_Node=" + nodeId + " AND ( FK_Station in (select FK_Station from " + com.kong.kflowweb.BP.WF.Glo.getEmpStation() + " WHERE FK_Emp='" + empId + "') ) ";
	...
	*/
	...
	{
		String sql = "select a.FK_Node, b.EmpID from WF_NodeStation  a,  " + com.kong.kflowweb.BP.WF.Glo.getEmpStation() + " b WHERE (a.FK_Station=b.FK_Station) AND (a.FK_Node=" + nodeId + " )";
	...
	*/
	public final String getVirPath()
	{
		if (_VirPath == null)
		{
			if (com.kong.kflowweb.BP.Sys.SystemConfig.getIsBSsystem() &&  com.kong.kflowweb.BP.Sys.Glo.getRequest()!=null)
			{
				_VirPath =com.kong.kflowweb.BP.Sys.Glo.getRequest().getRemoteAddr();
			}
			else
			{
				_VirPath = "";
	...
	*/
	public final String DoHungUp(HungUpWay way, String relData, String hungNote) throws Exception
	{
		...
		hu.setRec(com.kong.kflowweb.BP.Web.WebUser.getNo()); //挂起人
		...
		for (GenerWorkerList wl : wls.ToJavaList())
		{
			...
			//com.kong.kflowweb.BP.WF.Port.WFEmp emp = new WFEmp(wl.FK_Emp);
			emps += wl.getFK_Emp() + "," + wl.getFK_EmpText() + ";";

			//写入消息。
			com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendMsg(wl.getFK_Emp(), title, mailDoc, "HungUp" + wl.getWorkID(), com.kong.kflowweb.BP.WF.SMSMsgType.HungUp, wl.getFK_Flow(), wl.getFK_Node(), wl.getWorkID(), wl.getFID());
		}
	...
	*/
	public final String DoUnHungUp() throws Exception
	{
		...
		ps.Add("MyPK", com.kong.kflowweb.BP.DA.DBAccess.GenerGUID());
		...
		for (GenerWorkerList wl : wls.ToJavaList())
		{
			...
			//写入消息。
			com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendMsg(wl.getFK_Emp(), title, mailDoc, "HungUp" + wl.getFK_Node() + this.getWorkID(), com.kong.kflowweb.BP.WF.SMSMsgType.Self, getHisGenerWorkFlow().getFK_Flow(), getHisGenerWorkFlow().getFK_Node(), this.getWorkID(), this.getFID());
	...
	*/
	public final String DoUnShift() throws Exception
	{
		...
		//删除撤销信息.
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM WF_ShiftWork WHERE WorkID=" + this.getWorkID() + " AND FK_Node=" + wk.getFK_Node());
