...
import com.kong.kflowweb.BP.En.RefMethod;
import com.kong.kflowweb.BP.En.UAC;
import com.kong.kflowweb.BP.En.UIContralType;
import com.kong.kflowweb.BP.Port.Emp;
import com.kong.kflowweb.BP.Sys.AttachmentUploadType;
import com.kong.kflowweb.BP.Sys.EventListOfNode;
import com.kong.kflowweb.BP.Sys.FrmAttachment;
import com.kong.kflowweb.BP.Sys.FrmAttachmentDB;
import com.kong.kflowweb.BP.Sys.FrmAttachmentDBAttr;
import com.kong.kflowweb.BP.Sys.FrmAttachmentDBs;
import com.kong.kflowweb.BP.Sys.FrmAttachments;
import com.kong.kflowweb.BP.Sys.FrmEle;
import com.kong.kflowweb.BP.Sys.FrmEles;
import com.kong.kflowweb.BP.Sys.FrmEvent;
import com.kong.kflowweb.BP.Sys.FrmEvents;
import com.kong.kflowweb.BP.Sys.FrmImg;
import com.kong.kflowweb.BP.Sys.FrmImgAths;
import com.kong.kflowweb.BP.Sys.FrmImgs;
import com.kong.kflowweb.BP.Sys.FrmLab;
import com.kong.kflowweb.BP.Sys.FrmLabs;
import com.kong.kflowweb.BP.Sys.FrmLine;
import com.kong.kflowweb.BP.Sys.FrmLines;
import com.kong.kflowweb.BP.Sys.FrmLink;
import com.kong.kflowweb.BP.Sys.FrmLinks;
import com.kong.kflowweb.BP.Sys.FrmRB;
import com.kong.kflowweb.BP.Sys.FrmRBs;
import com.kong.kflowweb.BP.Sys.GEDtl;
import com.kong.kflowweb.BP.Sys.GEDtlAttr;
import com.kong.kflowweb.BP.Sys.GEDtls;
import com.kong.kflowweb.BP.Sys.GEEntity;
import com.kong.kflowweb.BP.Sys.GroupField;
import com.kong.kflowweb.BP.Sys.GroupFieldAttr;
import com.kong.kflowweb.BP.Sys.GroupFields;
...
import com.kong.kflowweb.BP.Sys.MapDtlAttr;
...
import com.kong.kflowweb.BP.Sys.MapFrame;
import com.kong.kflowweb.BP.Sys.MapFrames;
...
import com.kong.kflowweb.BP.Sys.SysEnumMainAttr;
import com.kong.kflowweb.BP.Sys.SysEnumMains;
...
import com.kong.kflowweb.BP.Tools.FileAccess;
...
import com.kong.kflowweb.BP.WF.Data.NDXRptBaseAttr;
import com.kong.kflowweb.BP.WF.Template.*;
import com.kong.kflowweb.BP.Web.GuestUser;
import com.kong.kflowweb.BP.Web.WebUser;

/**
 * 流程 记录了流程的信息． 流程的编号，名称，建立时间．
 */
public class Flow extends com.kong.kflowweb.BP.En.EntityNoName {
	...
	public final String getStartGuidePara2() {
		...
		str = str.replace("~", "'");
		if (StringHelper.isNullOrEmpty(str)) {
			if (this.getStartGuideWay() == com.kong.kflowweb.BP.WF.Template.StartGuideWay.ByHistoryUrl) {
			}
		}
		return str;
	}
	...
	public final Work NewWork(Emp emp, java.util.Hashtable paras) throws Exception {
		// 检查是否可以发起该流程？
		if (com.kong.kflowweb.BP.WF.Glo.CheckIsCanStartFlow_InitStartFlow(this) == false)
		...
		if (com.kong.kflowweb.BP.Sys.SystemConfig.getIsBSsystem() == true) {
			if (com.kong.kflowweb.BP.Sys.Glo.getRequest() != null) {
				Enumeration enu = com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameterNames();
				while (enu.hasMoreElements()) {
					...
					paras.put(key, com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter(key));
				}
			}
		...
		// 开始节点.
		com.kong.kflowweb.BP.WF.Node nd = new com.kong.kflowweb.BP.WF.Node(this.getStartNodeID());
		...
		if (this.getDraftRole() != com.kong.kflowweb.BP.WF.Template.DraftRole.None && nd.getIsStartNode()) {
			IsNewWorkID = true;
		}
		...
		try {
			...
			if (wk.getOID() == 0) {
				...
				wk.SetValByKey(WorkAttr.RDT, com.kong.kflowweb.BP.DA.DataType.getCurrentDataTime());
				wk.SetValByKey(WorkAttr.CDT, com.kong.kflowweb.BP.DA.DataType.getCurrentDataTime());
				...
				wk.SetValByKey(GERptAttr.FK_NY, com.kong.kflowweb.BP.DA.DataType.getCurrentYearMonth());
				...
				if (this.getPTable().equals(wk.getEnMap().getPhysicsTable())) {
					...
					rpt.setFID(0);
					rpt.setFlowStartRDT(com.kong.kflowweb.BP.DA.DataType.getCurrentDataTime());
					rpt.setMyNum(0);
					rpt.setTitle(com.kong.kflowweb.BP.WF.WorkFlowBuessRole.GenerTitle(this, wk));
					rpt.setWFState(WFState.Blank);
					rpt.setFlowStarter(emp.getNo());
					rpt.setFK_NY(DataType.getCurrentYearMonth());
					if (com.kong.kflowweb.BP.WF.Glo.getUserInfoShowModel() == UserInfoShowModel.UserNameOnly) {
						rpt.setFlowEmps("@" + emp.getName()+"@");
					}

					if (com.kong.kflowweb.BP.WF.Glo.getUserInfoShowModel() == UserInfoShowModel.UserIDOnly) {
						rpt.setFlowEmps("@" + emp.getNo()+"@");
					}

					if (com.kong.kflowweb.BP.WF.Glo.getUserInfoShowModel() == UserInfoShowModel.UserIDUserName) {
						rpt.setFlowEmps("@" + emp.getNo() + "," + emp.getName()+"@");
					}

					rpt.setFlowEnderRDT(com.kong.kflowweb.BP.DA.DataType.getCurrentDataTime());
				...
				rpt.setFID(0);
				rpt.setFlowStartRDT(com.kong.kflowweb.BP.DA.DataType.getCurrentDate());
				rpt.setFlowEnderRDT(com.kong.kflowweb.BP.DA.DataType.getCurrentDate());
				rpt.setMyNum(1);
			}
		...
		if (paras.containsKey(StartFlowParaNameList.IsDeleteDraft)
			...
			try {
				...
				wk.Update();
				com.kong.kflowweb.BP.DA.Log.DebugWriteError("创建新工作错误，但是屏蔽了异常,请检查默认值的问题：" + ex.getMessage());
			}
		...
		/// #region 判断是否装载上一条数据.
		if (this.getIsLoadPriData() == true && this.getStartGuideWay() == com.kong.kflowweb.BP.WF.Template.StartGuideWay.None) {
			// 如果需要从上一个流程实例上copy数据.
		...
		if (StringHelper.isNullOrEmpty(PNodeIDStr) == false && StringHelper.isNullOrEmpty(PWorkIDStr) == false) {
			...
			/// #region 从调用的节点上copy.
			com.kong.kflowweb.BP.WF.Node fromNd = new com.kong.kflowweb.BP.WF.Node(Integer.parseInt(PNodeIDStr));
			...
			if (CopyFormWorkID != null) {
				...
				if (!this.getBillNoFormat().equals("")) {
					rpt.SetValByKey(GERptAttr.BillNo, com.kong.kflowweb.BP.WF.WorkFlowBuessRole.GenerBillNo(this.getBillNoFormat(),
				...
				rpt.SetValByKey(GERptAttr.FlowStartRDT, com.kong.kflowweb.BP.DA.DataType.getCurrentDataTime());
				rpt.SetValByKey(GERptAttr.FlowEnderRDT, com.kong.kflowweb.BP.DA.DataType.getCurrentDataTime());
				...
				if (dtlsFrom.size() == dtls.size()) {
					for (MapDtl dtl : dtls.ToJavaList()) {
						...
						for (GEDtl geDtlFromData : dtlsFromData.ToJavaList()) {
							...
							if(PFlowNo.equals(this.getNo()) == false &&(this.getStartLimitRole() == com.kong.kflowweb.BP.WF.StartLimitRole.OnlyOneSubFlow) ){
								...
								if(dbs!=null && dbs.size()!=0){
									...
									for(FrmAttachmentDB db:dbs.ToJavaList() ){
										...
										newDB.setMyPK(com.kong.kflowweb.BP.DA.DBAccess.GenerGUID());
										newDB.Insert();
									}
								}
							}
				...
				if (fns.size() != 0) {
					...
					for (FrmNode fn : fns.ToJavaList()) {
						for (FrmNode fnFrom : fnsFrom.ToJavaList()) {
							...
							com.kong.kflowweb.BP.Sys.GEEntity geEnFrom = new GEEntity(fnFrom.getFK_Frm());
		...
		if (this.getBillNoFormat().length() > 3)
			if (this.getBillNoFormat().length() > 3) {
				...
				// 生成单据编号.
				rpt.setBillNo(com.kong.kflowweb.BP.WF.Glo.GenerBillNo(billNoFormat, rpt.getOID(), rpt, this.getPTable()));
		...
		if (paras.containsKey("JumpToNode") == true) {
			wk.setRec(WebUser.getNo());
			wk.SetValByKey(StartWorkAttr.RDT, com.kong.kflowweb.BP.DA.DataType.getCurrentDataTime());
			wk.SetValByKey(StartWorkAttr.CDT, com.kong.kflowweb.BP.DA.DataType.getCurrentDataTime());
			...
			com.kong.kflowweb.BP.WF.GenerWorkFlow gwf = new GenerWorkFlow(rpt.getOID());
		...
		if (mygwf.RetrieveFromDBSources() == 0) {
			...
			mygwf.setRDT(com.kong.kflowweb.BP.DA.DataType.getCurrentDataTime());
			mygwf.Insert();
		}
		mygwf.setStarter(WebUser.getNo());
		mygwf.setStarterName(WebUser.getName());
		mygwf.setFK_Dept(com.kong.kflowweb.BP.Web.WebUser.getFK_Dept());
		mygwf.setDeptName(com.kong.kflowweb.BP.Web.WebUser.getFK_DeptName());
		mygwf.setBillNo(rpt.getBillNo());
		mygwf.setTitle(rpt.getTitle());

		if (mygwf.getTitle().contains("@") == true) {
			mygwf.setTitle(com.kong.kflowweb.BP.WF.WorkFlowBuessRole.GenerTitle(this, rpt));
		}
		if (StringHelper.isNullOrEmpty(PNodeIDStr) == false && StringHelper.isNullOrEmpty(PWorkIDStr) == false) {
	...
	public final Work GenerWork(long workid, Node nd, boolean isPostBack) throws Exception {
		...
		if (SystemConfig.getIsBSsystem() == true && isPostBack == false && nd.getIsStartNode()
				&& com.kong.kflowweb.BP.Sys.Glo.getRequest() != null
				&& com.kong.kflowweb.BP.Sys.Glo.getRequest().getParameter("IsDeleteDraft").equals("1")) {
		...
		if (nd.getIsStartNode()) {
			try {
				...
				if (StringHelper.isNullOrEmpty(billNo) && nd.getHisFlow().getBillNoFormat().length() > 2) {
					// 让他自动生成编号
					wk.SetValByKey(NDXRptBaseAttr.BillNo, com.kong.kflowweb.BP.WF.WorkFlowBuessRole.GenerBillNo(
	...
	public final String DoBTableDTS(GERpt rpt, Node currNode, boolean isStopFlow) throws Exception {
		...
		com.kong.kflowweb.BP.Sys.SFDBSrc src = new com.kong.kflowweb.BP.Sys.SFDBSrc(this.getDTSDBSrc());
	...
	public final String DoAutoStartIt() throws Exception {
		...
		case SpecEmp: // 指定人员按时运行。
			...
			String info_send = com.kong.kflowweb.BP.WF.Dev2Interface.Node_StartWork(this.getNo(), null, null, 0, null, 0, null)
					.ToMsgOfText();
		...
		String msg = "";
		com.kong.kflowweb.BP.Sys.MapExt me = new MapExt();
		me.setMyPK("ND" + Integer.parseInt(this.getNo()) + "01_" + MapExtXmlList.StartFlow);
		int i = me.RetrieveFromDBSources();
		if (i == 0) {
			com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLineError("没有为流程(" + this.getName() + ")的开始节点设置发起数据,请参考说明书解决.");
		...
		for (DataRow dr : dtMain.Rows) {
			...
			try {
				...
				com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLineInfo(msg);
	...
	public final String ClearCash() throws Exception {
		com.kong.kflowweb.BP.DA.Cash.ClearCash();
		 //清空流程中的缓存
        //获取改流程中的节点数据
        ...
        for(Node nd : nds.ToJavaList()){
            ...
            if(nd.getHisFormType()  == NodeFormType.FoolForm || nd.getHisFormType()  == NodeFormType.FreeForm)
                com.kong.kflowweb.BP.Sys.CCFormAPI.AfterFrmEditAction("ND"+nd.getNodeID());
        }
		return "清除成功.";
	}
	...
	public final String DoCheck() throws Exception {
		...
		try {
		    ...
		    com.kong.kflowweb.BP.WF.Node node = new com.kong.kflowweb.BP.WF.Node();
			...
			for (Node nd : nds.ToJavaList()) {
				...
				// 从表检查。
				MapDtls dtls = new com.kong.kflowweb.BP.Sys.MapDtls("ND" + nd.getNodeID());
				for (MapDtl dtl : dtls.ToJavaList()) {
				...
				case ByDeptAndStation: // 按部门与岗位的交集计算.
					...
					if (com.kong.kflowweb.BP.WF.Glo.getOSModel() == com.kong.kflowweb.BP.Sys.OSModel.OneOne) {
						mysql = "SELECT No FROM Port_Emp WHERE No IN (SELECT No FK_Emp FROM Port_Emp WHERE FK_Dept IN ( SELECT FK_Dept FROM WF_NodeDept WHERE FK_Node="
								+ nd.getNodeID() + "))AND No IN (SELECT FK_Emp FROM " + com.kong.kflowweb.BP.WF.Glo.getEmpStation()
				...
				case ByPreviousNodeFormEmpsField:
					...
					MapAttrs rptAttrs = new com.kong.kflowweb.BP.Sys.MapAttrs();
					rptAttrs.Retrieve(MapAttrAttr.FK_MapData, "ND" + str + "Rpt", MapAttrAttr.KeyOfEn);
					if (rptAttrs.Contains(com.kong.kflowweb.BP.Sys.MapAttrAttr.KeyOfEn, nd.getDeliveryParas()) == false) {
						// 检查节点字段是否有FK_Emp字段
			...
			for (Node nd : nds.ToJavaList()) {
				...
				strs = com.kong.kflowweb.BP.WF.Glo.DealExp(strs, rpt, "err");
	...
	public final String GenerFlowXmlTemplete() throws Exception {
		...
		name = com.kong.kflowweb.BP.Tools.StringExpressionCalculate.ReplaceBadCharOfFileName(name);
	...
	public final void WriteToXml(DataSet ds) {
		...
		try {
			...
			isXmlLocked = false;
			com.kong.kflowweb.BP.DA.Log.DefaultLogWriteLineError("流程模板文件备份错误:" + e.getMessage());
		} catch (Exception e) {
	...
	public DataSet GetFlow(String path) throws Exception {
		...
		// 表单信息，包含从表.
		sql = "SELECT No FROM Sys_MapData WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "No");
		MapDatas mds = new MapDatas();
		mds.RetrieveInSQL(MapDataAttr.No, sql);
		ds.Tables.add(mds.ToDataTableField("Sys_MapData"));

		// Sys_MapAttr.
		sql = "SELECT MyPK FROM Sys_MapAttr WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		sql += " UNION "; // 增加多附件的扩展列.
		sql += "SELECT MyPK FROM Sys_MapAttr WHERE FK_MapData IN ( SELECT MyPK FROM Sys_FrmAttachment WHERE FK_Node=0 AND "
				+ com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData") + " ) ";
		...
		// Sys_MapDtl
		sql = "SELECT No FROM Sys_MapDtl WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		MapDtls mdtls = new MapDtls();
		mdtls.RetrieveInSQL(sql);
		ds.Tables.add(mdtls.ToDataTableField("Sys_MapDtl"));

		// Sys_MapExt
		sql = "SELECT MyPK FROM Sys_MapExt WHERE  " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		MapExts mexts = new MapExts();
		mexts.RetrieveInSQL(sql);
		ds.Tables.add(mexts.ToDataTableField("Sys_MapExt"));

		// Sys_GroupField
		sql = "SELECT OID FROM Sys_GroupField WHERE   " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FrmID"); // +"
																											// "
																											// +
		...
		// Sys_MapFrame
		sql = "SELECT MyPK FROM Sys_MapFrame WHERE" + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		MapFrames mfs = new MapFrames();
		mfs.RetrieveInSQL("MyPK", sql);
		ds.Tables.add(mfs.ToDataTableField("Sys_MapFrame"));

		// Sys_FrmLine.
		sql = "SELECT MyPK FROM Sys_FrmLine WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		FrmLines frmls = new FrmLines();
		frmls.RetrieveInSQL(sql);
		ds.Tables.add(frmls.ToDataTableField("Sys_FrmLine"));

		// Sys_FrmLab.
		sql = "SELECT MyPK FROM Sys_FrmLab WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		FrmLabs frmlabs = new FrmLabs();
		frmlabs.RetrieveInSQL(sql);
		ds.Tables.add(frmlabs.ToDataTableField("Sys_FrmLab"));

		// Sys_FrmEle.
		sql = "SELECT MyPK FROM Sys_FrmEle WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		...
		// Sys_FrmLink.
		sql = "SELECT MyPK FROM Sys_FrmLink WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		FrmLinks frmLinks = new FrmLinks();
		frmLinks.RetrieveInSQL(sql);
		ds.Tables.add(frmLinks.ToDataTableField("Sys_FrmLink"));

		// Sys_FrmRB.
		sql = "SELECT MyPK FROM Sys_FrmRB WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		FrmRBs frmRBs = new FrmRBs();
		frmRBs.RetrieveInSQL(sql);
		ds.Tables.add(frmRBs.ToDataTableField("Sys_FrmRB"));

		// Sys_FrmImgAth.
		sql = "SELECT MyPK FROM Sys_FrmImgAth WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		FrmImgAths frmIs = new FrmImgAths();
		frmIs.RetrieveInSQL(sql);
		ds.Tables.add(frmIs.ToDataTableField("Sys_FrmImgAth"));

		// Sys_FrmImg.
		sql = "SELECT MyPK FROM Sys_FrmImg WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		...
		// Sys_FrmEvent.
		sql = "SELECT MyPK FROM Sys_FrmEvent WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
	...
	public final DataSet GetFlow2017(String path) throws Exception {
		...
		sql = "SELECT * FROM Sys_MapData WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "No");
		dt = DBAccess.RunSQLReturnTable(sql);
		dt.TableName = "Sys_MapData";
		ds.Tables.add(dt);

		// Sys_MapAttr.
		sql = "SELECT * FROM Sys_MapAttr WHERE  " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData")
		...
		// Sys_MapDtl
		sql = "SELECT * FROM Sys_MapDtl WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		dt = DBAccess.RunSQLReturnTable(sql);
		...
		// Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		sql = "SELECT * FROM Sys_MapExt WHERE  " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData"); // +Glo.MapDataLikeKey(this.getNo(),
																											// "FK_MapData");
		...
		// Sys_GroupField
		sql = "SELECT * FROM Sys_GroupField WHERE   " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FrmID"); // +"
																											// "
																											// +
		...
		// Sys_MapFrame
		sql = "SELECT * FROM Sys_MapFrame WHERE" + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		dt = DBAccess.RunSQLReturnTable(sql);
		...
		// Sys_FrmLine.
		sql = "SELECT * FROM Sys_FrmLine WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		dt = DBAccess.RunSQLReturnTable(sql);
		dt.TableName = "Sys_FrmLine";
		ds.Tables.add(dt);

		// Sys_FrmLab.
		sql = "SELECT * FROM Sys_FrmLab WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		dt = DBAccess.RunSQLReturnTable(sql);
		dt.TableName = "Sys_FrmLab";
		ds.Tables.add(dt);

		// Sys_FrmEle.
		sql = "SELECT * FROM Sys_FrmEle WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		dt = DBAccess.RunSQLReturnTable(sql);
		dt.TableName = "Sys_FrmEle";
		ds.Tables.add(dt);

		// Sys_FrmLink.
		sql = "SELECT * FROM Sys_FrmLink WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		dt = DBAccess.RunSQLReturnTable(sql);
		dt.TableName = "Sys_FrmLink";
		ds.Tables.add(dt);

		// Sys_FrmRB.
		sql = "SELECT * FROM Sys_FrmRB WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		dt = DBAccess.RunSQLReturnTable(sql);
		dt.TableName = "Sys_FrmRB";
		ds.Tables.add(dt);

		// Sys_FrmImgAth.
		sql = "SELECT * FROM Sys_FrmImgAth WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		dt = DBAccess.RunSQLReturnTable(sql);
		dt.TableName = "Sys_FrmImgAth";
		ds.Tables.add(dt);

		// Sys_FrmImg.
		sql = "SELECT * FROM Sys_FrmImg WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		dt = DBAccess.RunSQLReturnTable(sql);
		...
		// Sys_FrmEvent.
		sql = "SELECT * FROM Sys_FrmEvent WHERE " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getNo(), "FK_MapData");
		dt = DBAccess.RunSQLReturnTable(sql);
	...
	private String CheckRptData(Nodes nds, DataTable dt) throws Exception {
		...
		for (DataRow dr : dt.Rows) {
			...
			rpt.setTitle(com.kong.kflowweb.BP.WF.WorkFlowBuessRole.GenerTitle(this, startWork));
			try {
				/*
	...
	private void DoCheck_CheckRpt(Nodes nds) throws Exception {
		...
		for (DataRow dr : dt.Rows) {
			...
			com.kong.kflowweb.BP.Sys.MapAttr ma = new com.kong.kflowweb.BP.Sys.MapAttr(mypk);
			if (pks.contains("@" + dr.getValue("KeyOfEn").toString() + "@")) {
				
				com.kong.kflowweb.BP.Sys.MapAttr rptma = new com.kong.kflowweb.BP.Sys.MapAttr("ND" + flowId + "Rpt_" + dr.getValue("KeyOfEn").toString());
		...
		// 创建mapData.
		com.kong.kflowweb.BP.Sys.MapData md = new com.kong.kflowweb.BP.Sys.MapData();
		...
		if (attrs.Contains(md.getNo() + "_" + GERptAttr.Title) == false) {
			// 标题
			MapAttr attr = new com.kong.kflowweb.BP.Sys.MapAttr();
		...
		if (attrs.Contains(md.getNo() + "_" + GERptAttr.FlowStarter) == false) {
			...
			attr.setUIBindKey("com.kong.kflowweb.BP.Port.Emps");
			attr.setUIContralType(UIContralType.DDL);
		...
		if (mapData.RetrieveFromDBSources() == 0) {
			...
			com.kong.kflowweb.BP.WF.Rpt.MapRpt rpt = new com.kong.kflowweb.BP.WF.Rpt.MapRpt();
			rpt.setNo(mapRpt);
			rpt.RetrieveFromDBSources();
		...
		if (attrs.Contains(mapData.getNo() + "_" + GERptAttr.OID) == false) {
			...
			attr.setKeyOfEn("OID");
			attr.setName("WorkID");
			attr.setMyDataType(com.kong.kflowweb.BP.DA.DataType.AppInt);
			attr.setUIContralType(UIContralType.TB);
			attr.setLGType(FieldTypeS.Normal);
			attr.setUIVisible(false);
			attr.setUIIsEnable(false);
			attr.setDefVal("0");
			attr.setHisEditType(com.kong.kflowweb.BP.En.EditType.Readonly);
			attr.Insert();
		}
	...
	public final String DoFlowEventEntity(String doType, Node currNode, Entity en, String atPara, SendReturnObjs objs,
		...
		// 调用通用方法.
		com.kong.kflowweb.BP.WF.OverrideClass.FEE(doType, currNode, en, atPara, objs, jumpToNode, jumpToEmps);
	...
	private com.kong.kflowweb.BP.WF.FlowEventBase _FDEventEntity = null;

	/**
	 * 节点实体类，没有就返回为空.
	 */
	private com.kong.kflowweb.BP.WF.FlowEventBase getFEventEntity() {
		if (_FDEventEntity == null && !this.getFlowMark().equals("") && !this.getFlowEventEntity().equals("")) {
			_FDEventEntity = com.kong.kflowweb.BP.WF.Glo.GetFlowEventEntityByEnName(this.getFlowEventEntity());
		}
	 ...
	 * 
	 */
	public final com.kong.kflowweb.BP.WF.Data.GERpt getHisGERpt() throws Exception {
		try {
			...
			this.DoCheck();
			com.kong.kflowweb.BP.WF.Data.GERpt wk1 = new com.kong.kflowweb.BP.WF.Data.GERpt("ND" + Integer.parseInt(this.getNo()) + "Rpt");
			return wk1;
		}
	}
	...
	protected boolean beforeUpdateInsertAction() throws Exception {
		// 获得事件实体.
		this.setFlowEventEntity(com.kong.kflowweb.BP.WF.Glo.GetFlowEventEntityStringByFlowMark(this.getFlowMark(), this.getNo()));
	...
	public final String DoExp() throws Exception {
		this.DoCheck();
		return com.kong.kflowweb.BP.WF.Glo.getCCFlowAppPath() + "WF/Admin/Exp.jsp?CondType=0&FK_Flow=" + this.getNo();
	}
	...
	public final String DoDRpt() throws Exception {
		...
		PubClass.WinOpen(ContextHolderUtils.getResponse(),
				com.kong.kflowweb.BP.WF.Glo.getCCFlowAppPath() + "WF/Admin/WFRpt.jsp?CondType=0&FK_Flow=" + this.getNo(), "单据", "cdsn",
				800, 500, 210, 300);
		return null;
	}
	...
	public static Flow DoLoadFlowTemplate(String fk_flowSort, String path, ImpFlowTempleteModel model,
		...
		try {
			...
			for (DataTable dt : ds.Tables) {
				...
				{
					for (DataRow dr : dt.Rows) {
						com.kong.kflowweb.BP.WF.Template.NodeSheet nd = new com.kong.kflowweb.BP.WF.Template.NodeSheet();

						com.kong.kflowweb.BP.WF.Template.CC cc = new CC(); // 抄送相关的信息.
						com.kong.kflowweb.BP.WF.Template.FrmWorkCheck fwc = new FrmWorkCheck();
					...
					for(DataRow dr : dt.Rows)
                    {
                        com.kong.kflowweb.BP.WF.Template.NodeExt nd = new com.kong.kflowweb.BP.WF.Template.NodeExt();
	...
	public final Node DoNewNode(int x, int y) throws Exception {
		...
		// 通用的人员选择器.
		com.kong.kflowweb.BP.WF.Template.Selector select = new Selector(nd.getNodeID());
		select.setSelectorModel(com.kong.kflowweb.BP.WF.Template.SelectorModel.GenerUserSelecter);
		select.Update();
	...
	public final void CreatePushMsg(Node nd) throws Exception {
		...
		// 创建发送短消息,为默认的消息.
		com.kong.kflowweb.BP.WF.Template.PushMsg pm = new com.kong.kflowweb.BP.WF.Template.PushMsg();
	...
	public final String DoNewFlow(String flowSort, String flowName, DataStoreModel model, String pTable,
		...
		try {
			...
			// 装载模版.
			String file = com.kong.kflowweb.BP.Sys.SystemConfig.getPathOfDataUser() + "XML/TempleteSheetOfStartNode.xml";
			if (new File(file).exists() == true) {
				...
				com.kong.kflowweb.BP.Sys.MapData.ImpMapData(nodeID, ds);
			}
	...
	public final void WritToGPM(String flowSort) {
		 ...
		 * "' AND FK_App='" + com.kong.kflowweb.BP.Sys.SystemConfig.getSysNo() + "'"; String
		 * parentNoOfMenu = DBAccess.RunSQLReturnStringIsNull(sql, null); if
		 * (parentNoOfMenu == null) { throw new RuntimeException("@没有找到该流程的(" +
		 * com.kong.kflowweb.BP.Sys.SystemConfig.getSysNo() + ")目录在GPM系统中,请重新新建此目录。"); }
		 * 
		 * // 取得该功能的主键编号. String treeNo =
		 * String.valueOf(DBAccess.GenerOID("com.kong.kflowweb.BP.GPM.Menu"));
		 * 
	...
	protected boolean beforeUpdate() throws Exception {
		this.setVer(com.kong.kflowweb.BP.DA.DataType.getCurrentDataTimess());
		Node.CheckFlow(this);
		 Flow fl = new Flow(this.getNo());
	...
	public static void UpdateVer(String flowNo) {
		String sql = "UPDATE WF_Flow SET VER='" + com.kong.kflowweb.BP.DA.DataType.getCurrentDataTimess() + "' WHERE No='" + flowNo + "'";
		DBAccess.RunSQL(sql);
	}
	...
	public String VerSetCurrentVer() throws Exception {
		...
		//清空缓存数据
		com.kong.kflowweb.BP.DA.Cash2019.DeleteRow("com.kong.kflowweb.BP.WF.Flow",oldFlowNo);
		com.kong.kflowweb.BP.DA.Cash2019.DeleteRow("com.kong.kflowweb.BP.WF.Flow",this.getNo());
