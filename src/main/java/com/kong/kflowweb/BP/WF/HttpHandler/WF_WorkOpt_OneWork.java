...
public class WF_WorkOpt_OneWork extends WebContralBase {
   ...
   {
       DataSet ds = com.kong.kflowweb.BP.WF.Dev2Interface.DB_JobSchedule(this.getWorkID());
	...
	public String FlowBBS_Delete() throws Exception{
		return com.kong.kflowweb.BP.WF.Dev2Interface.Flow_BBSDelete(this.getFK_Flow(), this.getMyPK(), WebUser.getNo());
	}
	...
	public String OP_UnSend() throws Exception{
		// 转化成编号.
		return com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoUnSend(this.getFK_Flow(), this.getWorkID());
	}
	@Override
	...
	*/
	...
	public String OP_GetStatus() throws Exception
	{
        ...
        if (SystemConfig.getCustomerNo().equals("TianYe"))
        {
            ...
            //前提，流程结束后才可以看到打印权限
            if (gwf.getWFState() == com.kong.kflowweb.BP.WF.WFState.Complete)
                isFlowEnd = true;

            boolean isAdmin = false;
            if (com.kong.kflowweb.BP.Web.WebUser.getNo() == "admin")
                isAdmin = true;

            // 判断是否可以打印.
            //string sql = "SELECT NDFrom,NDFromT,EmpFrom FROM ND" + int.Parse(this.FK_Flow) + "Track WHERE WorkID=" + this.WorkID + " AND (EmpFrom='" + com.kong.kflowweb.BP.Web.WebUser.No + "' OR  EmpTo='" + com.kong.kflowweb.BP.Web.WebUser.No + "')  ";
            ...
            for(DataRow dr : dt.Rows)
            {
                ...
                if (btn.getPrintPDFEnable() == true || btn.getPrintZipEnable() == true)
                {
                    ...
                    if (isFlowEnd==true && (isAdmin == true || com.kong.kflowweb.BP.Web.WebUser.getNo() == empFrom || gwf.getStarter() == WebUser.getNo()))
                    {
                        CanPackUp = true;
                        break;
                    }
		...
		switch (gwf.getWFState())
		{
			case Runing: // 运行时
				/*删除流程.*/
				if (com.kong.kflowweb.BP.WF.Dev2Interface.Flow_IsCanDeleteFlowInstance(this.getFK_Flow(), this.getWorkID(), WebUser.getNo()) == true)
			...
			case HungUp: // 挂起.
				/*撤销挂起*/
				if (com.kong.kflowweb.BP.WF.Dev2Interface.Flow_IsCanDoCurrentWork(this.getWorkID(), WebUser.getNo()) == false)
	...
	*/
	public String GetAthsAndBills() throws Exception
	{
		...
		{
			sql = "SELECT fadb.*,wn.Name NodeName FROM Sys_FrmAttachmentDB fadb INNER JOIN WF_Node wn ON wn.NodeID = fadb.NodeID WHERE fadb.FK_FrmAttachment IN (SELECT MyPK FROM Sys_FrmAttachment WHERE  " + com.kong.kflowweb.BP.WF.Glo.MapDataLikeKey(this.getFK_Flow(), "FK_MapData") + "  AND IsUpload=1) AND fadb.RefPKVal='" + this.getWorkID() + "' ORDER BY fadb.RDT";
		}
		else
		{
		...
		json += com.kong.kflowweb.BP.Tools.Json.ToJson(dt) + ",\"Bills\":";

		Bills bills = new Bills();
		bills.Retrieve(BillAttr.WorkID, this.getWorkID());

		json += bills.ToJson() + ",\"AppPath\":\"" + com.kong.kflowweb.BP.WF.Glo.getCCFlowAppPath()+ "\"}";
	...
	*/
	...
	{
		String name = "";
		name = com.kong.kflowweb.BP.Web.WebUser.getNo();
		return name;
	...
	{
		String name = "";
		name = com.kong.kflowweb.BP.Web.WebUser.getName();
		return name;
	}
	...
	public final String FlowBBSDept()
	{
		...
		return com.kong.kflowweb.BP.Tools.Json.ToJson(com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnString(ps));
	}
	...
	*/
	...
	{
		Paras pss = new Paras();
		pss.SQL = "SELECT * FROM ND" + Integer.parseInt(this.getFK_Flow()) + "Track WHERE ActionType=" + com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBVarStr() + "ActionType AND WorkID=" + com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBVarStr() + "WorkID AND  EMPFROMT='" + this.getUserName() + "'";
		pss.Add("ActionType", com.kong.kflowweb.BP.WF.ActionType.FlowBBS.getValue());
		pss.Add("WorkID", this.getWorkID());
		return com.kong.kflowweb.BP.Tools.Json.ToJson(com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(pss));
	...
	*/
	public final String FlowBBS_Save() throws Exception
	{
		...
		String mypk = com.kong.kflowweb.BP.WF.Dev2Interface.Flow_BBSAdd(this.getFK_Flow(), this.getWorkID(), this.getFID(), msg, WebUser.getNo(), WebUser.getName());
		Paras ps = new Paras();
		ps.SQL = "SELECT * FROM ND" + Integer.parseInt(this.getFK_Flow()) + "Track WHERE MyPK=" + com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBVarStr() + "MyPK";
		ps.Add("MyPK", mypk);
		return com.kong.kflowweb.BP.Tools.Json.ToJson(com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnTable(ps));
	}
	...
	*/
	public final String FlowBBS_Count()
	{
		...
		Paras ps = new Paras();
		ps.SQL = "SELECT COUNT(ActionType) FROM ND" + Integer.parseInt(this.getFK_Flow()) + "Track WHERE ActionType=" + com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBVarStr() + "ActionType AND WorkID=" + com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBVarStr() + "WorkID";
		ps.Add("ActionType", com.kong.kflowweb.BP.WF.ActionType.FlowBBS.getValue());
		ps.Add("WorkID", this.getWorkID());
		String count = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt(ps) + "";
		return count;
	}
...
*/
	public final String TimeBase_Init() throws Exception
	{
		...
		//获取track.
		DataTable dt = com.kong.kflowweb.BP.WF.Dev2Interface.DB_GenerTrackTable(this.getFK_Flow(), this.getWorkID(), this.getFID());
		ds.Tables.add(dt);
    ...
    {
        com.kong.kflowweb.BP.WF.HttpHandler.WF wf = new WF();
