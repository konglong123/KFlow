...
import com.kong.kflowweb.BP.WF.FlowDeleteRole;
...
import com.kong.kflowweb.BP.WF.GenerWorkFlowAttr;
import com.kong.kflowweb.BP.WF.GenerWorkFlows;
...
import com.kong.kflowweb.BP.WF.ReturnWork;
import com.kong.kflowweb.BP.WF.TrackAttr;
import com.kong.kflowweb.BP.WF.WFState;
import com.kong.kflowweb.BP.WF.WorkAttr;
import com.kong.kflowweb.BP.WF.WorkNode;
...
public class FlowExt extends EntityNoName
{
	...
	*/
	...
	public UAC getHisUAC()
	{
		...
		/*
		if (com.kong.kflowweb.BP.Web.WebUser.getNo().equals("admin") || this.getDesignerNo().equals(WebUser.getNo()))
		{
			uac.IsUpdate = true;
		} 
...
//		map.AddRefMethod(rm);
		...
		map.AddRefMethod(rm);*/
			...
			//rm.Icon = com.kong.kflowweb.BP.WF.Glo.CCFlowAppPath + "WF/Img/Btn/DTS.gif";
	...
	{
		return Glo.getCCFlowAppPath() + "WF/Comm/Search.htm?EnsName=com.kong.kflowweb.BP.WF.Data.GenerWorkFlowViews&FK_Flow=" + this.getNo() + "&WFSta=all";
	}
	public final String DoDataManger_Group()
	{
		return Glo.getCCFlowAppPath() + "WF/Comm/Group.htm?EnsName=com.kong.kflowweb.BP.WF.Data.GenerWorkFlowViews&FK_Flow=" + this.getNo() + "&WFSta=all";
	}
	...
	{
		return Glo.getCCFlowAppPath() + "WF/Comm/Search.htm?EnsName=com.kong.kflowweb.BP.WF.WorkFlowDeleteLogs&FK_Flow=" + this.getNo() + "&WFSta=all";
	}
	...
	*/
	...
	public final String DoDRpt()
	{
		...
		/**WF/Comm/RefFunc/UIEn.jsp?EnsName=com.kong.kflowweb.BP.WF.Rpt.MapRptExts&PK=ND185MyRpt;
		*/
		//return Glo.getCCFlowAppPath() + "WF/Comm/RefFunc/UIEn.jsp?EnsName=com.kong.kflowweb.BP.WF.Rpt.MapRptExts&PK=ND" + Integer.parseInt(this.getNo()) + "MyRpt";
		return Glo.getCCFlowAppPath() +"WF/Comm/En.htm?EnName=com.kong.kflowweb.BP.WF.Rpt.MapRptExt&PKVal=ND" + Integer.parseInt(this.getNo()) + "MyRpt";
		//  UIEn.jsp?EnsName=com.kong.kflowweb.BP.WF.Rpt.MapRptExts&PK=ND185MyRpt
	...
	*/
	public final String DoDelFlows(String dtFrom, String dtTo, String isDelCurrFlow) throws Exception
	{
		...
		for (DataRow dr : dt.Rows)
		{
			...
			com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoDeleteFlowByReal(fk_flow, workid, false);
			msg += " " + workid;
		}
		return msg;
	}
	...
	*/
	public final String DoRebackFlowData(long workid, int backToNodeID, String note) throws Exception
	{
		...
		try
		{
			...
			rw.setMyPK(com.kong.kflowweb.BP.DA.DBAccess.GenerGUID());
			...
			// 向接受人发送一条消息.
			com.kong.kflowweb.BP.WF.Dev2Interface.Port_SendMsg(currWl.getFK_Emp(), "工作恢复:" + gwf.getTitle(), "工作被:" + WebUser.getNo() + " 恢复." + note, "ReBack" + workid, com.kong.kflowweb.BP.WF.SMSMsgType.SendSuccess, this.getNo(), Integer.parseInt(this.getNo() + "01"), workid, 0);
	...
	*/
	public final String DoGenerFlowEmps() throws Exception
	{
		...
		com.kong.kflowweb.BP.Web.WebUser.SignInOfGener(emp1);
	...
	*/
	...
	{
		return "../../WF/Comm/Search.htm?s=d34&EnsName=com.kong.kflowweb.BP.WF.Data.GenerWorkFlowViews&FK_Flow=" + this.getNo() + "&ExtType=StartFlow&RefNo=";
	}
	/** 
	 绑定独立表单
	...
	*/
	public final String DoDelDataOne(int workid, String note) throws Exception
	{
		try
		{
			com.kong.kflowweb.BP.WF.Dev2Interface.Flow_DoDeleteFlowByReal(this.getNo(), workid, true);
	...
	*/
	...
	protected boolean beforeUpdate() throws Exception
	{
		...
		try
		{
			...
			String fee=com.kong.kflowweb.BP.WF.Glo.GetFlowEventEntityStringByFlowMark(this.getFlowMark(), this.getNo());
		...
		if (fl.getDTSWay() != FlowDTSWay.None)
		{
			...
			if (fl.getDTSTime() == FlowDTSTime.SpecNodeSend)
			{
				...
				for (String str : strs)
				{
					...
					if (com.kong.kflowweb.BP.DA.DataType.IsNumStr(str) == false)
					{
	...
	protected void afterInsertUpdateAction() throws Exception
	{
		...
		{
			com.kong.kflowweb.BP.Sys.MapData md = new MapData(ndxxRpt);
	...
	*/
	public final String DoSetFWCModel() throws Exception
	{
		...
		for (Node nd : nds.ToJavaList())
		{
			...
			com.kong.kflowweb.BP.WF.Template.FrmNodeComponent fnd = new FrmNodeComponent(nd.getNodeID());
