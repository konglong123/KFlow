...
public class RepariDB extends Method
{
	...
	*/
	...
	{
		String rpt =PubClass.DBRpt(com.kong.kflowweb.BP.DA.DBCheckLevel.High);

		//// 手动升级. 2011-07-08 补充节点字段分组.
		//string sql = "DELETE FROM Sys_EnCfg WHERE No='com.kong.kflowweb.BP.WF.Template.NodeSheet'";
		//com.kong.kflowweb.BP.DA.DBAccess.RunSQL(sql);

		//sql = "INSERT INTO Sys_EnCfg(No,GroupTitle) VALUES ('com.kong.kflowweb.BP.WF.Template.NodeSheet','NodeID=基本配置@WarningHour=考核属性@SendLab=功能按钮标签与状态')";
		//com.kong.kflowweb.BP.DA.DBAccess.RunSQL(sql);
		...
		for (MapData md : mds.ToJavaList())
		{
			...
			try
			{
				com.kong.kflowweb.BP.WF.Node nd = new Node(Integer.parseInt(nodeid));
				nd.RepareMap();
				continue;
			}
