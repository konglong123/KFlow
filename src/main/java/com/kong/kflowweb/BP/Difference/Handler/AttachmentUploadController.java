      ...
      //求主键. 如果该表单挂接到流程上.
		...
		if (athDesc.getAthSaveWay() == AthSaveWay.WebServer) {
			...
			if (savePath.contains("@") == true || savePath.contains("*") == true) {
				...
				savePath = com.kong.kflowweb.BP.WF.Glo.DealExp(savePath, en, null);
			...
			if (!DataType.IsNullOrEmpty(msg)) {
				com.kong.kflowweb.BP.Sys.Glo.WriteLineError("@AthUploadeBefore事件返回信息，文件：" + file.getName() + "，" + msg);
				file.delete();
				return;
			}
			...
			if (!DataType.IsNullOrEmpty(msg))
				com.kong.kflowweb.BP.Sys.Glo.WriteLineError("@AthUploadeAfter事件返回信息，文件：" + dbUpload.getFileName() + "，" + msg);
		}
		...
		if (athDesc.getAthSaveWay() == AthSaveWay.DB || athDesc.getAthSaveWay() == AthSaveWay.FTPServer) {
			...
			if (DataType.IsNullOrEmpty(msg) == false) {
				com.kong.kflowweb.BP.Sys.Glo.WriteLineError("@AthUploadeBefore事件返回信息，文件：" + fileName + "，" + msg);
