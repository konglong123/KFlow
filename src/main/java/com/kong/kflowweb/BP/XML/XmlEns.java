package com.kong.kflowweb.BP.XML;
...
public abstract class XmlEns extends ArrayList<XmlEn>
{
	...
	private DataTable GetTableTxts(List<File> fis)
	{
		...
		Object tempVar = com.kong.kflowweb.BP.DA.Cash.GetObj(this.getTname(),
		...
		com.kong.kflowweb.BP.DA.Cash.AddObj(this.getTname(), Depositary.Application, dt);
		return dt;
	}
	...
	public final DataTable GetTable()
	{
		...
		if (this.getFile().toLowerCase().indexOf(".xml") > 0)
		{
			...
			com.kong.kflowweb.BP.DA.Cash.AddObj(this.getTname(), Depositary.Application, mdt);
	...
	{
		com.kong.kflowweb.BP.DA.Cash.RemoveObj(this.getTname());
	...
	{
		com.kong.kflowweb.BP.DA.Log.DebugWriteInfo("开始读取:"+this.toString());
		this.clear(); // 清所有的信息。
		Object tempVar = com.kong.kflowweb.BP.DA.Cash.GetObj(this.toString(),
				Depositary.Application);
		...
		com.kong.kflowweb.BP.DA.Cash.AddObj(this.toString(), Depositary.Application, this);
		com.kong.kflowweb.BP.DA.Log.DebugWriteInfo("读取结束"+this.toString());
		return dt.Rows.size();
	}
