...
public class GPMEmp extends EntityNoName
{
	...
	*/
	...
	public Map getEnMap()
	{
		...
		map.AddDDLEntities(EmpAttr.FK_Dept, null, "主要部门", new com.kong.kflowweb.BP.Port.Depts(), true);
		...
		map.AddSearchAttr(EmpAttr.SignType);
			...
			//节点绑定部门. 节点绑定部门.
		map.getAttrsOfOneVSM().AddBranches(new EmpMenus(), new com.kong.kflowweb.BP.GPM.Menus(), com.kong.kflowweb.BP.GPM.EmpMenuAttr.FK_Emp, com.kong.kflowweb.BP.GPM.EmpMenuAttr.FK_Menu, "人员菜单", EmpAttr.Name, EmpAttr.No, "0");
		...
		map.AddRefMethod(rm);
			...
			//节点绑定部门. 节点绑定部门.
		map.getAttrsOfOneVSM().AddBranches(new DeptEmps(), new com.kong.kflowweb.BP.GPM.Depts(), com.kong.kflowweb.BP.GPM.DeptEmpAttr.FK_Emp, com.kong.kflowweb.BP.GPM.DeptEmpAttr.FK_Dept, "部门维护", EmpAttr.Name, EmpAttr.No, "@WebUser.FK_Dept");
	...
	{
		//增加拼音，以方便查找.
		String pinyinQP = com.kong.kflowweb.BP.DA.DataType.ParseStringToPinyin(en.getName()).toLowerCase();
		String pinyinJX = com.kong.kflowweb.BP.DA.DataType.ParseStringToPinyinJianXie(en.getName()).toLowerCase();
		...
		for (DeptEmpStation item : des.ToJavaList())
		{
			...
			//给拼音重新定义值,让其加上部门的信息.
			en.setPinYin(en.getPinYin() + pinyinJX + "/" + com.kong.kflowweb.BP.DA.DataType.ParseStringToPinyinJianXie(dept.getName()).toLowerCase() + ",");

			com.kong.kflowweb.BP.Port.Station sta = new com.kong.kflowweb.BP.Port.Station();
	...
	{
		//处理其他的数据.
		com.kong.kflowweb.BP.GPM.GPMEmp.GenerData(this);
