...
public class Glo
{
	...
	*/
	public static void DoInstallDataBase(String lang, String yunXingHuanjing) throws Exception
	{
		...
		for (Object obj : al)
		{
			...
			if (en.toString().contains("com.kong.kflowweb.BP.Port."))
			{
				continue;
			}
			//if (en.ToString().Contains("com.kong.kflowweb.BP.GPM."))
			//    continue;
			//if (en.ToString().Contains("com.kong.kflowweb.BP.Demo."))
			//    continue;
		...
		com.kong.kflowweb.BP.DA.DBAccess.RunSQLScriptGo(sqlscript);
		...
		//删除数据.
		com.kong.kflowweb.BP.DA.DBAccess.RunSQL("DELETE FROM GPM_EmpApp");
	...
	*/
	...
	{
	  //  string sqlscript = SystemConfig.PathOfWebApp + "\\GPM\\SQLScript\\CCIM_"+com.kong.kflowweb.BP.Sys.SystemConfig.AppCenterDBType+".sql";
	   // com.kong.kflowweb.BP.DA.DBAccess.RunSQLScriptGo(sqlscript);
	}
	...
	/** 
	 ...
	 @return 是否可以执行
	*/
	public static boolean IsCanDoIt(String ctrlObj, com.kong.kflowweb.BP.GPM.CtrlWay cw, String empID)
	{
		int n = 0;
		String sql = "";
		switch (cw)
		{
			...
			case ByStation:
				...
				n = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnValInt(sql, 0);
				break;
			case ByDept:
