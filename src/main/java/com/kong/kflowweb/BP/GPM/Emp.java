	...
	*/
	...
	public Map getEnMap()
	{
        ...
        map.AddSearchAttr(EmpAttr.SignType);
         ...
         //节点绑定部门. 节点绑定部门.
         map.getAttrsOfOneVSM().AddBranches(new DeptEmps(), new com.kong.kflowweb.BP.GPM.Depts(),
            com.kong.kflowweb.BP.GPM.DeptEmpAttr.FK_Emp,
            com.kong.kflowweb.BP.GPM.DeptEmpAttr.FK_Dept, "部门维护", EmpAttr.Name, EmpAttr.No, "@WebUser.FK_Dept");
     ...
     {
         //增加拼音，以方便查找.
         String pinyinQP = com.kong.kflowweb.BP.DA.DataType.ParseStringToPinyin(this.getName()).toLowerCase();
         String pinyinJX = com.kong.kflowweb.BP.DA.DataType.ParseStringToPinyinJianXie(this.getName()).toLowerCase();
         ...
         for (DeptEmpStation item : des.ToJavaList())
         {
             ...
             //给拼音重新定义值,让其加上部门的信息.
             this.setPinYin(this.getPinYin() + pinyinJX + "/" + com.kong.kflowweb.BP.DA.DataType.ParseStringToPinyinJianXie(dept.getName()).toLowerCase()+",");
		 ...
		 wfEmp.setNo(this.getNo());
		 if (wfEmp.RetrieveFromDBSources() == 0)
		 {
			 wfEmp.Copy(this);
			 wfEmp.Insert();
		 }else{
			 wfEmp.setFK_Dept(this.getFK_Dept());
			 wfEmp.Update();
		 }
