...
//				break;
		...
		for (Attr attr : attrs) {
			...
			case DataType.AppInt:
				...
				if (attr.getMyFieldType() == FieldType.Enum || attr.getMyFieldType() == FieldType.PKEnum) {
					...
					val = val + "," + com.kong.kflowweb.BP.DA.Cash.getCaseWhenSQL(mainTable, attr.getKey(), attr.getField(),
		...
		for (Attr attr : en.getEnMap().getAttrs()) {
			...
			case DataType.AppInt:
				...
				if (attr.getMyFieldType() == FieldType.Enum || attr.getMyFieldType() == FieldType.PKEnum) {
					...
					SysEnums ses = new com.kong.kflowweb.BP.Sys.SysEnums(attr.getUIBindKey(), attr.UITag);
	...
	public static Paras GenerParas(Entity en, String[] keys) {
		boolean IsEnableNull = com.kong.kflowweb.BP.Sys.SystemConfig.getIsEnableNull();
		String mykeys = "@";
		if (keys != null) {
			for (String key : keys) {
				mykeys += key + "@";
