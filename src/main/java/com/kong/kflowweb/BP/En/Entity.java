...
import com.kong.kflowweb.BP.DA.Cash2019;
import com.kong.kflowweb.BP.DA.CashEntity;
...
import com.kong.kflowweb.BP.Sys.EnVer;
import com.kong.kflowweb.BP.Sys.EnVerDtl;
import com.kong.kflowweb.BP.Sys.GEDtlAttr;
import com.kong.kflowweb.BP.Sys.GloVar;
...
import com.kong.kflowweb.BP.Sys.SysDocFile;
...
import com.kong.kflowweb.BP.Tools.StringUtils;
...
public abstract class Entity implements Serializable {
	...
	public Entities getGetNewEntities() {
		...
		if (_GetNewEntities == null) {
			...
			ArrayList al = ClassFactory.GetObjects("com.kong.kflowweb.BP.En.Entities");
			for (Object o : al) {
         ...
         try
         {
             ...
             switch (md.getDtlOpenType())
             {
                 case ForEmp:  // 按人员来控制.
                     ...
                     qo.addAnd();
                     qo.AddWhere(GEDtlAttr.Rec, com.kong.kflowweb.BP.Web.WebUser.getNo());
                     break;
	...
	public int Retrieve() throws Exception {
        ...
        {
        	Row row = com.kong.kflowweb.BP.DA.Cash2019.GetRow(this.toString(), this.getPKVal().toString());
            if (row != null)
            {
                this.setRow(row);
                return 1;
		...
		try {
			...
			if (i > 0){
                ...
                if (this.getEnMap().getDepositaryOfEntity() == Depositary.Application)
                {
                    com.kong.kflowweb.BP.DA.Cash2019.PutRow(this.toString(), this.getPKVal().toString(), this.getRow());
                }
				return i;
			}
			...
			if (msg.contains("无效") || msg.contains("field list")) {
				try {
					...
					this.CheckPhysicsTable();
					if (com.kong.kflowweb.BP.DA.DBAccess.IsExits(this.getEnMap().getPhysicsTable()) == true)
		...
		try {
			...
			if (msg.contains("无效") || msg.contains("field list")) {
				try {
					...
					if (com.kong.kflowweb.BP.DA.DBAccess.IsExits(this.getEnMap().getPhysicsTable()) == false)
	...
	protected void afterInsert() throws Exception {
		...
		String rdt = com.kong.kflowweb.BP.DA.DataType.getCurrentDataTime();
		...
		ver.setRDT(rdt);
		ver.setRec(com.kong.kflowweb.BP.Web.WebUser.getName());
		ver.Save();
		...
		for (Attr attr : attrs.ToJavaList()) {
			...
			dtl.setRDT(rdt);
			dtl.setRec(com.kong.kflowweb.BP.Web.WebUser.getName());
	...
	public final void SaveBigTxtToDB(String saveToField, String bigTxt) throws Exception {
		String temp = com.kong.kflowweb.BP.Sys.SystemConfig.getPathOfTemp() + "/" + this.getEnMap().getPhysicsTable() + this.getPKVal()
				+ ".tmp";
	...
	public final void SaveFileToDB(String saveToField, String filefullName) throws Exception {
		try {
			com.kong.kflowweb.BP.DA.DBAccess.SaveFileToDB(filefullName, this.getEnMap().getPhysicsTable(), this.getPK(),
					this.getPKVal().toString(), saveToField);
		} catch (RuntimeException ex) {
			// 为了防止任何可能出现的数据丢失问题，您应该先仔细检查此脚本，然后再在数据库设计器的上下文之外运行此脚本。
			String sql = "";
			if (com.kong.kflowweb.BP.DA.DBAccess.getAppCenterDBType().equals(DBType.MSSQL)) {
				sql = "ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + saveToField + " Image NULL ";
			}

			if (com.kong.kflowweb.BP.DA.DBAccess.getAppCenterDBType().equals(DBType.Oracle)) {
				sql = "ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + saveToField + " Blob NULL ";
			}

			if (com.kong.kflowweb.BP.DA.DBAccess.getAppCenterDBType().equals(DBType.MySQL)) {
	...
	public final byte[] GetFileFromDB(String saveToField, String filefullName) throws IOException {
		com.kong.kflowweb.BP.DA.DBAccess.GetFileFromDB(filefullName, this.getEnMap().getPhysicsTable(), this.getPK(),
				this.getPKVal().toString(), saveToField);
		return null;
	}
	...
	public final String GetBigTextFromDB(String imgFieldName) throws IOException {
		String tempFile = com.kong.kflowweb.BP.Sys.SystemConfig.getPathOfTemp() + "/" + this.getEnMap().getPhysicsTable()
				+ this.getPKVals() + ".tmp";
		...
		return com.kong.kflowweb.BP.DA.DBAccess.GetTextFileFromDB(tempFile, this.getEnMap().getPhysicsTable(), this.getPK(),
		...
		for (Attr attr : this.getEnMap().getAttrs()) {
			...
			for (DataRow dr : dt.Rows) {
				try {
					...
					com.kong.kflowweb.BP.DA.Log.DebugWriteWarning(ex.getMessage());
				}
			}
		}
...
//				String OWNER = DBAccess.RunSQLReturnString(sql);
	...
	private void CheckPhysicsTable_MySQL() throws Exception {
		...
		sql = "SELECT character_maximum_length as Len, table_schema as OWNER, column_Name FROM information_schema.columns WHERE TABLE_SCHEMA='"
				+ com.kong.kflowweb.BP.Sys.SystemConfig.getAppCenterDBDatabase() + "' AND table_name ='" + map.getPhysicsTable() + "'";
...
//				break;
	...
	public final String ToJson(Boolean isInParaFields) {
		...
		return com.kong.kflowweb.BP.Tools.Json.ToJson(ht, false);
	}
	...
	public final void ResetDefaultVal(String fk_mapdata,String fk_flow,int fk_node,MapAttrs attrs) throws Exception{
		for (MapAttr attr : attrs.ToJavaList()) {
			if(attr.getLGType()== com.kong.kflowweb.BP.En.FieldTypeS.FK)
	...
	public SQLCash getSQLCash() throws Exception {
		if (_SQLCash == null) {
			_SQLCash = com.kong.kflowweb.BP.DA.Cash.GetSQL(this.toString());
			if (_SQLCash == null) {
				_SQLCash = new SQLCash(this);
				com.kong.kflowweb.BP.DA.Cash.SetSQL(this.toString(), _SQLCash);
			}
		}
		return _SQLCash;
	}
	...
	public void clearSQLCash() {
		com.kong.kflowweb.BP.DA.Cash.getSQL_Cash().remove(this.toString());
		_SQLCash = null;
	}
