...
import com.kong.kflowweb.BP.Port.Current;
import com.kong.kflowweb.BP.Port.Emp;
import com.kong.kflowweb.BP.Port.Stations;
import com.kong.kflowweb.BP.Sys.OSDBSrc;
	...
	public static String SignInOfGener(Emp em, String lang, String auth, boolean isRememberMe, boolean IsRecSID) throws Exception {
		if (SystemConfig.getIsBSsystem()) {
			if("BP".equals(SystemConfig.getRunOnPlant())) {
				...
				com.kong.kflowweb.BP.Sys.Glo.WriteUserLog("SignIn", em.getNo(), "登录");
			}			
		}
		if (auth == null) {
			auth = "";
...
//					}
					 ...
					 * com.kong.kflowweb.BP.Glo.getHttpContextCurrent().Response.AppendCookie(cookie);
					 */
	...
	*/
	public static void SignInOfGener(Emp em, String lang, boolean isRememberMe, boolean IsRecSID, String authNo, String authName) throws Exception
	{
		...
		//登录模式？
		com.kong.kflowweb.BP.Web.WebUser.setUserWorkDev(UserWorkDev.PC);
		...
		if (com.kong.kflowweb.BP.Sys.SystemConfig.getOSDBSrc() == OSDBSrc.Database)
		{
			...
			{
				String sql = "";
				if (com.kong.kflowweb.BP.Sys.SystemConfig.getOSModel()== OSModel.OneOne)
				{
				...
				String deptNo = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnString(sql);
		...
		if (com.kong.kflowweb.BP.Sys.SystemConfig.getOSDBSrc() == OSDBSrc.WebServices)
		{
			...
			String strs = com.kong.kflowweb.BP.DA.DBAccess.GenerWhereInPKsString(dt);
			Paras ps = new Paras();
			...
			strs = com.kong.kflowweb.BP.DA.DBAccess.GenerWhereInPKsString(dt);
			ps = new Paras();
		...
		if (IsRecSID)
		{
			...
			if (com.kong.kflowweb.BP.DA.DBAccess.IsView("Port_Emp") == false){
			//如果记录sid
		...
		if (com.kong.kflowweb.BP.Sys.SystemConfig.getIsBSsystem())
		{
			...
			//HttpCookie hc = com.kong.kflowweb.BP.Sys.Glo.getRequest().getCookies("CCS");
	...
	private static void ChangeMainDept(String no, String deptNo) {
		...
		String sql = com.kong.kflowweb.BP.Sys.SystemConfig.GetValByKey("UpdataMainDeptSQL", "");
		if (sql.equals(""))
		{
			//如果没有配置, 就取默认的配置.
...
//		if (getIsBSMode()) {
//			/*
//			 * warning return com.kong.kflowweb.BP.Glo.getHttpContextCurrent().Session[key];
//			 */
	...
	public static String getDeptParentNo() throws Exception {
		...
		if (val == null) {
			...
			throw new RuntimeException("@err-001 DeptParentNo, FK_Dept 登录信息丢失。");
			   ...
			   com.kong.kflowweb.BP.Port.Dept dept = new com.kong.kflowweb.BP.Port.Dept( WebUser.getFK_Dept());
               com.kong.kflowweb.BP.Web.WebUser.setDeptParentNo(  dept.getParentNo());
...
//		}
	...
	public static String getNoOfSessionID() {
		...
		if (s == null) {
			/*
			 * warning return com.kong.kflowweb.BP.Glo.getHttpContextCurrent().Session.SessionID;
			 */
    ...
    {
            if ("admin".equals(com.kong.kflowweb.BP.Web.WebUser.getNo()))
                return true;
            try
            {
	...
	public static String getNo() throws Exception {
		...
		if (StringUtils.isNotBlank(userNoSessionKey)){
			...
			if (StringUtils.isNotBlank(userNo)){
				...
				if (com.kong.kflowweb.BP.WF.Glo.getIsAdmin(userNo)){
					userNo = "admin";
				}
...
*/
	...
	public static UserWorkDev getUserWorkDev() {
		...
		com.kong.kflowweb.BP.Web.UserWorkDev wd = UserWorkDev.forValue(s);
		return wd;
	}
	...
	public static String getHisStationsStr() throws Exception {
		...
		if (val == null) {
			Object tempVar = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnVal("SELECT Stas FROM WF_Emp WHERE No='" + WebUser.getNo() + "'");
	...
	public static String getHisDeptsStr() throws Exception {
		...
		if (val == null) {
			Object tempVar = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnVal("SELECT Depts FROM WF_Emp WHERE No='" + WebUser.getNo() + "'");
			val = (String) ((tempVar instanceof String) ? tempVar : null);
			if (val == null) {
				val = "";
			}
	...
	public static void SignInOfGener2017(String userNo, String userName, String deptNo, String deptName, String authNo, String authName) throws UnsupportedEncodingException
	{
		...
		if (com.kong.kflowweb.BP.Sys.SystemConfig.getOSDBSrc() == OSDBSrc.Database)
		{
			if (StringHelper.isNullOrEmpty(deptNo) == true)
			{
				...
				deptNo = com.kong.kflowweb.BP.DA.DBAccess.RunSQLReturnString(sql);
				if (StringHelper.isNullOrEmpty(deptNo) == true)
				{
